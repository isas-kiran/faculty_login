    ...
    <?php       if($_POST['formAction'])
                    {
                        if($_POST['formAction']=="delete")
                        {
                            for($r=0;$r<count($_POST['chkRecords']);$r++)
                            {
                                ...
                                $sql_query= "SELECT book_lost_id FROM lb_book_lost where book_issue_id ='".$del_record_id."'";
                                $ptr_query=mysql_query($sql_query);
								if(mysql_num_rows($ptr_query))
								{    
									$cust_data=mysql_fetch_array($ptr_query);  
									"<br>".$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('manage_lost_book','Delete','".$cust_data['book_lost_id']."','".$del_record_id."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
									$query=mysql_query($insert);              
									                                     
									$delete_query="delete from lb_book_lost where book_lost_id='".$del_record_id."'";
                    ...
                    if($_REQUEST['deleteRecord'] && $_REQUEST['record_id'])
                    {
                        ...
                        $sql_query= "SELECT book_lost_id FROM lb_book_lost where book_lost_id='".$del_record_id."'";
                        $ptr_query=mysql_query($sql_query);
						if(mysql_num_rows($ptr_query))
						{    
							$cust_data=mysql_fetch_array($ptr_query);  
...
<?php
                        ...
                        if($_GET['order'] !='' && ($_GET['orderby']=='book_return_id'))
                        {
							...
							$select_directory='order by i.book_lost_id desc';  
						$record_cat_id='';
						if($_GET['record_id'] !='')
						{
							$record_cat_id="and i.book_lost_id='".$_GET['record_id']."' ";
						}
						if($_SESSION['where'] !='')
						{
							$cm_ids="and i.cm_id='".$_SESSION['cm_id']."'";
						}
						$sql_query= "SELECT i.book_lost_id,i.book_id,i.lost_by,i.lost_date,i.user_type,i.status,i.fine FROM lb_book_lost i ".$record_cat_id." ".$cm_ids." ".$search_cm_id_S." ".$pre_keyword." ".$select_directory."";
                     ...
                     <input type="hidden" name="formAction" id="formAction" value=""/>
                      <tr class="grey_td" >
					   <?php
                        ...
                        <td width="9%" align="center"><strong>Lost by</strong></td>
                        <td width="4%" align="center"><strong>Lost date</strong></td>
                        ...
                        while($val_query=mysql_fetch_array($all_records))
                        {
							...
							$listed_record_id=$val_query['book_lost_id']; 
							...
							{
								$sel_user="select name from enrollment where enroll_id='".$val_query['lost_by']."'";
							...
							{
								$sel_user="select name from site_setting where admin_id='".$val_query['lost_by']."'";
								$query_user=mysql_query($sel_user);
								$data_user=mysql_fetch_array($query_user);
								$name=$data_user['name'];
							}	
							...
							echo '<td align="center">'.$val_query['lost_date'].'</td>';							
