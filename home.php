...
<script>
function change_topic(id,batch_id)
{
	var data1="action=topic&topic_id="+id+"&course_batch_id="+batch_id;
	//alert(data1);
	$.ajax ({
	...
	success: function (html)
	{
		//alert(html);
		content=html.split('##');
		document.getElementById('content_'+batch_id).innerHTML=content[0];
		document.getElementById('day_'+batch_id).innerHTML=content[1];
	}
	});
}
</script>
...
<div class="content-wrapper" style="margin-left:0px !important">
	...
	<section class="content-header">
      ...
      <h1>
        <center><?php echo date('D').' - '.date('d-M-Y'); ?></center>
      </h1>
      <ol class="breadcrumb">
		...
		if(mysql_num_rows($ptr_cate))
		{
			while($data_cat=mysql_fetch_array($ptr_cate))
			{
				...
				<div class="row">
					<!-- /.col -->
					<?php
					if($_POST['save_changes_'.$cat_name_small.''])
					{
						$agree=$_POST['agree'];
						if($agree !='')
						{
							...
							$update="update batch_timetable set status='Completed' where c_b_id='".$agree."' and date='".$attendenceDate."' and topic_id='".$topic_id."'";
					...
					$sel_batch="select * from course_batch_mapping where 1 and DATE(`start_date`)<='".date('Y-m-d')."' and DATE(`end_date`)>='".date('Y-m-d')."' and course_domain_id='".$cat_id."' order by c_b_id desc ";
					...
					if(mysql_num_rows($ptr_batch))
					{
						...
						<form method="post" name="frm_<?php echo $cat_name_small; ?>">
						<?php
						$it=1;
										...
										if(mysql_num_rows($ptr_timetable))
										{
											...
											if($no_of_records)
											{
												...
												<tr bgcolor="#F7F7F7">
												  ...
												  <?php
												  $sel_timet="Select batch_timetable_map_id,day,date,topic_id,topic_content from batch_timetable where c_b_id='".$course_batch_id."' and date ='".date('Y-m-d')."'";
												  $ptr_timet=mysql_query($sel_timet);
												  $data_timet=mysql_fetch_array($ptr_timet);
												  
												  echo 'Day &nbsp;'.$data_timet['day']; 
												  ?>
												  <input type="hidden" name="day_<?php echo $course_batch_id; ?>" id="day_<?php echo $course_batch_id; ?>" >
												  <!--<select class="form-control" name="day_<?php //echo $course_batch_id; ?>" id="day_<?php //echo $course_batch_id; ?>">
												  <option value="">Select Day</option>
												  <?php
												 /* $sel_timet="Select batch_timetable_map_id,day,date,topic_id,topic_content from batch_timetable where c_b_id='".$course_batch_id."'";
												  ...
												  while($data_time=mysql_fetch_array($ptr_timetable))
												  {
													 ...
													 if($data_time['date']==date('Y-m-d'))
													 {
													  ...
													  ?>
													  <option value="<?php echo $data_time['batch_timetable_map_id']; ?>" <?php echo $sel; ?>>Day <?php echo $data_time['day']; ?></option>
													  <?php
												  }*/
												  ?>
												  </select>-->
												  ...
												  <th colspan="3">
												  <?php
												  $sel_timet="Select batch_timetable_map_id,day,date,topic_id,topic_content from batch_timetable where c_b_id='".$course_batch_id."' and DATE(`date`)='".date('Y-m-d')."'";
												  $ptr_timet=mysql_query($sel_timet);
												  
												  ?>
												  <div id="content_<?php echo $course_batch_id; ?>">
												  <?php
												  while($data_timet=mysql_fetch_array($ptr_timet))
												  {
												  	echo $data_timet['topic_content'].'<br/>';
												  }
												  ?>
												  </div>
												  </th>
												...
												<tr>
												  ...
												  <!--<th align="center">Faculty Sign</th>-->
												...
												if($data_chk=='yes')
												{
													?>
													<tr>
														<td colspan="10" align="right"><input type="button" class=".btn btn-success" name="save_changes_<?php echo $cat_name_small;?>"  id="save_changes_<?php echo $cat_name_small;?>" value="Attendence Completed" disabled /></td>
													</tr>
												...
												else
												{
													?>
													<tr>
														<td colspan="10" align="right"><input type="radio" class="flat-red" name="agree" id="agree" value="<?php echo $course_batch_id; ?>" > <span style="font-size:16px"> &nbsp;&nbsp;&nbsp; I agreee  </span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="submit" class=".btn btn-success" name="save_changes_<?php echo $cat_name_small;?>"  id="save_changes_<?php echo $cat_name_small;?>" value="Save" /> </td>
													</tr>
													<input type="hidden" name="total_student"  id="total_student" value="<?php echo ($j-1); ?>" />
													</table>
													<?php
