    ...
    <?php       if($_POST['formAction'])
                    {
                        if($_POST['formAction']=="delete")
                        {
                            for($r=0;$r<count($_POST['chkRecords']);$r++)
                            {
                                ...
                                $sql_query= "SELECT saving_id FROM ".$GLOBALS["pre_db"]."saving where saving_id ='".$del_record_id."'";
                                if(mysql_num_rows($db->query($sql_record)))
                                    {                                                
                                        $delete_query="delete from ".$GLOBALS["pre_db"]."saving where saving_id='".$del_record_id."'";
...
<!--                                <option value="Active">Active</option>
    ...
    <?php
                        ...
                        if($keyword)
                            $pre_keyword=" and (saving_name like '%".$keyword."%' || date_saving like '%".$keyword."%')";
                        else
                            $pre_keyword="";
                        ...
                        if($_GET['orderby']=='saving_name' )
                            $img1 = $img;
							if($_GET['orderby']=='date_saving' )
                            $img2 = $img;
                        if($_GET['order'] !='' && ($_GET['orderby']=='saving_name' || $_GET['orderby']=='date_saving' ))
                        {
                            $select_directory .= " order by ".$_GET['orderby']." " .$_GET['order'] ;
                            $date_query='&order='.$_GET['order'].'&orderby='.$_GET['orderby'];
                        }
                        else
                            $select_directory='order by saving_id desc';                      
                            $sql_query= "SELECT * FROM bp_saving where 1  ".$pre_keyword." ".$select_directory.""; 
                       //echo $sql_query;
    ...
    <form method="post"  name="frmTakeAction" action="<?php echo $_SERVER['PHP_SELF'].'?#msgbox';?>" >
                     <input type="hidden" name="formAction" id="formAction" value=""/>
                      <tr class="grey_td" >
                        ...
                        <td width="10%"><a href="<?php echo $_SERVER['PHP_SELF'];?>?order=<?php echo $order."&orderby=saving_name".$query_string;?>" 
                        class="table_font"><strong>Title</strong></a> <?php echo $img1;?></td>
                     <td width="10%"> Discription</td>
                     <td width="10%"><a href="<?php echo $_SERVER['PHP_SELF'];?>?order=<?php echo $order."&orderby=date_saving".$query_string;?>" 
                     class="table_font"><strong>Date</strong></a> <?php echo $img3;?></td>
                       ...
                       <td width="15%"><strong>Tag</strong></td>
                        ...
                        <td width="10%" class="centerAlign"><strong>Action</strong></td>
                            ...
                            while($val_query=mysql_fetch_array($all_records))
                            {
                                ...
                                $listed_record_id=$val_query['saving_id']; 
                                
                                $position=120; // Define how many character you want to display.                                
                                $post = substr(strip_tags($val_query['discription']), 0, $position);
                                ...
                                echo '<td ><a href="add_savings.php?record_id='.$listed_record_id.'" >'.$val_query['saving_name'].'</a></td>';
                                echo '<td >'.$post.'</td>';
							    echo '<td >'.$val_query['date_saving'].'</td>';
                                echo '<td align="center">'.$val_query['added_date'].'</td>';
                                echo '<td align="center"><a href="add_savings.php?record_id='.$listed_record_id.'" ><img src="images/edit_icon.png" title="Edit Record" class="example-fade"/></a>&nbsp;&nbsp;
