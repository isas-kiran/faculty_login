...
$(document).ready(function() {
    $("#send_by").change(function() {
        ...
        if(selVal == 'email') 
		{
            $("#customise").append('<table width="100%"><tr><td width="20.5%" class="heading">Add Subject</span></td><td width="70%" colspan=2"><input type="text" class="input_text" name="subject" style="width:300px;"/></td></tr></table>');
			$("#for_sms").hide();
			$("#for_mail").show(); 
		...
		<?php
			...
			else
			{
				...
				if($_POST['send_by']=='sms')
				{
					if($_POST['staff_name']!='')
					{
						$sel_sms_cnt="select name,contact_phone from site_setting where admin_id='".$_POST['staff_name']."'";
						$ptr_sel_sms1=mysql_query($sel_sms_cnt);
						$ptr_sel_sms=mysql_fetch_array($ptr_sel_sms1);
						$mb_no=$ptr_sel_sms['contact_phone'];
					}
					if($_POST['student_name']!='')
					{
						$sel_sms_cnt="select name,contact from enrollment where enroll_id='".$_POST['student_name']."'";
						$ptr_sel_sms1=mysql_query($sel_sms_cnt);
						$ptr_sel_sms=mysql_fetch_array($ptr_sel_sms1);
						$mb_no=$ptr_sel_sms['contact'];
					}
					if($_POST['mobile_no']!='')
					{
					...
					send_sms_function($mb_no,addslashes($_POST['text']),$_POST['sms_type']);
					$insert_sms="insert into sms_log_history (`name`,`mobile_no`,`sms_text`,`sms_type`,`cm_id`,`added_date`) values('".$data_sel_cnt['name']."','".$mb_no."','".$_POST['text']."','".$_POST['send_by']."','".$cm_id."','".$start_date."')";
				...
				if($_POST['send_by']=='email')
				{
					if($_POST['staff_name']!='')
					{
						$sel_sms_cnt="select name,email from site_setting where admin_id='".$_POST['staff_name']."'";
						$ptr_email=mysql_query($sel_sms_cnt);
						$data_sel_sms=mysql_fetch_array($ptr_email);
						$email=$data_sel_sms['email'];
					}
					if($_POST['student_name']!='')
					{
						$sel_sms_cnt="select name,mail from enrollment where enroll_id='".$_POST['student_name']."'";
						$ptr_email=mysql_query($sel_sms_cnt);
						$data_sel_sms=mysql_fetch_array($ptr_email);
						$email=$data_sel_sms['mail'];
					}
					if($_POST['mobile_no']!='')
					{
						$email=$_POST['mobile_no'];
					}
					$insert_sms="insert into sms_log_history (`name`,`mobile_no`,`sms_text`,`sms_type`,`cm_id`,`added_date`) values('".$data_sel_cnt['name']."','".$email."','".addslashes($_POST['mail_text'])."','".$_POST['send_by']."','".$cm_id."','".$start_date."')";
					$ptr_sms=mysql_query($insert_sms);
					...
					{					
						...
						$email_from="infoisas@isasbeautyschool.com";
						...
						{
							$imgbelow="<br><br><img width='500' height='500' src='http://isasbeautyschool.org/faculty_login/email_photo/".$uploaded_url."'><br><br>";
						}
						else if($_POST['alignment']=='above')
						{
							$imgabove="<img width='500' height='500' src='http://isasbeautyschool.org/faculty_login/email_photo/".$uploaded_url."'><br><br>";
						}
						$email_message .="<div style='border: solid black;padding:10px;'><b>".$imgabove." ".$text." ".$_POST['text']."</b>".$imgbelow."</div>";
		...
		if($success==0)
		{
            ...
            <tr>
                <td>
                    ...
                    <tr>
                        <td><font color="red">*</font><b>Enter Mobile No. / Email Address</b></td>
                        <td>
                            <input type="text" name="mobile_no" id="mobile_no" class="input_select" value=""/>
                        </td>
                        <td width="30%">
                        </td>
                    </tr>
                    <tr>
                        <td><font color="red">*</font><b>Select Staff</b></td>
                        <td>
                            <?php 
                            $sel_branch = "SELECT admin_id,contact_phone,name FROM site_setting where 1 order by admin_id asc ";	 
                            $query_branch = mysql_query($sel_branch);
                            $total_Branch = mysql_num_rows($query_branch);
                            echo '<table width="100%"><tr><td>';
                            echo ' <select id="staff_name" name="staff_name" class="input_select">';
                             echo '<option value="">Select Staff </option>';
                            while($row_branch = mysql_fetch_array($query_branch))
                            {
                                ?>
                                <option value="<?php echo $row_branch['admin_id'];?>" ><? echo $row_branch['name']; ?> 
                                </option>
                                <?php
                            }
                            echo '</select>';
                    ...
                    <tr>
                        <td><font color="red">*</font><b>Select Student</b></td>
                        <td>
                            <?php 
                            $sel_branch1="SELECT enroll_id,contact,name FROM enrollment where 1 order by enroll_id asc ";	 
                            $query_branch1= mysql_query($sel_branch1);
                            $total_Branch1= mysql_num_rows($query_branch1);
                            echo'<table width="100%"><tr><td>';
                            echo'<select id="student_name" name="student_name" class="input_select">';
                            echo'<option value="">Select Student </option>';
                            while($row_branch1 = mysql_fetch_array($query_branch1))
                            {
                                ?>
                                <option value="<?php echo $row_branch1['enroll_id'];?>" ><? echo $row_branch1['name']; ?> 
                                </option>
                                <?php
                            }
                            echo '</select>';
                    ...
                    <tr>
                        <td colspan="3">
                            <table id="for_mail" style="display:none;">
                                <tr>
                                    <td width="23%" valign="top">Message <!--span class="orange_font">*</span --></td>
                                    <td colspan="2">
                    ...
                    <tr>
                        <td colspan="3" >
                            <table id="photo_id" style="display:none" width="100%">
                                <tr>
                                ...
                                <tr>
                                    ...
                                    <option value="">Select Image allignment</option>
                    ...
                    <tr>
                        <td>&nbsp;</td>
                        <td>
                            <input type="submit" id="mySubmit" class="input_btn" onclick="return validme()" value="Send" name="save_changes"  />
                        </td>
                        <td></td>
                    </tr>
                    <?php
