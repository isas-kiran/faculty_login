...
<body>
    <div class="heightSpacer"></div>
                        <?php
						  ...
						  $sql_records= "SELECT * from package_invoice where invoice_id ='".$record_id."'";
					    ...
					    $sql_record= "SELECT * FROM sales_package_voucher_memb where id='".$data_bill_master['v_id']."'";
						$_SESSION['sql_articles']=$sql_record;
						if(mysql_num_rows($db->query($sql_record)))
						$row_record=$db->fetch_array($db->query($sql_record));
						
						$select_customer = " select cust_name, cust_id from customer where cust_id='".$row_record['cust_id']."' ";
						$ptr_customer=mysql_query($select_customer);
						$data_customer = mysql_fetch_array($ptr_customer);
						 //=============================================================
						 	 $sql_records_invoice= "SELECT * FROM `package_invoice` WHERE v_id='".$data_bill_master['v_id']."' and (invoice_id < ".$record_id.") order by invoice_id desc limit 1";
...
<table align="center" border="0" width="450" style="border-left:solid 0.6mm; border-right:solid 0.6mm; border-top:solid 0.6mm;border-top-radius:5px;">
        ...
        <?php
		 ...
		 {
		 ?>
        <a href="invoice_generate_cust_serv.php?record_id=<?php echo $record_id ?>&action=print" style="text-decoration:none"><input type="button" name="print" value="Print" /></a>
        <?php } ?>
                        ...
                        <table align="center" border="1px"  width="450" cellpadding="0" cellspacing="0"
                                        ...
                                        <tr>
                                              ...
                                              <tr>
                                              	<td width="" height="10" style="padding-left:20px; font-size:16px;" > <strong>Customer Name :</strong></td>
                                                <td width="" height="10" style="font-size:16px;"> <strong><?php echo $data_customer['cust_name']; ?></strong> </td>
                                              </td>
                                              ...
                                              <tr><td width="" height="10" style="padding-left:20px; font-size:16px;" ><strong>Receipt No :</strong> <strong><?php echo $invoice_no; ?></strong></font></h3></td></tr>
											  <tr><td width="" height="10" style="padding-left:20px; font-size:16px;" ><strong>Invoice No : <?php echo $row_record['id']; ?></strong></font></h3></td></tr>
                                              <tr><td colspan="4"> <h3 align="center"><font size="+1">Price Details</font></h3></td></tr>
                                              </table>
                             ...
                             <tr>
                          		<td width="21%" align="left"  style="padding-left:10px; width:25%"><span style="font-size:16px; font-weight:bold"><?php echo $row_record['category']; ?> Name</span></td>
								<?php if($row_record['category']=='Package')
								{
								$p= "SELECT * from package where package_id ='".$row_record['package_id']."'";
								$pk =mysql_query($p);
								$pkg=mysql_fetch_array($pk);	
								$name=$pkg['package_name'];
								}	
								else
								{
								 $v= "SELECT * from voucher where voucher_id ='".$row_record['voucher_id']."'";
								$vc =mysql_query($v);
								$vcn=mysql_fetch_array($vc);	
								$name=$vcn['deal_name'];	
								}?>
                           		<td width="26%" width:20% align="center"><font size="4px;"><?php echo $name; ?></font></td>
