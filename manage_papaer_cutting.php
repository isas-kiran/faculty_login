    ...
    <?php       if($_POST['formAction'])
                    {
                        if($_POST['formAction']=="delete")
                        {
                            for($r=0;$r<count($_POST['chkRecords']);$r++)
                            {
                                ...
                                $sql_query= "SELECT  	gallery_id FROM ".$GLOBALS["pre_db"]."paper where  	gallery_id ='".$del_record_id."'";
                                if(mysql_num_rows($db->query($sql_record)))
                                    {                                                
                                        $delete_query="delete from ".$GLOBALS["pre_db"]."paper where gallery_id='".$del_record_id."'";
						...
						else if($_POST['formAction']=="Active")
                        {
                            for($r=0;$r<count($_POST['chkRecords']);$r++)
                            {
                                ...
                                $update_record= "update ".$GLOBALS["pre_db"]."paper set status='Active' where gallery_id='".$update_record_id."'";
                        ...
                        else if($_POST['formAction']=="Inactive")
                        {
                            ...
                            {
                                $update_record_id=$_POST['chkRecords'][$r];
                                $update_record= "update ".$GLOBALS["pre_db"]."paper set status='Inactive' where gallery_id='".$update_record_id."'";
                                $db->query($update_record);
                            }
                        ...
                        else if($_POST['formAction']=="Banned")
                        {
                            ...
                            {
                                $update_record_id=$_POST['chkRecords'][$r];
                                $update_record= "update ".$GLOBALS["pre_db"]."paper set status='Banned' where gallery_id='".$update_record_id."'";
                                $db->query($update_record);
                            }
                    ...
                    {
                        $update_query1="update ".$GLOBALS["pre_db"]."paper set status='".$_REQUEST['value']."' where gallery_id='".$_REQUEST['changeStatus']."'";
                        //echo $update_query1;
  ...
  <tr class="head_td">
    ...
    <?php
                        ...
                        if($_GET['order'] !='' && ($_GET['orderby']=='title' || $_GET['orderby']=='event_date' ))
                        {
                            ...
                            $sql_query= "SELECT * FROM bp_paper where 1  ".$pre_keyword." ".$select_directory.""; 
                       //echo $sql_query;
    ...
    <form method="post"  name="frmTakeAction" action="<?php echo $_SERVER['PHP_SELF'].'?#msgbox';?>" >
                        ...
                        <td width="14%" class="centerAlign"><strong>Action</strong></td>
                            ...
                            while($val_query=mysql_fetch_array($all_records))
                            {
                                ...
                                $listed_record_id=$val_query['gallery_id']; 
								 ...
								 $sql_record1= "SELECT img_id FROM ".$GLOBALS["pre_db"]."cutting_images where gallery_id='".$val_query['gallery_id']."'";
                                $total_images=mysql_num_rows($db->query($sql_record1));
								if($total_images !=0)
								$total_images = "<a href='manage_cutting.php?gallery_id=$listed_record_id'>$total_images</a>";
                                ...
                                echo '<td ><a href="add_papaer_cutting.php?record_id='.$listed_record_id.'" >'.$val_query['title'].'</a></td>';
                                echo '<td >'.$post.'</td>';
					...
					echo '</td>';
                                ...
                                echo '<td align="center"><a href="add_papaer_cutting.php?record_id='.$listed_record_id.'" ><img src="images/edit_icon.png" title="Edit Record" class="example-fade"/></a>&nbsp;&nbsp;
