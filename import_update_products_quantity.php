				...
				{					
					...
					{		
						...
						for($i=2;$i<=$data->sheets[0]['numRows'];$i++)
						{
							...
							"<br />Quantity In system:".$qty_system=trim($data->sheets[0]['cells'][$i][3]);
							"<br />Quantity in stock:".$qty_stock=trim($data->sheets[0]['cells'][$i][4]);
							"<br />Product ID:".$product_id=trim($data->sheets[0]['cells'][$i][5]);
							...
							if(mysql_num_rows($ptr_product_cat))
							{
								if($product_id !='')
								{
									echo"<br/>".$insert_product="update `product` set `quantity` =".$qty_stock." where `product_id` =".$product_id." ";
