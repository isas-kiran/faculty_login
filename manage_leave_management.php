...
<title>Manage Leave Management</title>
...
<?php
$edit_access='';
$sel_acc="select * from edit_previleges where admin_id='".$_SESSION['admin_id']."' and privilege_id='210'";
  ...
  <tr>
    ...
    <?php if($_POST['formAction'])

                    {
                        ...
                        if($_POST['formAction']=="delete")
                        {
                            for($r=0;$r<count($_POST['chkRecords']);$r++)
                            {
                                ...
                                $sql_query= "SELECT leave_id FROM ".$GLOBALS["pre_db"]."pr_leave_management where leave_id ='".$del_record_id."'";
                                if(mysql_num_rows($db->query($sql_query)))
                                {
									"<br>".$sql_query= "SELECT * FROM pr_leave_management where leave_id ='".$del_record_id."' ";              
									$query=mysql_query($sql_query);
									$fetch=mysql_fetch_array($query);
										
									"<br>".$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('leave_mgmt','Delete','leaves','".$del_record_id."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
									$query=mysql_query($insert);       
									                                     
                                    $delete_query="delete from ".$GLOBALS["pre_db"]."pr_leave_management where leave_id='".$del_record_id."'";
                    ...
                    if($_REQUEST['deleteRecord'] && $_REQUEST['record_id'])
                    {
                        ...
                        if(mysql_num_rows($db->query($sql_query)))
                        {
							"<br>".$sql_query= "SELECT name FROM pr_leave_management where leave_id ='".$del_record_id."' ";              
							$query=mysql_query($sql_query);
							$fetch=mysql_fetch_array($query);
...
<table cellspacing="0" cellpadding="0" class="table" width="95%">
  ...
  <tr class="head_td">
    ...
    <?php
                        ...
                        if($keyword !='')
                            ...
                            {                            
                               ...
                               $pre_keyword="and (branch_name like '%".$keyword."%' or year like '%".$keyword."%' or  month like '%".$keyword."%')";
                        ...
                        if($_GET['order'] !='' && ($_GET['orderby']=='course_name'))
                        {
                            ...
                            $select_directory='order by leave_id desc';                      
                        ...
                        $sql_query= "SELECT * FROM pr_leave_management where 1 ".$_SESSION['where']." ".$pre_keyword." ".$select_directory.""; 
    ...
    <td width="7%"><strong>Number Of Working Days</strong></td>
    <td width="7%"><strong>Number Of Holidays</strong></td>
  	<?php if($_SESSION['type']=='S' ||  $edit_access='yes')
	{ ?>
	...
	while($val_query=mysql_fetch_array($all_records))
    {
                                ...
                                if($bgColorCounter%2==0)
                                    ...
                                    $bgcolor="";                
                               $listed_record_id=$val_query['leave_id']; 
                                ...
                                */
								...
								echo '<td >'.$month.'</td>';
                                echo '<td >'.$val_query['no_of_days'].'</td>';
								echo '<td >'.$val_query['no_of_working_days'].'</td>';
								echo '<td >'.$val_query['no_of_holidays'].'</td>';						
								if($_SESSION['type']=='S' ||  $edit_access='yes')
								{   
									echo '<td align="center"><a href="leave_management.php?record_id='.$val_query['leave_id'].'"><img src="images/edit_icon.png" height="20px" width="20px" title="Edit Event" class="example-fade"/></a>&nbsp;
