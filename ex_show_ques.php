	...
	$ques_concat="and question_title like '%".$question_title."%'";
		...
		while($data_unit=mysql_fetch_array($ptr_unit))
		{
			...
			if($total_contact && $record_id =='')
			{
				...
				echo "<table><tr><td valign='top'>Select Question from ".$data_unit['topic_name']."<span class='orange_font'>*</span></td><td colspan='2'> Select all questions - &nbsp;&nbsp;<input type='checkbox' name='Check_All' id='Check_All' value='Check All' onClick='CheckAll(document.jqueryForm.chapter_id".$data_unit['topic_id'].")'></td></tr>";
				echo "<tr><td width='9%'></td><td width='41%'>";
				$member_result='';
				...
				if($total_contact = mysql_num_rows($query_contact))
				{
					...
					{
						echo  "<td id='qs".$row_contact['question_id']."'><input type='checkbox' name='chapter_id[]' onclick='select_me(\"qs".$row_contact['question_id']."\")' value='".$row_contact['question_id']."' id='chapter_id".$data_unit['topic_id']."' class='case ".$row_contact['question_id']."' style='vertical-align:top'><input type='hidden' name='split'>".$row_contact['question_id'].". ".$row_contact['question_title']." ";
				...
				while($row_contact = mysql_fetch_array($query_contact))
					{
						...
						if($tot_ques !=0 || $tot_ques !='')
						{
							...
							echo  "<td id='qs".$row_contact['question_id']."'><input type='checkbox' name='chapter_id[]' onclick='select_me(\"qs".$row_contact['question_id']."\")' ".$checked."  value='".$row_contact['question_id']."' id='chapter_id".$data_unit['topic_id']."' class='case ".$row_contact['question_id']."' style='vertical-align:top'/><input type='hidden' name='split'>".$row_contact['question_id'].". ".$row_contact['question_title']." ";
						echo "<input type='hidden' name='split'><input type='hidden' name='selected_unit' id='selected_units".$row_contact['question_id']."' value='".$data_unit['topic_name']."' >";
