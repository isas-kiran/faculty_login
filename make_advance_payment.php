...
$sel_course="select * from pr_staff_advance_salary_management where staff_advance_salary_id='".$record_id."'";
$ptr_course=mysql_query($sel_course);
$data_course=mysql_fetch_array($ptr_course);
$assigned_to=$data_course['assigned_to'];
$course_name="select * from site_setting where attendence_id='".$data_course['staff_id']."'";
$ptr_c_name=mysql_query($course_name);
$data_cnama=mysql_fetch_array($ptr_c_name);

$staff_name=$data_cnama['course_name'];
?>
    					...
    					<?php
    						if($_POST['save_changes'])
                        	{
								...
								if($record_id)
								{
									...
									$insert_new_invoice = " INSERT INTO `invoice` (`enroll_id`, `course_id`, `amount`,`balance_amt`, `paid_type`, `cust_bank_name`,`bank_name`, `cheque_detail`, `chaque_date`, `credit_card_no`,`online_transc_details`, `admin_id`, `added_date`,`status`,`cm_id`,`cgst_tax_in_per`,`cgst_tax`,`sgst_tax_in_per`,`sgst_tax`,`total_paid_gst`,`assigned_to`) VALUES ('$enroll_id', '$course_id', '$amount_paid','".$data_record['balance_amt']."', '".$payment_type."','".$cust_bank_name."', '".$_POST['bank_name']."', '".$chaque_no."', '".$chaque_date."','".$bank_name."', '".$_POST['online_transc_details']."', '".$_SESSION['admin_id']."', '".$data_record['added_date']."','$status','".$cm_id1."','".$cgst_tax_in_per."','".$cgst_tax."','".$sgst_tax_in_per."','".$sgst_tax."','".$total_paid_gst."','".$assigned_to."'); ";
                                    ...
                                    while($val_record=mysql_fetch_array($all_records))
                                    {
                                       ...
                                       <?php }
										...
										?><tr>
											...
											<td width="49%"><input type="text" id="added_date" name="added_date" class="input_text datepicker" value="<?php if($_POST['added_date']) echo $_POST['added_date']; else if($row_record['added_date']!=''){$arrage_date= explode('-',$row_record['added_date'],3);echo $arrage_date[1].'/'.$arrage_date[2].'/'.$arrage_date[0];}else{echo date("d/m/Y");}?>" />
											</td>
										</tr> <?php
										...
										$phone_no ='';
									  	...
									  	$select_firstname = " select * from site_setting where attendence_id='".$data_course['staff_id']."' ";
									  	$ptr_query=mysql_query($select_firstname);
										$data_select = mysql_fetch_array($ptr_query);
										
										
										echo '<tr><td><strong>Staff Name</strong></td><td align="left" style="padding-left:5px;"><b>'.$data_select['name'].'
                                        ...
                                        echo '<tr><td><strong>Total</strong></td><td align="left">'.$data_course['advance_payment'].' <input type="hidden" name="total" id="total" value='.$data_course['advance_payment'].'></td></tr>';
                                      
									    $paid=$data_select['paid'];
										
										echo '<tr><td><strong>Number Of Installments</strong></td><td align="left">'.$data_course['no_of_installments'].'<input type="hidden" name="down_payment" id="down_payment" value='.$data_course['no_of_installments'].'></td></tr>'; 
										if($data_select['paid'] !='')
										{
									   ...
									   ?> 
										<tr><td width="37%"><strong>Deposite Amount</strong></td><td width="63%" align="left"><input type="text" name="amount_paid" id="amount_paid" onKeyUp="calculate_total(this.value,0)"	value="" > </td></tr>
                                        ...
                                        <table width="95%" align="center">
                                        	...
                                        	<td >
												...
												$i=$data_select['no_of_installments'];
