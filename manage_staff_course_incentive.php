...
<title>Manage Staff Course Management</title>
    ...
    <?php if($_POST['formAction'])

                    {
                        ...
                        if($_POST['formAction']=="delete")
                        {
							...
							echo 	"</br>".$month=$val[1];
							echo	"</br>".$year=$val[2];
                            echo    $sql_query= "SELECT staff_id,month,year FROM ".$GLOBALS["pre_db"]."pr_staff_course_incentive where staff_id ='".$del_record_id."' AND month='".$month."' AND year='".$year."'";
                                if(mysql_num_rows($db->query($sql_query)))
                                {
									"<br>".$sql_query= "SELECT * FROM pr_staff_course_incentive where staff_id ='".$del_record_id."' AND month='".$month."' AND year='".$year."'";              
                                    ...
                                    $delete_query="delete from ".$GLOBALS["pre_db"]."pr_staff_course_incentive where staff_id='".$del_record_id."' AND month='".$month."' AND year='".$year."'";
                    ...
                    if($_REQUEST['deleteRecord'] && $_REQUEST['record_id'])
                    {
                        ...
                        $sql_query= "SELECT staff_id,year,month FROM ".$GLOBALS["pre_db"]."pr_staff_course_incentive where staff_id='".$del_record_id."' AND month='".$_REQUEST['month']."' AND year='".$_REQUEST['year']."'";
                        if(mysql_num_rows($db->query($sql_query)))
                        {
							"<br>".$sql_query= "SELECT * FROM pr_staff_course_incentive where staff_id ='".$del_record_id."' AND month='".$_REQUEST['month']."' AND year='".$_REQUEST['year']."'";              
							$query=mysql_query($sql_query);
							$fetch=mysql_fetch_array($query);
								
							"<br>".$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('manage_user','Delete','".$fetch['name']."','".$del_record_id."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
							$query=mysql_query($insert);                        
                        	$delete_query="delete from ".$GLOBALS["pre_db"]."pr_staff_course_incentive where staff_id='".$del_record_id."' AND month='".$_REQUEST['month']."' AND year='".$_REQUEST['year']."'";
                        	$db->query($delete_query);
  ...
  <tr class="head_td">
    ...
    <?php
						...
						if($_GET['record_id'] !='')
						{
						 ...
						 $sql_query= "SELECT DISTINCT u.attendence_id,u.name,o.staff_id,o.year,o.month FROM site_setting u, pr_staff_course_incentive o where ".$pre_keyword." AND o.staff_id=u.attendence_id"; 
                       // $sql_query= "SELECT DISTINCT staff_id,year,month FROM pr_staff_course_incentive"; 
   ...
   <td width="12%"><a href="<?php echo $_SERVER['PHP_SELF'];?>?order=<?php echo $order."&orderby=name".$query_string;?>" class="table_font"><strong>Staff Name</strong></a> <?php echo $img1;?></td>
    ...
    <td width="7%" class="centerAlign"><strong>Action</strong></td>
							...
							//	 echo '<td >'.$val_query['grand_total'].'</td>';
									echo '<td align="center"><a href="staff_course_incentive.php?record_id='.$val_query['staff_id'].'&month='.$val_query['month'].'&year='.$val_query['year'].'"><img src="images/edit_icon.png" height="20px" width="20px" title="Edit Record" class="example-fade"/></a>&nbsp;
