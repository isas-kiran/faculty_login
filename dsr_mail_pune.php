...
//}   
		...
		if($total_bank=mysql_num_rows($ptr_amnt1))
		{
			...
			while($data_ptr_sel1=mysql_fetch_array($ptr_amnt1))
			{
				...
				//24/10/20 $sel_expense="select SUM(amount) as total_amt from expense where bank_id='".$data_ptr_sel1['bank_name']."' and added_date='".$date."' ".$branch_id."";
				...
				$sele_yesterday_bal="select todays_balance from dsr_bank_summery where bank_id='".$data_ptr_sel1['bank_name']."' and DATE(added_date)=DATE_SUB('".$date."', INTERVAL 1 DAY) ".$branch_id."  order by added_date desc limit 0,1 "; //and added_date = '".$prv_date."'
...
//======================================================CASH SUMMARY ============================================================
 ...
 <tr style="background-color:#999;color: black">
	...
	<td width="10%" align="center" style="border:1px solid #CCC"><strong>Total Cash Expenses From Petty Cash</strong></td>
...
//					}
			...
			//--------------------------------------------------------------------------------------------------------------
				...
				$cash_in_hands=$opening_cash1 + $data_ptr_sel['total_amt'] +$data_sel_recipt['total_amt'] +$total_service +$data_sales_product['total_amt']+$data_cash_received_sir['total_amt'] +$data_sel_voucher['total_amt']+$data_sales_membership['total_amt']+$data_sales_package['total_amt']+$data_sales_voucher['total_amt']+$data_cash_rec['total_amt']-$tam-$cash_transfer_in_petty_cash- $data_cash_from_sir_business['total_amt'] - $tota_exp_business -$tots ;//+ $data_cash_from_innocent['total_amt'] - $tt  - $data_total_expense['total_amt'] (3/6/19)
				//===========================================
				//=========================================PETTY CASH IN HAND==================================================
...
$message.='<tr>
 ...
 <table cellspacing="2" cellpadding="2" width="100%" style="border:1px solid #999">
	...
	<td width="12%" align="center" style="border:1px solid #CCC"><strong>Businees Booked</strong></td>
		...
		$data_total_inc=mysql_fetch_array($ptr_total_inc);
		//============================================
		$sel_total_servicec="select SUM(payable_amount) as total_amt from customer_service where payable_amount>'0' and DATE(`added_date`) >= '".$date_for_month."-01' and DATE(`added_date`) <= '".date('Y-m-d')."' ".$branch_id."";
		$ptr_total_service=mysql_query($sel_total_servicec);
		$data_total_service=mysql_fetch_array($ptr_total_service);
...
$message.='</table>';
						...
						$subject = " DSR Report for Pune Branch";
						...
						try {
							...
							try {
									...
									///usr/local/bin/php -q /home/isasadmin007/isastest/faculty_login/dsr_mail.php?&send_mail=mail
									///bin/touch /home/isasadmin007/public_html/cron_test.txt >/dev/null 2>&1 && /bin/echo "Cron ran at: `date`" >> /home/isasadmin007/public_html/cron_test.txt
									$mail->Subject = 'DSR Report For Pune Branch- '.$date.'';
