  ...
  <tr>
    ...
    <td class="mid_mid">
    						...
    						if($_POST['save_changes'])
                        	{
                                ...
                                {
									$success=1;
									$insert_followup = "INSERT INTO `installment_followup_details` (`student_id`,`lead_category_followup`,  `followup_date`, `lead_grade`,`followup_details`,`response`,`added_date`,`cm_id`,`admin_id`) VALUES ('".$student_id."','".$lead_category_followup."', '".$followup_date."', '".$lead_grade."','".$followup_details."','".$response."','".date('Y-m-d H:i:s')."','".$cm_id."','".$admin_id."')";
									...
									setTimeout('document.location.href="installment_followup_details.php?record_id=<?php echo $record_id; ?>";',1000);
									</script>
								...
								?>
                                    ...
                                    <?
										...
										$select_followup = "select * from installment_followup_details where student_id='".$record_id."' order by added_date desc limit 0,1 ";
...
<table cellspacing="0" cellpadding="0" class="table" width="95%">
  ...
  <tr class="head_td">
    ...
    <?php
						...
						"<br/>". $sql_query= "select enroll_id,name,contact,mail,course_id,added_date,installment_followup_date from enrollment where installment_followup_date!='' and installment_followup_date >= '".date('Y-m-d')."' and installment_followup_date <='".date('Y-m-d', strtotime('+2 days'))."' ".$_SESSION['where']." ".$pre_from_date." ".$pre_to_date." ".$pre_lead_grade." order by enroll_id asc"; 
    ...
    <td width="15%" align="center"><strong>Lead Grade</strong></td>
     ...
     <td width="10%" align="center"><strong>Added Date</strong></td>
                            ...
                            while($val_query=mysql_fetch_array($all_records))
                            {
								...
								$sele_latest_rec="select followup_date,lead_category_followup, lead_grade,added_date,cm_id,admin_id from installment_followup_details where student_id = '".$val_query['enroll_id']."' and followup_date >= '".date('Y-m-d')."' and followup_date <='".date('Y-m-d', strtotime('+2 days'))."' order by followup_id desc";
