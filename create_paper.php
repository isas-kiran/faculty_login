...
if($_REQUEST['record_id'])
{
    ...
    $sql_record= "SELECT * FROM paper where paper_id='".$record_id."'";
    if(mysql_num_rows($db->query($sql_record)))
    $row_record=$db->fetch_array($db->query($sql_record));
    else
    $record_id=0;
	
	 $select_question="select * from question_subject_topic_map where paper_id='".$record_id."'";
	$query=mysql_query($select_question);
    $row_record_question=mysql_fetch_array($query);
	 $row_record_question['selected_question'];
}
...
<script>
	 function show_subject(course_id)
	 {
		...
		if(course_id !='')
		{
		  ...
		  var data1="course_id="+course_id;
				   //alert(data1);
					 $.ajax({
				url: "get-subject_course.php", type: "post", data: data1, cache: false,
				success: function (html)
					{
						
						 document.getElementById('subject_new').innerHTML=html;
					}
				});
  ...
  <tr>
						...
						else 
						{
							...
							else
							{
								...
								$paper_date=$tan_date[2].'-'.$tan_date[0].'-'.$tan_date[1];
								 ...
								 '<br/>'.$insert_query = "insert into paper (`admin_id`,`exam_id`,`add_class_id`,`subject_id`,`course_id`,`paper_title`,`description`,`select_question`,`paper_time`,`paper_date`,`paper_start_timing`,`total_time`,`total_mark`,`created_date`,`exam_status`) values('".$_SESSION['admin_id']."','".$_POST['exam_id']."','".$_POST['add_class_id']."','".$_POST['subject_id']."',
							 ...
							 // echo $_POST['quation_id'.$i] ;
							  ...
							  // echo $_POST['chapter_id'][0];
							   for($h=0;$h<count($_POST['chapter_id']);$h++)
								{
									 '<br/>'.$insert_into_question_subject_topic_map = " insert into question_subject_topic_map(`paper_id`,`subject_id`, `topic_id`,`selected_question`)
	...
	<table border="0" cellspacing="15" cellpadding="0" width="100%">
            ...
            <tr>
                ...
                <td width="40%">
                  <input type="text" name="paper_title" id="paper_title"  class="input_text" value="<?php if($_POST['save_changes']) echo $_POST['paper_title']; else echo $row_record['paper_title'];?>"/> <td width="40%"></td>
            </tr>
             <tr>
                <td width="20%">Discription </td>
                <td width="40%">
                     <textarea name="description" rows="1" class="input_textarea" cols="50" ><?php if($_POST['save_changes']) echo $_POST['description']; else echo $row_record['description'];?></textarea></td>
            </tr>
            ...
            <td width="40%" >
                    <select name="course_id" id="course_id" class="validate[required] input_select" onchange="show_subject(this.value);">  
                        ...
                        <?php
														   ...
														   while($data_cat = mysql_fetch_array($ptr_course_cat))
														   {
															...
															{
																$selected='';
																if($row['course_id']==$row_record['course_id'])
             ...
             <tr><td width="50%" colspan="3"><div id="subject_new"> </div></td></tr>
             
             <?php
			  if($record_id)
			  { ?>
              <script>
				  show_subject(<?php echo $row_record['course_id'] ?>)
				  </script>
			  <?php }
			  ?>
            ...
            <tr>
                ...
                <td width="40%">
                    <input type="radio" name="select_question" onclick="hide();" <?php if($row_record['select_question']=='automatic') { echo 'checked="checked"'; }  ?> value="automatic"  id='select_question'/>Automatic   &nbsp; &nbsp; 
                    <input type="radio" id='select_question' name="select_question" onclick="show();" <?php if ($row_record['select_question']=='manual') { echo 'checked="checked"'; } ?> value="manual"  />Manual  </td> 
                <td width="40%"></td>
            </tr>
            ...
            <tr>
                ...
                <td width="40%" ><input type="text" name="total_mark" id="total_mark" class="input_text" value="<?php if($_POST['save_changes']) echo $_POST['total_mark']; else echo $row_record['total_mark'];?>" style="width:140px" /></td> 
                <td width="40%"></td>
            </tr>
             <tr>
                <td width="20%">Total Time<span class="orange_font">*</span></td>
                <td width="40%" ><input type="text" name="total_time" id="total_time" class="input_text" style="width:140px" value="<?php if($_POST['save_changes']) echo $_POST['total_time']; else echo $row_record['total_time'];?>" /></td> 
                <td width="40%"></td>
            </tr>
             <tr>
                <td colspan="3">
           ...
           <!-- <tr>
            ...
            <tr>
                ...
                <td width="20%" colspan="2"><input type="text" name="total_quations" id="total_quations" class="input_text" style="width:140px"  value="<?php if($_POST['save_changes']) echo $_POST['total_quations']; else echo $row_record['total_quations'];?>" />	&nbsp; Selected Quetions &nbsp; 
                      <input type="text"  name="total_checked_question"id="total_checked_question" class="input_text" style="width:140px" onkeyup="cal_total()" value="<?php if($_POST['save_changes']) echo $_POST['total_checked_question']; else echo $row_record_question['selected_question'];?>" /> </td>
            </tr>
