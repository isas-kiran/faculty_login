...
<table border="0" cellspacing="0" cellpadding="0" width="100%">
  ...
  <tr>
	...
	if($_GET['action']=="delete")
	{
		...
		$id = $_GET['img_id'];
		$ok2 = mysql_query("DELETE FROM ".$GLOBALS["pre_db"]."cutting_images  WHERE img_id='$id'");
...
if($_POST['save'])
{
	 ...
	 if($icon !='')
	 {
	 $update_icons= "update ".$GLOBALS["pre_db"]."cutting_images set icon='' where gallery_id='$gallery_id' ";
	 $ptr_reset= mysql_query($update_icons);
	 
	 
	 $update_icon= "update ".$GLOBALS["pre_db"]."cutting_images set icon='y' where img_id='$icon'";
	 ...
	 for($j=1;$j<=$total_images;$j++)
	 {
		...
		if($_FILES['imge_name'.$j]['tmp_name'] !='')
		{
		 ...
		 $update_imges= " update ".$GLOBALS["pre_db"]."cutting_images set  image_tag='".addslashes($_POST['image_tag'.$j])."' $set_image , img_comment='".addslashes($_POST['img_comment'.$j])."' where  img_id='$img_id' ";
...
$catqry="Select * from ".$GLOBALS["pre_db"]."cutting_images where gallery_id='$gallery_id' ";
$strSQL=$catqry;
...
else
{
	...
	echo "<td align='center'><a href='manage_cutting.php?gallery_id=$gallery_id&img_id=$img_id&action=delete' onclick=\"return confirm('do you really want to delete this image');\"><img src='../images/delete.png' border='0' /></a></td>";
	echo("</tr>");
	$i++;
	}
	$total_imgsss= $i-1;
