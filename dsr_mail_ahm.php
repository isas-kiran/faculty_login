...
<title>DSR Mail Ahmadabad</title>
...
$exp_branch_id="and i.cm_id = '60'";//For expense_invoice and expense Join query
$cm_id1= '60';
?>
<script>
...
$message.=' <td align="left" colspan="10" style="color:#F00"><strong>Total Incomming</strong> :'.$total_todays_bal.'</td> 
						...
						<td width="15%" align="center" style="border:1px solid #CCC"><strong>Sale Type</strong></td>
						<td width="15%" align="center" style="border:1px solid #CCC"><strong>Student Name</strong></td>
						...
						<td width="15%" align="center" style="border:1px solid #CCC"><strong>Chaque No</strong></td>
 					...
 					<tr class="grey_td" style="background-color:#999;color: black">
                        ...
                        if($no_of_records)
                        {
                            ...
                            while($val_query=mysql_fetch_array($ptr_db))
                            {
								...
								$sel_sales_products="select * from sales_product where sales_product_id='".$val_query['sales_product_id']."'";	
...
$sele_recep="select receipt_id,category,payment_mode_id,bank_id,amount,added_date,emp_type,description from receipt where 1 and category!='cash_transfer' and cash_transfer_mode is NULL and added_date='".$date."' ".$branch_id."";
...
<tr>
 <td align="center" colspan="10">&nbsp;</td>
  ...
  <tr>';
  	...
  	$select_inv="select SUM(payable_amount) as total from expense_invoice where DATE(added_date)='".$date."' ".$branch_id."";
 ...
 <tr style="background-color:#999;color: black">
	...
	if(mysql_num_rows($ptr_exp))
	{
		while($data_expense_inv=mysql_fetch_array($ptr_exp))
		{
			...
			$message.= '<td align="center" style="border:1px solid #CCC">'.$data_bank_name['bank_name'].'</td>';
			$message.= '<td align="center" style="border:1px solid #CCC">'.$data_bank_name['account_no'].'</td>';
			$message.= '<td align="center" style="border:1px solid #CCC">'.$data_expense_inv['payable_amount'].'</td></tr>';
		$i++;
		}
	}
	...
	if(mysql_num_rows($ptr_recep))
	{
		while($data_recep=mysql_fetch_array($ptr_recep))
		{
			...
			$message.= '<tr>';
			$message.= '<td align="center" style="border:1px solid #CCC">'.$i.'</td>';       
			$message.= '<td align="center" style="border:1px solid #CCC">'.$data_recep['category'].' '.$cash_modes.'</td>';
			$message.= '<td align="center" style="border:1px solid #CCC">--</td>';
			$message.= '<td align="center" style="border:1px solid #CCC">'.$data_pay_name['payment_mode'].'</td>';
			$message.= '<td align="center" style="border:1px solid #CCC">--</td>';
			$message.= '<td align="center" style="border:1px solid #CCC">'.$data_bank_name['bank_name'].'</td>';
	...
	if(mysql_num_rows($ptr_exp))
	{
		while($val_query=mysql_fetch_array($ptr_exp))
		{
			...
			$sele_pay_name="select payment_mode from payment_mode where payment_mode_id='".$val_query['paid_type']."'";
			$ptr_pay_name=mysql_query($sele_pay_name);
			$data_pay_name=mysql_fetch_array($ptr_pay_name);
...
{
$message.= '<tr><td class="text" align="center" colspan="10"><br><div class="msgbox" style="width:50%;background-color:green;color:white">No Receipt added on today</div><br></td></tr>';
}
$message.='</table>
   </td>
  </tr>';
...
//}   
		...
		$sel_inv1="select DISTINCT(bank_id) as bank_name from bank where 1 ".$branch_id."  and status='Active' and show_in_report!='No'";
		$ptr_amnt1=mysql_query($sel_inv1);
		if($total_bank=mysql_num_rows($ptr_amnt1))
		{
			...
			while($data_ptr_sel1=mysql_fetch_array($ptr_amnt1))
			{
				...
				$sele_yesterday_bal="select todays_balance from dsr_bank_summery where bank_id='".$data_ptr_sel1['bank_name']."' and DATE(added_date)=DATE_SUB('".$date."', INTERVAL 1 DAY) ".$branch_id."  order by dsr_bank_id desc limit 0,1 "; //and added_date = '".$prv_date."'
...
$message.='  </table>
 ...
 <tr style="background-color:#999;color: black">
    ...
    <td width="12%" align="center" style="border:1px solid #CCC"><strong>Cash Deposited in Bank From Business Cash</strong></td>
    ...
    // <td width="15%" align="center" style="border:1px solid #CCC"><strong>Cash Received from Innocent</strong></td>
				...
				$message.= '<td align="center" style="border:1px solid #CCC">'.$data_cash_from_sir_business['total_amt'].'<br/><hr style="color: #1395D2">Cash Given to Director From Petty Cash<br/>'.$data_cash_from_sir_petty['total_amt'].'</td>';
				$message.= '<td align="center" style="border:1px solid #CCC">';
				
				//======================================Cash Deposited in Bank From Business Cash=========================================
...
$message.='<tr>
 ...
 <table cellspacing="2" cellpadding="2" width="100%" style="border:1px solid #999">
	...
	<td width="12%" align="center" style="border:1px solid #CCC"><strong>Businees Booked</strong></td>
		...
		//==================================================
		$sel_total_recp="select SUM(amount) as total_amt from receipt where category='incoming' and cash_type='business_cash' and DATE(`added_date`) >='".$date_for_month."-01' and DATE(`added_date`) <= '".$date."' ".$branch_id."";
		...
		$message.= '<td align="center" style="border:1px solid #CCC">'.round($data_total_bus['total_amt']).'</td>';
		$message.= '</tr>';
		$message.='</table>
   </td>
  </tr>';
...
$message.='   <tr>
  ...
  $message.='</table>';
						...
						try {
								...
								$mail->Subject ='DSR Report For Ahmedabad Branch- '.$date.'';
							    ...
							    $mail->Send();
								echo "Email Sent to Ahmedabad Successfully.";
                        ...
                        <?php
						$sel_dsr="select dsr_id from dsr where added_date='".$date."' and cm_id='".$cm_id1."'";
						...
						$sel_bank_id="select bank_id from bank where cm_id='".$cm_id1."' and status='Active' ";
						...
						if(mysql_num_rows($ptr_bank_ids))
						{
							while($data_bank_id=mysql_fetch_array($ptr_bank_ids))
							{
								...
								$ptr_expense_bank_id=mysql_query($sel_expense);
								$data_sel_expense=mysql_fetch_array($ptr_expense_bank_id);// changes on 29-2-20*/
								
								//$sel_expense="select SUM(amount) as total_amt from expense where bank_id='".$data_bank_id['bank_id']."' and added_date='".$date."' ".$branch_id."";
								$sel_expense="select SUM(payable_amount) as total_amt from expense_invoice where bank_id='".$data_bank_id['bank_id']."' and DATE(added_date)='".$date."' ".$branch_id." ";
								$ptr_expense_bank_id=mysql_query($sel_expense);
								$data_sel_expense=mysql_fetch_array($ptr_expense_bank_id);
								...
								$sel_exp="select SUM(payable_amount) as total_amt from inventory_invoice where bank_id='".$data_bank_id['bank_id']."' and DATE(added_date)='".$date."' ".$branch_id." ";
								...
								$sel_bank_out="select SUM(amount) as total_amt from receipt where from_bank_name='".$data_bank_id['bank_id']."' and category='cash_transfer' and cash_transfer_mode='bank_to_bank' and added_date='".$date."' ".$branch_id." ";
								$ptr_bank_out=mysql_query($sel_bank_out);
								$data_bank_out=mysql_fetch_array($ptr_bank_out);
								
								
								$sel_refund="select SUM(total_refund) as total_amt from enrollment_refund where bank_name='".$data_bank_id['bank_id']."' and DATE(added_date)='".$date."' ".$branch_id."";
								$ptr_refund_bank_id=mysql_query($sel_refund);
								$data_sel_refund=mysql_fetch_array($ptr_refund_bank_id);
								
								//====================================INCOMMING QUERY==================================================
								//Changes on 29-2-20
								$sel_total_inc="select SUM(amount) as total_amt from invoice where bank_name='".$data_bank_id['bank_id']."' and DATE(`added_date`) = '".$date."' ".$branch_id." ";
								$ptr_total_inc=mysql_query($sel_total_inc);
								$data_total_inc=mysql_fetch_array($ptr_total_inc);
								
								$sel_total_inc2="select SUM(amount) as total_amt from receipt where bank_id='".$data_bank_id['bank_id']."' and (cash_transfer_mode = 'outword' OR cash_transfer_mode IS NULL ) and added_date='".$date."' ".$branch_id." ";
								$ptr_total_inc2=mysql_query($sel_total_inc2);
								$data_total_inc2=mysql_fetch_array($ptr_total_inc2);
								
								$sel_total_inc3="select SUM(payable_amount) as total_amt from sales_product_invoice where bank_id='".$data_bank_id['bank_id']."' and DATE(added_date)='".$date."' ".$branch_id." ";
								$ptr_total_inc3=mysql_query($sel_total_inc3);
								$data_total_inc3=mysql_fetch_array($ptr_total_inc3);
								...
								$sel_total_inc6="select SUM(amount) as total_amt from receipt where bank_id='".$data_bank_id['bank_id']."' and category='cash_transfer' and cash_transfer_mode='bank_to_bank' and added_date='".$date."' ".$branch_id." ";
								$ptr_total_inc6=mysql_query($sel_total_inc6);
								$data_total_inc6=mysql_fetch_array($ptr_total_inc6);
...
//===================================================================================================================================	
								//CHANGES on 29-2-20							
								$total_incomming=$data_total_inc['total_amt']+$data_total_inc2['total_amt']+$data_total_inc3['total_amt']+$data_total_inc4['total_amt']+$data_total_inc5['total_amt']+$data_total_inc6['total_amt']; //DATE(added) = DATE_SUB(CURDATE(), INTERVAL 1 DAY)
																
								$total_outgoing=intval($data_sel_expense['total_amt']+$data_exp_purchase['total_amt']+$data_sel_recipt_out['total_amt']+$data_cc_expense['total_amt'] + $data_bank_out['total_amt'] + $data_sel_refund['total_amt']);
								...
								//echo "<br/>-".$total_incomming."--".$data_yesterday['todays_balance']."--".$total_outgoing;
//=========================================================================================================================================
		
								$sel_bank_sm="select dsr_bank_id from dsr_bank_summery where bank_id='".$data_bank_id['bank_id']."' and cm_id='".$cm_id1."' and added_date='".$date."'";
								$ptr_bank_sm=mysql_query($sel_bank_sm);
								if(mysql_num_rows($ptr_bank_sm))
								{
									$update_bank_sm="update dsr_bank_summery set incoming='".$total_incomming."',outgoing='".$total_outgoing."',yesterdas_balance='".$data_yesterday['todays_balance']."', todays_balance='".$today_total_bal."' where bank_id='".$data_bank_id['bank_id']."' and cm_id='".$cm_id1."' and added_date='".$date."'  ";
									$ptr_update_sm=mysql_query($update_bank_sm);
									
								}
								else
								{
									$insert_into_dsr_bank=" insert into dsr_bank_summery (`bank_id`,`account_no`,`incoming`,`outgoing`,`yesterdas_balance`,`todays_balance`,`added_date`,`cm_id`) values('".$data_bank_id['bank_id']."','".$data_bank_name['account_no']."','".$total_incomming."','".$total_outgoing."','".$data_yesterday['todays_balance']."','".$today_total_bal."','".$date."','".$cm_id1."')";
