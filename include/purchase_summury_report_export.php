...
$total_found =0;	
	...
	if($_REQUEST['keyword']!="Keyword")
  	...
  	$sel_for_Num_rows="select i.invoice_id,e.vendor_id,i.cm_id,i.inventory_id,i.paid_type,i.bank_id,i.cheque_detail,i.chaque_date,i.credit_card_no,i.payable_amount,i.added_date from inventory_invoice i, inventory e where 1 and i.inventory_id=e.inventory_id ".$search_cm_id." ".$pre_keyword_i." ".$pay_type." ".$bank_ids." ".$paid_from_date." ".$paid_to_date." ".$select_directory." "; 
  	$my_querxy=mysql_query($sel_for_Num_rows);
  	if($no_of_rows=mysql_num_rows($my_querxy))
	{
		while($data_total=mysql_fetch_array($my_querxy))
		{
			$sel_prod="select inventory_id,product_id from inventory_product_map where inventory_id='".$data_total['inventory_id']."'";
...
$objPHPExcel->setActiveSheetIndex(0);
 ...
 $objPHPExcel->setActiveSheetIndex()->setCellValueByColumnAndRow('3','3', 'Mo No','3');
  ...
  $objPHPExcel->setActiveSheetIndex()->setCellValueByColumnAndRow('13','3', 'Status','3');*/
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('C4', 'Receipt No','6');
	$objPHPExcel->getActiveSheet()->SetCellValue('D4', 'Customer Name','6');
	$objPHPExcel->getActiveSheet()->SetCellValue('E4', 'Product Details','6');
	$objPHPExcel->getActiveSheet()->SetCellValue('L4', 'Payment Type','6');
	$objPHPExcel->getActiveSheet()->SetCellValue('M4', 'Bank Name','6');
	$objPHPExcel->getActiveSheet()->SetCellValue('N4', 'Bank Details','6');
	$objPHPExcel->getActiveSheet()->SetCellValue('O4', 'Discount Price','6');
	$objPHPExcel->getActiveSheet()->SetCellValue('P4', 'Deposit Amount','6');
	$objPHPExcel->getActiveSheet()->SetCellValue('Q4', 'Date','6');
	...
	while($val_query=mysql_fetch_array($query_order))
	{
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('C'.$rowCount, $val_query['invoice_id']);
		
		$sel_name="select name from vendor where 1 and vendor_id='".$val_query['vendor_id']."'";
		$ptr_name=mysql_query($sel_name);
		$data_name=mysql_fetch_array($ptr_name);
		...
		$product_name ='';
		$total_inst=0;
		$sel_prod="select * from inventory_product_map where inventory_id='".$val_query['inventory_id']."'";
		$ptr_prod= mysql_query($sel_prod);
		if($total_inst=mysql_num_rows($ptr_prod))
		{
			$ins=1;
			$amount=0;
			$ins=1;
			...
			while($product=mysql_fetch_array($ptr_prod))
			{
				...
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('E'.$rowcnt, $data_p['product_name']);
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('F'.$rowcnt, $product['discounted_price']);
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('G'.$rowcnt, $product['sin_product_qty']);
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('H'.$rowcnt, $product['cgst_tax']);
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('I'.$rowcnt, $product['sgst_tax']);
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('J'.$rowcnt, $product['igst_tax']);
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('K'.$rowcnt, $product['sin_product_total']);
				
				$totals_pro_price +=intval($product['discounted_price']);
				$totals_qty +=intval($product['sin_product_qty']);
				$totals_cgst +=intval($product['cgst_tax']);
				$totals_sgst +=intval($product['sgst_tax']);
				$totals_igst +=intval($product['igst_tax']);
				$totals_amnts +=intval($product['sin_product_total']);
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('L'.$rowCount, $payment_mode);
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('M'.$rowCount, $data_bank['bank_name']."\n Acc. No.- ".$data_bank['account_no']);
		...
		if($val_query['credit_card_no'] !='')
		  ...
		  $objPHPExcel->getActiveSheet()->SetCellValue('N'.$rowCount, $b_details."\n".$b_details1."\n".$b_details2."\n".$b_details3);
		  $objPHPExcel->getActiveSheet()->SetCellValue('O'.$rowCount, $val_query['discount_price']);		
		  $objPHPExcel->getActiveSheet()->SetCellValue('P'.$rowCount, $val_query['payable_amount']);		
		$sep=explode(" ",$val_query['added_date']);
		$sep_date=explode("-",$sep[0]);
		$date=$sep_date[2]."/".$sep_date[1]."/".$sep_date[0];
		$objPHPExcel->getActiveSheet()->SetCellValue('Q'.$rowCount, $date);	
		/* $sel_inst_amnt="select course_id,enroll_id,balance_amt from enrollment where ref_id='".$val_query['enroll_id']."' || enroll_id='".$val_query['enroll_id']."'";
		$ptr_ins_amnt=mysql_query($sel_inst_amnt);
		if($total_inst=mysql_num_rows($ptr_ins_amnt))
		{
...
header('Content-Disposition: attachment;filename="purchse_summury_report.xls"');
