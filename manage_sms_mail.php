    ...
    <?php       if($_POST['formAction'])
                    {
                        if($_POST['formAction']=="delete")
                        {
                            for($r=0;$r<count($_POST['chkRecords']);$r++)
                            {
                                ...
                                $sql_query= "SELECT id FROM ".$GLOBALS["pre_db"]."sms_mail_configuration where id ='".$del_record_id."'";
                                if(mysql_num_rows($db->query($sql_query)))
                                    {                                                
                                        $delete_query="delete from ".$GLOBALS["pre_db"]."sms_mail_configuration where id='".$del_record_id."'";
...
<table cellspacing="0" cellpadding="0" class="table" width="95%">
  ...
  <tr class="head_td">
    ...
    <?php
                        ...
                        if($keyword)
                            $pre_keyword=" and (name like '%".$keyword."%' || email_id like '%".$keyword."%' || mobile like '%".$keyword."%' || address like '%".$keyword."%')";
                        else
                            $pre_keyword="";
                        ...
                        if($_GET['order'] !='' && ($_GET['orderby']=='lab_id'))
                        {
                            ...
                            $sql_query= "SELECT id,name,email_id,mobile,address,added_date,status FROM sms_mail_configuration where 1  ".$_SESSION['where']." ".$pre_keyword." ".$select_directory." "; 
                       //echo $sql_query;
...
if($_SESSION['type']=="A" || $_SESSION['type'] =='S')
    ...
    <td width="20%"><a href="<?php echo $_SERVER['PHP_SELF'];?>?order=<?php echo $order."&orderby=lab_name".$query_string;?>" class="table_font"><strong>Name</strong></a> <?php echo $img1;?></td>
...
if($_SESSION['type']=="A" || $_SESSION['type'] =='S')
	...
	?>
                            ...
                            while($val_query=mysql_fetch_array($all_records))
                            {
								...
								$sel_name="select name from site_setting where admin_id='".$val_query['name']."'";  
								$ptr_names=mysql_query($sel_name);
								$data_name=mysql_fetch_array($ptr_names);
								if($_SESSION['type']=="A" || $_SESSION['type'] =='S')
									{
                                echo '<td ><a href="add_sms_mail.php?record_id='.$listed_record_id.'" class="table_font">'.$data_name['name'].'</a></td>';
									}
                                echo '<td >'.$val_query['email_id'].'</td>';
								echo '<td >'.$val_query['mobile'].'</td>';
								echo '<td >'.$val_query['address'].'</td>';
								echo '<td >'.$val_query['status'].'</td>';
								echo '<td >'.$val_query['added_date'].'</td>';
								if($_SESSION['type']=="A" || $_SESSION['type'] =='S')
									{
                                echo '<td align="center"><a href="add_sms_mail.php?record_id='.$listed_record_id.'" ><img src="images/edit_icon.png" title="Edit Record" class="example-fade"/></a>&nbsp;&nbsp;
