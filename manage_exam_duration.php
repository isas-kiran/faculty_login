...
<title>Manage Exam Duration</title>
    ...
    <?php if($_POST['formAction'])
                    {
                        if($_POST['formAction']=="delete")
                        {
                            for($r=0;$r<count($_POST['chkRecords']);$r++)
                            {
                                ...
                                $sql_query= "SELECT exam_duration_id FROM ".$GLOBALS["pre_db"]."course_exam_duration where exam_duration_id ='".$del_record_id."'";
                                if(mysql_num_rows($db->query($sql_record)))
                                    {                                                
                                        $delete_query="delete from ".$GLOBALS["pre_db"]."course_exam_duration where exam_duration_id ='".$del_record_id."'";
  ...
  <tr class="head_td">
    ...
    <?php
                        ...
                        if($keyword)
                            {
//                            $questionss = '';
                                 $select_question = "select * from PB_courses where course_name like '%".$keyword."%' ";
                                if(mysql_num_rows($db->query($select_question)))
                                {
                                    $val_question = $db->fetch_array($db->query($select_question));
                                    $questionss = "or course_id = '".$val_question['course_id']."' ";
                                }
                                
                                $pre_keyword=" and (exam_name like '%".$keyword."%' $questionss)";
                            }                            
                        else
                            $pre_keyword="";
                        ...
                        if($_GET['orderby']=='exam_duration_id' )
                            $img1 = $img;

                        if($_GET['order'] !='' && ($_GET['orderby']=='exam_duration_id'))
                        {
                            $select_directory .= " order by ".$_GET['orderby']." " .$_GET['order'] ;
                            $date_query='&order='.$_GET['order'].'&orderby='.$_GET['orderby'];
                        }
                        else
                            $select_directory='order by exam_duration_id desc';                      
                            $sql_query= "SELECT * FROM PB_course_exam_duration where 1  ".$pre_keyword." ".$select_directory.""; 
                       //echo $sql_query;
                     ...
                     <form method="post"  name="frmTakeAction" action="<?php echo $_SERVER['PHP_SELF'].'?#msgbox';?>" >
                         <input type="hidden" name="formAction" id="formAction" value=""/>
                          <tr class="grey_td" >
                            ...
                            <td width="20%"><strong>Exam Title</strong><?php echo $img1;?></td>
                            ...
                            while($val_query=mysql_fetch_array($all_records))
                            {
                                ...
                                $listed_record_id=$val_query['exam_duration_id']; 
                                ...
                                echo '<td >'.$val_query['exam_duration'].'</td>';                                
                                echo '<td align="center"><a href="add_exam_duration.php?record_id='.$listed_record_id.'" ><img src="images/edit_icon.png" title="Edit Record" class="course_example-fade"/></a>&nbsp;&nbsp;
