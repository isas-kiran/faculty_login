...
$objPHPExcel->getProperties()->setDescription("Test document for Office 2007 XLSX, generated using PHP classes.");
	...
	$select_directory=' order by DATE(i.added_date) asc';                     
  	$sel_for_Num_rows="select i.*, e.name,e.course_id,e.net_fees from invoice i, enrollment e where 1 and i.enroll_id=e.enroll_id and i.status='Paid' ".$search_cm_id." ".$pre_keyword_i." ".$pay_type." ".$bank_ids." ".$paid_from_date." ".$paid_to_date." ";
	//and amount>0
...
$objPHPExcel->setActiveSheetIndex(0);
 ...
 $objPHPExcel->setActiveSheetIndex()->setCellValueByColumnAndRow('3','3', 'Mo No','3');
  ...
  $objPHPExcel->setActiveSheetIndex()->setCellValueByColumnAndRow('13','3', 'Status','3');*/
	...
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('C4', 'Enroll ID','6');
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('D4', 'Invoice No','6');
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('E4', 'Receipt No','6');
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('F4', 'Date','6');
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('G4', 'Student Name','6');
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('H4', 'Course Name','6');
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('I4', 'Total Fee','6');
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('J4', 'Fees Paid','6');
	...
	$select_directory=' order by DATE(i.added_date) asc';                         
	$sql_query= "select i.*,e.name,e.course_id,e.net_fees,e.installment_display_id from invoice i, enrollment e where 1 and i.enroll_id=e.enroll_id and i.status='Paid' ".$search_cm_id." ".$pre_keyword_i." ".$pay_type." ".$bank_ids." ".$paid_from_date." ".$paid_to_date." ".$select_directory." "; 
	//and amount>0
	...
	while($val_query=mysql_fetch_array($query_order))
	{
		...
		{
			$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('D'.$rowCount, $val_query['enroll_id']);
		}
		else
		{
			$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('D'.$rowCount, $val_query['invoice_id']);
		}
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('E'.$rowCount, $val_query['receipt_no']);		
		
		$sep=explode(" ",$val_query['added_date']);
		$sep_date=explode("-",$sep[0]);
		$date=$sep_date[2]."/".$sep_date[1]."/".$sep_date[0];
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('F'.$rowCount, $date);	
		...
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('H'.$rowCount, $data_course['course_name']);	
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('I'.$rowCount, $val_query['net_fees']);	
	...
	header('Content-Disposition: attachment;filename="bank_summury_report.xls"');
