	...
	<table border="0" cellspacing="0" cellpadding="0" width="99%" align="center">
              <tr>
                ...
                <!--<td width="20%">
                        ...
                        <td width="15%" id="admin_id_by">
							<select id="admin_id" name="admin_id" class="input_select">
							<option value="">Select Staff</option>
                                ...
                                while($data_cat = mysql_fetch_array($ptr_course_cat))
                                {
                                    ...
                                    $sel_source="SELECT * FROM site_setting where 1 and cm_id='".$data_cat['cm_id']."' and status is NOT NULL order by name asc";
                                    $ptr_src=mysql_query($sel_source);
                                    while($data_src=mysql_fetch_array($ptr_src))
                                    {
                                         $sele_source="";
                                        if($data_src['admin_id'] == $_REQUEST['admin_id'] || $_POST['admin_id']== $data_src['admin_id'] )
                                        {
                                            $sele_source='selected="selected"';
                                        }
                                        ?>
                                        <option <?php echo $sele_source?> value ="<?php echo $data_src['admin_id']?>" <? if (isset($_REQUEST['admin_id']) && $_REQUEST['enquiry_src'] == $data_src['admin_id']) echo "selected";?> > <?php echo $data_src['name'] ?> </option>
                                        <?php
                                    }
                                    echo " </optgroup>";
                                }?>
						 ...
						 <!--<td width="15%">
							<select name="type" id="type"  class="input_select_login"  style="width: 150px;" onchange="change_type(this.value);">
								...
								<option value="institute" <?php //if($_REQUEST['type']=='institute') echo 'selected="selected"' ?>>Institute</option>
                                <option value="service" <?php //if($_REQUEST['type']=='service') echo 'selected="selected"' ?>>Service</option>
							</select>
						</td>-->
                        <!--<td width="15%" id="camaign">
							...
							<option value="">Select Campaign</option>
                               ...
                               /* $course_category = " select DISTINCT(cm_id),branch_name from site_setting where type='A' ".$_SESSION['where']."";
                                ...
                                {
                                    echo "<optgroup label='".$data_cat['branch_name']."'>";
                                    $sel_source="SELECT * FROM campaign where 1 and cm_id='".$data_cat['cm_id']."' order by campaign_for asc";
                                    $ptr_src=mysql_query($sel_source);
                                    while($data_src=mysql_fetch_array($ptr_src))
                                    {
                                         $sele_source="";
                                        ...
                                        ?>
                                        <option <?php echo $sele_source?> value ="<?php echo $data_src['campaign_id']?>" <? if (isset($_REQUEST['campaign_name']) && $_REQUEST['campaign_name'] == $data_src['campaign_name']) echo "selected";?> > <?php echo $data_src['campaign_for']." - ".$data_src['campaign_name'] ?> </option>
                                        <?
                                    }
                                    echo " </optgroup>";
                                }*/?>
  ...
  <?php
						...
						if($_SESSION['type']=="S")
						{
							...
							else
							{
								...
								{
									$sele_cm="select cm_id,branch_name from site_setting where admin_id='".$_REQUEST['admin_id']."'";
									$ptr_cm=mysql_query($sele_cm);
									$data_cm=mysql_fetch_array($ptr_cm);
									$search_cm_id=" and cm_id='".$data_cm['cm_id']."'";
									$branch_name=$data_cm['branch_name'];
									$cm_ids=$data_cm['cm_id'];
								}
							}
						}
						$where_admin_id='';
						if($_REQUEST['admin_id'])
						{
							...
							$where_admin_id=" and admin_id='".$admin_id."'";
						}
                        ...
                        if($_GET['order'] !='' && ($_GET['orderby']=='firstname'))
                        {
							...
							$select_directory='order by admin_id asc';                      
                       		$sql_query= "SELECT name,admin_id FROM site_setting where 1 ".$search_cm_id." ".$where_admin_id." ".$select_directory.""; 
	                    ...
	                    if($no_of_records)
                        {
                            ...
                            $query_string='&keyword='.$keyword.'&branch_name='.$_REQUEST['branch_name'].'&admin_id='.$_REQUEST['admin_id'].'&campaign_name='.$_REQUEST['campaign_name'].'&from_date='.$_GET['from_date'].'&to_date='.$_GET['to_date'];
    ...
    <form method="post"  name="frmTakeAction"  action="<?php echo $_SERVER['PHP_SELF'].'?#msgbox';?>">
                     ...
                     <input type="hidden" name="formAction" id="formAction" value=""/>
                      ...
                      <tr class="grey_td" >
                        ...
                        <td width="20%" align="center"><strong>Total Inquiry</strong></td>
                        <td width="20%" align="center"><strong>Total Enrollment</strong></td>
                        <td width="20%" align="center"><strong>Total Walk-in Inquiry</strong></td>
                      </tr>
                      <?php
                            while($val_query=mysql_fetch_array($all_records))
                            {
								...
								$total_source='';	       
                                $sel_inq_source="select inquiry_id from inquiry where 1 and (employee_id='".$val_query['admin_id']."') ".$search_cm_id." ".$_SESSION['where']." ".$pre_from_date." ".$pre_to_date." ".$enquiry_src." ";
								$ptr_inq_source=mysql_query($sel_inq_source);
								$total_inq_source=mysql_num_rows($ptr_inq_source);
								$total_inq +=$total_inq_source;
								
								
								"<br/>".$sel_enroll_src="select enroll_id from enrollment where 1 and (admin_id='".$val_query['admin_id']."') ".$search_cm_id." ".$_SESSION['where']." ".$pre_from_date." ".$pre_to_date." ".$enroll_src." ";
								$ptr_enroll_src=mysql_query($sel_enroll_src);
								$total_enroll_src=mysql_num_rows($ptr_enroll_src);
								$total_enroll +=$total_enroll_src;
								
								$sel_inq_src="select inquiry_id from inquiry where 1 and response=1 and (admin_id='".$val_query['admin_id']."' or employee_id='".$val_query['admin_id']."') ".$search_cm_id." ".$_SESSION['where']." ".$pre_from_date." ".$pre_to_date." ".$enquiry_src." ";
								$ptr_inq_src=mysql_query($sel_inq_src);
								$total_inq_src=mysql_num_rows($ptr_inq_src);
								$total_walkin +=$total_inq_src;
								...
								echo '<td align="center">'.$total_inq_src.'</td>';                   
                                echo '</tr>';
								$bgColorCounter++;                                                              
							}
							echo '<tr '.$bgcolor.'>';
							echo '<td align="center" colspan="2" style="font-weight:800;"><strong>Total</strong></td>';
							echo '<td align="center" style="font-weight:800"><strong>'.$total_inq.'</strong></td>';
							echo '<td align="center" style="font-weight:800"><strong>'.$total_enroll.'</strong></td>';
							echo '<td align="center" style="font-weight:800"><strong>'.$total_walkin.'</strong></td>';                   
							echo '</tr>';
                          ?>
