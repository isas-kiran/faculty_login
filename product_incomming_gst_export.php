...
$total_found =0;	
	...
	if($_SESSION['where']!='')
	{
  	...
  	$sel_for_Num_rows="SELECT DISTINCT(s.sales_product_id),s.customer_id,s.added_date,e.cust_id FROM sales_product s, sales_product_map sm, customer e WHERE 1 and s.sales_product_id=sm.sales_product_id  and  s.customer_id=e.cust_id and (s.show_gst is NULL or s.show_gst!='no') ".$where_cm." ".$search_cm_id." ".$where_gst_i." ".$pre_keyword_i." ".$paid_from_date_i." ".$paid_to_date_i." and (sm.cgst_tax_in_per >0 or sm.sgst_tax_in_per >0) and s.payable_amount>0 ".$select_directory." ";
...
$objPHPExcel->setActiveSheetIndex(0);
 ...
 $objPHPExcel->setActiveSheetIndex()->setCellValueByColumnAndRow('3','3', 'Mo No','3');
  ...
  $objPHPExcel->setActiveSheetIndex()->setCellValueByColumnAndRow('13','3', 'Status','3');*/
	...
	$sql_query= "SELECT DISTINCT(s.sales_product_id),s.customer_id,s.added_date,e.cust_id FROM sales_product s, sales_product_map sm, customer e WHERE 1 and s.sales_product_id=sm.sales_product_id  and s.customer_id=e.cust_id and (s.show_gst is NULL or s.show_gst!='no') ".$where_cm." ".$search_cm_id." ".$where_gst_i." ".$pre_keyword_i." ".$paid_from_date_i." ".$paid_to_date_i." and (sm.cgst_tax_in_per >0 or sm.sgst_tax_in_per >0) and s.payable_amount>0 ".$select_directory." "; 
	...
	while($val_query=mysql_fetch_array($query_order))
	{
		...
		$gst_per='';
		$gst_val='';
		$sel_inst_amnt="select * from sales_product_map where 1 and sales_product_id='".$val_query['sales_product_id']."' and (cgst_tax_in_per >0 or sgst_tax_in_per >0)";
		...
		if($total_inst=mysql_num_rows($ptr_ins_amnt))
		{
			...
			while($data_inst_amnt=mysql_fetch_array($ptr_ins_amnt))
			{
				...
				/* if($data_inst_amnt['discounted_price'] >0)
					...
					$product_price .=$data_inst_amnt['sales_product_price'];
