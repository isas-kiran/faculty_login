...
.table2 tr th:last-child {
  border-right: 0;
	...
	function send_mail($branch_name,$month,$year,$employee_id)
	{
		...
		$mail->Username = 'erp.isas@gmail.com';					//Sets SMTP username
		$mail->Password = 'erp@frespa';					//Sets SMTP password
		...
		$mail->addReplyTo('info@isasbeautyschool.com', 'ISAS Beauty School');		//Adds a "To" address
...
//====================================================================
	...
	if($_POST['save_changes'])
	{
		...
		{			
			...
			$data_record_type1['expense_action'] =$_POST['expense_action'.$z];
			...
			$data_record_type1['payment_date'] =$payment_date;
				...
				{
					if(isset($_POST['expense_action'.$z]))
					{
...
function CheckExpense(chk)
{
	if ($('#Check_All_Expense').attr('checked'))
	{
		for (i = 0; i < chk.length; i++)
		chk[i].checked = true ;
