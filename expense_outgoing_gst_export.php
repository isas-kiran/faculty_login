...
$total_found =0;	
	...
	if($_SESSION['where']!='')
	{
  	...
  	$sel_for_Num_rows="SELECT DISTINCT(s.expense_id),s.vendor_id,s.added_Date FROM expense s, expense_map sm,vendor e WHERE 1 and s.expense_id=sm.expense_id and s.vendor_id=e.vendor_id ".$where_cm." ".$search_cm_id." ".$where_gst_i." ".$pre_keyword_i." ".$paid_from_date_i." ".$paid_to_date_i." and (sm.cgst_tax_in_per >0 or sm.sgst_tax_in_per >0) ".$select_directory." ";
 ...
 $objPHPExcel->setActiveSheetIndex()->setCellValueByColumnAndRow('3','3', 'Mo No','3');
  ...
  $objPHPExcel->setActiveSheetIndex()->setCellValueByColumnAndRow('13','3', 'Status','3');*/
	...
	while($val_query=mysql_fetch_array($query_order))
	{
		...
		$gst_per='';
		$gst_val='';
		$sel_inst_amnt="select * from expense_map where 1 and expense_id='".$val_query['expense_id']."' and (cgst_tax_in_per >0 or sgst_tax_in_per >0)";
		...
		if($total_inst=mysql_num_rows($ptr_ins_amnt))
		{
			...
			while($data_inst_amnt=mysql_fetch_array($ptr_ins_amnt))
			{
				...
				$product_name .= $data_inst_amnt['expense_name'];
...
header('Content-Disposition: attachment;filename="expense_outgoing_gst.xls"');
