...
<title>Manage sales voucher/Package</title>
...
<?php
$edit_access='';
$sel_acc="select * from edit_previleges where admin_id='".$_SESSION['admin_id']."' and privilege_id='126'";
    ...
    <?php       if($_POST['formAction'])
                    {
                        if($_POST['formAction']=="delete")
                        {
                            for($r=0;$r<count($_POST['chkRecords']);$r++)
                            {
                                ...
                                $sql_query= "SELECT id,category FROM sales_package_voucher_memb where id ='".$del_record_id."'";
								$ptr_query11=mysql_query($sql_query); 
                                if(mysql_num_rows($ptr_query11))
								{    
									 $data_cust=mysql_fetch_array($ptr_query11);   
									"<br>".$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('manage_sales_memb_package','Delete','".$data_cust['category']."','".$del_record_id."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
...
<table cellspacing="0" cellpadding="0" class="table" width="95%">
  ...
  <tr class="head_td">
    ...
    <?php
						...
						$cust_name='';
						$sel_cust_name="select cust_id from customer where cust_name like '%".$keyword."%' || mobile1 like '%".$keyword."%' "; 
                        ...
                        if($_GET['order'] !='' && ($_GET['orderby']=='id'))
                        {
                            ...
                            $sql_query= "SELECT * FROM sales_package_voucher_memb where 1 ".$_SESSION['where']." ".$record_cat_id." ".$pre_keyword." ".$select_directory.""; 
						//echo $sql_query;
	...
	<?php } ?>
                        ...
                        <!--<td width="10%" class="centerAlign"><strong>Action</strong></td>-->
                            ...
                            while($val_query=mysql_fetch_array($all_records))
                            {
								...
								$cust_name="select cust_name,mobile1 from customer where cust_id='".$val_query['cust_id']."'";
								...
								echo '<td align="center">'.$data_cust['mobile1'].'</td>';
								...
								echo '<td><center><a href="package_payment_details.php?record_id='.$listed_record_id.'">
								...
								if($_SESSION['type']=="S" || $edit_access='yes')
								{
									...
									//echo'<a href="sale_memb_package_voucher.php?record_id='.$listed_record_id.'" ><img src="images/edit_icon.png" title="Edit Record" class="example-fade"/></a>&nbsp;&nbsp;';
