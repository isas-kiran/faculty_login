                    ...
                    <tr>
                   		<td width="60%" valign="top" style="width:100%;padding:0; margin:0" >
                            ...
                            <ul id="qm0" class="qmmc">
                                    ...
                                    {
                                       $sele_latest="select * from inquiry where status = 'Enquiry' and campaign_id!='' and followup_date IS NULL and (DATE(added_date) >= '".date('Y-m-d', strtotime('-2 days'))."' and DATE(added_date) <= '".date('Y-m-d')."') ".$_SESSION['where']." ".$search_cm_id." order by added_date asc";//and  '".date('Y-m-d', strtotime('+2 days'))."'
                                        $ptr_select_letest=mysql_query($sele_latest);
                                        $count=mysql_num_rows($ptr_select_letest);
                                        ?>
