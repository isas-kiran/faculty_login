...
if(isset($_POST['submit']))
{
	...
	if($username && $pass && $captch )
	{
		...
		$sql_site_setting= "SELECT username, email, type, admin_id, cm_id, branch_name, status FROM site_setting where username='".$username."' and pass='".$pass."'  and system_status='Enabled' limit 0,1";
	   	...
	   	if(mysql_num_rows($ptr_site_setting))
		{
			if((trim(strtolower($_POST['captcha'])) == $_SESSION['captcha']))
			{
				...
				if($_COOKIE[$cookiess_name]==$username || $row_site_setting['type']=="S")
				{
