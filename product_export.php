...
$total_found =0;	
	...
	if($keyword)
		$pre_keyword=" and (p.product_name like '%".$keyword."%' || p.product_code like '%".$keyword."%' || p.description like '%".$keyword."%' || p.amount like '%".$keyword."%' || p.commission like '%".$keyword."%' || p.price like '%".$keyword."%' || p.vender like '%".$keyword."%' || p.type like '%".$keyword."%')";
	else
		$pre_keyword="";
	...
	$sel_for_Num_rows= "SELECT p.product_name, p.product_code,p.status, p.description, p.size,unit, p.commission, p.price, p.pcategory_id, p.sub_id, p.product_id,p.quantity,p.quantity_in_shelf,p.quantity_in_consumable,p.added_date,p.admin_id,p.hsn_code FROM product p, product_category pc, product_subcategory sub where 1  and (p.product_name like '%".$keyword."%' or p.product_code like '%".$keyword."%' or p.description like '%".$keyword."%' or p.size like '%".$keyword."%' or p.commission like '%".$keyword."%' or p.price like '%".$keyword."%' or pc.pcategory_name like '%".$keyword."%' or sub.sub_name like '%".$keyword."%' ) ".$status_type." and p.pcategory_id=pc.pcategory_id and p.sub_id=sub.sub_id ".$vendor_ids." ".$cm_ids."  ".$search_cm_id_S." ".$search_admin_id_S." ".$select_directory.""; 
...
$objPHPExcel->setActiveSheetIndex(0)->setSharedStyle($sharedStyle2, "A1:P1");
...
$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('C1', 'Product Code');
...
$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('F1', 'Product Description');
$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('G1', 'Size');
$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('H1', 'Unit');
$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('I1', 'Commission (in %)');
$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('J1', 'Non Tax Price');
$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('K1', 'Brand');
$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('L1', 'Select Vender');
$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('M1', 'Type');
$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('N1', 'Quantity');
$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('O1', 'Staff');
$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('P1', 'Product ID');
$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('Q1', 'Status');

$select_directory='order by p.product_name asc';                      

	/*if($pre_keyword=='') 
	{                   
		$sql_query= "SELECT * FROM product where 1 ".$record_cat_id." ".$_SESSION['where']." ".$_SESSION['user_id']." ".$search_cm_id." ".$search_admin_id."  ".$select_directory."";
	}
	else
	{*/
	...
	$sql_query= "SELECT p.product_name,p.product_id, p.product_code,p.status, p.description, p.size,p.unit, p.commission, p.price, p.pcategory_id, p.sub_id, p.product_id,p.quantity,p.quantity_in_shelf,p.quantity_in_consumable,p.added_date,p.admin_id,p.hsn_code,p.brand FROM product p, product_category pc, product_subcategory sub where 1  and (p.product_name like '%".$keyword."%' or p.product_code like '%".$keyword."%' or p.description like '%".$keyword."%' or p.size like '%".$keyword."%' or p.commission like '%".$keyword."%' or p.price like '%".$keyword."%' or pc.pcategory_name like '%".$keyword."%' or sub.sub_name like '%".$keyword."%' ) ".$status_type." and p.pcategory_id=pc.pcategory_id and p.sub_id=sub.sub_id ".$vendor_ids." ".$cm_ids."  ".$search_cm_id_S." ".$search_admin_id_S." ".$select_directory.""; //".$admin_ids."
	...
	while($val_query=mysql_fetch_array($query_order))
	{
		...
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('J'.$rowCount, trim($val_query['price']));
		...
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('K'.$rowCount, $data_brand['brand_name']);
		...
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('L'.$rowCount, trim($vendor_name));
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('M'.$rowCount, $val_query['type']);						
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('N'.$rowCount, $val_query['quantity']);
		
		$sel_name="select name from site_setting where admin_id='".$val_query['admin_id']."'";
		$ptr_name=mysql_query($sel_name);
		$data_name=mysql_fetch_array($ptr_name);
		
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('O'.$rowCount, $data_name['name']);
		
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('P'.$rowCount, $val_query['product_id']);
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('Q'.$rowCount, $val_query['status']);
		$k++;
		$rowCount++;
		$i++;
	}
