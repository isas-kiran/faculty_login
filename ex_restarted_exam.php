...
<?php include "ex_include/ps_pagination.php"; ?>
	...
	function reattempt_exam(stud_id,status)
	{
		if(confirm("Are you sure, Do you want to Restart exam?"))
                {					
					document.location.href="stopped_student.php?action=stop&student_id="+stud_id+"&status="+status;
				}
            else
                return false;
	...
	if($_GET['action']=='stop')
	{
		...
		?>
                    ...
                    if($_REQUEST['stopexam'] && $_REQUEST['stud_login_id'])
                    {
                        ...
                        {                           
                          $delete_query="update ex_stud_login set status='inactive' , exam_stoped='yes', stopped_time='".date('Y-m-d H:i:s')."'  where stud_login_id='".$stud_login_id."'";
  ...
  <tr class="head_td">
    ...
    <?php
                        ...
                        if($_GET['order'] !='' && ($_GET['orderby']=='exam_name'))
                        {
                            ...
                            $sql_query= "SELECT DISTINCT(stud_login_id) FROM ex_stud_login where reattempt_permission='yes'  ".$pre_keyword." ".$select_directory.""; 
                       //echo $sql_query;
                     ...
                     <form method="post"  name="frmTakeAction" action="<?php echo $_SERVER['PHP_SELF'].'?#msgbox';?>" >
                         <input type="hidden" name="formAction" id="formAction" value=""/>
                          <tr class="grey_td" >
                            ...
                            <td width="30%"><strong>Student Name </strong><?php echo $img1;?></td>
                            <td width="15%"><strong>Exam Number</strong><?php echo $img1;?></td>
                            <td width="15%"><strong>School Code </strong><?php echo $img1;?></td>
                           <!-- <td width="20%"><strong>Reattempt Permission</strong><?php echo $img1;?></td>-->
                            <td width="25%" class="centerAlign"><strong>Date Time</strong></td>
                          </tr>
                            <?php
							$i=1;
