...
<title>Product Incomming GST</title>
	...
	?>
              <tr>
                ...
                <!--<td width="20%">
                        ...
                        <td><a href="product_incomming_gst_export.php<?php echo $sep_url_string; ?>"><img src="images/excel.png" height="30px" width="30px"/></a></td>  
            </tr>          
        </table>
        </form>	
    </td>
     ...
     <?php
                        ...
                        if($_GET['order'] !='' && ($_GET['orderby']=='firstname'))
                        {
							...
							$select_directory='order by s.sales_product_id desc';                     
							$where_cm='';
							if($_SESSION['where']!='')
							{
							...
							$branch_id='';
                       		...
                       		"<br/>".$sql_query= "SELECT DISTINCT(s.sales_product_id),s.customer_id,s.added_date,e.cust_id,s.type FROM sales_product s, sales_product_map sm, customer e WHERE 1 and s.sales_product_id=sm.sales_product_id and (s.show_gst is NULL or s.show_gst!='no') and s.customer_id=e.cust_id ".$where_cm." ".$search_cm_id." ".$where_gst_i." ".$pre_keyword_i." ".$paid_from_date_i." ".$paid_to_date_i." and (sm.cgst_tax_in_per >0 or sm.sgst_tax_in_per >0) and s.payable_amount>0 ".$select_directory." "; 
				...
				<form method="post"  name="frmTakeAction"  action="<?php echo $_SERVER['PHP_SELF'].'?#msgbox';?>">
                     <input type="hidden" name="formAction" id="formAction" value=""/>
                      <tr class="grey_td" >
						...
						<table width="100%">
						<tr>
                        <td align="center" colspan="4"><strong>Product Details</strong></td></tr>
                        
						<tr><td width="40%" align="center">Product Name</td>
                        <td width="10%" align="center">Price (incl. GST)</td><td width="25%" align="center">GST in %</td><td width="25%" align="center">GST in Value</td></tr>
						</table>
                        ...
                        <td width="14%" align="center"><strong>Installments</strong></td>-->
                            ...
                            while($val_query=mysql_fetch_array($all_records))
                            {
								...
								{
									$sel_name = " select cust_name,cust_id,cust_gst_no,cm_id from customer where cust_id='".$val_query['customer_id']."' ";
									$ptr_name = mysql_query($sel_name);
									$data_name = mysql_fetch_array($ptr_name);
									$name=$data_name['cust_name'];
									$cust_gst_no=$data_name['cust_gst_no'];
								}
								else
								if($val_query['type']=='Employee')
								{
									$sel_name = " select name,admin_id,cm_id from site_setting where admin_id='".$val_query['customer_id']."' ";
									$ptr_name = mysql_query($sel_name);
									$data_name = mysql_fetch_array($ptr_name);
									$name=$data_name['name'];
								}
								...
								{
									$sel_name = " select name,cm_id from enrollment where enroll_id='".$val_query['customer_id']."' ";
									$ptr_name = mysql_query($sel_name);
									$data_name = mysql_fetch_array($ptr_name);
									$name=$data_name['name'];
								}
								
								/*"<br/>".$sel_name="select cust_name,cust_gst_no,cm_id from customer where cust_id='".$val_query['customer_id']."'";
								$ptr_name=mysql_query($sel_name);
                                $data_name=mysql_fetch_array($ptr_name);*/
								...
								if($cust_gst_no)
								{
									$gstno='<br/>GST no: '.$cust_gst_no.'';
								}
								$sel_branch="select branch_name from site_setting where cm_id='".$data_name['cm_id']."' and type='A'";
								$ptr_ranch=mysql_query($sel_branch);
								$data_branchs=mysql_fetch_array($ptr_ranch);
								
								echo '<td align="center">'.$sr_no.'</td>';
								echo '<td align="center">'.$data_branchs['branch_name'].'</td>';
								echo '<td align="center">'.$name.''.$gstno.'</td>';
								
								
								echo "<td colspan='4'>";
								echo "<table width='100%'>";
								$totals_products_gst=0;
								"<br/>".$sel_inst_amnt="select * from sales_product_map where 1 and sales_product_id='".$val_query['sales_product_id']."' and (cgst_tax_in_per >0 or sgst_tax_in_per >0)";
								...
								if($total_inst=mysql_num_rows($ptr_ins_amnt))
								{
									...
									while($data_inst_amnt=mysql_fetch_array($ptr_ins_amnt))
									{
										...
										/* if($data_inst_amnt['discounted_price'] >0)
											...
											echo "<td width='10%' align='center'>".$data_inst_amnt['sales_product_price']."</td>";
