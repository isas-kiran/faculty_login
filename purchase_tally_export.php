...
//styles
	...
	if($_GET['order'] !='' && ($_GET['orderby']=='firstname'))
	{
		...
		$select_directory='order by DATE(i.added_date) asc,order by DATE(i.inventory_id) asc';                  
...
$objPHPExcel->setActiveSheetIndex(0);
 ...
 $objPHPExcel->setActiveSheetIndex()->setCellValueByColumnAndRow('3','3', 'Mo No','3');
  ...
  $objPHPExcel->setActiveSheetIndex()->setCellValueByColumnAndRow('13','3', 'Status','3');*/
	...
	$select_directory='order by i.inventory_id asc';
	$sql_query="select i.*,e.invoice_id from inventory_invoice e, inventory i where 1 and i.inventory_id=e.inventory_id ".$search_cm_id." ".$pre_keyword_i." ".$pay_type." ".$bank_ids." ".$paid_from_date." ".$paid_to_date." group by inventory_id ".$select_directory."";
	...
	while($val_query=mysql_fetch_array($query_order))
	{
		...
		//$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('C'.$rowCount, 'Purchase');
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('C'.$rowCount, 'Purchase');
		
		$sel_name="select name,vendor_id from vendor where vendor_id='".$val_query['vendor_id']."'";
		...
		$sel_prod="select SUM(i.cgst_tax) as cgst_total,SUM(i.discounted_price * i.sin_product_qty) as total_val,SUM(i.sin_product_base_price * i.sin_product_qty) as base_total, i.sin_product_disc as product_disc,SUM(i.discounted_price) as total_cgst_val, s.total_cost as total_price, (i.cgst_tax_in_per + i.sgst_tax_in_per) as total_gst from inventory_product_map i, inventory s where 1 and i.inventory_id='".$val_query['inventory_id']."' and i.inventory_id=s.inventory_id and (i.cgst_tax_in_per >0 || i.sgst_tax_in_per >0) group by i.cgst_tax_in_per";
			...
			if($tot_prod=mysql_num_rows($ptr_prod))
			{
				while($data_prod=mysql_fetch_array($ptr_prod))
				{
					...
					if($val_query['discount'] >0)
					{
						...
						$tot_disc_price_gst=number_format($data_prod['total_val'] - $tot_disc,3,'.','');
					...
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('A'.$rowCount, $date);	
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('B'.$rowCount, $val_query['ext_invoice_no']);
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('E'.$rowCount, 'Purchase @ '.$data_prod['total_gst'].'%');	
					...
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('H'.$rowCount, number_format($tot_price_gst,3,'.',''));
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('I'.$rowCount, $disc);
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('J'.$rowCount, number_format($tot_disc_price_gst,3,'.',''));
					$rowCount++;
		...
		//============================= FOR IGST ============================
			...
			$sel_prod="select SUM(i.igst_tax) as igst_total,SUM(i.discounted_price * i.sin_product_qty) as total_val,SUM(i.sin_product_base_price * i.sin_product_qty) as base_total, i.sin_product_disc as product_disc,SUM(i.discounted_price) as total_cgst_val, s.total_cost as total_price, (i.igst_tax_in_per) as total_gst from inventory_product_map i, inventory s where 1 and i.inventory_id='".$val_query['inventory_id']."' and i.inventory_id=s.inventory_id and (i.igst_tax_in_per >0) group by i.igst_tax_in_per";
			$ptr_prod= mysql_query($sel_prod); //sin_product_base_price and i.sin_product_total>0 and s.total_cost > 0
			if($tot_prod=mysql_num_rows($ptr_prod))
			{
				while($data_prod=mysql_fetch_array($ptr_prod))
				{
					...
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('F'.$rowCount, 'Item');
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('G'.$rowCount, '1');
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('H'.$rowCount, number_format($tot_price_igst,3,'.',''));
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('I'.$rowCount, $disc);
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('J'.$rowCount, number_format($tot_disc_price_igst,3,'.',''));
					$rowCount++;
			...
			$tot_disc_price=0;
			 ...
			 $sel_prod="select SUM(i.igst_tax) as igst_total,SUM(i.discounted_price * i.sin_product_qty) as total_val,SUM(i.sin_product_base_price * i.sin_product_qty) as base_total, i.sin_product_disc as product_disc,SUM(i.discounted_price) as total_cgst_val, s.total_cost as total_price, (i.igst_tax_in_per) as total_gst from inventory_product_map i, inventory s where 1 and i.inventory_id='".$val_query['inventory_id']."' and i.inventory_id=s.inventory_id and (i.cgst_tax_in_per <=0 and i.sgst_tax_in_per <=0 and  i.igst_tax_in_per <=0) group by i.igst_tax_in_per";
			$ptr_prod= mysql_query($sel_prod); //sin_product_base_price and i.sin_product_total>0 and s.total_cost > 0
			if($tot_prod=mysql_num_rows($ptr_prod))
			{
				while($data_prod=mysql_fetch_array($ptr_prod))
				{
					...
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('F'.$rowCount, 'Item');
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('G'.$rowCount, '1');
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('H'.$rowCount, number_format($tot_price,3,'.',''));
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('I'.$rowCount, $disc);
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('J'.$rowCount, number_format($tot_disc_price,3,'.',''));
					$rowCount++;
					$total_amnts =number_format($tot_disc_price);
					$k++;
				}
			}
		//=========================CGST=======================
		...
		$sel_cgst_prod="select SUM(i.cgst_tax) as cgst_total,SUM(i.sin_product_base_price) as base_total,i.sin_product_disc, SUM(i.discounted_price) as total_cgst_val,i.cgst_tax_in_per from inventory_product_map i, inventory s where i.inventory_id='".$val_query['inventory_id']."' and i.inventory_id=s.inventory_id and i.sin_product_total>0 and s.total_cost > 0 group by i.cgst_tax_in_per";
		...
		if($tot_cgst_num=mysql_num_rows($ptr_cgst_prod))
		{
			while($data_cgst=mysql_fetch_array($ptr_cgst_prod))
			{
				if($data_cgst['cgst_tax_in_per']>0)
				{
					...
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('J'.$rowCount, number_format($tot_cgst,3,'.',''));
					$rowCount++;
		...
		$sel_sgst_prod="select SUM(i.sgst_tax) as sgst_total,SUM(i.sin_product_base_price) as base_total,i.sin_product_disc, SUM(i.discounted_price) as total_sgst_val,i.sgst_tax_in_per from inventory_product_map i, inventory s where i.inventory_id='".$val_query['inventory_id']."' and i.inventory_id=s.inventory_id and i.sin_product_total>0 and s.total_cost > 0 group by i.sgst_tax_in_per";
		...
		if($tot_sgst_num=mysql_num_rows($ptr_sgst_prod))
		{
			while($data_sgst=mysql_fetch_array($ptr_sgst_prod))
			{
				if($data_sgst['sgst_tax_in_per']>0)
				{
					...
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('J'.$rowCount, number_format($tot_sgst,3,'.',''));
					$rowCount++;
		...
		$sel_igst_prod="select SUM(i.igst_tax) as igst_total,SUM(i.sin_product_base_price) as base_total,i.sin_product_disc, SUM(i.discounted_price) as total_igst_val,i.igst_tax_in_per from inventory_product_map i, inventory s where i.inventory_id='".$val_query['inventory_id']."' and i.inventory_id=s.inventory_id and i.sin_product_total>0 and s.total_cost > 0 group by i.igst_tax_in_per";
		...
		if($tot_igst_num=mysql_num_rows($ptr_igst_prod))
		{
			while($data_igst=mysql_fetch_array($ptr_igst_prod))
			{
				if($data_igst['igst_tax_in_per']>0)
				{
					...
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('J'.$rowCount, number_format($tot_igst,3,'.',''));
					$rowCount++;
		...
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('H'.$rowCount, '');
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('I'.$rowCount, '');
		$total_actual= number_format($tot_disc_price_gst+$tot_disc_price_igst+$tot_disc_price+$tot_cgst+$tot_sgst+$tot_igst,3,'.','');
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('J'.$rowCount,  number_format($total_actual,3,'.',''));
		//==========================================================		
		$k++;
	...
	header('Content-Disposition: attachment;filename="Purchase Tally Report.xls"');
