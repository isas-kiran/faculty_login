        ...
        <?php
                    ...
                    <?php
                        ...
                        else
                        {
                            ...
                            if($record_id)
                            {
								 $select_pre_leave="select * from pr_previous_leave_management where month='".$_REQUEST['month']."' AND year='".$_REQUEST['year']."' AND staff_id='".$_REQUEST['staff_id']."'";
							...
							else
							{
								...
								"<br>".$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('staff_leaves','Update','Staff Leaves','".$record_id."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
							...
							else
							{
								...
								$data_record1['branch_name']=$_SESSION['branch_name'];
								   ...
								   $record_comission1=$db->query_insert("pr_previous_leave_management", $data_record1); 
											...
											"<br>".$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('staff_leaves','Add','Staff Leaves','".$slab_id."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
									$query=mysql_query($insert);  
...
function getmonthdays(i)
{
 ...
 function finalleavebalance() {
  ...
  $('#extra_leave_type').prop('disabled', false).trigger("chosen:updated");
	...
	$('#total_working_days').val(precisionRound(leave_balance,2));
	 }	 
 }
</script>
<script language="javascript">
