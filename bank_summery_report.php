...
<title>Bank Summery Report</title>
     ...
     <?php
						...
						if($diff)
						{
                            ...
                            <tr>
                                ...
                                <td colspan="4" style="font-size:12px"><strong>From Date- <?php echo $frm_dates; ?></strong></td>
                                <td colspan="5" style="font-size:12px"><strong>To Date- <?php echo $to_dates; ?></strong></td>
                            </tr>
                            <tr class="grey_td">
                            	<td width="4%" rowspan="2" align="center" style="font-size:12px"><strong>Sr. No</strong></td>
                                <td width="8%" rowspan="2" align="center" style="font-size:12px"><strong>Added Date</strong></td>
                                <td colspan="5" align="center" style="font-size:12px"><strong>Incomming</strong></td>
                                <td colspan="3" align="center" style="font-size:12px"><strong>Outgoing</strong></td>
                                <td width="12%" rowspan="2" align="center" style="font-size:12px"><strong>Deposite</strong></td>
                                <td width="12%" rowspan="2" align="center" style="font-size:12px"><strong>Withdrawl</strong></td>
                            </tr>
                            <tr class="grey_td" >
                                <td width="8%" align="center" style="font-size:12px"><strong>Enrollment</strong></td>
                                <td width="10%" align="center" style="font-size:12px"><strong>Receipt</strong></td>
                                <td width="10%" align="center" style="font-size:12px"><strong>Sale Prod.</strong></td>
                                <td width="10%" align="center" style="font-size:12px"><strong>Service</strong></td> 
                                <td width="10%" align="center" style="font-size:12px"><strong>Cash Deposited</strong></td> 
                                <td width="10%" align="center" style="font-size:12px"><strong>Expense</strong></td>
                                <td width="10%" align="center" style="font-size:12px"><strong>Purchase</strong></td>
                                 <td width="10%" align="center" style="font-size:12px"><strong>Cash Withdrawl</strong></td> 
							...
							for($i=0;$i<$data_difs;$i++)
							{
								...
								$tot_depo='';
								echo '<tr '.$bgcolor.'>';
								echo '<td align="center">'.$sr_nos.'</td>';
								echo '<td align="center">'.$dates.'</td>';
								//========================Enrollment==========================
								echo '<td align="center">';
								$sel_inv="select amount,enroll_id from invoice where DATE(added_date)='".$newdates."' ".$bank_ids." ".$search_cm_id." ";
								...
								while($data_inv=mysql_fetch_array($ptr_inv))
								{
									...
									echo $data_inv['amount']." (".$data_names['name'].") "."<br/>";
									$tot_inv +=$data_inv['amount'];
								}
								if($tot_inv >0)
									echo "<hr/><br/><strong>Total- ".$tot_inv."</strong>";
								$total_enroll +=$tot_inv;
								echo'</td>';
								//============================================================
								//========================Recipt==============================
								echo '<td align="center">';
								$sel_rec="select amount from receipt where 1 and (cash_transfer_mode ='' or cash_transfer_mode is NULL) and DATE(added_date)='".$newdates."' and amount>0 ".$banks_id." ".$search_cm_id."";
								$ptr_rec=mysql_query($sel_rec);
								while($data_rec=mysql_fetch_array($ptr_rec))
								{
									echo $data_rec['amount']."<br/>";
									$tot_rec +=$data_rec['amount'];
								}
								if($tot_rec >0)
									echo "<hr/><br/><strong>Total- ".$tot_rec."</strong>";
								$total_receipt +=$tot_rec;
								echo'</td>';
								//===============================================================
								//========================Sales Product==========================
								echo '<td align="center">';
								$sel_sp="select payable_amount from sales_product_invoice where DATE(added_date)='".$newdates."' and payable_amount>0 ".$banks_id." ".$search_cm_id."";
								$ptr_sp=mysql_query($sel_sp);
								while($data_sp=mysql_fetch_array($ptr_sp))
								{
									echo $data_sp['payable_amount']."<br/>";
									$tot_prod +=$data_sp['payable_amount'];
								}
								if($tot_prod>0)
									echo "<hr/><br/><strong>Total- ".$tot_prod."</strong>";
								$total_prod +=$tot_prod;
								echo'</td>';
								//===============================================================
								...
								//========================Service================================
								echo '<td align="center">';
								$sel_ser="select payable_amount from customer_service_invoice where DATE(added_date)='".$newdates."' and amount>0 ".$banks_id." ".$search_cm_id."";
								$ptr_ser=mysql_query($sel_ser);
								while($data_ser=mysql_fetch_array($ptr_ser))
								{
									echo $data_ser['payable_amount']."<br/>";
									$tot_ser +=$data_ser['payable_amount'];
								}
								if($tot_ser>0)
									echo "<hr/><br/><strong>Total- ".$tot_ser."</strong>";
								$total_serv +=$tot_ser;
								echo'</td>';
								//============================================================
								//========================Cash deposited==============================
								echo '<td align="center">';
								$sel_dep1="select SUM(amount) as total_amt from receipt where 1 ".$banks_id." and category='cash_transfer' and cash_transfer_mode='outword' and DATE(added_date)='".$newdates."' ".$search_cm_id." ";
								$ptr_dep1=mysql_query($sel_dep1);
								if(mysql_num_rows($ptr_dep1))
								{
									while($data_inward=mysql_fetch_array($ptr_dep1))
									{
										echo $data_inward['total_amt']."<br/>";
										$tot_depo +=$data_inward['total_amt'];
									}
								}
								$sel_bank_out="select SUM(amount) as total_amt from receipt where 1 ".$banks_id." and category='cash_transfer' and cash_transfer_mode='bank_to_bank' and DATE(added_date)='".$newdates."' ".$search_cm_id." ";
								...
								if($tot_depo >0)
									echo "<hr/><br/><strong>Total- ".$tot_depo."</strong>";
								$total_dep_bank +=$tot_depo;
								echo'</td>';
								//===============================================================
								//========================Expemse ==========================
								echo '<td align="center">';
								$sel_exp="select amount from expense where DATE(added_date)='".$newdates."' and amount>0 ".$banks_id." ".$search_cm_id."";
								$ptr_exp=mysql_query($sel_exp);
								while($data_exp=mysql_fetch_array($ptr_exp))
								{
									echo $data_exp['amount']."<br/>";
									$tot_exp +=$data_exp['amount'];
								}
								if($tot_exp>0)
									echo "<hr/><br/><strong>Total- ".$tot_exp."</strong>";
								$total_expe +=$tot_exp;
								echo'</td>';
								//============================================================
								//========================Purchse==========================
								echo '<td align="center">';
								$sel_pur="select payable_amount from inventory_invoice where DATE(added_date)='".$newdates."' and payable_amount>0 ".$banks_id." ".$search_cm_id."";
								$ptr_pur=mysql_query($sel_pur);
								while($data_pur=mysql_fetch_array($ptr_pur))
								{
									echo $data_pur['payable_amount']."<br/>";
									$tot_pur +=$data_pur['payable_amount'];
								}
								if($tot_pur>0)
									echo "<hr/><br/><strong>Total- ".$tot_pur."</strong>";
								...
								if(mysql_num_rows($ptr_bank_with))
								{
									...
									{
										echo $data_with_bank['total_amt']."<br/>";
										$tot_with +=$data_with_bank['total_amt'];
									}
								}
								if($tot_with >0)
									echo "<hr/><br/><strong>Total- ".$tot_with."</strong>";
								...
								echo '<td align="center">'.$total_credit.'</td>';
								echo '<td align="center">'.$total_debit.'</td>';
								echo '</tr>';
							}
