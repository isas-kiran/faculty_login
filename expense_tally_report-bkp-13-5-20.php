...
?>   
	...
	if($no_of_records)
	{
		...
		while($val_query=mysql_fetch_array($my_db))
		{
				...
				while($data_prod=mysql_fetch_array($ptr_prod))
				{
					...
					if($val_query['discount'] >0)
					{
						...
						else
						{
							...
							$tot_disc_per_org=($val_query['discount'])*100 / $data_prod['total_val'];
							$tot_disc_per=round($tot_disc_per_org);
							$disc=$tot_disc_per;
						}
					...
					echo '<td align="center" width="10%">'.round($tot_price).'</td>';
					echo '<td align="center" width="10%">'.$disc.'</td>';
					echo "<td align='center' width='10%'>".round($tot_disc_price)."</td>";
					echo '</tr>';
			...
			if($tot_prod_gst<=0 && $tot_prod_igst <=0 && $tot_prod_nullgst <=0 )
			{
				...
				if($tot_prod_nullgst=mysql_num_rows($ptr_prod))
				{
					while($data_prod=mysql_fetch_array($ptr_prod))
					{
						...
						if(($data_prod['total_val'] <=0 || $data_prod['total_val']='') &&( $data_prod['discount']>='100' && $data_prod['discount_type']=='percentage'))
						{
							$tot_price=$data_prod['amount_wo_tax'];
							$disc=100;
						}
			...
			if($tot_cgst=mysql_num_rows($ptr_cgst_prod))
			{
				while($data_cgst=mysql_fetch_array($ptr_cgst_prod))
				{
					if($data_cgst['cgst_tax_in_per']>0)
					{
						...
						echo '<td align="center" width="10%"></td>';
						echo '<td align="center" width="10%"></td>';
						echo "<td align='center' width='10%'>".round($data_cgst['cgst_total'])."</td>";
						echo '</tr>';
			...
			if($tot_sgst=mysql_num_rows($ptr_sgst_prod))
			{
				while($data_sgst=mysql_fetch_array($ptr_sgst_prod))
				{
					if($data_sgst['sgst_tax_in_per']>0)
					{
						...
						echo '<td align="center" width="10%"></td>';
						echo '<td align="center" width="10%"></td>';
						echo "<td align='center' width='10%'>".round($data_sgst['sgst_total'])."</td>";
						echo '</tr>';
			...
			if($tot_igst=mysql_num_rows($ptr_igst_prod))
			{
				while($data_igst=mysql_fetch_array($ptr_igst_prod))
				{
					if($data_igst['igst_tax_in_per']>0)
					{
						...
						echo '<td align="center" width="10%"></td>';
						echo '<td align="center" width="10%"></td>';
						echo "<td align='center' width='10%'>".round($data_igst['igst_total'])."</td>";
						echo '</tr>';
						$total_amnts +=round($data_igst['igst_total'],2);
					}
				}
			}
			echo'<tr>';
			...
			echo '<td align="center" width="10%"></td>';
			echo '<td align="center" width="10%"></td>';
			echo "<td align='center' width='10%'>".round($total_price)."</td>";
			echo '</tr>';
			echo '</table></td>';
