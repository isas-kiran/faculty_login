						...
						else if($_POST['formAction']=="change_owner")
						{
							if($_POST['councillior_admin_id']!="")
                        	{
								...
								for($r=0;$r<count($_POST['chkRecords']);$r++)
								{
									...
									if($_SESSION['type']!="S" && $_SESSION['type']!="A" && $data_admin['employee_id']!=$admin_id)
									{
	...
	<td colspan="18">
        <table border="0" cellspacing="0" cellpadding="0" width="99%" align="center">
           	<tr>
                ...
                <?php if($_SESSION['type']=='S') {?><td width="4%" align="center"> <a href="excel.php<?php echo $sep_url_string; ?>"><img src="images/excel.png" height="30px" width="30px"/></a></td><?php } ?>
			</tr>
        </table>
    </td>
</tr>
...
<?php
                        ...
                        {          
								...
								{
									$pre_keyword =" and (mobile1 like '%".$keyword."%' || mobile2 like '%".$keyword."%' )";
								}
                            }                            
                       		else
                            	$pre_keyword="";
						...
						{
							$intval=1;
							$sql_query= "SELECT * FROM inquiry where 1 ".$status_type." ".$record_cat_id." ".$cm_ids." ".$search_cm_id." ".$pre_keyword." ".$enquiry_added." ".$assigned_to." ".$enquiry_src." ".$resps." ".$response." ".$lead_grade_by." ".$where_followup." ".$pre_from_date." ".$pre_to_date." ".$c_id." ".$select_directory.""; 
						}
						else
						{
							$intval=$pre_keyword;
...
//                                $val_contry = $db->fetch_array($db->query($select_country));
								...
								else
								{
									...
									//echo '<a href="add_student_gst.php?record_id='.$listed_record_id.'" class="table_font">';
									//echo '</a>';
								}
								echo'</td>';
                               ...
                               /* echo '<td align="center">'.$val_query['contact'].'</td>';*/
								...
								if($_SESSION['type']=='S' || ($val_query['campaign_id']!='' && $val_query['followup_date']==''))
								{
                    ...
                    <table border="0" cellspacing="15" cellpadding="0" width="100%">
						...
						if($_SESSION['type']=="S" || $_SESSION['type']=="A")
						{
							...
							<tr>
								...
								<?php
								$sle_name="select admin_id,name,branch_name from site_setting where 1 and system_status='Enabled' and (type='C' or type='A') order by name asc"; 
								$ptr_name=mysql_query($sle_name);
								while($data_name=mysql_fetch_array($ptr_name))
								{
