...
<table border="0" cellspacing="0" cellpadding="0" width="100%">
  <tr>
    ...
    <td class="top_mid" valign="bottom"><?php include "include/page_menu.php";?></td>
    ...
    <?php       if($_POST['formAction'])
                    {
                        if($_POST['formAction']=="delete")
                        {
                            for($r=0;$r<count($_POST['chkRecords']);$r++)
                            {
                                ...
                                $sql_query= "SELECT page_id FROM ".$GLOBALS["pre_db"]."pages where page_id ='".$del_record_id."'";
                                if(mysql_num_rows($db->query($sql_query)))
                                    {                                                
                                        $delete_query="delete from ".$GLOBALS["pre_db"]."pages where page_id='".$del_record_id."'";
...
<!--                                <option value="Active">Active</option>
    ...
    <?php
                        ...
                        if($keyword)
                            $pre_keyword=" and (page_title like '%".$keyword."%' || description like '%".$keyword."%' )";
                        else
                            $pre_keyword="";
                        ...
                        if($_GET['orderby']=='page_title' )
                            $img1 = $img;

                        if($_GET['order'] !='' && ($_GET['orderby']=='page_title'))
                        {
                            $select_directory .= " order by ".$_GET['orderby']." " .$_GET['order'] ;
                            $date_query='&order='.$_GET['order'].'&orderby='.$_GET['orderby'];
                        }
                        else
                            $select_directory='order by page_id desc';                      
                            $sql_query= "SELECT * FROM pages where 1  ".$pre_keyword." ".$_SESSION['where_admin_id']." ".$select_directory.""; 
                       //echo $sql_query;
    ...
    <form method="post"  name="frmTakeAction" action="<?php echo $_SERVER['PHP_SELF'].'?#msgbox';?>" >
                     <input type="hidden" name="formAction" id="formAction" value=""/>
                      <tr class="grey_td" >
                        ...
                        <td width="10%"><a href="<?php echo $_SERVER['PHP_SELF'];?>?order=<?php echo $order."&orderby=page_title".$query_string;?>" class="table_font"><strong>Page Title</strong></a> <?php echo $img1;?></td>
                       ...
                       <td width="15%"><strong>Tag</strong></td>
                        ...
                        <td width="10%" class="centerAlign"><strong>Action</strong></td>
                            ...
                            while($val_query=mysql_fetch_array($all_records))
                            {
                                ...
                                $listed_record_id=$val_query['page_id']; 
                                ...
                                echo '<td ><a href="add_pages.php?record_id='.$listed_record_id.'" >'.$val_query['title'].'</a></td>';
                                echo '<td >'.$post.'</td>';
                                echo '<td align="center">'.$val_query['added_date'].'</td>';
                                echo '<td align="center"><a href="add_pages.php?record_id='.$listed_record_id.'" ><img src="images/edit_icon.png" title="Edit Record" class="example-fade"/></a>&nbsp;&nbsp;
