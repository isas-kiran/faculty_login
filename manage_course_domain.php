...
<title>Manage Course Domain</title>
    ...
    <?php       if($_POST['formAction'])
                    {
                        if($_POST['formAction']=="delete")
                        {
                            for($r=0;$r<count($_POST['chkRecords']);$r++)
                            {
                                ...
                                $sql_query= "SELECT cat_id FROM course_domain_category  where cat_id ='".$del_record_id."'";
                                if(mysql_num_rows($db->query($sql_query)))
								{                                                
									$delete_query="delete from course_domain_category where cat_id='".$del_record_id."'";
...
<table cellspacing="0" cellpadding="0" class="table" width="95%">
    ...
    <?php
		...
		if($_GET['order'] !='' && ($_GET['orderby']=='category_name'))
		{
			...
			$select_directory='order by cat_id asc';                      
			$sql_query= "SELECT * FROM course_domain_category where 1  ".$pre_keyword." ".$select_directory." "; 
	   //echo $sql_query;
		...
		if($no_of_records)
		{
		   ...
		   // $pager = new PS_Pagination($sql_query,$_SESSION['show_records'],$_SESSION['show_records_pages'],$query_string1);
            ...
            <tr class="grey_td" >
                ...
                <td width="20%"><a href="<?php echo $_SERVER['PHP_SELF'];?>?order=<?php echo $order."&orderby=country_name".$query_string;?>" class="table_font"><strong>Domain Name</strong></a> <?php echo $img1;?></td>
			...
			while($val_query=mysql_fetch_array($all_records))
			{
				if($bgColorCounter%2==0)
					...
					$bgcolor="";                
				$listed_record_id=$val_query['cat_id']; 
				...
				echo '<td ><a href="add_course_domain.php?record_id='.$listed_record_id.'" class="table_font">'.$val_query['cat_name'].'</a></td>';
				//echo '<td >'.$val_query['country_id'].'</td>';
				if($_SESSION['type'] =='S')
				{ 
					echo '<td align="center"><a href="add_course_domain.php?record_id='.$listed_record_id.'" ><img src="images/edit_icon.png" title="Edit Record" class="example-fade"/></a>&nbsp;&nbsp;<a onclick="return validationToDelete(\''.$_SERVER['PHP_SELF'].'\');" href="'.$_SERVER['PHP_SELF'].'?deleteRecord=1&record_id='.$listed_record_id.'&page='.$_REQUEST['page'].$query_string1.'"><img src="images/delete_icon.png" title="Delete Record" class="example-fade" /></a>&nbsp;&nbsp;';
					echo '</td>';
				}
				echo '</tr>';
