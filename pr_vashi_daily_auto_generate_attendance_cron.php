...
<title id="title">Vashi Auto Generate Attendance</title>
...
<?php
	...
	$year='2021';//date('Y');
	$todays_date='2021-08-10';//date('Y-m-d');
	...
	echo"<br/>".$sel_serv_id="select service_tag_id from biometric_device where cm_id='".$cm_id."'";
	$ptr_serv_id=mysql_query($sel_serv_id);
	echo '<br/>tot- '.mysql_num_rows($ptr_serv_id);
	$data_serv_id=mysql_fetch_array($ptr_serv_id);
	echo'<br/>'.$service_tag_id=$data_serv_id['service_tag_id'];
	...
	echo "<br/>".$select_att="select DISTINCT(UserID) from pr_pune_biometric_attendance where cm_id='".$cm_id."' and ServiceTagId='".$service_tag_id."' and year='".$year."' and month='".$month."' and DATE(PunchTime)='".$todays_date."' order by UserID asc ";
...
{ 
	...
	try {
			...
			$mail->Username = 'info@isas.in';                   // SMTP username
			$mail->Password = 'info@isas08';                            // SMTP password
			$mail->SMTPSecure = 'ssl';                            // Enable encryption, 'tls' also accepted
			$mail->Port = 465;
			$mail->setFrom('info@isas.in', 'ISAS LLP');
			$mail->addAddress("erp.isas@gmail.com"); 
			//$mail->addCC("santosh.sapke@gmail.com"); 
			$mail->addCC("hrd@isasbeautyschool.com");
			...
			$mail->Subject = 'ISAS LLP'.$branch_name.' Daily Attendace is generated for '.date('Y-m-d').'';
