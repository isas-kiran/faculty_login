...
<table border="0" cellspacing="0" cellpadding="0" width="100%">
    ...
    <tr>
        ...
        <td class="mid_mid" align="center">
			...
			if($_REQUEST['from_date'] && $_REQUEST['from_date']!=="0000-00-00" && $_REQUEST['from_date']!="From Date")
			{
				...
				$invoice_from_date_i=" and DATE(iv.added_date) >='".date('Y-m-d',strtotime($from_date))."'";
			...
			else
			{
				...
				if($_REQUEST['to_date']=='')
				{
					...
					$paid_from_date=" and DATE(added_date) >='".date('Y-m-d',strtotime($from_date))."'";
					$installment_from_date=" and DATE(installment_date) >='".date('Y-m-d',strtotime($from_date))."'";
					$invoice_from_date_i=" and DATE(iv.added_dare) >='".date('Y-m-d',strtotime($from_date))."'";
					$installment_from_date_i=" and DATE(i.installment_date) >='".date('Y-m-d',strtotime($from_date))."'";
				}
				else
				{
			...
			if($_REQUEST['to_date']  && $_REQUEST['to_date']!="To Date")
			{
				...
				$invoice_to_date_i=" and DATE(iv.added_date)<='".date('Y-m-d',strtotime($to_dates))."'";
	...
	{
		echo "<br/>1- ".$sql_query="SELECT DISTINCT(e.enroll_id),e.cm_id,e.name,e.course_id,e.net_fees,e.down_payment,e.paid,balance_amt,e.assigned_to FROM enrollment e, installment i WHERE 1 and (e.student_status ='Active' or e.student_status is NULL) and e.enroll_id=i.enroll_id ".$where_staff_id_e." ".$where_cm." ".$search_cm_id_i." ".$installment_from_date_i." ".$installment_to_date_i." ".$pre_keyword." ".$select_directory."  ";
		
		echo "<br/>".$sql_qa="select e.* from `enrollment` e,`installment` i, `invoice` iv where e.enroll_id=i.enroll_id and e.enroll_id=iv.enroll_id ".$installment_from_date_i." ".$installment_to_date_i." ".$invoice_from_date_i." ".$invoice_to_date_i." ";
	}
	else
	{
