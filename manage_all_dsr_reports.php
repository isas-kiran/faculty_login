				...
				while($data_cmids=mysql_fetch_array($ptr_cm))
				{
					...
					if($no_of_records)
					{
						...
						while($data_ptr_sel1=mysql_fetch_array($ptr_records))
						{
							...
							if($i==1)
							{
								echo '<td rowspan="'.$no_of_records.'" align="center" style="border:1px solid #CCC">'.round($cash_in_hands,2).'</td> <input type="hidden" name="cash_in_hand" value="'.round($cash_in_hands,2).'"  />';
								echo '<td rowspan="'.$no_of_records.'" align="center" style="border:1px solid #CCC">'.round($petty_cash_in_hands,2).'</td> <input type="hidden" name="petty_cash_in_hand" value="'.round($petty_cash_in_hands,2).'" >';	//(3/6/19)
							}
							echo '</tr>';
							$i++;
							$k++;
							$bgColorCounter++;
							$rno++;
						} // while loop for bank balance
					} // num rows check for bank bal
				} // while loop for CM id
			} //if condition for cm id
			//==============**************===== ISAS LLP ====****************==============
			$cash_in_hands='';
			$petty_cash_in_hands='';
			$opening_cash1='';
				...
				while($data_cmids=mysql_fetch_array($ptr_cm))
				{
					...
					if($no_of_records2)
					{
						...
						while($data_ptr_sel2=mysql_fetch_array($ptr_records1))
						{
							...
							if($i==1)
							{
								echo '<td rowspan="'.$no_of_records2.'" align="center" style="border:1px solid #CCC">'.round($cash_in_hands,2).'</td> <input type="hidden" name="cash_in_hand" value="'.round($cash_in_hands,2).'"  />';
								echo '<td rowspan="'.$no_of_records2.'" align="center" style="border:1px solid #CCC">'.round($petty_cash_in_hands,2).'</td> <input type="hidden" name="petty_cash_in_hand" value="'.round($petty_cash_in_hands,2).'" >';	//(3/6/19)
							}
							echo '</tr>';
							$i++;
							$k++;
