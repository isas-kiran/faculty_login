...
{    
	...
	else			//========Kit offer===============
	{
		...
		if(document.getElementById('off_toal_fees') && vals !='' && no==1)
		{
			courses_fees=roundNumber(document.getElementById('off_toal_fees').value,3);
				...
				else
				{
					...
					if($record_id)
					{
						...
						if(!mysql_num_rows($ptr_exist))
						{
							...
							//==============Update Invoice No.=====================
							$sel_inv="select ext_invoice_no from enrollment where cm_id='".$data_record_invoice['cm_id']."' and ext_invoice_no IS NOT NULL order by ext_invoice_no desc limit 0,1";
...
<script language="javascript">
	show_course(<?php echo $row['course_id']; ?>)
</script>
<script>
if(document.getElementById("branch_name"))
{
