...
<title>Manage Staff Product Management</title>
  ...
  <tr>
    ...
    <?php if($_POST['formAction'])

                    {
                        ...
                        if($_POST['formAction']=="delete")
                        {
                            ...
                            for($r=0;$r<count($_POST['chkRecords']);$r++)
                            {
								...
								$val=explode('-',$ds);
                              echo   "</br>".$del_record_id=$val[0];
							echo 	"</br>".$month=$val[1];
							echo	"</br>".$year=$val[2];
                            echo    $sql_query= "SELECT staff_id,month,year FROM ".$GLOBALS["pre_db"]."pr_staff_product_incentive where staff_id ='".$del_record_id."' AND month='".$month."' AND year='".$year."'";
                                if(mysql_num_rows($db->query($sql_query)))
                                {
									"<br>".$sql_query= "SELECT * FROM pr_staff_product_incentive where staff_id ='".$del_record_id."' AND month='".$month."' AND year='".$year."'";              
                                    ...
                                    $delete_query="delete from ".$GLOBALS["pre_db"]."pr_staff_product_incentive where staff_id='".$del_record_id."' AND month='".$month."' AND year='".$year."'";
                    ...
                    if($_REQUEST['deleteRecord'] && $_REQUEST['record_id'])
                    {
                        ...
                        $sql_query= "SELECT staff_id,year,month FROM ".$GLOBALS["pre_db"]."pr_staff_product_incentive where staff_id='".$del_record_id."' AND month='".$_REQUEST['month']."' AND year='".$_REQUEST['year']."'";
                        if(mysql_num_rows($db->query($sql_query)))
                        {
							"<br>".$sql_query= "SELECT * FROM pr_staff_product_incentive where staff_id ='".$del_record_id."' AND month='".$_REQUEST['month']."' AND year='".$_REQUEST['year']."'";              
							$query=mysql_query($sql_query);
							$fetch=mysql_fetch_array($query);
								
							"<br>".$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('manage_user','Delete','".$fetch['name']."','".$del_record_id."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
							$query=mysql_query($insert);                        
                        	$delete_query="delete from ".$GLOBALS["pre_db"]."pr_staff_product_incentive where staff_id='".$del_record_id."' AND month='".$_REQUEST['month']."' AND year='".$_REQUEST['year']."'";
                        	$db->query($delete_query);
    ...
    <?php
                        ...
                        if($keyword !='')
                            ...
                            {                            
                              ...
                              $pre_keyword=" ((u.name like '%".$keyword."%') || (o.year like '%".$keyword."%') || (o.month like '%".$keyword."%' || (o.branch_name like '%".$keyword."%')))";
                            }                            
						...
						if($_GET['record_id'] !='')
						{
						 ...
						 $sql_query= "SELECT DISTINCT u.attendence_id,u.name,o.staff_id,o.year,o.month FROM site_setting u, pr_staff_product_incentive o where ".$pre_keyword." AND o.staff_id=u.attendence_id"; 
                       // $sql_query= "SELECT DISTINCT staff_id,year,month FROM pr_staff_product_incentive"; 
   ...
   <td width="12%"><a href="<?php echo $_SERVER['PHP_SELF'];?>?order=<?php echo $order."&orderby=name".$query_string;?>" class="table_font"><strong>Staff Name</strong></a> <?php echo $img1;?></td>
    ...
    <td width="7%" class="centerAlign"><strong>Action</strong></td>
...

                            {
                               ...
                               $listed_record_id=$val_query['staff_id']; 
							   $listed_record_id1=$val_query['staff_id'].'-'.$val_query['month'].'-'.$val_query['year']; 
                                $select_event= "select * from site_setting where attendence_id = '".$val_query['staff_id']."' ";                                           
                                ...
                                */
                                ...
                                echo '<td align="center">'.$sr_no.'</td>';                                
                               	...
                               	echo '<td >'.$val_event['name'].'</td>';
                                echo '<td >'.$val_query["year"].'</td>';
                                echo '<td >'.$month.'</td>';
								// echo '<td >'.$val_query['grand_total'].'</td>';
									echo '<td align="center"><a href="staff_product_incentive.php?record_id='.$val_query['staff_id'].'&month='.$val_query['month'].'&year='.$val_query['year'].'"><img src="images/edit_icon.png" height="20px" width="20px" title="Edit Event" class="example-fade"/></a>&nbsp;
									
									<a onclick="return validationToDelete(\''.$_SERVER['PHP_SELF'].'\');" href="'.$_SERVER['PHP_SELF'].'?deleteRecord=1&record_id='.$listed_record_id.'&year='.$val_query['year'].'&month='.$val_query['month'].'&page='.$_REQUEST['page'].$query_string1.'"><img src="images/delete_icon.png" title="Delete Record" class="example-fade" /></a>&nbsp;&nbsp;';
									echo '</td>';
