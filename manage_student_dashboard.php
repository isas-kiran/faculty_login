                ...
                <table cellspacing="0" cellpadding="0" class="table" width="98%">
                    ...
                    <tr>
                   		<td width="60%" valign="top" style="width:100%;padding:0; margin:0" >
							...
							<table width="100%">
                            	<tr>
                                	<td colspan="2"><img src="images/new_gif.gif" width="40" height="12"/>Note : Due to multiple followups pending, page loaded very slow, so we show data only between respected dates shown in red color. Also if you want to find your pending followups, create filter in <a href="manage_student.php" target="_blank">Manage Enquiry</a></td>
                                </tr>
                            </table>   
							<?php
							$branch_name='';
                            ...
                            <ul id="qm0" class="qmmc">
                                    ...
                                    <?php if($_SESSION['type']=='A' || $_SESSION['type']=='S' || $_SESSION['type']=='Z' || $_SESSION['type']=='LD' || $_SESSION['type']=='Z' || $_SESSION['type']=='LD' || $_SESSION['type']=='C')
                                    {
                                       $sele_latest="select inquiry_id,firstname, course_id, mobile1,cm_id,response,admin_id,employee_id from inquiry where status = 'Enquiry' and (response !='7' and response!='8' or response is NULL) and lead_category_followup='phone_followup' and (followup_date >= '".date('Y-m-d', strtotime('-2 days'))."' and DATE(followup_date) <='".date('Y-m-d')."') ".$_SESSION['where']." ".$search_cm_id." order by followup_date desc "; //followup_date <= '".date('Y-m-d', strtotime('+2 days'))."' and  
                                        $ptr_select_letest=mysql_query($sele_latest);
                                        $count=mysql_num_rows($ptr_select_letest);
                                        ?>
                                        <li><a class="qmparent" href="javascript:void(0)"><span style="color:#0088CB;">Enquiry Followup By Call <span style="color:red; font-weight:600">(<?php echo date('d-M-y', strtotime('-2 days'))." to ".date('d-M-y')?>) </span> Total - (<?php echo $count; ?>)</span></a>
                                        <ul>
                                        ...
                                        if(mysql_num_rows($ptr_select_letest))
                                        {
                                            ...
                                            {
                                               "<br/>".$sele_latest_rec="select followup_date,lead_category_followup, added_date,lead_grade,response,admin_id,cm_id from followup_details where student_id = '".$val_select_letest['inquiry_id']."' and (response!='7' and response!='8' or response is NULL) and (followup_date >= '".date('Y-m-d', strtotime('-2 days'))."' and followup_date <='".date('Y-m-d')."') ".$_SESSION['where']." ".$search_cm_id." order by followup_id desc limit 0,1";//followup_date <= '".date('Y-m-d', strtotime('+2 days'))."' and 
                                    ...
                                    <?php if($_SESSION['type']=='A' || $_SESSION['type']=='S' || $_SESSION['type']=='Z' || $_SESSION['type']=='LD' || $_SESSION['type']=='C')
                                    {
                                        $sele_latest="select inquiry_id,firstname, course_id, mobile1,cm_id,response,admin_id ,employee_id from inquiry where status = 'Enquiry' and (response!='7' and response!='8' or response is NULL) and lead_category_followup='walkin_followup' and (followup_date >= '".date('Y-m-d', strtotime('-2 days'))."' and followup_date <= '".date('Y-m-d')."') ".$_SESSION['where']." ".$search_cm_id." order by followup_date desc";//and  '".date('Y-m-d', strtotime('+2 days'))."'
                                        $ptr_select_letest=mysql_query($sele_latest);
                                        $count=mysql_num_rows($ptr_select_letest);
                                        ?>
                                        <li><a class="qmparent" href="javascript:void(0)"><span style="color:#0088CB;">Enquiry Followup By Walk-in <span style="color:red; font-weight:600">(<?php echo date('d-M-y', strtotime('-2 days'))." to ".date('d-M-y')?>)</span> Total - (<?php echo $count; ?>)</span></a>
                                        <ul>
                                        <li><span style="color:#000;"><table class="adminlist">
                                        <thead>
                                        <?php
                                        ...
                                        if(mysql_num_rows($ptr_select_letest))
                                        {
                                            while($val_select_letest=mysql_fetch_array($ptr_select_letest))
                                            {
                                                $sele_latest_rec="select followup_date,lead_category_followup, added_date,lead_grade,response,cm_id,admin_id from followup_details where student_id = '".$val_select_letest['inquiry_id']."' and (followup_date >= '".date('Y-m-d', strtotime('-2 days'))."' and followup_date <= '".date('Y-m-d')."') ".$_SESSION['where']." ".$search_cm_id." order by followup_id desc";//, strtotime('+2 days')
                                                $ptr_select_letest_rec=mysql_query($sele_latest_rec);
                                                $val_select_letest_rec=mysql_fetch_array($ptr_select_letest_rec);	
                                    ...
                                    <?php if($_SESSION['type']=='A' || $_SESSION['type']=='S' || $_SESSION['type']=='Z' || $_SESSION['type']=='LD' || $_SESSION['type']=='C'	 || $_SESSION['type']=='F')
                                    {
                                       $sele_latest="select * from inquiry where status = 'Enquiry' and followup_date IS NULL and (DATE(added_date) >= '".date('Y-m-d', strtotime('-2 days'))."' and DATE(added_date) <= '".date('Y-m-d')."') ".$_SESSION['where']." ".$search_cm_id." order by added_date asc";//and  '".date('Y-m-d', strtotime('+2 days'))."'
                                        $ptr_select_letest=mysql_query($sele_latest);
                                        $count=mysql_num_rows($ptr_select_letest);
                                        ?>
                                        <li><a class="qmparent" href="javascript:void(0)"><span style="color:#0088CB;">Campaign Enquiries <span style="color:red; font-weight:600">(<?php echo date('d-M-y', strtotime('-2 days'))." to ".date('d-M-y')?>)</span> Total - (<?php echo $count; ?>)</span></a>
                                        <ul>
                                        ...
                                        if(mysql_num_rows($ptr_select_letest))
                                        {
                                            while($val_select_letest=mysql_fetch_array($ptr_select_letest))
                                            {
                                                ...
                                                $campaign_id=$val_select_letest['enquiry_source'];
                                                
                                                $sel_cmp="select campaign_name from campaign where campaign_id='".$campaign_id."'";
                                    ...
                                    <?php if($_SESSION['type']=='A' || $_SESSION['type']=='S' || $_SESSION['type']=='Z' || $_SESSION['type']=='LD' || $_SESSION['type']=='C' || $_SESSION['type']=='F')
                                    {
                                        ...
                                        $count=mysql_num_rows($ptr_select_letest);
                                        ?>
                                        <li><a class="qmparent" href="javascript:void(0)"><span style="color:#0088CB;">Today's Enquiries <span style="color:red; font-weight:600">(<?php echo date('d-M-y'); ?>)</span> Total - <?php echo $count; ?></span></a>
                                        <ul>
                                        <li><span style="color:#000;">
                                    ...
                                    <?php if($_SESSION['type']=='A' || $_SESSION['type']=='S' || $_SESSION['type']=='Z' || $_SESSION['type']=='LD' || $_SESSION['type']=='C')
                                    {	
                                        $select_letest="select enroll_id,installment_amount,installment_date,status from installment where 1 ".$_SESSION['where']." ".$search_cm_id." and installment_date < '".date('Y-m-d')."' and installment_date !='' and installment_date!='0000-00-00' and installment_date!='--' and installment_date >='".date('d-M-y', strtotime('-2 days'))."' and installment_date < '".date('d-M-y', strtotime('+1 days'))."' and installment_id NOT IN( select installment_id from installment_followup_details where DATE(added_date) ='2018-01-15') and  status ='not paid' order by installment_date asc";
                                        $ptr_select_letest=mysql_query($select_letest);
                                        $count=mysql_num_rows($ptr_select_letest);
                                        ?>
                                        <li><a class="qmparent" href="javascript:void(0)"><span style="color:#0088CB;">Pending & Upcomming Installments - <span style="color:red; font-weight:600">(<?php echo date('d-M-y', strtotime('-2 days'))." to ".date('d-M-y', strtotime('+1 days'))?>)</span> Total - (<?php echo $count; ?>)</span> </a>
                                        <ul>
                                        <li><span style="color:#000;"><table class="adminlist">
                                        <thead>
                                        <?php
                                    ...
                                    <?php
                                        $select_letest="select enroll_id,installment_amount,installment_date,status from installment where 1 ".$_SESSION['where']." and installment_date >= '".date('Y-m-d')."' and installment_date<='".date('Y-m-d', strtotime('+2 days'))."' and installment_date !='' and installment_id NOT IN( select installment_id from installment_followup_details where DATE(added_date) ='2018-01-15') and status ='not paid' order by installment_date asc";
                                        $ptr_select_letest=mysql_query($select_letest);
                                        if(mysql_num_rows($ptr_select_letest))
                                        {
                                            ?>
                                    ...
                                    <?php
                                        ...
                                        while($val_query=mysql_fetch_array($ptr_select_letest))
                                        {
                                            "<br/>".$sele_latest_rec="select followup_date,lead_category_followup,lead_grade,added_date,cm_id,admin_id,installment_id from installment_followup_details where student_id = '".$val_query['enroll_id']."' and followup_date >= '".date('Y-m-d')."' and followup_date <='".date('Y-m-d', strtotime('+2 days'))."' and status!='paid' order by followup_id desc";
                                            $ptr_select_letest_rec=mysql_query($sele_latest_rec);
                                            if(mysql_num_rows($ptr_select_letest_rec))
                                            {
                                                $val_select_letest_rec=mysql_fetch_array($ptr_select_letest_rec);	
...
<!-- =========================================================================NEW Enquiry==================================================================================--> 
 ...
 <!--=============================================================================END Enquiry=================================================================================-->
                                    ...
                                    //} ?>
  <!--==========================================================================Batch Schedule================================================================================-->          
                                    <?php if($_SESSION['type']=='A' || $_SESSION['type']=='S' || $_SESSION['type']=='Z' || $_SESSION['type']=='LD')
                                    {?>
                                      ...
                                      <? }?>
 <!--=================================================================Schedule Induction====================================================================================-->
                                    <?php if($_SESSION['type']=='A' || $_SESSION['type']=='S' || $_SESSION['type']=='Z' || $_SESSION['type']=='LD' || $_SESSION['type']=='B')
                                    {?>
 ...
 <!--==============================================================================Inform BOP===================================================================================-->
                                    ...
                                    {?>
                                      ...
                                      <? } ?>
 <!--=========================================================================================================================================================================-->
                                    <?php if($_SESSION['type']=='B' || $_SESSION['type']=='Z' || $_SESSION['type']=='LD')
                                    {?>
 ...
 <!--=========================================================================================================================================================================-->
                                    ...
                                    <? } ?>  
 <!--========================================================================================================================================================================-->
                                    <?php if($_SESSION['type']=='S' || $_SESSION['type']=='Z' || $_SESSION['type']=='LD' || $_SESSION['type']=='C' || $_SESSION['type']=='A'  )
                                    {?>
                                    ...
                                    <!--<li><a class="qmparent" href="javascript:void(0)"> <span style="color:#0088CB;">Pending Course Logsheet</span></a>
                                     ...
                                     <? } ?>   
 <!--=========================================================================================================================================================================-->
                                    <?php if($_SESSION['type']=='S' || $_SESSION['type']=='Z' || $_SESSION['type']=='LD' || $_SESSION['type']=='F' || $_SESSION['type']=='A'  )
                                    {?>
