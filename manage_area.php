    ...
    <td class="mid_mid">
     <?php 
        if($_REQUEST['deleteRecord'])
        {
            ...
            $sql_query= "SELECT id FROM city_area where area_id='".$del_record_id."'";
            if(mysql_num_rows($db->query($sql_query)))
            {                           
            $delete_query="delete from city_area where area_id='".$del_record_id."'";
...
/* function validme()
	 {
        ...
        <?php
        $sql_records= "SELECT * FROM city_area where 1 ".$pre_transcation_id." order by area_name asc";
        ...
        if($no_of_records)
        {
            ...
            <table cellpadding="0" cellspacing="0" width="100%" border="0">
                ...
                <tr align="center" class="grey_td" >
                    ...
                    <td width="11%" class="tr-header"><strong>Area name</strong></td>
                    <td width="11%" class="tr-header"><strong>City name</strong></td>
                ...
                while($val_record=mysql_fetch_array($all_records))
                {
					...
					$sql_state= "SELECT name FROM states where 1 and id ='".$val_record['state_id']."' ";
                    $ptr_state=mysql_query($sql_state);
                    $val_state=mysql_fetch_array($ptr_state);
					
					$sql_city= "SELECT name FROM cities where 1 and id ='".$val_record['city_id']."' ";
                    $ptr_city=mysql_query($sql_city);
                    $val_city=mysql_fetch_array($ptr_city);
                    ...
                    echo "<td align='center'>".$val_record['area_name']."</td>";
					echo "<td align='center'>".$val_city['name']."</td>";
					echo "<td align='center'>".$val_state['name']."</td>";
        ...
        if($_POST['save_changes'])
        {
            ...
            else
            {
                ...
                for($j=1;$j<=$total_floor;$j++)
                {
                    if($_POST['sub_name'.$j] !='')
                    {
                        $data_record_type1['city_id']=$city_id;
						$data_record_type1['state_id']=$state_id;
                        $data_record_type1['area_name']=addslashes(trim($_POST['sub_name'.$j]));
                       
                        $record_ids=$db->query_insert("city_area",$data_record_type1);
                ...
                setTimeout('document.location.href="manage_area.php";',1000);
                </script>
                <?php
            }
        }
                ...
                <td>
                    <div id="show_cities">
                        <table width="100%">
                            <tr>
                                <td>
                                <select id="city" name="city" style="width:460px"> <!--onchange="select_area(this.value)"-->
                ...
                <td colspan="3">
                    ...
                    <tr>
                        ...
                        <table cellpadding="5" width="100%" >
                            ...
                            {
                                var shows_data='<div id="floor_id'+idss+'"><table cellspacing="3" id="tbl'+idss+'" width="100%"><tr><td valign="top" width="10%" align="center">'+idss+'</td><td valign="top" width="40%" align="center">Enter City Name</td><td valign="top" width="50%" align="center"><input type="text" name="sub_name'+idss+'" id="sub_name'+idss+'" style="width:90%" required/></td><input type="hidden" name="row_deleted'+idss+'" id="row_deleted'+idss+'" value="" /><tr></table></div>';
