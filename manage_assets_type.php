                	...
                	if($_POST['formAction']=="delete")
                    	{
                            for($r=0;$r<count($_POST['chkRecords']);$r++)
                            {
                                ...
                                $sql_query= "SELECT asset_type_id,asset_name FROM assets_type where asset_type_id ='".$del_record_id."'";
								...
								{    
									...
									$delete_query="delete from assets_type where asset_type_id='".$del_record_id."'";
									$db->query($delete_query);  
									
									$delete_query12="delete from assets_vendor_map where asset_type_id='".$del_record_id."'";
  ...
  <tr class="head_td">
	...
	<table border="0" cellspacing="0" cellpadding="0" width="99%" align="center">
              <tr>
				...
				<?php
				/*if($_REQUEST['branch_name'] !='' || $_SESSION['type']!='S')
				{
					if($_SESSION['type']=='S')
					{
    ...
    <?php
                        ...
                        if($_GET['order'] !='' && ($_GET['orderby']=='asset_name'))
                        {
							...
							$select_directory='order by asset_type_id desc';  
						...
						{*/
							...
							$sql_query= "SELECT p.asset_name, p.asset_code,p.status, p.description, p.size, p.price, p.category_id, p.asset_type_id,p.quantity,p.quantity_in_shelf,p.quantity_in_consumable,p.added_date,p.admin_id,p.hsn_code,p.brand FROM assets_type p, assets_category pc where 1  and (p.asset_name like '%".$keyword."%' or p.asset_code like '%".$keyword."%' or p.description like '%".$keyword."%' or p.size like '%".$keyword."%' or p.commission like '%".$keyword."%' or p.price like '%".$keyword."%' or pc.name like '%".$keyword."%' ) ".$status_type." and p.category_id=pc.asset_category_id ".$vendor_ids." ".$cm_ids."  ".$search_cm_id_S." ".$search_admin_id_S." ".$select_directory.""; //".$admin_ids."
						//}
                       //echo $sql_query;
    				...
    				<form method="post"  name="frmTakeAction" action="<?php echo $_SERVER['PHP_SELF'].'?#msgbox';?>" >
                     ...
                     <input type="hidden" name="councillior_admin_id" id="councillior_admin_id" value=""  />
                      <tr class="grey_td" >
                        ...
                        <td width="20%"><a href="<?php echo $_SERVER['PHP_SELF'];?>?order=<?php echo $order."&orderby=product_name".$query_string;?>" class="table_font"><strong>Asset Name</strong></a> <?php echo $img1;?></td>
                        <td width="7%" align="center"><strong>Asset Code</strong></td>
						<!--<td width="5%" align="center"><strong>HSN Code</strong></td>-->
                        <td width="8%" align="center"><strong>Asset Category</strong></td>
						...
						?>
                            ...
                            while($val_query=mysql_fetch_array($all_records))
                            {
                                ...
                                $listed_record_id=$val_query['asset_type_id']; 
                                ...
                                echo '<td >'.$val_query['asset_name'].'</td>';							
								echo '<td align="center">'.$val_query['asset_code'].'</td>';
								//echo '<td align="center">'.$val_query['hsn_code'].'</td>';
								$select_prod_cat="select name,asset_category_id from assets_category where asset_category_id='".$val_query['category_id']."'";
								$query_cat=mysql_query($select_prod_cat);
								$fetch_cat=mysql_fetch_array($query_cat);
								
								echo '<td align="center">'.$fetch_cat['name'].'</td>';
								...
								echo '<td align="center">';
								$select_vendor_map="select asset_type_id,vendor_id from assets_vendor_map where asset_type_id='".$val_query['asset_type_id']."' ";
								...
								//if( $_SESSION['type'] =='S' || $_SESSION['type'] =='ST')
						       	///{
								echo '<a href="add_assets_type.php?record_id='.$listed_record_id.'" ><img src="images/edit_icon.png" title="Edit Record" class="example-fade"/></a>&nbsp;&nbsp;';
								//}
								if( $_SESSION['type'] =='S')
						       	{
