...
$total_found =0;	
	$not_found=0; 
  	...
  	$sel_for_Num_rows="select * from bank_records where 1 ".$pre_keyword_i." ".$bank_ids." ".$paid_from_date." ".$paid_to_date." ".$select_directory." "; 	
...
$objPHPExcel->setActiveSheetIndex(0);
 ...
 $objPHPExcel->setActiveSheetIndex()->setCellValueByColumnAndRow('3','3', 'Mo No','3');
  ...
  $objPHPExcel->setActiveSheetIndex()->setCellValueByColumnAndRow('13','3', 'Status','3');*/
	...
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('C4', 'Type','6');
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('D4', 'Bank Name','6');
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('E4', 'Credit','6');
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('F4', 'Debit','6');
	...
	$branch_id='';
	$select_directory='order by DATE(added_Date) desc';                         
	 $sql_query= "select * from bank_records where 1 ".$pre_keyword_i." ".$bank_ids." ".$paid_from_date." ".$paid_to_date." ".$select_directory." "; 
	...
	while($val_query=mysql_fetch_array($query_order))
	{
		...
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('C'.$rowCount, $val_query['type']);		
		...
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('D'.$rowCount, "Bank Name.-". $data_bank['bank_name']."\n Acc. No.- ".$data_bank['account_no']);
		
		if($val_query['type']=='enrollment' || $val_query['type']=='receipt' || $val_query['type']=='sales product' || $val_query['type']=='service')
								{
								$credit=$val_query['amount'];
								$totals_credit +=intval($credit);
								}
								
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('E'.$rowCount, $credit);						
							
								if($val_query['type']=='expense' || $val_query['type']=='purchase')
								{
								$debit=$val_query['amount'];
								$totals_debit +=intval($debit);
								}
		  
		  $objPHPExcel->setActiveSheetIndex(0)->SetCellValue('F'.$rowCount, $debit);
		...
		$rowCount++;
		$i++;
		//$totals_tds +=$val_query['amount'];
	}
	
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('D'.$rowCount, 'Total');
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('E'.$rowCount,intval($totals_credit));
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('F'.$rowCount,intval($totals_debit));
...
header('Content-Disposition: attachment;filename="all_bank_summury_report.xls"');
