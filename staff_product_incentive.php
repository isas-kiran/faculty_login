...
function calc_product_price(prod_id)
{
	...
	card_value=0;
			card_percent=parseFloat(document.getElementById("sin_product_card"+prod_id).value).toFixed(2);
			if(card_percent >0)
			{
				//alert(card_percent);
				...
				document.getElementById("sin_product_card_price"+prod_id).value=card_price;
			}
			else
			{
				document.getElementById("sin_product_card_price"+prod_id).value=0;
			}
	totals_price=Number(total_price) - (Number(cgst_value) + Number(sgst_value)+ Number(igst_value)+ Number(card_value));
                           ...
                           if($record_id)
			{
				...
				for($z=1;$z<=$total_floor;$z++)
				{
					...
					if($_POST['del_floor'.$z]=='yes')
					{
						...
						{
							"<br />".$delete_row = "delete from pr_staff_product_incentive where staff_product_incentive_id='".$_POST['floor_id_'.$z]."' ";
					...
					if($_POST['del_floor'.$z] !='yes')
					{
											...
											$data_record_type1['sin_product_card'] =addslashes(trim($_POST['sin_product_card'.$z]));
											$data_record_type1['sin_product_card_price'] =addslashes(trim($_POST['sin_product_card_price'.$z]));
					...
					//echo ">>>>>>>".$_POST['floor_id_'.$z].">>>>>>>>>>>>>>".$_POST['product_id'.$z];
					if($_POST['floor_id_'.$z]=='' && $_POST['product_id'.$z] !='')
					{
					...
					else
					{
						...
						$where_record="staff_product_incentive_id='".$_POST['floor_id_'.$z]."'";
				 ...
				 echo ' <br></br><div id="msgbox" style="width:40%;">Record Updated successfully</center></div> <br></br>';
                            ...
                            else
                            {
								...
								for($j=1;$j<=$total_floor;$j++)
								{
											...
											$data_record_type1['sin_product_card'] =addslashes(trim($_POST['sin_product_card'.$j]));
											$data_record_type1['sin_product_card_price'] =addslashes(trim($_POST['sin_product_card_price'.$j]));
...
<?php
			...
			<script language="javascript">
						...
						{
							var shows_data='<div id="floor_id'+idss+'"><table cellspacing="3" id="tbl'+idss+'" width="100%"><tr><td width="10%" align="center"><input type="hidden" name="exclusive_id'+idss+'" id="exclusive_id'+idss+'" value="'+idss+'" /><select name="product_id'+idss+'" id="product_id'+idss+'" style="width:130px;" onchange="show_product_qty(this.value,'+idss+')"><option value="">Select Product</option>				<?php
		...
		<tr>
			<td colspan="6">
			<?php	if($record_id){ $pdnm_width='width="12%"'; } else { $pdnm_width='width="12%"'; }
...
//	$select_exc = "select m.* from customer_service_map m, customer_service s where s.customer_service_id=m.customer_service_id and MONTH(s.added_date)='".$month."' and YEAR(s.added_date)='".$year."' and m.admin_id='".$staff_id."' order by  m.customer_service_map_id asc ";
	...
	{ 
		...
		<select name="product_id<?php echo $t; ?>" id="product_id<?php echo $t; ?>" style="width:130px;" onchange="show_product_qty(this.value,<?php echo $t; ?>)"><option value="">Select Product</option><?php
		...
		if($record_id)
		{
			...
			<input type="hidden" name="floor_id_<?php echo $t; ?>" id="floor_id_<?php echo $t; ?>" value="<?php echo $data_exclusive['staff_product_incentive_id'] ?>" />
