...
<?php
$edit_access='';
$sel_acc="select * from edit_previleges where admin_id='".$_SESSION['admin_id']."' and privilege_id='255'";
...
<table cellspacing="0" cellpadding="0" class="table" width="95%">
  ...
  <tr class="head_td">
              ...
              <tr>
				...
				<td width="15%" id="emp_id">
					<!--<select name="therapist_name" id="therapist_name"  class="input_select_login"  style="width: 150px; ">
						...
						<?php 
							/*$therapist="select DISTINCT admin_id from customer_service_map where 1 and admin_id!='0' and admin_id!='' AND admin_id IS NOT NULL";
							$ptr_therapist=mysql_query($therapist);
							while($data_ptr_therapist=mysql_fetch_array($ptr_therapist))
							{
								$therapist_nm="select name,admin_id from site_setting where admin_id='".$data_ptr_therapist['admin_id']."' and system_status='Enabled'";
								$ptr_therapist_nm=mysql_query($therapist_nm);
								if($toal=mysql_num_rows($ptr_therapist_nm))
								{
                    ...
                    <select name="employee_id" id="employee_id"  class="input_select_login"  style="width: 150px;">
    ...
    <?php
						...
						{
							$therapist_name=" and csm.admin_id='".$_REQUEST['employee_id']."'";
						}
						else
						{
							$therapist_name='';
                        ...
                        if($_GET['order'] !='' && ($_GET['orderby']=='service_name'))
                        {
                            ...
                            $select_directory='order by cs.customer_service_id desc';  
						...
						$sql_query= "SELECT distinct(cs.customer_service_id) as customer_service_id,csm.service_id as service_id,csm.admin_id as admin_id,csm.total_price as total_price FROM customer_service cs, customer c, servies s, site_setting ss, customer_service_map csm where 1  and (cs.service_price like '%".$keyword."%' or cs.discount_price like '%".$keyword."%' or cs.service_tax like '%".$keyword."%' or cs.total_cost like '%".$keyword."%' or c.cust_name like '%".$keyword."%' or c.mobile1 like '%".$keyword."%' or s.service_name like '%".$keyword."%' or s.service_price like '%".$keyword."%' or ss.name like '%".$keyword."%') and cs.customer_id=c.cust_id and cs.customer_service_id=csm.customer_service_id  and ss.admin_id=csm.admin_id and  cs.status='Completed' and csm.service_id=s.service_id ".$cmids." ".$pre_from_date."  ".$pre_to_date." ".$therapist_name."  ".$branch_keyword_p." ".$record_cat_idss." ".$where_service_id." ".$select_directory.""; 
						...
						$total_rows=mysql_num_rows($ptr_query);
                       //echo $sql_query;
                        ...
                        if($no_of_records)
                        {
                            ...
                            $query_string='&keyword='.$keyword.'&branch_name='.$_REQUEST['branch_name'].'&employee_id='.$_REQUEST['employee_id'].'&from_date='.$_REQUEST['from_date'].'&to_date='.$_REQUEST['to_date'];
                     ...
                     <input type="hidden" name="formAction" id="formAction" value=""/>
					  ...
					  ?>
						...
						<?php
							...
							<td width="25%" align="center"><strong>Therapist</strong></td>
							...
							while($val_querys=mysql_fetch_array($all_records))
							{
								...
								echo '<td align="center">';
									...
									$select_service_name="select service_name,service_price from servies where service_id='".$val_querys['service_id']."'";
								...
								echo '</td>';
								 ...
								 $sel_th="select admin_id,name from site_setting where admin_id='".$val_querys['admin_id']."'";
								$ptr_th=mysql_query($sel_th);
								$data_name=mysql_fetch_array($ptr_th);
								echo '<td >'.$data_name['name'].'</td>';
								
								echo '<td align="center">'.$val_querys['total_price'].'</td>';
								
								//$total +=$val_querys['total_price'];
                                ...
                                echo '</tr>';
                                $tot +=$val_querys['total_price'];                                
                                $bgColorCounter++;
                            }    
                            
							$sqls_query= "SELECT SUM(csm.total_price) as total_price FROM customer_service cs, customer c, servies s, site_setting ss, customer_service_map csm where 1  and (cs.service_price like '%".$keyword."%' or cs.discount_price like '%".$keyword."%' or cs.service_tax like '%".$keyword."%' or cs.total_cost like '%".$keyword."%' or c.cust_name like '%".$keyword."%' or c.mobile1 like '%".$keyword."%' or s.service_name like '%".$keyword."%' or s.service_price like '%".$keyword."%' or ss.name like '%".$keyword."%') and cs.customer_id=c.cust_id and cs.customer_service_id=csm.customer_service_id  and ss.admin_id=csm.admin_id and  cs.status='Completed' and csm.service_id=s.service_id ".$cmids." ".$pre_from_date."  ".$pre_to_date." ".$therapist_name."  ".$branch_keyword_p." ".$record_cat_idss." ".$where_service_id." ".$select_directory.""; 
							$ptrs_query=mysql_query($sqls_query);
							$total_cost=mysql_fetch_array($ptrs_query);
							?>
	<tr>
  		<td colspan="2"><b>Total services</b></td>
  		<td colspan="2"><b> <?php echo $total_rows; ?></b></td>
        <td colspan=""></td>
  		<td colspan=""></td>
        <td><?php echo $total_cost['total_price']; ?></td>
        <td></td>
        <td></td>
  	</tr>
  	<tr class="head_td">
