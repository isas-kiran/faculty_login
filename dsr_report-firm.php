    ...
    <td class="mid_mid" align="center">
        <table cellspacing="0" cellpadding="0" class="table" width="95%">
            ...
            if(isset($_POST['sending_mail']))
            {
                ...
                {
                    ?><script>
                    document.location.href="dsr_mail_isasfirm.php?<?php echo $sep_url_string; ?>&send_mail=mail";
                    </script>
                    <?php
                }
			...
			?>
  			<form method="post" name="search">
  				...
  				<tr>
                    ...
                    /*"<br>".$sel_total_inc2="select SUM(amount) as total_amt from receipt where added_date='".$date."' and category !='cash_transfer' and category !='voucher' ".$branch_id."";
...
//==============================================================================
  ?>
  <td align="left" colspan="10" style="color:#F00"><strong>Total Incoming</strong> : <?php echo $total_sales_pack; ?></td>
 ...
 <tr style="background-color:#999">
	...
	$select_directory='order by invoice_id asc';   
	echo "<br/>".$sele_exp="select * from expense_invoice where DATE(`added_date`)='".$date."' and payable_amount>0 ".$branch_id." ".$select_directory."";
	$ptr_exp=mysql_query($sele_exp);
	$i=1;
	if(mysql_num_rows($ptr_exp))
	{
		while($data_expense_inv=mysql_fetch_array($ptr_exp))
		{
			...
			echo "<br/>".$sele_pay_name="select payment_mode from payment_mode where payment_mode_id='".$data_expense_inv['paid_type']."'";
			$ptr_pay_name=mysql_query($sele_pay_name);
			$data_pay_name=mysql_fetch_array($ptr_pay_name);
			...
			echo "<br/>".$sel_bank="select bank_name,account_no from bank where bank_id='".$data_expense_inv['bank_id']."'";
			$ptr_bank=mysql_query($sel_bank);
			$data_bank_name=mysql_fetch_array($ptr_bank);
