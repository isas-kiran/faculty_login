...
function handle_attendance_log($request_data)
{
	...
	$file = fopen("bangalore-biometric-attendance-record.txt","a");
	fwrite($file, $content);
...
// date 6-1-2020
	...
	$sql_user="select * from pr_pune_biometric_attendance where UserId='".trim($UserId)."' and ServiceTagId='".$ServiceTagId."' and DATE(PunchTime)='".date($ChkPunchTime)."' and cm_id='".$cm_id."' order by ID desc";
	...
	if ($result_att->num_rows > 0) 
	{
		...
		if($data_attendace['AttendanceType']=='CheckIn')
		{
			...
			if($differTime >=1) //check out if verified after 10 minute
			{
				...
				if ($conn->query($sql) === TRUE) {
					fwrite($file, " -- Checkout Added after Checkin <br/>");
				} else {
			...
			{
				$sql = "Update pr_pune_biometric_attendance set AttendanceTime=".$AttendanceTime.", PunchTime='".$punchTime."' where UserId='".$UserId."' and AttendanceType='CheckIn' and DATE(PunchTime)='".$ChkPunchTime."' and cm_id='".$cm_id."' and ServiceTagId='".$ServiceTagId."'";
				if ($conn->query($sql) === TRUE) {
		...
		else if($data_attendace['AttendanceType']=='CheckOut')
		{
			...
			$CheckIn=$PunchInData[1];
			$differTime=abs($data_attendace['AttendanceTime']- $AttendanceTime)/60;
			if($differTime >=1) //check In if verified after 10 min
			{
			...
			{
				$sql ="Update pr_pune_biometric_attendance set AttendanceTime=".$AttendanceTime.", PunchTime='".$punchTime."' where UserId='".$UserId."' and AttendanceType='CheckOut' and DATE(PunchTime)='".$ChkPunchTime."' and cm_id='".$cm_id."' and ServiceTagId='".$ServiceTagId."' ";
				if ($conn->query($sql) === TRUE) {
					fwrite($file, " -- Verified CheckIn");
				} else {
					fwrite($file, " -- DB Error: " . $sql . "<br>" . $conn->error);
	...
	else
	{
		...
		fwrite($file,$sql);
		if ($conn->query($sql) === TRUE) {
			fwrite($file, " -- <br>Inserted in DB <br/>");
		} else {
			fwrite($file, " -- DB Error: " . $sql . "<br>" . $conn->error);
		}
	}
