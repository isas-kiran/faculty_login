...
function send_emil_to_oceanone(email_id,subject,message,headers)
{
	...
	//alert(data12);
	$.ajax({
            url: "send_email.php", type: "post", data: data12, cache: false,
...
{
	"<br/>".$sel_act=" select email from site_setting where admin_id='".$data_sel_cnt['staff_id']."' and email!='' ";
						...
						try {
								...
								if($_SESSION['type']!='S')
								{
									...
									if(mysql_num_rows($ptr_cnt))
									{
										...
										while($data_cnt=mysql_fetch_array($ptr_cnt))
										{
											"<br/> 2nd mail-  ".$email_ids-$data_cnt['email'];
											$mail->addCC("$email_ids"); 
											$j++;
										}
									}
								}
								...
								$mail->Subject = " DSR Report of ".$GLOBALS['domainName']." ".$branch_name." on- ".$date."";
