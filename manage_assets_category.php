...
<table border="0" cellspacing="0" cellpadding="0" width="100%">
  ...
  <tr>
	...
	if($_REQUEST['deleteRecord'])
	{
		...
		$sql_query= "SELECT assets_category_id FROM assets_category where asset_category_id='".$del_record_id."'";
		if(mysql_num_rows($db->query($sql_query)))
		{
			$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('Manage_asset_category','Delete','asset_category','".$del_record_id."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
			$query=mysql_query($insert); 

			$delete_query="delete from assets_type where category_id='".$del_record_id."'";
			$db->query($delete_query);
			
			$delete_query="delete from assets_category where asset_category_id='".$del_record_id."'";
			$db->query($delete_query);
			?>
...
if($_GET['record_id'] !='')
{
	$record_cat_id="and asset_type_id='".$_GET['record_id']."' ";
...
if($keyword)
{
	$pre_keyword =" and (asset_type like '%".$keyword."%')";
	$pre_keyword_cat =" and (name like '%".$keyword."%')";
}
$sql_records="select * from assets_category where 1 ".$record_cat_id." ".$pre_keyword_cat." ";
...
<tr align="center" class="grey_td" ><td width="7%" class="tr-header"><strong>Sr.</strong></td><td width="11%" class="tr-header"><strong>Asset  Category</strong></td><td width="11%" class="tr-header"><strong>Asset Name</strong></td><?php if($_SESSION['type']=="S")
{ ?><td width="12%" class="tr-header"><strong>Action</strong></td><?php } ?></tr><?php
	while($val_record=mysql_fetch_array($all_records))
	{
		...
		$paid_totas=0;
		echo $listed_record_id=$val_record['asset_type_id'];
		...
		$se_kit= "SELECT * FROM assets_type where 1 and category_id='".$val_record['asset_category_id']."' order by asset_type_id asc";
		...
		{	
			...
			{
				echo $data_kit_name['asset_type']."<br/>";
				if($k !=$total)
		...
		{
			echo '<a onclick="return validationToDelete(\''.$_SERVER['PHP_SELF'].'\');" href="'.$_SERVER['PHP_SELF'].'?deleteRecord='.$val_record['asset_category_id'].'"><img src="images/delete_icon.png" title="Delete Record" class="example-fade" /></a>&nbsp;&nbsp;<a href="asset_type_edit.php?record_id='.$listed_record_id.'&cat_id='.$val_record['asset_category_id'].'" ><img src="images/edit_icon.png" title="Edit Record" class="example-fade"/></a>&nbsp;&nbsp;';
		}
		echo '</td>';
		//}
		echo '</tr>';
...
if($_POST['save_changes'])
{
	...
	{
		$success=0;
		$errors[$i++]="Enter Asset Category";
	}

	$select_kit="select asset_category_id from assets_category where name='".$_POST['asset_category'] ."' ";
	$ptr_sel_kit=mysql_query($select_kit);
	if(mysql_num_rows($ptr_sel_kit))
	{
		$success=0;
		$errors[$i++]="Asset Category Already Exist";
	}
...
{
	$success=1;
	$data_record['name'] = $_POST['asset_category'];
...
//====================================================================
	...
	if($data_record['name'] !='')
	{
		...
		$select_kit="select asset_category_id from assets_category where name='".$asset_category."' ";
		$ptr_sel_kit=mysql_query($select_kit);
		if(!mysql_num_rows($ptr_sel_kit))
		{
			$insert_for_prevelgegeis = "insert into assets_category (`name`,`added_date`,`cm_id`) values('".$data_record['name']."','".date('Y-m-d H:i:s')."','".$cm_id."')";
		...
		/*for($j=1;$j<=$total_floor;$j++)
		{
			if($_POST['sub_name'.$j] !='')
			{
				...
				$data_record_type1['asset_type'] = addslashes(trim($_POST['sub_name'.$j])); 
				$record_ids=$db->query_insert("assets_type",$data_record_type1);
				$sub_id=mysql_insert_id();  
			}
		}*/   
		"<br>".$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('asset_type','Add','asset','".$sub_id."','".date('Y-m-d')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
	$query=mysql_query($insert);    
				
		echo ' <br></br><div id="msgbox" style="width:40%;">Record added successfully</center></div> <br></br>';
		?><div id="statusChangesDiv" title="Record Deleted"><center><br><p>Asset type added successfully</p></center></div>
		<script type="text/javascript">
		$(document).ready(function() {
		$( "#statusChangesDiv" ).dialog({
			modal: true,
...
<input type="text" name="asset_category" id="asset_category" >
</td>
<!--<td width="20%" ><input type="button" onClick="ajax_kit()" value="Add New Item" /></td> -->
</tr>
<!--<tr>
<td width="15%" class="tr-header" align="center">Asset Type</td>
<td colspan="2">
