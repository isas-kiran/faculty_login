...
<title>Expense Tally Report</title>
...
?>   
	...
	<table border="0" cellspacing="0" cellpadding="0" width="100%" align="center">
              <tr>
                <td class="width5"></td>
                    ...
                    <td width="10%" align="center"><input type="text" name="from_date" class="input_text datepicker" placeholder="From Date" readonly="1" id="from_date" title="From Date" value="<?php if($_REQUEST['from_date']!="From Date") echo $_REQUEST['from_date'];?>">
                    </td>
                    <td width="10%" align="center"><input type="text" name="to_date" class="input_text datepicker" placeholder="To Date" readonly="1" id="to_date"  title="To Date" value="<?php if($_REQUEST['from_date']!="To Date") echo $_REQUEST['to_date'];?>">
                    </td>
                    ...
                    <td><a href="expense_tally_export.php<?php echo $sep_url_string; ?>"><img src="images/excel.png" height="30px" width="30px"/></a></td>
            </tr>          
        </table>
        </form>	
    </td>
	...
	if($no_of_records)
	{
		...
		while($val_query=mysql_fetch_array($my_db))
		{
			...
			echo '<td align="center">Purchase</td>';
			echo '<td align="center">Expense</td>';

			echo '<td align="center">'.$data_name['name'].' - '.$cust_id.'</td>';
				...
				while($data_prod=mysql_fetch_array($ptr_prod))
				{
					...
					if($val_query['discount'] >0)
					{
						...
						else
						{
							...
							$tot_disc_per_org=number_format(($val_query['discount'])*100 / $data_prod['total_val'],3,'.','');
							$tot_disc_per=number_format($tot_disc_per_org,3,'.','');
							$disc=$tot_disc_per;
						}
					...
					echo'<tr>';
					echo '<td align="center" width="40%">Purchase @ '.$data_prod['total_gst'].'%</td>';
				...
				if($tot_prod_nullgst=mysql_num_rows($ptr_prod))
				{
					while($data_prod=mysql_fetch_array($ptr_prod))
					{
						...
						if($val_query['discount'] >0)
						{
							...
							$tot_disc=number_format(($data_prod['total_val'] * $val_query['discount'] ) / 100,3,'.','');
							$tot_disc_price_map=number_format($data_prod['total_val'] - $tot_disc,3,'.','');
							$disc=$val_query['discount'];
						}
						if(($data_prod['total_val'] <=0 || $data_prod['total_val']='') &&( $data_prod['discount']>='100' && $data_prod['discount_type']=='percentage'))
						{
							$tot_price_map=$data_prod['amount_wo_tax'];
							$disc=100;
						}
						
						echo'<tr>';
						echo '<td align="center" width="40%">Purchase @ 0%</td>';
						echo '<td align="center" width="20%">Item </td>';
						echo '<td align="center" width="10%">1</td>';				
						echo '<td align="center" width="10%">'.number_format($tot_price_map,3,'.','').'</td>';
						echo '<td align="center" width="10%">'.$disc.'</td>';
						echo "<td align='center' width='10%'>".number_format($tot_disc_price_map,3,'.','')."</td>";
						echo '</tr>';
						$total_amnts =number_format($tot_disc_price_map,3,'.','');
					}
				}	
			}
			...
			if($tot_cgst_num=mysql_num_rows($ptr_cgst_prod))
			{
				while($data_cgst=mysql_fetch_array($ptr_cgst_prod))
				{
					...
					{
						echo'<tr>';
						echo '<td align="center" width="40%">Input CGST '.$data_cgst['cgst_tax_in_per'].'%</td>';
			...
			if($tot_sgst_num=mysql_num_rows($ptr_sgst_prod))
			{
				while($data_sgst=mysql_fetch_array($ptr_sgst_prod))
				{
					...
					{
						echo'<tr>';
						echo '<td align="center" width="40%">Input SGST '.$data_sgst['sgst_tax_in_per'].'%</td>';
						echo '<td align="center" width="20%"></td>';
						echo '<td align="center" width="10%"></td>';				
						echo '<td align="center" width="10%"></td>';
						echo '<td align="center" width="10%"></td>';
			...
			if($tot_igst=mysql_num_rows($ptr_igst_prod))
			{
				while($data_igst=mysql_fetch_array($ptr_igst_prod))
				{
					...
					{
						echo'<tr>';
						echo '<td align="center" width="40%">Input IGST '.$data_igst['igst_tax_in_per'].' %</td>';
						echo '<td align="center" width="20%"></td>';
						echo '<td align="center" width="10%"></td>';				
						echo '<td align="center" width="10%"></td>';
						echo '<td align="center" width="10%"></td>';
			...
			echo '</tr>';
			echo '</table></td>';
			//$tot_num_gst=$tot_cgst+$tot_sgst+$tot_igst;
			...
			$totals_tds +=intval($total_actual);
