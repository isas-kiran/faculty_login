...
<title>Manage Customer Membership</title>
    ...
    <?php       if($_POST['formAction'])
                    {
                        if($_POST['formAction']=="delete")
                        {
                            for($r=0;$r<count($_POST['chkRecords']);$r++)
                            {
                                ...
                                $sql_query= "SELECT map_id FROM customer_membership_map where map_id ='".$del_record_id."'";
                                if(mysql_num_rows($db->query($sql_query)))
                                    {                                                
                                        $delete_query="delete from customer_membership_map where map_id='".$del_record_id."'";
...
<table cellspacing="0" cellpadding="0" class="table" width="95%">
  ...
  <tr class="head_td">
    ...
    <?php
                        ...
                        if($keyword)
                            $pre_keyword=" and (membership_name like '%".$keyword."%' || start_date like '%".$keyword."%' || end_date like '%".$keyword."%' )";
                        else
                            $pre_keyword="";
                        ...
                        if($_GET['orderby']=='map_id' )
                            $img1 = $img;

                        if($_GET['order'] !='' && ($_GET['orderby']=='map_id'))
                        {
                            $select_directory .= " order by ".$_GET['orderby']." " .$_GET['order'] ;
                            $date_query='&order='.$_GET['order'].'&orderby='.$_GET['orderby'];
                        }
                        else
                            $select_directory='order by map_id desc';                      
                            $sql_query= "SELECT * FROM customer_membership_map where 1  ".$pre_keyword." ".$select_directory.""; 
                       //echo $sql_query;
    ...
    <form method="post"  name="frmTakeAction" action="<?php echo $_SERVER['PHP_SELF'].'?#msgbox';?>" >
                     <input type="hidden" name="formAction" id="formAction" value=""/>
                      <tr class="grey_td" >
                        ...
                        <td width="10%" align="center"><strong>Membership Name</strong></td>
                        ...
                        <td width="10%" class="centerAlign"><strong>Action</strong></td>
                            ...
                            while($val_query=mysql_fetch_array($all_records))
                            {
                                ...
                                $listed_record_id=$val_query['map_id']; 
								...
								$select_customer="select cust_id,cust_name from customer where cust_id='".$val_query['cust_id']."'";
								$customer_query=mysql_query($select_customer);
								$fetch_customer=mysql_fetch_array($customer_query);
								       
                                echo '<td >'.$fetch_customer['cust_name'].'</td>';
								
								$select_membership="select membership_id,membership_name from membership where membership_id='".$val_query['membership_id']."'";
								$membership_query=mysql_query($select_membership);
								$fetch_membership=mysql_fetch_array($membership_query);
								
                                echo '<td align="center">'.$fetch_membership['membership_name'].'</td>';
                                ...
                                echo '<td align="center"><a href="add_customer_membership.php?record_id='.$listed_record_id.'" ><img src="images/edit_icon.png" title="Edit Record" class="example-fade"/></a>&nbsp;&nbsp;
