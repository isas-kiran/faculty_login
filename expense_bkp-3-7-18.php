...
<link href="css/style.css" rel="stylesheet" type="text/css" />
	...
	function payment(value)
	{
		...
		if(payment_mode[0]=="cheque")
		{
			...
			show_bank_for_payment_mode(branch_name,"cheque",payment_mode[1])
		}
		else if(payment_mode[0]=="Credit Card")
		{
			...
			document.getElementById("paytm_details").style.display = 'none';
			show_bank_for_payment_mode(branch_name,"credit_card",payment_mode[1])
		...
		else if(payment_mode[0]=="paytm")
		{
			...
			show_bank_for_payment_mode(branch_name,"paytm",payment_mode[1])
		}
		else if(payment_mode[0]=="online")
		{
			...
			document.getElementById("credit_details").style.display = 'none';
			document.getElementById("paytm_details").style.display = 'none';
			show_bank_for_payment_mode(branch_name,"online",payment_mode[1])
		}
		else
		{
			document.getElementById("chaque_details").style.display = 'none';
	...
	function show_bank_for_payment_mode(branch_id,vals,payment_ids)
	{
		if(document.getElementById("record_id"))
		{
			record_id= document.getElementById("record_id").value;
		...
		if(vals=='credit_card')
		{
			document.getElementById("cc_bank_record").style.display="none";
			var bank_data="action=expense&action_for=for_credit_card&show_bnk=1&branch_id="+branch_id+"&payment_type="+vals+"&record_id="+record_id+"&payment_id="+payment_ids;
			$.ajax({
			url: "show_bank.php",type:"post", data: bank_data,cache: false,
			success: function(bankdata)
			{
				//alert(bankdata);
				document.getElementById("cc_bank_id").innerHTML=bankdata;
				if(document.getElementById("cc_bank_name"))
				{
					var bank_ids=document.getElementById("cc_bank_name").value;
					show_acc_no(bank_ids)
				}
			}
			});
...
function calc_product_price(prod_id)
{
	...
	//===================================Calculate Base Price============================== Changes 18/1/18
		...
		else if(igsttax > 0)
		{
			...
			document.getElementById("sin_prod_igst_price"+prod_id).value=precisionRound(total_base_taxable_value,2);
		}
...
<table border="0" cellspacing="0" cellpadding="0" width="100%">
  ...
  <tr>
	...
	if($_POST['save_changes'])
	{
		...
		{
			$cc_bank_name=$_POST['cc_bank_name'] ? $_POST['cc_bank_name'] : "";
		}
	...
	<?php
		...
		else
		{
			...
			$data_record['cc_bank_name'] =$cc_bank_name;
...
<tr>
	...
	<td width="25%"><input style="width:200px" type="text" id="added_date" name="added_date" class="input_text datepicker" value="<?php if($_POST['added_date']) echo $_POST['added_date']; else if($row_expense['added_Date']!=''){$arrage_date= explode('-',$row_expense['added_Date'],3);echo $arrage_date[2].'/'.$arrage_date[1].'/'.$arrage_date[0];}else{echo date("d/m/Y");}?>" />
...
<div id="credit_details" <?php  if($data_payment_mode1['payment_mode']=='Credit Card') echo ' style="display:block"'; else echo ' style="display:none"'; ?>>
	<table width="100%">
    	<tr>
			<td width="22%" class="tr-header"  align="center">CC Bank Name<span class="orange_font">*</span></td>
			<td>
				<div id="cc_bank_record">
				<?php 
				if($record_id !='')
				{
					?>
					<select name="cc_bank_name" class="input_text" style="width:200px" id="cc_bank_name" onChange="show_acc_no(this.value)">
    ...
    The Amount Deposited in Below Selected Bank
</div>
