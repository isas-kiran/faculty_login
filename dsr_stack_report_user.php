...
<?php
	...
	if($no_of_records)
	{
		...
		while($val_query=mysql_fetch_array($ptr_db))
		{
			...
			$subject = "Hi ".$val_query['name']." Your Daily Stack Report is Ready for Today - ISAS BEAUTY SCHOOL ";
			...
			try {
					...
					$sel_sms_cnt="select * from sms_mail_configuration_map where previlege_id='283' and staff_id='".$val_query['admin_id']."' and cm_id='".$cm_id1."'";
					$ptr_sel_sms=mysql_query($sel_sms_cnt);
					$i=0;
					if($tot_num_rows=mysql_num_rows($ptr_sel_sms))
					{
						$data_sel_cnt=mysql_fetch_array($ptr_sel_sms);
					...
					$mail->Subject ='Hi '.$val_query['name'].' Your Daily Stack Report is Ready for Today - ISAS BEAUTY SCHOOL';
					...
					$mail->Send();
					echo " Hi ".$val_query['name']." Email Sent Successfully.";
