...
$sel_for_Num_rows="SELECT * FROM inquiry where 1 ".$status_type." ".$record_cat_id." ".$cm_ids." ".$search_cm_id." ".$pre_keyword." ".$enquiry_added." ".$assigned_to." ".$enquiry_src." ".$response." ".$where_followup." ".$pre_from_date."".$pre_to_date." ".$c_id." order by inquiry_id desc ";
...
$objPHPExcel->setActiveSheetIndex(0)->setSharedStyle($sharedStyle2, "A4:O4");
...
$objPHPExcel->setActiveSheetIndex(0)->getColumnDimension('I')->setWidth("17");
$objPHPExcel->setActiveSheetIndex(0)->getColumnDimension('J')->setWidth("14");
$objPHPExcel->setActiveSheetIndex(0)->getColumnDimension('K')->setWidth("5");
...
$objPHPExcel->setActiveSheetIndex(0)->getStyle('O1:O'.$objPHPExcel->setActiveSheetIndex(0)->getHighestRow())->getAlignment()->setWrapText(true); 
...
$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('J4', 'Inquiry Date','5');
$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('K4', 'Sr No','5');
$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('L4', 'Call Date','5');
$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('M4', 'Description','5');
$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('N4', 'Next Followup Date','5');
$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('O4', 'Status','5');
//$select_directory='order by student_id desc';                      
		
if($_GET['keyword']!='' || $_GET['keyword']!="Keyword")
{
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('C2', 'Search by =>');
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('D2', ''.$_GET['keyword'].'');
}
if($_SESSION['type']=="S")
{
	if($_REQUEST['branch_name']!='')
	{
		...
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('B3', ''.$branch_name.'');
	}
}
...
if($_GET['assigned_to'] !="")
{
	...
	$ptr_name=mysql_query($sel_name);
	$data_names=mysql_fetch_array($ptr_name);
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('G2', 'Assigned to =>');
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('G3', ''.$data_names['name'].'');
}
if($_GET['enquiry_src']!='')
{
	...
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('H2', 'Enquiry source by =>');
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('H3', ''.$enq_source.'');
}
if($_GET['response']!='')
{
	...
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('I2', 'Response by =>');
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('I3', ''.$response.'');
}
if($_GET['start_date']!='')
{
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('I2', 'From Date =>');
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('J2', ''.$_GET['start_date'].'');
}
if($_GET['end_date']!='')
{
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('I3', 'End Date =>');
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('J3', ''.$_GET['end_date'].'');
}

$sql_query= "SELECT * FROM inquiry where 1 ".$status_type." ".$record_cat_id." ".$cm_ids." ".$search_cm_id." ".$pre_keyword." ".$enquiry_added." ".$assigned_to." ".$enquiry_src." ".$response." ".$where_followup." ".$pre_from_date."".$pre_to_date." ".$c_id." order by inquiry_id desc "; 
...
while($val_query=mysql_fetch_array($query_order))
{
	...
	$select_last_followup = " select * from followup_details where student_id='".$val_query['inquiry_id']."' and followup_details!='' order by added_date asc ";
	...
	if($total_foll >0)
	{
		...
		while($data_last_followup = mysql_fetch_array($ptr_last_followup))
		{
			...
			$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('K'.$rowCount, $l);
			$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('L'.$rowCount, $added_date);	
			$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('M'.$rowCount, $data_last_followup['followup_details']);
			$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('N'.$rowCount, $data_last_followup['followup_date']);
			$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('O'.$rowCount, $status);
...
header('Content-Disposition: attachment;filename="followups_inquiry.xls"');
