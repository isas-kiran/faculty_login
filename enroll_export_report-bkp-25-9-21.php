...
$objPHPExcel->getProperties()->setCreator("Maarten Balliauw")
	...
	$whr_transfer_from='';
	if($_REQUEST['transfer_from'])
	{
		$transfer_from=$_REQUEST['transfer_from'];
		$whr_transfer_from=" and transfer_from_cm_id = '".$transfer_from."'";
	}
	$whr_transfer_to='';
	if($_REQUEST['transfer_to'])
	{
		$transfer_to=$_REQUEST['transfer_to'];
		$whr_transfer_to=" and cm_id = '".$transfer_to."'";
	}
	if($_REQUEST['page'])
		$page=$_REQUEST['page'];
	else
	...
	$select_directory='order by enroll_id desc';                      
  	...
  	$sel_enroll="SELECT * FROM enrollment where 1 ".$_SESSION['where']." ".$where_course." ".$where_assign_to." ".$where_added_by." ".$enquiry_src." ".$search_cm_id." ".$whr_transfer_from." ".$whr_transfer_to." ".$pre_from_date." ".$pre_to_date."  ".$select_directory."";
	...
	$objPHPExcel->setActiveSheetIndex(0)->setSharedStyle($sharedStyle2, "A4:P4");
	...
	$objPHPExcel->setActiveSheetIndex(0)->getStyle('P1:O'.$objPHPExcel->setActiveSheetIndex(0)->getHighestRow())->getAlignment()->setWrapText(true); 
	...
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('O4', 'Enroll No','6');
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('P4', 'Source Name','6');
	...
	$sql_query= "SELECT * FROM enrollment where 1 ".$_SESSION['where']." ".$where_course." ".$where_assign_to." ".$where_added_by." ".$enquiry_src." ".$search_cm_id." ".$whr_transfer_from." ".$whr_transfer_to." ".$pre_from_date." ".$pre_to_date." ".$select_directory."";
	...
	while($val_query=mysql_fetch_array($query_order))
	{
		...
		$enq_source='';
		$enq_src="select campaign_name from campaign where campaign_id='".$val_query['source']."'";
		...
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('P'.$rowCount, $enq_source);	
