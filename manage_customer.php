...
<?php
$edit_access='';
$sel_acc="select * from edit_previleges where admin_id='".$_SESSION['admin_id']."' and privilege_id='117'";
    ...
    <?php       if($_POST['formAction'])
                    {
                        if($_POST['formAction']=="delete")
                        {
                            for($r=0;$r<count($_POST['chkRecords']);$r++)
                            {
                                ...
                                $sql_query= "SELECT cust_id,cust_name FROM customer where cust_id ='".$del_record_id."'";
                                $ptr_query=mysql_query($sql_query);
								if(mysql_num_rows($ptr_query))
								{     
										$data_cust=mysql_fetch_array($ptr_query);   
										"<br>".$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('manage_customer','Delete','".$data_cust['cust_name']."','".$del_record_id."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
  ...
  <tr class="head_td">
    ...
    <?php
						...
						if($_GET['record_id'] !='')
						{
                            ...
                            $sql_query= "SELECT * FROM customer where 1 ".$record_cat_id." ".$_SESSION['where']."  ".$pre_keyword." ".$select_directory.""; 
                       //echo $sql_query;
    ...
    <form method="post"  name="frmTakeAction" action="<?php echo $_SERVER['PHP_SELF'].'?#msgbox';?>" >
                     <input type="hidden" name="formAction" id="formAction" value=""/>
					  ...
					  <?php } ?>
                        ...
                        <td width="8%" align="center"><strong>Birth Date</strong></td>
                        <td width="8%" align="center"><strong>Mobile 1</strong></td>
                        
                        <td width="10%" align="center"><strong>Email</strong></td>
                        <td width="15%" ><strong>Address</strong></td>
                        <td width="5%" align="center"><strong>Membership</strong></td>
                        <td width="5%" align="center"><strong>Add/Edit Membership</strong></td>
                        <!--<td width="5%" align="center"><strong>Details</strong></td>-->
						 ...
						 if($_SESSION['type']=="S" || $edit_access='yes')
						 {
                            ...
                            while($val_query=mysql_fetch_array($all_records))
                            {
                                ...
                                echo '<td >'.$val_query['cust_name'].'</td>';
								
								/*echo '<td align="center">'.$val_query['gender'].'</td>';*/
								
								//$birth_date_sep=explode('/',$val_query['birth_date']);
							   // $sep_birth_date=$birth_date_sep[2].'/'.$birth_date_sep[1].'/'.$birth_date_sep[0];
								
								echo '<td align="center">'.$val_query['birth_date'].'</td>';
								
                                echo '<td align="center">'.$val_query['mobile1'].'</td>';
								
								/*echo '<td align="center">'..'</td>';*/
								
								echo '<td align="center">'.$val_query['email'].'</td>';
								
								echo '<td>'.$val_query['address'].'</td>';
								
								echo '<td align="center">'.$val_query['membership'].'</td>';
								
								
								
								echo '<td align="center">';
								if($val_query['membership']=="yes")
								{
									echo'<a href="add_customer_membership.php?record_id='.$listed_record_id.'" ><img src="images/edit_icon.png" title="Edit Membership" class="example-fade"/></a>';
								}
								else
								{
									echo'<a href="add_customer_membership.php?record_id='.$listed_record_id.'" ><img src="images/add.png" title="Add Membership" class="example-fade"/></a>';
								}
								echo'</td>';
