...
//             $('input:radio[name=status][value=Inactive]').click();
    ...
    <script type="text/javascript">
        ...
        function select_sudent(batch)
        {
            var data1="action=stud_report&batch_id="+batch;	
            //alert(data1);
            $.ajax({
            url: "show_list_batch_student.php", type: "post", data: data1, cache: false,
            success: function (html)
            {
                if(html !='')
                {
                    sep=html.split("###");
                    document.getElementById("enroll_id").innerHTML=sep[0];
              ...
              <tr>
                ...
                <td class="rightAlign"> 
              	...
              	<tr>
                    ...
                    <td width="38%" >
                        <select name="batch_id" id="batch_id" class="validate[required] input_select" style="width:200px" onchange="select_sudent(this.value)">  
                        <option value=""> Select Batch</option>
                        <?php
                        $select_cb = "select * from course_batch_mapping where 1 ".$_SESSION['where']." order by c_b_id asc";
                        ...
                        {               
                            ...
                            $sel='';
                            if($_REQUEST['batch_id']==$data_cb['c_b_id'])
                            {
                                $sel='selected="selected"';
                            }
                            
                            echo '<option '.$sel.' value='.$data_cb['batch_name'].'>'.$data_cb['batch_name'].'&nbsp;&nbsp;('.$data_course['course_name'].')&nbsp;&nbsp;('.$data_staff['name'].')</option>';
                        } 
                        ?>
                        </select>
                    </td>
                    ...
                    <td class="width5" style="padding-left:px; width:10px;"></td>
                       <td width="38%" >
                        <select name="enroll_id" id="enroll_id" class="validate[required] input_select" style="width:200px" >  
                       
                        <?php
                        // $select_cb = "select * from student_course_batch_map where 1 ".$_SESSION['where']." order by c_b_id asc";
                        // $ptr_cb = mysql_query($select_cb);
                        // while($data_en = mysql_fetch_array($ptr_cb))
                        // {               
                        //     $sel_course="select name from `enrollment` where enroll_id='".$data_en['enroll_id']."'";
                        //     $ptr_course=mysql_query($sel_course);
                        //     $data_name=mysql_fetch_array($ptr_course);
                            
                        //     $sel='';
                        //     if($_REQUEST['enroll_id']==$data_en['enroll_id'])
                        //     {
                        //         $sel='selected="selected"';
                        //     }
                            
                        //     echo '<option '.$sel.' value='.$data_en['enroll_id'].'>'.$data_name['name'].'</option>';
                        // } 
                        ?>
                        </select>
                    </td>
                    ...
                    <?php if($_SESSION['type']=='S' || $_SESSION['type']=='Z' || $_SESSION['type']=='LD') {?><td width="4%" align="center"> <a href="student_record_excel.php<?php echo $sep_url_string; ?>"><img src="images/excel.png" height="30px" width="30px"/></a></td><?php } ?>
                        ...
                        if($no_of_records)
                        {
                           ...
                           // $pager = new PS_Pagination($sql_query,$_SESSION['show_records'],$_SESSION['show_records_pages'],$query_string1);
                            ...
                            <tr class="grey_td" >
                                ...
                                <td width="10%" align="center"><strong> Without Uniform</strong></td>
                                <td width="10%" align="center"><strong> No. of latemark</strong></td>
                                <td width="10%" align="center"><strong>Mobile submitted</strong></td>
                            ...
                            while($val_query=mysql_fetch_array($all_records))
                            {
                                ...
                                $select_latemark= "SELECT * from student_attendence where enroll_id= ".$val_query['enroll_id']." and course_batch_id='".$val_query['c_b_id']."' and latemark='no' ";
								$pre_latemark= mysql_query($select_latemark);
                                $count_latemark= mysql_num_rows($pre_latemark);
                                
                                $select_uniform= "SELECT * from student_attendence where enroll_id= ".$val_query['enroll_id']." and course_batch_id='".$val_query['c_b_id']."' and uniform='no' ";
								$pre_uniform= mysql_query($select_uniform);
                                $count_uniform= mysql_num_rows($pre_uniform);
                                
                                $select_mobile= "SELECT * from student_attendence where enroll_id= ".$val_query['enroll_id']." and course_batch_id='".$val_query['c_b_id']."' and mobile_submit='no' ";
								$pre_mobile= mysql_query($select_mobile);
								$count_mobile= mysql_num_rows($pre_mobile);
								
								echo '<td align="center"><a href="batch_student_record.php?enroll_id='.$val_query['enroll_id'].'&c_b_id='.$val_query['c_b_id'].'" >'.$total_attendence.'</a></td>';
								echo '<td align="center"><a href="email_details.php?enroll_id='.$val_query['enroll_id'].'&c_b_id='.$val_query['c_b_id'].'" >'.$count_absent.'</a></td>';
								 echo '<td align="center">'.$count_uniform.'</td>';
                                echo '<td align="center">'.$count_latemark.'</td>';
                                echo '<td align="center">'.$count_mobile.'</td>';
