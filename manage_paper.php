    ...
    <?php if($_POST['formAction'])
                    {
                        if($_POST['formAction']=="delete")
                        {
                            for($r=0;$r<count($_POST['chkRecords']);$r++)
                            {
                                ...
                                $sql_query= "SELECT  	paper_id FROM ".$GLOBALS["pre_db"]."paper where  	paper_id ='".$del_record_id."'";
                                if(mysql_num_rows($db->query($sql_query)))
                                    {                                                
                                        $delete_query="delete from ".$GLOBALS["pre_db"]."paper where paper_id='".$del_record_id."'";
						...
						else if($_POST['formAction']=="Active")
                        {
                            for($r=0;$r<count($_POST['chkRecords']);$r++)
                            {
                                ...
                                $update_record= "update ".$GLOBALS["pre_db"]."paper set status='Active' where paper_id='".$update_record_id."'";
                        ...
                        else if($_POST['formAction']=="Inactive")
                        {
                            ...
                            {
                                $update_record_id=$_POST['chkRecords'][$r];
                                $update_record= "update ".$GLOBALS["pre_db"]."paper set status='Inactive' where paper_id='".$update_record_id."'";
                                $db->query($update_record);
                            }
                        ...
                        else if($_POST['formAction']=="Banned")
                        {
                            ...
                            {
                                $update_record_id=$_POST['chkRecords'][$r];
                                $update_record= "update ".$GLOBALS["pre_db"]."paper set status='Banned' where paper_id='".$update_record_id."'";
                                $db->query($update_record);
                            }
                    ...
                    {
                        $update_query1="update ".$GLOBALS["pre_db"]."paper set status='".$_REQUEST['value']."' where paper_id='".$_REQUEST['changeStatus']."'";
                        //echo $update_query1;
...
<table cellspacing="0" cellpadding="0" class="table" width="95%">
  <tr class="head_td">
    ...
    <?php
                        ...
                        if($keyword)
                            {                            
                                $select_name = "select user_id from PB_user where (first_name like '%".$keyword."%' or last_name like '%".$keyword."%')";                                           
                        ...
                        if($_GET['order'] !='' && ($_GET['orderby']=='course_name'))
                        {
                            ...
                            $select_directory='order by paper_id desc';                      
                            $sql_query= "SELECT paper_id,paper_title,course_id,subject_id,status FROM paper where 1  ".$pre_keyword." ".$select_directory.""; 
                       //echo $sql_query;
  ...
  <tr class="grey_td" >
    ...
    <td width="20%"><a href="<?php echo $_SERVER['PHP_SELF'];?>?order=<?php echo $order."&orderby=course_name".$query_string;?>" class="table_font"><strong>Paper Name</strong></a> <?php echo $img1;?></td>
    <td width="15%"><strong>Course  Name</strong></td>
      <td width="10%"><strong>Subject   Name</strong></td>
    <td width="13%"><strong>No.OF Question  </strong></td>
    <td width="7%"><strong>Staus </strong></td>
...
<td width="20%"><strong>Delete</strong></td>-->    
    <td width="10%" class="centerAlign"><strong>Action</strong></td>
                            ...
                            while($val_query=mysql_fetch_array($all_records))
                            {
                                ...
                                $listed_record_id=$val_query['paper_id']; 
                                include "include/paging_script.php";
                               
                                echo '<tr '.$bgcolor.' >
                                      <td align="center"><input type="checkbox" name="chkRecords[]" value="'.$listed_record_id.'"></td>';
                                echo '<td align="center">'.$sr_no.'</td>'; 
								 echo '<td align="center">'.$val_query['paper_title'].'</td>'; 
								$select_course_name=mysql_query("select * from courses where course_id ='".$val_query['course_id']."'");
								$val_course=mysql_fetch_array($select_course_name);  
								                           
                                echo '<td >'.$val_course['course_name'].'</td>';
                               
							    $select_sub_name=mysql_query("select * from subject where subject_id ='".$val_query['subject_id']."'");
								$val_sub=mysql_fetch_array($select_sub_name);  
							    echo '<td >'.$val_sub['name'].'</td>';
								$count_quetions=0;
								$count_quation_map="select * from quation_add_to_paper where paper_id='".$val_query['paper_id']."' and subject_id='".$val_query['subject_id']."'";
								$ptr_count_map=mysql_query($count_quation_map);
								$count_quetions =mysql_num_rows($ptr_count_map);
								$paper_id=$val_query['paper_id'];
								$subject_id=$val_query['subject_id'];
								$count_quation="<a href='show_exam_quation_paper.php?paper_id=$paper_id&subject_id=$subject_id'> ".$count_quetions." (CLICK HERE TO SHOW )"."</a>";
					...
					echo '</td>';
                                ...
                                echo '<td align="center"> <a href="create_paper.php?record_id='.$listed_record_id.'" ><img src="images/edit_icon.png" title="Edit Record" class="example-fade"/></a>&nbsp;&nbsp;
