...
// Add some data
	...
	if($_REQUEST['start_date'] && $_REQUEST['start_date']!=="0000-00-00" && $_REQUEST['start_date']!="From Date")
	{
		...
		$pre_from_date_i=" and DATE(i.added_date) >='".date('Y-m-d',strtotime($frm_dates))."'";
	}
	else
	{
		$pre_from_date="";     
...
if($pre_keyword=='')
{                     
	$sql_query= "SELECT * FROM service_inquiry where 1 ".$record_cat_id." ".$_SESSION['where']." ".$pre_from_date." ".$pre_to_date." ".$branch_keyword." ".$pre_keyword." ".$select_directory.""; 
}
else
{
	$sql_query= "SELECT distinct(i.inquiry_id) as inquiry_id FROM service_inquiry i, servies s, service_inquiry_map sim where 1  and (i.service_price like '%".$keyword."%' or i.total_cost like '%".$keyword."%' or i.name like '%".$keyword."%' or i.mobile like '%".$keyword."%' or s.service_name like '%".$keyword."%' or sim.service_price like '%".$keyword."%') and i.inquiry_id=sim.inquiry_id  and sim.service_id=s.service_id ".$cmids." ".$branch_keyword_p." ".$pre_from_date_i." ".$pre_to_date_i." ".$record_cat_idss." order by i.inquiry_id desc "; 
}

$no_of_rows=mysql_num_rows($sql_query);
...
while($val_query=mysql_fetch_array($query_order))
{
	...
	$asssign_name=='';
	"<br />".$sel_name_assign="select name from site_setting where admin_id='".$val_query['staff_id']."'";
	...
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('D'.$rowCount, $val_query['mobile']);
	...
	$enq_src="select campaign_name from campaign where campaign_id='".$val_query['enquiry_src']."'";
	$ptr_enq_src=mysql_query($enq_src);
	$data_enq_src=mysql_fetch_array($ptr_enq_src);	
	...
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('L'.$rowCount, $val_query['followup_desc']);
...
header('Content-Disposition: attachment;filename="service_inquiry.xls"');
