						   ...
						   $sel_inventory="select vendor_id, inventory_id, price, discount, discount_price, total_cost, amount1, payable_amount, remaining_amount, cm_id from inventory where inventory_id='".$record_id."'";
    						...
    						if($_POST['save_changes'])
                        	{
								...
								if($record_id)
								{
									 ...
									 '<br/>'.$update_inventory="update inventory set remaining_amount='".$_POST['remaining_amount']."', payable_amount=(payable_amount + ".$payable_amount.")  where inventory_id='".$inventory_id."'";
									...
									$sep_check_date=$chaque_date_exp[2].'-'.$chaque_date_exp[1].'-'.$chaque_date_exp[0];
									 ...
									 '<br/>'.$insert_inventory_invoice = " INSERT INTO `inventory_invoice` (`inventory_id`, `price`, `total_cost`, `amount1`, `payable_amount`,`remaining_amount`, `paid_type`, `cust_bank_name`, `bank_id`, `cheque_detail`, `chaque_date`, `credit_card_no`, `admin_id`, `added_date`,`status`,`cm_id`) VALUES ('".$record_id."', '".$fetch_query_inventory['price']."', '".$fetch_query_inventory['total_cost']."', '".$fetch_query_inventory['amount1']."', '".$payable_amount."','".$_POST['remaining_amount']."', '".$payment_type."', '".$cust_bank_name."', '".$bank_name."', '".$_POST['chaque_no']."', '".$sep_check_date."','".$credit_card_no."', '".$_SESSION['admin_id']."', '".date('Y-m-d H:i:s')."','".$status."','".$cm_id1."'); ";
                                ...
                                if($no_of_records)
                                {
                                    ...
                                    while($val_record=mysql_fetch_array($all_records))
                                    {
                                       ...
                                       <?php }
										...
										<input type="hidden" name="cm_id_branch" value="'.$fetch_query_inventory['cm_id'].'"/></td></tr>';
										
                                        echo '<tr><td><strong>Total Product Price</strong></td><td align="left">'.$fetch_query_inventory['price'].'</td></tr>';
										
                                        echo '<tr><td><strong>Discount</strong></td><td align="left">';
                                        	echo $fetch_query_inventory['discount'];
                                        echo '</td></tr>'; 
									    
										echo '<tr><td><strong>Total Discounted Price</strong></td><td align="left">'.$fetch_query_inventory['total_cost'].'</td></tr>';
										
										echo '<tr><td><strong>Final Amount</strong></td><td align="left">'.$fetch_query_inventory['amount1'].'</td></tr>';
										...
										echo '<tr><td><strong>Total paid Amount</strong></td><td align="left">'.$fetch_sum['payable_amount'].'</td></tr>';
