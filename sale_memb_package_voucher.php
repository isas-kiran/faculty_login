...
<?php
$edit_access='';
$sel_acc="select * from edit_previleges where admin_id='".$_SESSION['admin_id']."' and privilege_id='127'";
...
<title><?php if($record_id) echo "Edit"; else echo "Add";?>Sale Package / Voucher</title>
...
mail1=Array();
<?php
$sel_sms_cnt="select * from sms_mail_configuration_map where previlege_id='127' ".$_SESSION['where']."";
...
"<br/>".$sel_mail_text="select email_text from previleges where privilege_id='127'";
...
//alert("hi");
	...
	{
		var memb_disc=document.getElementById('memb_discs').value;
	...
	else if(category=="Package")
	{
		...
		var package_start_dates=document.getElementById('package_start_dates').value;
		var package_end_dates=document.getElementById('package_end_dates').value;
		var pkg_qtys=document.getElementById('pkg_qtys').value;
		var pkg_prices=document.getElementById('pkg_prices').value;
	}
	...
	//alert(users_mail);
		...
		data1='action=sale&branch_name='+branch_name+'&category='+category+'&realtxt='+realtxt+'&cust_id='+cust_id+'&voucher_id='+voucher_id+'&vouchers_name='+vouchers_name+'&vouchers_start_date='+vouchers_start_date+'&vouchers_end_date='+vouchers_end_date+'&vouchers_price='+vouchers_price+'&redeems_price='+redeems_price+'&total_quantities='+total_quantities+'&issue_qty='+issue_qty+'&memb_disc='+memb_disc+'&days='+days+'&memb_price='+memb_price+'&package_names='+package_names+'&package_start_dates='+package_start_dates+'&package_end_dates='+package_end_dates+'&pkg_qtys='+pkg_qtys+'&pkg_prices='+pkg_prices+'&selling_amount='+selling_amount+'&total_type1='+total_type1+'&tax_type='+tax_type+'&tax_value='+tax_value+'&start_date='+start_date+'&end_date='+end_date+'&amount='+amount+'&payment_mode='+payment_mode+concat_string_voucher+'&users_mail='+users_mail+'&mail='+mail+'& bank_name='+bank_name+'&account_no='+account_no+'&chaque_no='+chaque_no+'&cheque_date='+cheque_date+'&credit_card_no='+credit_card_no+"&email_text_msg="+email_text_msg;
		//alert(data1);
		$.ajax({
        ...
        $.ajax({
            ...
            success: function (html)
            {
				...
				else
				{
					...
					document.getElementById('memb_discs').value=memb_disc;
        ...
        $.ajax({
            ...
            success: function (html)
            {
				...
				else
				{
					...
					document.getElementById("remaining_amount").value=price;
...
function searchSel(value) 
{
	var data1="action=sale_voucher&mobile_no="+value;	
	$.ajax({
	url: "get_name.php", type: "post", data: data1, cache: false,
	success: function (html)
	{
...
function find_remain()
{
	...
	remain=0;
					if(payable_amount!='')
					{
					var remain=amount-payable_amount;
					document.getElementById("remaining_amount").value=remain;
					}
					else
					{
					document.getElementById("remaining_amount").value=amount;	
					}
}
</script>
  ...
  <tr>
        ...
        <?php
                        ...
                        if($_POST['save_changes'])
                        {
							...
							if($category=="Package")
							{
								...
								$package_id=( ($_POST['package_id'])) ? $_POST['package_id'] : "0";
							}
							if($category=="Voucher")
							{
								//$voucher_id=$_POST['voucher_id'];
								$voucher_id=( ($_POST['voucher_id'])) ? $_POST['voucher_id'] : "0";
			...
			$payment_type_val=( ($payment_type_val)) ? $payment_type_val : "";
							...
							$issue_quantity=( ($_POST['issue_qty'])) ? $_POST['issue_qty'] : "0";
							//$service_tax=$_POST['service_tax'];
							$service_tax=( ($_POST['service_tax'])) ? $_POST['service_tax'] : "0";
							///$total_cost_with_tax=$_POST['total_cost_with_tax'];
							$total_cost_with_tax=( ($_POST['total_cost_with_tax'])) ? $_POST['total_cost_with_tax'] : "0";
							//$selling_amount=$_POST['selling_amount'];
							$selling_amount=( ($_POST['selling_amount'])) ? $_POST['selling_amount'] : "0";
							...
							$total_type1=( ($_POST['total_type1'])) ? $_POST['total_type1'] : "";
			...
			else
			{
				...
				if($record_id)
				{
					...
					"<br>".$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('add_sales_memb_package','Edit','".$category."','".$del_record_id."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
				...
				else
				{
					...
					echo "<br/>".$insert_cust_service_invoice = " INSERT INTO `package_invoice` (`v_id`, `total_cost`, `amount`, `payable_amount`,`remaining_amount`, `total_paid`,`paid_type`, `bank_id`, `cust_bank_name`,`cheque_detail`, `chaque_date`, `credit_card_no`,`isas_paytm_no`,`cust_paytm_no`, `admin_id`, `added_date`,`status`,`cm_id`) VALUES ('".$id."', '".$selling_amount."', '".$amount."', '".$payable_amount."','".$remaining_amount."', '".$payable_amount."', '".$payment_mode_id."','".$bank_name."','".$cust_bank_name."', '".$chaque_no."', '".$sep_check_date."','".$credit_card_no."','".$isas_paytm_no."','".$cust_paytm_no."', '".$_SESSION['admin_id']."', '".date('Y-m-d H:i:s')."','".$status."','".$cm_id."'); ";
					$ptr_cust_service_invoice = mysql_query($insert_cust_service_invoice);	
					$ins_id=mysql_insert_id($ptr_cust_service_invoice);
									...
									if($category=="Voucher")
									{
										...
										//------send notification on Sale voucher--------------------
											$notification_args['reference_id'] =$id;
											$notification_args['on_action'] = 'sale_voucher';
									...
									if($category=="Package")
									{
										...
										{*/
											...
											while($data_vsm=mysql_fetch_array($ptr_vsm))
											{
												$qty=intval($_POST['issue_qty'])*intval($data_vsm['quantity']);
												 "<br>".$insert_qty="insert into sales_customer_service_voucher_map(`id`,`customer_id`,`package_id`,`service_id`,`quantity`) values('".$id."','".$cust_id."','".$package_id."','".$data_vsm['service_id']."','".$qty."')";
												$query_prod_qty=mysql_query($insert_qty);
											}
										//}
										//------send notification on Sale voucher--------------------
											$notification_args['reference_id'] =$id;
											$notification_args['on_action'] = 'sale_package';
											$notification_status = addNotifications($notification_args);
										//---------------------------------------------------------------
									}
									
									$sel_cust="select cust_name,mobile1,rewards_point from customer where cust_id ='".$cust_id."'";
									...
									$selct_rpc="SELECT * FROM `reward_point_config` where cm_id='".$cm_id."'";
									...
									$total_rewards_pt=$data_cust_name['rewards_point']+$total_rp;
									...
									"<br/>".$sel_sms_cnt="select * from sms_mail_configuration_map where previlege_id='127' ".$_SESSION['where']."";
									$ptr_sel_sms=mysql_query($sel_sms_cnt);
									while($data_sel_cnt=mysql_fetch_array($ptr_sel_sms))
									{
									...
									"<br>".$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('add_sales_memb_package','Add','".$category."','".$id."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
									$query=mysql_query($insert); 
   ...
   <form method="post" id="jqueryForm" name="jqueryForm" enctype="multipart/form-data" onSubmit="return validme()">
	<table border="0" cellspacing="15" cellpadding="0" width="100%">
           ...
           <tr>
            	<td colspan="3"><div id="show_customer"></div><input type="hidden" name="mail" id="mail" value=""  /></td>	
           </tr>
           <tr>
                ...
                <table  width="100%" style="border:1px solid gray; ">
                    ...
                    <script language="javascript">
...

						{
							...
							var shows_data='<div id="type1_id'+idss+'"><table cellspacing="3" id="tbl'+idss+'" width="100%"><tr><td valign="top" width="8%"><input type="hidden" name="exclusive_id'+idss+'" id="exclusive_id'+idss+'" value="" /><select name="tax_type'+idss+'" id="tax_type'+idss+'" onChange="get_tax_value('+idss+',this.value)" ><option value="">Select Tax</option>'+res1+'</select></td><td valign="top" width="8%" align="left"><input type="text" name="tax_value'+idss+'" id="tax_value'+idss+'" onKeyUp="calculte_amount_tax('+idss+')"/><input type="hidden" name="tax_amount'+idss+'" id="tax_amount'+idss+'" /></td><input type="hidden" name="total_tax[]" id="total_tax'+idss+'" /><input type="hidden" name="row_deleted'+idss+'" id="row_deleted'+idss+'" value="" /><tr></table></div>';
	...
	<td>Payable Amount</td>
    		<td><input type="text" name="payable_amount" id="payable_amount" value="<?php if($_POST['payable_amount']) echo $_POST['payable_amount']; else echo $row_record['payable_amount']; ?>" onKeyUp="find_remain();" /></td>
	</tr>
	
		<tr>
	<td>Remaining Amount</td>
    		<td><input type="text" name="remaining_amount" id="remaining_amount" value="<?php if($_POST['remaining_amount']) echo $_POST['remaining_amount']; else echo $row_record['remaining_amount']; ?>" /></td>
	</tr>
	
  	<tr>
   			<td>&nbsp;</td>
    		<td><input type="submit" class="input_btn"  value="<?php if($record_id) echo "Update"; else echo "Sales";?> Vouchers/Package/membership" name="save_changes"  /></td>
    		<td></td>
  	</tr>
</table>
</form>
