...
if($_POST['branch_id']) //$_POST:-because of that we can use this  in to the function for ajax 
{
	if($_POST['action_for']=="for_credit_card")
	{
		...
		"<br/>".$sle_bank_name="select b.bank_id,b.bank_name,b.is_credit_card,b.is_paytm from bank b, bank_payment_map bp where b.bank_id = bp.bank_id and bp.payment_mode_id='".$payment_id."' and b.cm_id='".$data_cm_id['cm_id']."' "; //".$pay_type."
	...
	else
	{
		...
		"<br/>".$sle_bank_name="select bank_id,bank_name,is_credit_card,is_paytm from bank where cm_id='".$data_cm_id['cm_id']."' ".$pay_type." "; //".$pay_type."
		$ptr_bank_name=mysql_query($sle_bank_name);
		if(mysql_num_rows($ptr_bank_name))
		{
			while($data_bank=mysql_fetch_array($ptr_bank_name))
