	...
	while($val_query=mysql_fetch_array($query_order))
	{
		...
		if($tot_prod=mysql_num_rows($ptr_prod))
		{
			while($data_prod=mysql_fetch_array($ptr_prod))
			{
				...
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('A'.$rowCount, $date);	
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('B'.$rowCount, $val_query['receipt_no']);
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('E'.$rowCount, 'Product @ '.$data_prod['total_gst'].'');	
				...
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('H'.$rowCount, round($tot_price,2));
		...
		//=====================PRODUCT GST NUll=============
		$sel_prod="select SUM(i.cgst_tax) as cgst_total,SUM(i.base_prod_price * i.product_qty) as base_total,SUM(i.discounted_price * i.product_qty) as total_val,s.total_price, i.product_disc, SUM(i.discounted_price) as total_cgst_val from sales_product_map i, sales_product s where i.sales_product_id='".$val_query['sales_product_id']."' and i.sales_product_id=s.sales_product_id and (i.cgst_tax_in_per <=0 and i.sgst_tax_in_per <=0 i.igst_tax_in_per <=0) group by i.igst_tax_in_per";
		$ptr_prod= mysql_query($sel_prod);
		if($tot_prod=mysql_num_rows($ptr_prod))
		{
			while($data_prod=mysql_fetch_array($ptr_prod))
