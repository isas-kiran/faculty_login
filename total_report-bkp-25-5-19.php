    						...
    						if($_POST['save_changes'])
                        	{
								...
								$repeat_date = $_GET['date'];
									...
									$batch_id=$_GET['batch_id'];
								   	$delete_existing = " delete from student_attendence where c_b_id='".$_GET['batch_id']."' and repeated='$repeat_date' ";
              ...
              <td width="30">Select Batch<span class="orange_font"></span></td>
                                 <td width="38%" >
                                    ...
                                    <?php
                                        ...
                                        {
											$select_batch = "select * from batch where batch_id='".$data_cb['batch_id']."' ".$_SESSION['where']." order by batch_id asc";
                                        	$ptr_batch = mysql_query($select_batch);
											$data_batch = mysql_fetch_array($ptr_batch);
											...
											$sel='';
											if($_GET['batch_id']==$data_cb['batch_id'])
											{
												$sel='selected="selected"';
											}
											
                                        	echo '<option '.$sel.' value='.$data_cb['batch_id'].'>'.$data_batch['batch_name'].' ('.$data_staff['name'].')</option>';
                                        } 
                                        ?>  
                                         <?php //if(!$record_id || !$record_id_course)
                                      //{ echo ' <option value="new_batch" ><b>Add New Batch</b></option>  ';}
                      ...
                      <?php
                       	if($_REQUEST['from_date'] && $_REQUEST['from_date']!=="0000-00-00" && $_REQUEST['from_date']!="From Date")
                      	{
                  			$pre_from_date=" and date_format(added_date,'%Y-%m-%d')>='".date('Y-m-d',strtotime($_REQUEST['from_date']))."'";
								...
								{	
									$batch_id=$_GET['batch_id'];
									 $sql_records= "SELECT * FROM student_course_batch_map where c_b_id=".$batch_id." ".$_SESSION['where']." ";
