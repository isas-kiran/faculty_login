...
<title>Campaign Staff Report</title>
...
<?php
$edit_access='';
$sel_acc="select * from edit_previleges where admin_id='".$_SESSION['admin_id']."' and privilege_id='257'";
...
?>
	...
	{            
		...
		$("#admin_id").chosen({allow_single_deselect:true});
	});
	</script>
    <script type="text/javascript">
...
<table cellspacing="0" cellpadding="0" class="table" width="95%">
                ...
                <!--<td width="20%">
                        ...
                        <td width="15%" id="admin_id_by">
							...
							<option value="">Select Staff</option>
								<?php 
                                $course_category = "select DISTINCT(cm_id),branch_name from site_setting where 1 and (type='A' or type='C') and system_status='Enabled' ".$_SESSION['where']."";
                                ...
                                while($data_cat = mysql_fetch_array($ptr_course_cat))
                                {
                                    ...
                                    $sel_source="SELECT * FROM site_setting where 1 and cm_id='".$data_cat['cm_id']."'  and system_status='Enabled' order by name asc";
  ...
  <?php
						...
						if($_REQUEST['admin_id'])
						{
							...
							$where_admin_id=" and employee_id='".$admin_id."'";
							$where_assigned_id=" and assigned_to='".$admin_id."'";
	                    ...
	                    if($no_of_records)
                        {
                            ...
                            $query_string='&keyword='.$keyword.'&branch_name='.$_REQUEST['branch_name'].'&admin_id='.$_REQUEST['admin_id'].'&from_date='.$_GET['from_date'].'&to_date='.$_GET['to_date'];
                           ...
                           // $pager = new PS_Pagination($sql_query,$_SESSION['show_records'],$_SESSION['show_records_pages'],$query_string1);
							...
							while($val_query=mysql_fetch_array($all_records))
							{
								...
								$total_source='';	       
                                $sel_inq_source="select inquiry_id from inquiry where 1 and enquiry_source ='".$val_query['campaign_id']."' ".$_SESSION['where']." ".$enquiry_from_date." ".$enquiry_to_date." ".$where_admin_id." ".$enquiry_src." ";
								$ptr_inq_source=mysql_query($sel_inq_source);
								$total_inq_source=mysql_num_rows($ptr_inq_source);

								$sel_enroll_src="select enroll_id from enrollment where source ='".$val_query['campaign_id']."' ".$_SESSION['where']." ".$enquiry_from_date." ".$enquiry_to_date." ".$where_assigned_id." ".$enroll_src." ";
