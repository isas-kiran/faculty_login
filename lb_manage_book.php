    ...
    <?php       if($_POST['formAction'])
                    {
                        if($_POST['formAction']=="delete")
                        {
                            for($r=0;$r<count($_POST['chkRecords']);$r++)
                            {
                                ...
                                $sql_query= "SELECT book_id,book_name FROM lb_book where book_id ='".$del_record_id."'";
                                $ptr_query=mysql_query($sql_query);
								if(mysql_num_rows($ptr_query))
								{    
									$cust_data=mysql_fetch_array($ptr_query);  
									"<br>".$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('manage_book','Delete','".$cust_data['book_name']."','".$del_record_id."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
									$query=mysql_query($insert);              
									                                     
									$delete_query="delete from lb_book where book_id='".$del_record_id."'";
...
<?php
                        ...
                        if($keyword)
                            $pre_keyword=" and (book_name like '%".$keyword."%' || isbn_no like '%".$keyword."%' || description like '%".$keyword."%' || language like '%".$keyword."%' || publisher like '%".$keyword."%' || published_year like '%".$keyword."%' || author like '%".$keyword."%' || rack_location like '%".$keyword."%')";
                        else
                            $pre_keyword="";
                        ...
                        if($_GET['order'] !='' && ($_GET['orderby']=='product_name'))
                        {
							...
							$select_directory='order by book_id desc';  
						$record_cat_id='';
						if($_GET['record_id'] !='')
						{
							$record_cat_id="and book_id='".$_GET['record_id']."' ";
						}
						$sql_query= "SELECT * FROM lb_book where 1 ".$record_cat_id." ".$_SESSION['where']." ".$pre_keyword." ".$select_directory."";
                     ...
                     <input type="hidden" name="formAction" id="formAction" value=""/>
                      <tr class="grey_td" >
					   <?php
                        ...
                        <td width="9%" align="center"><strong>Description</strong></td>
                        <td width="4%" align="center"><strong>Language</strong></td>
                        <td width="4%" align="center"><strong>Publisher</strong></td>
                        <td width="4%" align="center"><strong>Published Year</strong></td>
                        <td width="10%" align="center"><strong>Author</strong></td>
                        <td width="6%" align="center"><strong>Rack Location</strong></td>
                        <td width="5%" align="center"><strong>Quantity</strong></td>
                        <td width="5%" align="center"><strong>Price</strong></td>                       
                        ...
                        while($val_query=mysql_fetch_array($all_records))
                        {
							...
							$listed_record_id=$val_query['book_id']; 
							...
							echo '<td >'.$val_query['book_name'].'</td>';			
															
							$select_prod_cat="select category_name,category_id from lb_book_category where category_id='".$val_query['category_id']."'";
							$query_cat=mysql_query($select_prod_cat);
							$fetch_cat=mysql_fetch_array($query_cat);
							
							echo '<td align="center">'.$fetch_cat['category_name'].'</td>';
							echo '<td align="center">'.$val_query['isbn_no'].'</td>';
							echo '<td align="center">'.$val_query['description'].'</td>';
							echo '<td align="center">'.$val_query['language'].'</td>';							
							echo '<td align="center">'.$val_query['publisher'].'</td>';
							echo '<td align="center">'.$val_query['published_year'].'</td>';
							echo '<td align="center">'.$val_query['author'].'</td>';
							echo '<td align="center">'.$val_query['rack_location'].'</td>';
