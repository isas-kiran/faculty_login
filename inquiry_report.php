...
<title>Student Inquiry Report</title>
...
?>  
	...
	<table border="0" cellspacing="0" cellpadding="0" width="99%" align="center">
              <tr>
                ...
                <td width="8%">
                        ...
                        <td width="4%"> <a href="excel.php<?php echo $sep_url_string; ?>"><img src="images/excel.png" height="30px" width="30px"/></a></td>
                        ...
                        <td width="10%" id="added_by">
                        <select name="enquiry_added" class="input_select">
                        	<option value="">Added to</option>
                             <?php
                        ...
                        <td width="10%"><select id="response" name="response" class="input_select">
                            ...
                            <td width="10%">
                                <select id="date_by" name="date_by" class="input_select">
                                    ...
                                    <option value="inquiry_by">Inquiry Added</option>
                                    <option value="followup_by">Followup date</option>
                                </select>
                            </td>
							<td width="10%"><input type="text" class="input_text datepicker" name="start_date" placeholder="From" id="dob" value="<?php if($_REQUEST['start_date']!="") echo $_REQUEST['start_date'];?>" /><td>
							<td width="10%"><input type="text" class="input_text datepicker" name="end_date" id="end_date" placeholder="To" value="<?php if($_REQUEST['end_date']!="") echo $_REQUEST['end_date'];?>"  /></td>
							<!--<td width="10%"><input type="submit" name="search" value="Search" class="input_Button"/></td>-->
							<td width="10%"><input type="text" value="<?php if($_REQUEST['keyword']!="Keyword") echo $_REQUEST['keyword'];?>" name="keyword" class="defaultText search_box" title="Keyword" /></td><td class="width5"></td><td class="width5"></td>
    ...
    <?php
                        ...
                        {          
								...
								if(mysql_num_rows($ptr_branch)) 
								{
									...
									$cm_id_filter="|| i.cm_id = '".$data_branch['cm_id']."'";
								} 
								$course_name_filter='';
								...
								{
								 $course_name_filter.= " || i.course_id =".$data_installment['course_id'];
									if($i !=$total)
									{
										//$xx.= '<br /><hr >';
									}
								...
								if(mysql_num_rows($ptr_enq))
								{
									...
									$enquiry_added="|| i.admin_id = '".$data_enq['admin_id']."'";
								}
                                $pre_keyword =" and (i.firstname like '%".$keyword."%' || i.middlename like '%".$keyword."%' || i.lastname like '%".$keyword."%' || i.username like '%".$keyword."%' || i.mobile1 like '%".$keyword."%' ".$cm_id_filter." ".$course_name_filter." ".$enquiry_added." || i.address like '%".$keyword."%' || i.email_id like '%".$keyword."%' || i.status like '%".$keyword."%' || i.enquiry_source like '%".$keyword."%' || i.response like '%".$keyword."%')";
                            }                            
                       	else
                            $pre_keyword="";
						...
						if($_REQUEST['date_by']=="inquiry_by")
						{						
							if($_REQUEST['start_date'] && $_REQUEST['start_date']!=="0000-00-00" && $_REQUEST['start_date']!="From Date")
							{
								...
								$pre_from_date=" and DATE(i.added_date) >='".date('Y-m-d',strtotime($frm_dates))."'";
							}
							else
							{
								$pre_from_date="";                            
							...
							if($_REQUEST['end_date'] && $_REQUEST['end_date']!=="0000-00-00" && $_REQUEST['end_date']!="To Date")
							{
								...
								$pre_to_date=" and DATE(i.added_date) <='".date('Y-m-d',strtotime($to_dates))."'";						
							}
							else
								$pre_to_date="";
						}
						...
						if($_REQUEST['enquiry_added'])
						{
							...
							$enquiry_added=" and i.admin_id='".$enquiry_ad."'";
						}
						$assigned_to='';
						...
						if($_REQUEST['enquiry_src'])
						{
							...
							$enquiry_src=" and i.enquiry_source = '".$enq_src."'";
						}
						...
						if($_GET['record_id'] !='')
						{
							$record_cat_id="and i.inquiry_id='".$_GET['record_id']."' ";
						} 
						$c_id='';
						if($_GET['c_id'] !='')
						{
							$c_id="and i.campaign_id='".$_GET['c_id']."' ";
						} 
						$cm_ids=='';
						if($_SESSION['where'] !='')
						{
                        ...
                        $select_directory='order by i.inquiry_id desc';    
						"<br/>".$sql_query= "SELECT i.*,f.added_date as followup_added_date,f.followup_date as followup_date,f.response as followup_response FROM inquiry i, followup_details f where 1 and i.inquiry_id=f.student_id ".$status_type." ".$record_cat_id." ".$cm_ids." ".$search_cm_id." ".$pre_keyword." ".$enquiry_added." ".$assigned_to." ".$enquiry_src." ".$response." ".$pre_from_date." ".$pre_to_date." ".$c_id." ".$select_directory.""; 
						//echo $sql_query;
                        ...
                        if($no_of_records)
                        {
                            ...
                            $query_string='&keyword='.$keyword.'&branch_name='.$branch_name.'&enquiry_added='.$enquiry_ad.'&enquiry_src='.$enq_src.'&response='.$response.'&start_date='.$_REQUEST['start_date'].'&date_by='.$_REQUEST['date_by'].'&end_date='.$_REQUEST['end_date'].'&status_type='.$_REQUEST['status_type'];
  ...
  <tr class="grey_td" >
    ...
    <td width="11%" align="center"><strong>Added By</strong></td>
	<td width="11%" align="center"><strong>Assigned To</strong></td>
   ...
   <!-- <td width="5%" align="center"><strong>Status</strong></td>
    <td width="6%" align="center"><strong>Followup</strong></td>-->
    <td width="6%" align="center"><strong>Followup Date</strong></td>
    <!--<td width="6%" align="center"><strong>Enrollment</strong></td>-->
	<?php
	if($_SESSION['type']=='S' )
	{?>
...
//                                $val_contry = $db->fetch_array($db->query($select_country));
								...
								$sel_date="select followup_id from followup_details where 1 and student_id='".$val_query['inquiry_id']."' and added_date >='".$val_query['followup_date']."'";
								$pre_date=mysql_query($sel_date);
								if(mysql_num_rows($pre_date))
								{
									$bg_color='style="background-color:#D5FADF !important"';
									$followup_count +=1;
								}
								else
									$followup_pending +=1;
                               ...
                               /* echo '<td align="center">'.$val_query['contact'].'</td>';*/
								...
								$response='';
								if($val_query['followup_response'] == "1")
								{
									$response="Walk in";
								}
								else if($val_query['followup_response'] == "2")
								{
									$response="Will walk in";
								}
								else if($val_query['followup_response'] == "3")
								{
									$response="Call Back Later";
								}
								else if($val_query['followup_response'] == "4")
								{
									$response="Call did not pick up";
								}
								else if($val_query['followup_response'] == "5")
								{
									$response="Not reachable";
								}
								else if($val_query['followup_response'] == "6")
								{
									$response="Call Taken";
								}
								...
								echo '<td style="color:#FF0000" align="center"><a href="followup_details.php?record_id='.$val_query['inquiry_id'].'" ><img title="Followup" src="images/followup.png" height="30" width="30"></a></td>';*/
    ...
    <td width="10%" colspan="8" align="left"><strong><?php echo $no_of_records;?></strong></td>
    <!--<td width="6%" colspan="2" align="center"><strong>Total Followups completed</strong></td>
    <td width="10%" colspan="2" align="left"><strong><?php echo $followup_count;?></strong></td>
     <td width="6%" colspan="2" align="center"><strong>Total Followups Pending</strong></td>
    <td width="10%" colspan="1" align="left"><strong><?php echo $followup_pending;?></strong></td>-->
