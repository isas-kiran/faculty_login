...
<table border="0" cellspacing="0" cellpadding="0" width="100%">
			...
			{ 
				if($_POST['formAction']=="delete")
				{
					for($r=0;$r<count($_POST['chkRecords']);$r++)
					{
						...
						$sql_query= "SELECT followup_id FROM ".$GLOBALS["pre_db"]."installment_followup_details where followup_id ='".$del_record_id."'";
						...
						if(mysql_num_rows($my_query))
							{       
								...
								$delete_query="delete from ".$GLOBALS["pre_db"]."installment_followup_details where followup_id='".$del_record_id."'";
					 ...
					 ?>
							...
							// $("#statusChangesDiv").dialog();
								...
								setTimeout('document.location.href="installment_followup_details.php";',2000);
							</script>
					<?php                            
						}                       
			 }
  ...
  <!-- ================================================Start Table=============================================================================================-->
<?php
 $sql_query= "select enroll_id,name,contact,mail,course_id,added_date,cm_id from enrollment where enroll_id='".$record_id."' order by enroll_id asc "; 
 $db=mysql_query($sql_query);
 $data_ptr=mysql_fetch_array($db);
 
 $select_course_id="select course_name from courses where course_id='".$data_ptr['course_id']."' ";
 ...
 if($invoice_id)
 { 
	$sel_invoice="select status from invoice where invoice_id='".$invoice_id."'";
	$ptr_invoice_id=mysql_query($sel_invoice);
	$data_invoice=mysql_fetch_array($ptr_invoice_id);
	$invoice_status=$data_invoice['status'];
 }
?>
...
<table cellspacing="0" cellpadding="0" class="table" width="95%">
  ...
  <tr class="head_td">
	...
	<table border="0" cellspacing="0" cellpadding="0" width="99%" align="center">
              <tr>
                ...
                <td><strong>Course Name- </strong><?php echo $val_select_course['course_name']; ?></td>       
				<?php 
				if($invoice_status !="paid" && $show !="false")
				{
					?>
					<td  align="right"><a href="installment_followup_summery.php?record_id=<?php echo $record_id; ?>&installment_id=<?php echo $installment_id; ?>&ref_id=<?php echo $ref_id; ?>"><input type="button" name="update_followup" value="Update Followup" /></a></td>
					<?php 
				}
				?>
                <td  align="right"><a href="invoice-summary.php?record_id=<?php echo $record_id; ?>"><input type="button" name="back" value="Back" /></a></td>
                <td class="rightAlign" > 
    ...
    <?php
						...
						{
							$where_installments_ids=" and invoice_id='".$invoice_id."'";
						}
						else
						{
							//$where_installments_ids=" and student_id='".$invoice_id."'";//For wrong condition
						}
						
						
                        $select_directory='order by followup_id desc';                      
                        $sql_query= "select * from installment_followup_details where student_id ='".$record_id."' ".$where_installments_ids." ".$_SESSION['where']." ".$pre_keyword." ".$select_directory.""; 
					   	$db=mysql_query($sql_query);
  ...
  <tr class="grey_td" >
    ...
    <td width="15%" align="center"><strong>Payment Date</strong></td>
    ...
    <td width="15%" align="center"><strong>Followup By </strong></td>
     <td width="10%" align="center"><strong>Added Date</strong></td>
                            ...
                            while($val_query=mysql_fetch_array($all_records))
                            {
								...
								/*$sele_latest_rec="select followup_date,lead_category_followup,lead_grade, added_date,followup_details from followup_details where student_id = '".$record_id."' order by followup_id desc";
...
//                                $val_contry = $db->fetch_array($db->query($select_country));
								...
								if($val_query['payment_date'] !='')
								{
									$sep_date = explode('-',$val_query['payment_date']);
									$payment_date = $sep_date[2].'/'.$sep_date[1].'/'.$sep_date[0];
								}
								if($val_query['followup_date'] !='')
								{
									$sep_date = explode('-',$val_query['followup_date']);
								...
								echo '<td align="center">'.$payment_date.'</td>';  
								echo '<td align="center">'.$followup_date.'</td>';  
