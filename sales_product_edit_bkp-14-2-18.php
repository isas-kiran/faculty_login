...
function show_data(id)
{
	...
	var data1="action=show_data&action_page=sales_product&id="+id+'&record_id='+record_id;
	if (record_id!='')
	{
	}
	else
...
function calc_product_price(prod_id)
{
	...
	total_exit_qty=Number(prod_qty)+ Number(prev_prod_qty);
	alert(total_exit_qty);
	total_prod_qty=document.getElementById("product_total_qty"+prod_id).value;
	
	if(Number(total_exit_qty) > Number(total_prod_qty) )
	{
                            ...
                            else
                            {
								...
								//=======================================LEMEURICE==================================================
									if($customer_id=="904")
									{
										...
										echo "<br/>inv1-".$record_id=mysql_insert_id($db2);
										...
										for($i=1;$i<=$total_floor;$i++)
										{
											if(trim($_POST['product_id'.$i])!='')
											{
												...
												echo "<br/>inv2-".$record_id;
												echo"<br/>sele1-".$sel_product_name="select product_id,product_name,product_code,pcategory_id,sub_id,size,unit,commission,price, vender,type,added_date,cm_id,quantity,admin_id from product where product_id='".$product_id."'";
												...
												if(mysql_num_rows($ptr_sel))
												{
													...
													echo"<br/>sel-".$sele_cate="select product_id from product where product_name='".$data_product['product_name']."' ".$prd_admin_id."  ";
													...
													{
														echo "<br/>inv3-".$record_id;
														echo"<br/>1".$sel_category="select pcategory_name from product_category where pcategory_id='".$data_product['pcategory_id']."'";
														$ptr_category=mysql_query($sel_category,$con1);
														$data_cate=mysql_fetch_array($ptr_category);//=====================
														
														echo"<br/>2".$sel_subcategory1="select sub_name from product_subcategory where sub_id='".$data_product['sub_id']."'";
														$ptr_subcategory1=mysql_query($sel_subcategory1,$con1);
														$data_subcategory=mysql_fetch_array($ptr_subcategory1);//=================pune=========================
														
														echo"<br/>3".$sele_cateahm="select pcategory_id from product_category where pcategory_name='".$data_cate['pcategory_name']."' order by  pcategory_id asc";
														...
														{
															echo"<br/>4".$insert_cat="insert into product_category (`pcategory_name`,`added_date`,`cm_id`,`admin_id`) values('".$data_cate['pcategory_name']."','".date('Y-m-d H:i:s')."','2','".$data_record_ahm['admin_id']."')";
															$ptr_ins_cat=mysql_query($insert_cat,$db2);
															$cat_id=mysql_insert_id($db2);
														}
														
														echo"<br/>5".$sele_subcateahm="select sub_id from product_subcategory where sub_name='".$data_subcategory['sub_name']."' order by  sub_id asc";
														...
														echo "<br/>inv4-".$record_id;											
														echo"<br/>7".$inser_prod="insert into product (`product_name`,`product_code`,`pcategory_id`,`sub_id`,`size`,`unit`,`commission`,`price`,`type`,`added_date`,`cm_id`,`quantity`,`admin_id`) values ('".$data_product['product_name']."','".$data_product['product_code']."','".$cat_id."','".$sub_cat_id."','".$data_product['size']."','".$data_product['unit']."','".$data_product['commission']."','".$data_product['price']."','".$data_product['type']."','".date('Y-m-d H:i:s')."','".$cm_ids."','".$data_record_service_inv['sin_product_qty']."','".$data_record_ahm['admin_id']."')";
                        ...
                        <td colspan="6">
							...
							if($record_id!='')
							{
                            	...
                            	{ 
									...
									echo "<br/>".$sel_tel = "select p.product_id,p.product_name,p.price,p.quantity,p.admin_id from product p, site_setting s where 1 and p.status='Active' and p.admin_id=s.admin_id ".$where_cm." order by product_id asc";//remove ".$_SESSION['user_id']." on 14/5/2018 --and s.type='ST' on 13/6/18
									$query_tel = mysql_query($sel_tel);
									$qty_in_stock='';
									...
									<td width="5%" align="center"><input type="text" name="product_total_qty<?php echo $t; ?>" readonly="readonly" id="product_total_qty<?php echo $t; ?>" style=" width:40px" value="<?php echo $qty_in_stock; ?>" onKeyUp="getDiscount(<?php echo $t; ?>)"/></td>
