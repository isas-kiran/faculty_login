...
$total_found =0;	
	...
	if($_REQUEST['keyword']!="Keyword")
                        ...
                        if($_GET['order'] !='' && ($_GET['orderby']=='firstname'))
                        {
							...
							$select_directory='order by DATE(added_Date) asc';                    
...
$objPHPExcel->setActiveSheetIndex(0)->getStyle('I1:I'.$objPHPExcel->setActiveSheetIndex()->getHighestRow())->getAlignment()->setWrapText(true);$objPHPExcel->setActiveSheetIndex(0)->getStyle('J1:J'.$objPHPExcel->setActiveSheetIndex()->getHighestRow())->getAlignment()->setWrapText(true);$objPHPExcel->setActiveSheetIndex(0)->getStyle('K1:K'.$objPHPExcel->setActiveSheetIndex()->getHighestRow())->getAlignment()->setWrapText(true);$objPHPExcel->setActiveSheetIndex(0)->getStyle('L1:L'.$objPHPExcel->setActiveSheetIndex()->getHighestRow())->getAlignment()->setWrapText(true); 
...
$objPHPExcel->setActiveSheetIndex(0);
 ...
 $objPHPExcel->setActiveSheetIndex()->setCellValueByColumnAndRow('3','3', 'Mo No','3');
  ...
  $objPHPExcel->setActiveSheetIndex()->setCellValueByColumnAndRow('13','3', 'Status','3');*/
	...
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('C4', 'Employee Name','6');
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('D4', 'Expense Details','6');
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('E4', 'Payment Type','6');
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('F4', 'Bank Name','6');
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('G4', 'Bank Details','6');
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('H4', 'Total Amount','6');
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('I4', 'Total Paid Amount','6');
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('J4', 'Total Remaining Amount','6');
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('K4', 'Date','6');
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('L4', 'Description','6');
	...
	while($val_query=mysql_fetch_array($query_order))
	{
		...
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('C'.$rowCount, $name);		
		...
		$sel_p1= " select expense_type from expense_type where expense_type_id='".$val_query['expense_type_id']."'";
		$ptr_p1= mysql_query($sel_p1);
		$data_p1 = mysql_fetch_array($ptr_p1); 
		$exp_type= 'Expense Type :-'.$data_p1['expense_type'];
								
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('D'.$rowCount, $exp_cat."\n".$exp_type);
		...
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('E'.$rowCount, $payment_mode);
		...
		{
			$bank_detail="Bank Name.-". $data_bank['bank_name']."\n Acc. No.- ".$data_bank['account_no'];
		}
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('F'.$rowCount,$bank_detail);		
		...
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('G'.$rowCount, $b_details."\n".$b_details1."\n".$b_details2."\n".$b_details3);
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('H'.$rowCount, $val_query['amount']);
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('I'.$rowCount, $val_query['payable_amount']);
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('J'.$rowCount, $val_query['remaining_amount']);
		  
		$sep=explode(" ",$val_query['added_Date']);
		$sep_date=explode("-",$sep[0]);
		$date=$sep_date[2]."/".$sep_date[1]."/".$sep_date[0];
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('K'.$rowCount, $date);	
		
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('L'.$rowCount,htmlspecialchars($val_query['description']));	
...
header('Content-Disposition: attachment;filename="expense_summury_report.xls"');
