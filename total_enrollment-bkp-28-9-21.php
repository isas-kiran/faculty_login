	...
	<table border="0" cellspacing="0" cellpadding="0" width="99%" align="center">
              <tr>
                ...
                <!--<td width="20%">
						...
						?>
						 <td width="12%">
							<select name="branch_name" id="branch_name"  class="input_select_login" onchange="select_branch(this.value)" >
                        ...
                        <tr>
                        <td width="6%">Select Source<span class="orange_font">*</span></td>
                        <td width="8%" >
                        ...
                        <td width="6%">Transfer From<span class="orange_font">*</span></td>
                        <td class="customized_select_box" width="12%">
                            <select id="transfer_from" name="transfer_from" class="input_text" >
                                <option value="">Select Transfer From</option>
                                <?php
                                $sel_admin="select branch_name,cm_id from site_setting where type='A' group by branch_name";
                                $ptr_admin=mysql_query($sel_admin);
                                while($data_admin= mysql_fetch_array($ptr_admin))
                                {
                                    $sel_br_assign='';
                                    if($_REQUEST['transfer_from'] == $data_admin['cm_id'])
                                    {
                                        $sel_br_assign='selected="selected"';
                                    }
                                    ?>
                                    <option <?php echo $sel_br_assign; ?> value ="<?php echo $data_admin['cm_id']?>" > <?php echo $data_admin['branch_name'] ?> </option>
                                    <?php 
                                }
                                ?>
                            </select>
                        </td>
                        <td width="6%">Transfer To<span class="orange_font">*</span></td>
                        <td class="customized_select_box" width="12%">
                            <select id="transfer_to" name="transfer_to" class="input_text" >
                                ...
                                {
                                    $sel_br_to_assign='';
                                    if($_REQUEST['transfer_to'] == $data_admin['cm_id'])
                                    {
                                        $sel_br_to_assign='selected="selected"';
                                    }
                                    ?>
                                    <option <?php echo $sel_br_to_assign; ?> value ="<?php echo $data_admin['cm_id']?>" > <?php echo $data_admin['branch_name'] ?> </option>
                                    <?php 
                                }
                                ?>
                            </select>
     ...
     <?php
                        ...
                        $sql_query= "SELECT * FROM enrollment where 1 ".$_SESSION['where']." ".$where_course." ".$enquiry_src." ".$where_assign_to." ".$where_added_by." ".$whr_transfer_from." ".$whr_transfer_to." ".$search_cm_id." ".$enquiry_from_date." ".$enquiry_to_date."  ".$select_directory."";
                        ...
                        if($no_of_records)
                        {
                            ...
                            $query_string='&keyword='.$keyword.'&from_date='.$_REQUEST['from_date'].'&to_date='.$_REQUEST['to_date'].'&branch_name='.$_REQUEST['branch_name'].'&course_id='.$_REQUEST['course_id'].'&assign_to='.$_REQUEST['assign_to'].'&added_by='.$_REQUEST['added_by'].'&enquiry_src='.$enquiry_src;
                           ...
                           // $pager = new PS_Pagination($sql_query,$_SESSION['show_records'],$_SESSION['show_records_pages'],$query_string1);
                            ...
                            <tr class="grey_td" >
                                ...
                                <td width="7%" align="center"><strong>Source</strong></td>
