...
if($_REQUEST['record_id'])
{
    ...
    $sql_record= "SELECT * FROM product_quotation where quotation_id='".$record_id."'";
...
<title><?php if($record_id) echo "Edit"; else echo "Add";?> Quotation</title>
...
<table border="0" cellspacing="0" cellpadding="0" width="100%">
  ...
  <tr>
    ...
    <td class="mid_mid">
		...
		if(isset($_POST['save_changes']))
		{
			...
			if(count($errors)==0 && $_FILES['ref_invoice']["name"])
			{
				...
				{
					$update_news="update product_quotation set ref_invoice='' where quotation_id='".$record_id."'";
			...
			else
			{
				...
				$data_record['amount1'] = $total_cost;//$amount1;
				...
				if($record_id)
				{
					$sel_inv="select sin_product_qty,product_id from product_quotation_map where quotation_id='".$record_id."'";
					...
					while($data_inv=mysql_fetch_array($ptr_inv))
					{
						...
						$ins_data="INSERT INTO `product_daily_report`(`product_id`,`cm_id`,`stockiest_id`,`type`, `purchase_qty`,`sales_qty`,`checkout_qty`,`vendor_id`,`cust_type`,`cust_id`,`todays_qty`,`todays_shelf_qty`, `todays_consumable_qty`,`description`,`admin_id`,`added_date`) VALUES ('".$product_id."','".$cm_id."','".$data_stock['admin_id']."','purchase','".$$qty."','0','0','".$vendor_id."','','','sin_product_qty -".$qty."','','','Product QUotation Edit Remove','".$_SESSION['admin_id']."','".date('Y-m-d H:i:s')."')";
						$ptr_ins=mysql_query($ins_data);
					}
					
					$delete_query1="delete from product_quotation_map where quotation_id='".$record_id."'";
					$db->query($delete_query1); 
					 
					$delete_query2="delete from product_quotation_map where quotation_id='".$record_id."'";
					$db->query($delete_query2);
					
					$where_record="quotation_id='".$record_id."'";
					$db->query_update("product_quotation",$data_record,$where_record);
					
					for($i=1;$i<=$total_floor;$i++)
					{
						if($_POST['product_id'.$i]!='')
						{
							$data_record_service['quotation_id'] =$record_id; 
							...
							$customer_service_id=$db->query_insert("product_quotation_map", $data_record_service);
					...
					$update_invoice="update `product_quotation_invoice` set `price`='".$price."',`total_cost`='".$total_cost."', `amount1`='".$amount1."',`payable_amount`='".$_POST['payable_amount']."',`remaining_amount`='".$_POST['remaining_amount']."', `paid_type`='".$payment_mode_id."',`bank_id`='".$bank_name."',`cheque_detail`='".$chaque_no."',`chaque_date`='".$sep_check_date."',`credit_card_no`='".$credit_card_no."',bank_ref_no='".$bank_ref_no."',`admin_id`='".$_SESSION['admin_id']."',`total_paid`='".$payable_amount."' where quotation_id='".$record_id."' order by invoice_id asc LIMIT 1";
					...
					$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('add_quotation','Edit','quotation edit','".$record_id."','".date('Y-m-d H:i:s')."','".$data_record['cm_id']."','".$_SESSION['admin_id']."')";
				...
				{
					//==============Update Invoice No.=====================
					$sel_inv="select ext_invoice_no from product_quotation where cm_id='".$data_record['cm_id']."' and ext_invoice_no IS NOT NULL order by ext_invoice_no desc limit 0,1";
					$ptr_inv=mysql_query($sel_inv);
					...
					$record_id=$db->query_insert("product_quotation", $data_record);
					...
					$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('add_quotation','Add','add quotation','".$record_id."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
					$query=mysql_query($insert);
					...
					//===============Update Receipt no================
					$sel_recpt="select receipt_no from product_quotation_invoice where cm_id='".$cm_id1."' and (receipt_no IS NOT NULL and receipt_no !='') order by receipt_no desc limit 0,1";
					...
					//------send notification on Sale voucher--------------------
						...
						$notification_args['on_action'] = 'product_quotation';
                    ...
                    setTimeout('document.location.href="manage_quotation.php";',500);
                    </script>
					<?php
				}
			}
    ...
    <form method="post" id="jqueryForm" name="jqueryForm" enctype="multipart/form-data" onSubmit="return validme()">
		<table border="0" cellspacing="15" cellpadding="0" width="100%">
           ...
           <?php 
		      ...
		      $rowSQL = mysql_query("SELECT MAX(quotation_id) as max FROM `product_quotation`" );
                	...
                	<table cellspacing="3" id="tbl" width="100%">
                    	<tr>
                   		...
                   		<tr>
							...
							if($record_id!='')
							{
            	       			$select_exc ="select * from product_quotation_map where quotation_id='".$record_id."' and product_id!='' order by map_id asc ";
	...
	<tr>
		...
		<td width="40%">--><input type="hidden" name="payable_amount" style="width:200px" id="payable_amount" value="<?php if($_POST['save_changes']) echo $_POST['payable_amount']; else echo $row_record['payable_amount']; ?>" onKeyUp="cal_remaining_amt();"/><!--</td>
	</tr> -->
	<!--<tr>
