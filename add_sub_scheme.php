...
if($_REQUEST['record_id'])
{
    ...
    $sql_record= "SELECT * FROM bp_sub_scheme where sub_scheme_id='".$record_id."'";
...
{
    $update_news="update bp_sub_scheme set photo='' where sub_scheme_id='".$record_id."'";
    //echo $update_events;
...
<title><?php if($record_id) echo "Edit"; else echo "Add";?> Sub Scheme</title>
  ...
  <tr>
    ...
    <td class="mid_mid">
        ...
        <?php
                        ...
                        {
                            $sub_scheme_name=$_POST['sub_scheme_name'];
                            $rules=$_POST['rules'];  
							$benifits=$_POST['benifits']; 
							$needed_documents=$_POST['needed_documents'];
                            ...
                            {
                                    $success=0;
                                    $errors[$i++]="Please Select Scheme";
                            }
							 if($sub_scheme_name=="")
                            {
                                    $success=0;
                                    $errors[$i++]="Entere Name";
                            }
							$uploaded_url="";
                            ...
                            {
                            $success=1;
							$data_record['sub_scheme_name'] =$sub_scheme_name;
							$data_record['description'] =($description);
							$data_record['benifits'] =$benifits;
							$data_record['rules'] =($rules);
							$data_record['needed_documents'] =$needed_documents;
							$data_record['scheme_id'] =$scheme_id;
                               ...
                               if($record_id)
                                {
                                   ///print_r($data_record); //    exit;
                                    $where_record=" sub_scheme_id='".$record_id."'";
                                    $db->query_update("sub_scheme", $data_record,$where_record);
                                    echo '<br></br><div id="msgbox" style="width:40%;">Record updated successfully</center></div><br></br>';
                                }
                                else
                                {
                                    $data_record['added_date'] =date("Y-m-d H:i:s");
                                    $record_id=$db->query_insert("sub_scheme", $data_record);
	...
	<table border="0" cellspacing="15" cellpadding="0" width="100%">
              <tr>
                ...
                <tr>
                  <td width="20%" valign="top">Select Scheme <span class="orange_font">*</span></td>
                <td width="70%">
                <select name="scheme_id"  class="input_select_login">
                <option value="0">Select Scheme</option>
                <?
                $select_schems="select * from bp_scheme";
				$ptr_schems=mysql_query($select_schems);
				while($val_records=mysql_fetch_array($ptr_schems))
				{
					$scheme_id=$val_records['scheme_id'];
					$scheme_name=$val_records['scheme_name'];
				
					if($_POST['scheme_id']==$schess_ids )
					echo  '<option value='.$scheme_id.' selected="selected">'.$scheme_name.' </option>';
					else
			echo  '<option value='.$scheme_id.'>'.$scheme_name.' </option>';
              ...
              <tr>
                  <td width="20%" valign="top">Sub Scheme Name<span class="orange_font">*</span></td>
                <td width="70%"><input type="text"  class="validate[required] input_text" name="sub_scheme_name" id="sub_scheme_name" 
                value="<?php if($_POST['save_changes']) echo $_POST['sub_scheme_name']; else echo $row_record['sub_scheme_name'];?>" /></td> 
                <td width="10%"></td>
              </tr>
                <tr>
                  ...
                  <td>
                      <?php
                                    ...
                                    if($_POST['save_changes'])
                                        $oFCKeditor->Value		= stripslashes($_POST['rules']);
                                    else
                                        $oFCKeditor->Value		= stripslashes($row_record['rules']);
              ...
              <tr>
                  ...
                  <td>
                      <?php
                                    include("../FCKeditor/fckeditor.php");
                                    $BasePath = "../FCKeditor/";
                                    $oFCKeditor 			= new FCKeditor('benifits') ;
                                    $oFCKeditor->BasePath	= $BasePath ;
                                    if($_POST['save_changes'])
                                        $oFCKeditor->Value		= stripslashes($_POST['benifits']);
                                    else
                                        $oFCKeditor->Value		= stripslashes($row_record['benifits']);
                                    //$oFCKeditor->ToolbarSet	= "MyToolBar";
                                    $oFCKeditor->Height		= "200";
                                    $oFCKeditor->Create() ;
                     ?>
             ...
             <tr>
                  <td valign="top">Needed Documents</td>
                  <td>
                      <?php
                                    include("../FCKeditor/fckeditor.php");
                                    $BasePath = "../FCKeditor/";
                                    $oFCKeditor 			= new FCKeditor('needed_documents') ;
                                    $oFCKeditor->BasePath	= $BasePath ;
                                    if($_POST['save_changes'])
                                        $oFCKeditor->Value		= stripslashes($_POST['needed_documents']);
                                    else
                                        $oFCKeditor->Value		= stripslashes($row_record['needed_documents']);
                                    //$oFCKeditor->ToolbarSet	= "MyToolBar";
                                    $oFCKeditor->Height		= "200";
                                    $oFCKeditor->Create() ;
                     ?>
             ...
             <tr>
                  <td valign="top">Table Description</td>
                  <td>
                      <?php
                                    include("../FCKeditor/fckeditor.php");
                                    $BasePath = "../FCKeditor/";
