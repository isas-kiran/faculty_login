...
<title>Manage Staff Service Management</title>
    ...
    <?php if($_POST['formAction'])

                    {
                        ...
                        if($_POST['formAction']=="delete")
                        {
							...
							echo 	"</br>".$month=$val[1];
							echo	"</br>".$year=$val[2];
                            echo    $sql_query= "SELECT staff_id,month,year FROM ".$GLOBALS["pre_db"]."pr_staff_service_incentive where staff_id ='".$del_record_id."' AND month='".$month."' AND year='".$year."'";
                                if(mysql_num_rows($db->query($sql_query)))
                                {
									"<br>".$sql_query= "SELECT * FROM pr_staff_service_incentive where staff_id ='".$del_record_id."' AND month='".$month."' AND year='".$year."'";              
                                    ...
                                    $delete_query="delete from ".$GLOBALS["pre_db"]."pr_staff_service_incentive where staff_id='".$del_record_id."' AND month='".$month."' AND year='".$year."'";
                    ...
                    if($_REQUEST['deleteRecord'] && $_REQUEST['record_id'])
                    {
                        ...
                        $sql_query= "SELECT staff_id,year,month FROM ".$GLOBALS["pre_db"]."pr_staff_service_incentive where staff_id='".$del_record_id."' AND month='".$_REQUEST['month']."' AND year='".$_REQUEST['year']."'";
                        if(mysql_num_rows($db->query($sql_query)))
                        {
							"<br>".$sql_query= "SELECT * FROM pr_staff_service_incentive where staff_id ='".$del_record_id."' AND month='".$_REQUEST['month']."' AND year='".$_REQUEST['year']."'";              
							$query=mysql_query($sql_query);
							$fetch=mysql_fetch_array($query);
								
							"<br>".$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('manage_user','Delete','".$fetch['name']."','".$del_record_id."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
							$query=mysql_query($insert);                        
                        	$delete_query="delete from ".$GLOBALS["pre_db"]."pr_staff_service_incentive where staff_id='".$del_record_id."' AND month='".$_REQUEST['month']."' AND year='".$_REQUEST['year']."'";
                        	$db->query($delete_query);
  ...
  <tr class="head_td">
    ...
    <?php
						...
						if($_GET['record_id'] !='')
						{
						 ...
						 $sql_query= "SELECT DISTINCT u.admin_id,u.name,o.staff_id,o.year,o.month FROM site_setting u, pr_staff_service_incentive o where ".$pre_keyword." AND o.staff_id=u.admin_id"; 
                       // $sql_query= "SELECT DISTINCT staff_id,year,month,grand_total FROM pr_staff_service_incentive"; 
    ...
    <td width="7%" class="centerAlign"><strong>Action</strong></td>
...

                            {
							   ...
							   $listed_record_id1=$val_query['staff_id'].'-'.$val_query['month'].'-'.$val_query['year']; 
                                $select_event= "select * from site_setting where admin_id = '".$val_query['staff_id']."' ";                                           
                                ...
                                */
                                ...
                                echo '<td >'.$month.'</td>';
									echo '<td align="center"><a href="staff_service_incentive.php?record_id='.$val_query['staff_id'].'&month='.$val_query['month'].'&year='.$val_query['year'].'"><img src="images/edit_icon.png" height="20px" width="20px" title="Edit Event" class="example-fade"/></a>&nbsp;
