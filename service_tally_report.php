...
<title>Service Tally Report</title>
...
?>   
    		...
    		<form method="get" name="search">
				<table border="0" cellspacing="0" cellpadding="0" width="100%" align="center">
              		<tr>
                        ...
                        <td><a href="service_tally_export.php<?php echo $sep_url_string; ?>"><img src="images/excel.png" height="30px" width="30px"/></a></td>  
            		</tr>          
        		</table>
        	</form>	
    	</td>
	...
	if($no_of_records)
	{
		...
		while($val_query=mysql_fetch_array($my_db))
		{
			...
			{
				$gst_vals='CGST '.$val_query['cgst_tax_in_percent'].' % &nbsp;&nbsp;&nbsp; SGST '.$val_query['sgst_tax_in_percent'].'%';
			...
			{
				$gst_vals='IGST '.$val_query['igst_tax_in_percent'].' %';
			...
			//echo '<br/> - '.$val_query['cgst_tax_in_percent'];
			//echo "<br/>".$val_query['gst_type'];
			echo '<td align="center">'.$cust_name.' - '.$cust_id.'</td>';
			echo '<td colspan="6"><table width="100%" class="table11"><tr>';
			echo '<td align="center" width="40%">Sales @ '.$gst_tot.'%</td>';
			...
			if($val_query['nonmemb_discount']>=100 && $val_query['nonmemb_discount_type']=='percentage')
			{
				...
				/*if($val_query['apply_gst']!='no')
				{
					$rate = number_format($val_query['service_price'] * 100/(100 + $gst_tot),3,'.','');
					$price = number_format($val_query['service_price'] * 100/(100 + $gst_tot),3,'.','');
				}*/
			}else if($val_query['apply_gst']!='no')
			{
				$disc = '0';
				//$rate=number_format($val_query['excluded_tax'],3,'.','');
			...
			echo '<td align="center" width="10%">'.$rate.'</td>';
			echo '<td align="center" width="10%">';
			echo $disc;					
			echo'</td>';
			echo "<td align='center' width='10%'>".$price."</td>";
			...
			if(trim($val_query['gst_type'])=="m_gst" && $val_query['apply_gst']!='no')
			{
				...
				echo '<td align="center">'.number_format($gst_amnts,3,'.','').'</td>';
				echo '</tr>';
				echo '<tr>';
			...
			{
				echo '<tr>';
				echo '<td align="center">Output IGST '.$val_query['igst_tax_in_percent'].' %</td>';
				echo '<td align="center"></td>';
				echo '<td align="center"></td>';
				echo '<td align="center"></td>';
				echo '<td align="center"></td>';
				$IGST_Amount = $price * $val_query['igst_tax_in_percent'] /(100 + $val_query['igst_tax_in_percent']);
				echo '<td align="center">'.number_format($IGST_Amount,3,'.','').'</td>';
				echo '</tr>';
			}
			else if($val_query['apply_gst']!='no')
			{
			...
			echo '<td align="center">'.number_format($total_amount,3,'.','').'</td>';
			echo '</tr>';
			echo '</table></td>';
			...
			echo "</td>";*/
			$totals_tds +=number_format($total_amount,3,'.','');
