   	...
   	<tr>
    	...
    	<td class="mid_mid" align="center">
    				...
    				<td colspan="17">
        				<form method="get" name="search">
							<table border="0" cellspacing="0" cellpadding="0" width="99%" align="center">
                                ...
                                <td width="10%">
                                <input type="text" name="start_date" class="input_text datepicker" placeholder="From Date" readonly="1" id="start_date" title="From Date" value="<?php if($_REQUEST['start_date']) echo $_REQUEST['start_date']; else echo $enquiry_from_date=date('d/m/Y',strtotime('-180 days')) ?>">
                                </td>
            ...
            else
            {
                ...
                if($_REQUEST['to_date']=='')
                {
                    $enquiry_from_date=" and DATE(`added_date`) >='".date('Y-m-d',strtotime('-180 days'))."'"; //date('Y-m-d',strtotime('-1 days'))
					$enquiry_from_date_i=" and DATE(`i.added_date`) >='".date('Y-m-d',strtotime('-180 days'))."'"; //date('Y-m-d',strtotime('-1 days'))
                    $followup_from_date=" and DATE(`followup_date`) >='".date('Y-m-d',strtotime('-180 days'))."'";//date('Y-m-d',strtotime('-1 days'))
            ...
            $sql_query="select * from enrollment where 1 and enroll_id NOT IN (select enroll_id from student_course_batch_map) ".$enquiry_from_date." ".$enquiry_to_date." ".$search_cm_id." ".$_SESSION['where']." order by enroll_id asc ";
            ...
            if($no_of_records)
            {
               ...
               // $pager = new PS_Pagination($sql_query,$_SESSION['show_records'],$_SESSION['show_records_pages'],$query_string1);
                ...
                <td width="7%" align="center"><strong>Enroll Date</strong></td>
                <td width="7%" align="center"><strong>Course Start Date</strong></td>
                <td width="7%" align="center"><strong>Councillor Name</strong></td>
                ...
                while($val_query=mysql_fetch_array($all_records))
                {
                    ...
                    $data_course=mysql_fetch_array($query_course);
					...
					//------------------------------------------------------------------
					$select_admin="select name, branch_name from site_setting where 1 and admin_id='".$val_query['assigned_to']."'";
                    $query_admin=mysql_query($select_admin);
                    $data_admin=mysql_fetch_array($query_admin);								
                    echo '<td align="center">'.$data_admin['name'].'</td>';
					//--------------------------------------------------------------------
					echo '<td align="center">'.$data_admin['branch_name'].'</td>';
					//--------------------------------------------------------------------
					echo '</tr>';
                    $i++;
                }
