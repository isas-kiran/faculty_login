...
function validme()
{
	 ...
	 if(frm.stockiest.value=='')
	 {
		 disp_error +='Select Stockiest Name\n';
		 document.getElementById('stockiest').style.border = '1px solid #f00';
		 frm.stockiest.focus();
		 error='yes';
	 }
	 ...
	 if(frm.kit_qty.value=='')
	 {
		 disp_error +='Enter Kit Quantity\n';
		 document.getElementById('kit_qty').style.border = '1px solid #f00';
		 frm.kit_qty.focus();
		 error='yes';
	 }
	 
	 total_kit=document.getElementById("no_of_floor").value;
	 for(i=1; i<=total_kit;i++)
	 {
		id="ava_product_qty"+i;
		i_qty="sin_product_qty"+i;
		var ava_qty =parseInt( document.getElementById(id).value);
		var issue_qty=parseInt( document.getElementById(i_qty).value);
		if(ava_qty < issue_qty)
		{
			disp_error +='Available quantity should be equal or greater than issue quantity \n';
			document.getElementById('ava_product_qty'+i).style.border = '1px solid #f00';
			error='yes';
		}
	 }
...
function show_data(id)
{
	...
	var data1="action=show_data&action_page=kit_product&id="+id+'&record_id='+record_id+'&branch_name='+branch_name;
...
function show_product_desc(ids)
{
	...
	$.ajax({
		...
		success: function (html)
		{
			...
			document.getElementById("ava_product_qty"+ids).value=quantity;
		}
	});
}
</script> 
    ...
    <td class="mid_mid">
        ...
        <?php
			...
			if($_POST['save_changes'])
			{
				...
				$kit_qty=( ($_POST['kit_qty'])) ? $_POST['kit_qty'] : "0";
				...
				else
				{
					...
					{*/
						...
						$record_id=$db->query_insert("product_kit_sale", $data_record);
						for($i=1;$i<=$total_floor;$i++)
						{
							$data_record_service['kit_sale_id'] =$record_id; 
							...
							$customer_service_id=$db->query_insert("product_kit_sale_map", $data_record_service);
							...
							$up_prod_qty="update product set quantity='".$total_quantity."' where product_id='".$_POST['product_id'.$i]."'";
							$query_prod_qty=mysql_query($up_prod_qty); 
						}
						
						$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('sale_product_kit','Add','Sale kit','".$record_id."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
						$query=mysql_query($insert);
            ...
            <tr>
            	<td>Enter Kit Quantity <span class="orange_font">*</span></td>
                <td><input type="text" name="kit_qty" id="kit_qty" class="input_text" style="width:200px" value="<?php if($_POST['kit_qty']) echo $_POST['kit_qty']; else echo 1; ?>" onblur="cal_kit_qty(this.value)"/></td>
            </tr>
			<?php 
            ...
            <tr>
                ...
                <table  width="100%" style="border:1px solid gray; ">
					...
					function floors(idss)
					{
						...
						var shows_data='<div id="floor_id'+idss+'"><table cellspacing="3" id="tbl'+idss+'" width="100%"><tr><td><div id="prod_list"></div></td></tr><tr><td width="3%" align="center"><input type="hidden" name="exclusive_id'+idss+'" id="exclusive_id'+idss+'" value="'+idss+'" /><select name="product_id'+idss+'" id="product_id'+idss+'" style="width:400px" onChange="show_product_desc('+idss+')"><option value="">Select Product</option>'+res+'</select></td><td width="9%" align="center"><input type="text" disabled="disabled" name="ava_product_qty'+idss+'" id="ava_product_qty'+idss+'" style=" width:100px" /></td><td width="6%" align="center"><input type="text" name="sin_product_qty'+idss+'" id="sin_product_qty'+idss+'" style=" width:100px" /></td><td valign="top" width="1%" align="center"><input type="hidden" name="total_product[]" id="total_product'+idss+'" /></td><input type="hidden" name="row_deleted'+idss+'" id="row_deleted'+idss+'" value="" /></tr><tr><td colspan="7"><div id="product_details'+idss+'"><table style="width:100%" align="center"><tr><td width="10%">Price : <span id="price_desc'+idss+'"></span></td><td width="10%">Brand : <span id="brand'+idss+'"></span></td><td width="10%">Unit : <span id="unit_desc'+idss+'"></span><span id="measure_desc'+idss+'"></span></td><td width="70%">Product Desc.: <span id="description_desc'+idss+'"></span></td></tr></table></div></td></tr></table></div>';
    				...
    				<tr>
                        <td colspan="7">
                        	<table cellspacing="3" id="tbl" width="100%">
                        		<tr>
                        			<td valign="top" width="19%" align="center">Product Name</td>
                                   	<td valign="top" width="5%"  align="center">Available Qty</td>
                        			<td valign="top" width="12%"  align="center">Issue Qty</td>
                            			...
                            			{ 
											...
											<table cellspacing="5" id="tbl<?php echo $t; ?>" width="100%">
												<tr>
                                                    ...
                                                    <?php
													$sql_qty="select quantity from product where product_id='".$data_exclusive['product_id']."' ";
													$ptr_qty=mysql_query($sql_qty);
													$data_qty= mysql_fetch_array($ptr_qty);
													?>
                                                    <td width="28%" align="center"><input disabled="disabled" type="text"name="ava_product_qty<?php echo $t;?>" id="ava_product_qty<?php echo $t;?>" style=" width:100px" value="<?php echo $data_qty['quantity'] ?>" /></td>
													<td width="18%" align="center"><input type="text"name="sin_product_qty<?php echo $t; ?>" id="sin_product_qty<?php echo $t; ?>" style=" width:100px" value="<?php echo $data_exclusive['sin_product_qty'] ?>" />
                                                    <input type="hidden"name="kit_prod_qty<?php echo $t; ?>" id="kit_prod_qty<?php echo $t; ?>" style=" width:100px" value="<?php echo $data_exclusive['sin_product_qty'] ?>" />
                                                    </td>
       ...
       <!--============================================================END TABLE=========================================-->
              <tr>
                  <td>&nbsp;</td>
                  <td><input type="submit" class="input_btn" value="Checkout Kit" name="save_changes"  /></td>
                  <td></td>
              </tr>
        </table>
        </form>
...
<!--footer end-->
<script>
function cal_kit_qty(qty)
{
	 var msg='';
	 total_kits=document.getElementById("no_of_floor").value;
	 for(i=1; i<=total_kits;i++)
	 {
		//id="ava_product_qty"+i;
		i_qty="sin_product_qty"+i;
		i_kit_qty="kit_prod_qty"+i;
		ms="no";
		if(document.getElementById(i_qty).value > 0)
		{
			tot_qty=parseInt(Number(document.getElementById(i_kit_qty).value) * qty ); 
			document.getElementById(i_qty).value = tot_qty;
		}
		else
		{
			//var msg +='Please set kit quantity 1 or more in kit configuration \n';
			document.getElementById('ava_product_qty'+i).style.border = '1px solid #f00';
			ms="yes";
		}
		/*if(ms=="yes")
