...
$total_found =0;	
	...
	$sel_for_Num_rows="select i.*, e.ext_invoice_no,e.type,e.customer_id from sales_product_invoice i, sales_product e where 1 and i.sales_product_id=e.sales_product_id and i.status='Paid' ".$search_cm_id." ".$pre_keyword_i." ".$pay_type." ".$bank_ids." ".$paid_from_date." ".$paid_to_date." ".$select_directory." "; //and amount>0
	//and amount>0
...
$objPHPExcel->setActiveSheetIndex(0);
 ...
 $objPHPExcel->setActiveSheetIndex()->setCellValueByColumnAndRow('3','3', 'Mo No','3');
  ...
  $objPHPExcel->setActiveSheetIndex()->setCellValueByColumnAndRow('13','3', 'Status','3');*/
	...
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('F1', 'Student Name','6');
	...
	$sql_query= "select i.*, e.ext_invoice_no,e.type,e.customer_id from sales_product_invoice i, sales_product e where 1 and i.sales_product_id=e.sales_product_id and i.status='Paid' ".$search_cm_id." ".$pre_keyword_i." ".$pay_type." ".$bank_ids." ".$paid_from_date." ".$paid_to_date." ".$select_directory." "; //and amount>0
	...
	{		
		...
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('C'.$rowCount, 'Receipt');
		...
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('F'.$rowCount, $cust_name.' - '.$cust_id);		
		
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('G'.$rowCount, round($val_query['payable_amount'],2));
		//'Product CGST  '.$val_query['cgst_tax_in_per'].'%   SGST  '. $val_query['sgst_tax_in_per'].'%'
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('H'.$rowCount, 'Product Amount Collected');	
		if($val_query['cheque_detail']!='')
		{
			$chaque_detail=$val_query['cheque_detail'];
		}
		else
		{
			$chaque_detail="";	
	...
	header('Content-Disposition: attachment;filename="Product Receipt Tally Report.xls"');
