					...
					else
					{
						...
						if($record_id)
						{
							...
							while($data_times=mysql_fetch_array($ptr_timet_data))
							{
								...
								for($i=1;$i<=$totals;$i++)
								{
									if((in_array(date("l", $timestamp), $skipdays)) || (in_array(date("Y-m-d", $timestamp), $skipdates)) || in_array(date("Y-m-d", $timestamp),$skipbatchholiday) )
									{	
									...
									{
										// We need to add add_holiday table and add dates of holiday and take in timestamp and check in this condition so it will skip in when we change topic
										if((in_array(date("l", $timestamp), $skipdays)) || (in_array(date("Y-m-d", $timestamp), $skipdates)) || in_array(date("Y-m-d", $timestamp),$skipbatchholiday)  )
										{	
											
											echo "</br>holiday- ".$timestamp=strtotime("+ 1 day", $timestamp);
											//$dates[$i]=date("d/m/Y",strtotime("+ 1 day", $timestamp));
											//$timestamp = strtotime("+ 1 day", $timestamp);
											$totals++;
										}
										else if(in_array(date("Y-m-d", $timestamp),$skipholidays))
										{
											...
											echo "<br/>change day - ".$update="update batch_timetable set date='".date("Y-m-d", $timestamp)."' where day='".$change_day."' and c_b_id='".$c_b_id."'";
										...
										if((in_array(date("l", $timestamp), $skipdays)) || (in_array(date("Y-m-d", $timestamp), $skipdates)) || in_array(date("Y-m-d", $timestamp),$skipbatchholiday)  )
										{
											echo "</br>holiday1- ".$timestamp=strtotime("+ 1 day", $timestamp);
											//$dates[$i]=date("d/m/Y",strtotime("+ 1 day", $timestamp));
											//$timestamp = strtotime("+ 1 day", $timestamp);
											$totals++;
										}
										...
										{
											echo "<br/>next day- ".$data_record_timetable['date']=date("Y-m-d", $timestamp);
