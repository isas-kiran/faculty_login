...
<table cellspacing="0" cellpadding="0" class="table" width="95%">
  <tr class="head_td">
    ...
    <?php
                        ...
                        if($keyword)
						{                            
							$select_name = "select topic_name from topic where (topic_name like '%".addslashes($keyword)."%' or description like '%".addslashes($keyword)."%')";                                           
							if(mysql_num_rows($db->query($select_name)))
							{
								$val_name = $db->fetch_array($db->query($select_name));
								$name_to_array = "or topic_name like concat(concat('%','".addslashes($val_name['topic_name'])."','%')) ";
							}
							$subject = '';
							...
							$pre_keyword=" and (topic_name like '%".addslashes($keyword)."%' or  description like '%".addslashes($keyword)."%' $subject $name_to_array)";
						}
                        else
                            $pre_keyword="";
						...
						$sql_query= "SELECT topic_id,subject_id,topic_name,duration,description,model_required FROM topic where 1 ".$pre_keyword." ".$select_directory.""; 
                       //echo $sql_query;
  ...
  <?php
   ...
   ?>
    ...
    <td width="25%"><a href="<?php echo $_SERVER['PHP_SELF'];?>?order=<?php echo $order."&orderby=course_name".$query_string;?>" class="table_font"><strong>Topic Name</strong></a> <?php echo $img1;?></td>
    <td width="10%"><strong>Duration </strong></td>
    <td width="20%"><strong>Subject Linked</strong></td>
    <td width="20%"><strong>Description</strong></td>
	<td width="10%"><strong>Model required</strong></td>
	<?php
   	if($_SESSION['type'] =='S' || $_SESSION['type'] =='Z' || $_SESSION['type'] =='LD' )
    {	
   		?>
    	<td width="15%" class="centerAlign"><strong>Action</strong></td>
		<?php
	}
	?>
  </tr>
    ...
    while($val_query=mysql_fetch_array($all_records))
    {
        ...
        if($_SESSION['type'] =='S')
        {								
            echo '<td ><a href="add_topic.php?record_id='.$listed_record_id.'" class="table_font">'.stripslashes($val_query['topic_name']).'</a></td>';
        }
        else
        {
            echo '<td >'.stripslashes($val_query['topic_name']).'</td>';
        }
        
        echo '<td >'.$val_query['duration'].'</td>';
		
		$select_sub="select subject_id from topic_map where topic_id='".$listed_record_id."'";
		$ptr_sub = mysql_query($select_sub);
		$total_sub= mysql_num_rows($ptr_sub);
		$sub_names='';
		while($data_sub=mysql_fetch_array($ptr_sub))
		{
			$sele_sub_name="select name,subject_id from subject where 1 and subject_id ='".$data_sub['subject_id']."' order by subject_id asc";
			$ptr_sub_name=mysql_query($sele_sub_name);
			$row_sub_name= mysql_fetch_array($ptr_sub_name);
			
			$sub_names .=$row_sub_name['name'].', ';
		}
		echo '<td >'.$sub_names.'</td>';
        echo '<td >'.$val_query['description'].'</td>';
        echo '<td >'.$val_query['model_required'].'</td>';
        if( $_SESSION['type'] =='S' || $_SESSION['type'] =='Z' || $_SESSION['type'] =='LD')
        {
    ...
    echo '<img src="images/discount_off.png" title="'.$val_qusddery['discount'].' Discount. Valid Date start='.$val_qusddery['start_date'].' end ='.$val_qusddery['end_date'].'  " class="example-fade"/>&nbsp;&nbsp;';*/
            ...
            echo '<a href="add_topic.php?record_id='.$listed_record_id.'" ><img src="images/edit_icon.png" title="Edit Record" class="example-fade"/></a>&nbsp;&nbsp;<a onclick="return validationToDelete(\''.$_SERVER['PHP_SELF'].'\');" href="'.$_SERVER['PHP_SELF'].'?deleteRecord=1&record_id='.$listed_record_id.'&page='.$_REQUEST['page'].$query_string1.'"><img src="images/delete_icon.png" title="Delete Record" class="example-fade" /></a>&nbsp;&nbsp;';
            echo '</td>';
        }
        echo '</tr>';
        $bgColorCounter++;
