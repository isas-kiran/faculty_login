...
//==============Change Note==============
 ...
 /*Changes on 30-4-20 by kiran - Change in Discount Structure -as discount updated from database and automatically calculated as per course wise, discont is on One time and Installment is different discount rate*/
/*CHange in name field as making name in uppercase for certificate*/
...
{    
	...
	current_offer_type =frm.current_offer.value;
	if(frm.current_offer[0].checked == true) //Now Discount
	{
		//================================PAID TYPE==========================
		paid_type =frm.paid_type.value;
		now_disc_type =frm.now_disc.value;
	...
	else			//========Kit offer===============
	{
		...
		{
			courses_fees=roundNumber(document.getElementById('toal_fees').value,3);
			document.getElementById('off_course_only_fee').value=courses_fees;
		}
		...
		if(frm.now_disc[1].checked == true) //Now Discount
		{
			if(paid_type=='installments')
			{
				...
				var discount_inst=document.getElementById('off_discount_now_otp_inst').value;
				document.getElementById('concession').value=discount_inst;
				
				totals_fees = roundNumber(document.getElementById('off_course_only_fee').value,3);
				document.getElementById('toal_fees').value=totals_fees;
				document.getElementById("dropdown").disabled  = true;
				down_fees=roundNumber(document.getElementById('down_payment').value,3);
			...
			{
				var discount_inst=document.getElementById('off_discount_now_inst').value;
				document.getElementById('concession').value=discount_inst;
			...
			{
				var discount_otp=document.getElementById('off_discount_now_otp').value;
				document.getElementById('concession').value=discount_otp;
		...
		else //Normal Discount
		{
			if(paid_type=='installments')
			{
				...
				document.getElementById("dropdown").disabled  = false;
				down_fees=document.getElementById('down_payment').value;*/
				var discount_inst=document.getElementById('off_discount_otp_inst').value;
				document.getElementById('concession').value=discount_inst;
			...
			{
				var discount_inst=document.getElementById('off_discount_inst').value;
				document.getElementById('concession').value=discount_inst;
			...
			{
				var discount_otp=document.getElementById('off_discount_otp').value;
				document.getElementById('concession').value=discount_otp;
    ...
    <td class="mid_mid">
		...
		else
		{
			...
			if($record_id)
			{
				...
				//==============Update Invoice No.=====================
				$sel_inv="select ext_invoice_no from enrollment where cm_id='".$data_record['cm_id']."' and ext_invoice_no IS NOT NULL order by ext_invoice_no desc limit 0,1";
				...
				$update_enroll_id=" update enrollment set installment_display_id='".$inst_student_id."', 	upgrade_status='no' where enroll_id='".$courses_id."' ";
				...
				<table border="0" cellspacing="15" cellpadding="0" width="100%">		
                    ...
                    <tr>
                        ...
                        <td>
                            <div id="show_cities">
                                ...
                                echo '<table width="100%"><tr><td>';
                                echo '<select id="city" name="city" onchange="select_area(this.value)" style="width:460px"><option value="">Select City</option>';
                    ...
                    <tr>
                        ...
                        <td>
                            <div id="show_area">
                                ...
                                $where_area_id='';
                                if($row_record['city_id']!='' && $row_record['state_id']!='')
                                {
                                    $where_area_id=" and city_id='".$row_record['city_id']."' and state_id='".$row_record['state_id']."'";
                                }
                                $sel_city = "select * from area where 1 ".$where_area_id."" ;
                                $ptr_city = mysql_query($sel_city);
                                echo '<table width="100%"><tr><td>';
                                echo '<select id="area" name="area" style="width:460px"><option value="">Select Area</option>';
                                while($city_data= mysql_fetch_array($ptr_city))
                                {
                                    $sel='';
                                    if($row_record['area_id']==$city_data['area_id'])
                                    {
                                        $sel='selected="selected"';
                                    }
                                    echo '<option value="'.$city_data['area_id'].'" '.$sel.'>'.$city_data['area_name'].'</option>';
                                }
                                echo '</select></td></tr></table>';
                    ...
                    <tr>
                        ...
                        <tr>
                            ...
                            <input type="hidden" name="reg_toal_fees" id="reg_toal_fees" value="" />
                            <input type="hidden" name="off_toal_fees" id="off_toal_fees" value="" />
                            <input type="hidden" name="off_discount_otp" id="off_discount_otp" value="" />
                            <input type="hidden" name="off_discount_otp_inst" id="off_discount_otp_inst" value="" />
                            <input type="hidden" name="off_discount_inst" id="off_discount_inst" value="" />
                            <input type="hidden" name="off_discount_now_otp" id="off_discount_now_otp" value="" />
                            <input type="hidden" name="off_discount_now_otp_inst" id="off_discount_now_otp_inst" value="" />
                            <input type="hidden" name="off_discount_now_inst" id="off_discount_now_inst" value="" />
                            </td>
                        </tr>
                        <tr>
                            <td width="22%">Current Discount</td>
                            <td width="38%">
                            <input type="radio" name='now_disc' id="normal_disc" <?php if($_POST['now_disc'] =="normal_disc") echo 'checked="checked"'; else echo 'checked="checked"'; ?> value="normal_disc" onChange="show_now_discount()" />Normal Discount<input type="radio" name='now_disc' id="now_disc" <?php if($_POST['now_disc'] =="now_disc")  echo 'checked="checked"'; ?> value="now_disc" onChange="show_now_discount()" />Now Discount
                            </td>                
                            <td width="40%"><p id="countdown" style="color:#F00;font-size:15px;font-weight:600"></p></td>
                        </tr>
                        <tr>
                            <td width="22%">Paid Type </td>
                            <td width="38%"><input type="radio" checked="checked" name='paid_type' id="paid_type" value="one_time" <?php if($_POST['paid_type'] =="one_time")  echo 'checked="checked"'; else if($row_record['paid_type'] == "one_time") echo 'checked="checked"'; ?> onChange="show_record()" />One Time <input type="radio" name='paid_type' id="paid_type" value="installments" <?php if($_POST['paid_type'] =="installments")  echo 'checked="checked"'; else if($row_record['paid_type'] == "installments") echo 'checked="checked"'; ?> onChange="show_record()" />OTP with Installment <!--<span id="inst_tax_id"><?php //if($_SESSION['type']!='S' && $_SESSION['type']!='Z' && $_SESSION['type']!='LD' ){ echo $_SESSION['installment_tax'];} ?></span>% add-->
            ...
            <input type="radio" name='paid_type' id="paid_type" value="installments_zero"  <?php if($_POST['paid_type'] =="installments_zero")  echo 'checked="checked"'; else if($row_record['paid_type'] == "installments_zero") echo 'checked="checked"';  ?> onChange="show_record()" />Installment 0% add</td>             
                        ...
                        if($_SESSION['tax_type']=='GST')
                        {
                            ...
                            <tr>      
                              ...
                              <td><input type="text" class="validate[required] input_text" readonly="readonly" name="cgst_tax" id="cgst_tax"  value="<?php if($_POST['cgst_tax']) echo $_POST['cgst_tax']; ?>" /></td>
                            </tr>
                            <tr>      
                                  ...
                                  <td><input type="text" class="validate[required] input_text" readonly="readonly" name="sgst_tax" id="sgst_tax"  value="<?php if($_POST['sgst_tax']) echo $_POST['sgst_tax']; ?>" /></td>
                            </tr>
							<?php
						}
						else
						{
                            ...
                            <tr>      
                                ...
                                <td><input type="text" class="validate[required] input_text" name="service_tax" id="service_tax"  value="<?php if($_POST['service_tax']) echo $_POST['service_tax']; ?>" /></td>
                            </tr>
                            <?php
						}
						?>	
                        ...
                        ?>
                        <!--<tr>      
                              <td width="20%" class="heading">CGST <span id="down_cgst_id"> on down payment<?php //if($_SESSION['type']!='S' && $_SESSION['type']!='Z' && $_SESSION['type']!='LD' ){ echo $_SESSION['cgst'];} ?></span>%<span class="orange_font">*</span></td><input type="hidden" id="down_cgst_taxes" name="down_cgst_taxes" value="<?php //if($_SESSION['type']!='S' && $_SESSION['type']!='Z' && $_SESSION['type']!='LD' ){ echo $_SESSION['cgst'];} ?>"  />
                              <td><input type="text" class="validate[required] input_text" readonly="readonly" name="down_cgst_tax" id="down_cgst_tax"  value="<?php //if($_POST['down_cgst_tax']) echo $_POST['down_cgst_tax']; else echo $row_record['cgst_tax'];?>" /></td>
                        </tr>
                        <tr>      
                              <td width="20%" class="heading">SGST <span id="down_sgst_id"> on down payment<?php //if($_SESSION['type']!='S' && $_SESSION['type']!='Z' && $_SESSION['type']!='LD' ){ echo $_SESSION['sgst'];} ?></span>%<span class="orange_font">*</span></td><input type="hidden" id="down_sgst_taxes" name="down_sgst_taxes" value="<?php //if($_SESSION['type']!='S' && $_SESSION['type']!='Z' && $_SESSION['type']!='LD' ){ echo $_SESSION['sgst'];} ?>"  />
                              <td><input type="text" class="validate[required] input_text" readonly="readonly" name="down_sgst_tax" id="down_sgst_tax"  value="<?php //if($_POST['down_sgst_tax']) echo $_POST['down_sgst_tax']; else echo $row_record['sgst_tax'];?>" /></td>
                        </tr>-->
                        <!--<tr>
                        ...
                        <tr>
                              <td width="20%" class="heading">Customer <?php if($_SESSION['tax_type']=='GST') echo 'GST'; else echo 'VAT'; ?>(if availale)<span class="orange_font">*</span></td>
                ...
                <input type="hidden" name="off_course_only_fee" id="off_course_only_fee" value="" />
                <table>
                    <tr>
                        <td align="right">
            ...
            <form method="post" id="jqueryForm" enctype="multipart/form-data">
                <table border="0" cellspacing="15" cellpadding="0" width="100%">
                    ...
                    <tr>
                        <td></td>
                        <td>
                        	<input type="button" class="inputButton custom_cuorse_submit" value="Submit" name="submit"/>&nbsp;
