    						...
    						if($_POST['save_changes'])
                        	{
								...
								if($my_query=mysql_num_rows($ptr_enroll))
								{
									...
									for($i=1;$i<=$total_topics;$i++)
									{
										...
										if($_POST['position'.$i] !='' )
										{
										 ...
										 "<br />". $update_query="update logsheet set faculty_sign='".$_POST['action_'.$i]."',student_sign= '".$_POST['action1_'.$i]."',faculty_sign_date='".$faculty_date."',student_sign_date='".$student_date."',repeated='".$_POST['repeated_'.$i]."' ,selected_topic_id = '".$_POST['chkRecords'.$i]."',stud_status='".$_POST['stud_status'.$i]."',repeat_date='".$repeat_date."',position='".$position."' where enroll_id='".$record_id."'  and topic_id='".$_POST['topic_id_'.$i]."'";
								...
								else
								{
									...
									for($i=1;$i<=$total_topics;$i++)
									{
										...
										"<br />".$insert_logsheet="insert into logsheet (`enroll_id`,`course_id`,`topic_id`,`selected_topic_id`,`faculty_sign` ,`faculty_sign_date` ,`student_sign` ,`student_sign_date`,`repeated`,`added_date`,`stud_status`,`repeat_date`,`position`) 
										values('$record_id','$course_id','".$_POST['topic_id_'.$i]."','".$_POST['chkRecords'.$i]."','".$_POST['action_'.$i]."','".$faculty_date."','".$_POST['action1_'.$i]."','".$student_date."','".$_POST['repeated_'.$i]."','".date('Y-m-d H:i:s')."','".$_POST['stud_status'.$i]."','".$repeat_date."','".$position."')";
    ...
    <img src='images/print1.jpeg'
                                ...
                                <td width="6%" align="center" class="tr-header"><strong>Select Topic</strong><br /><!--<input type="checkbox" name="chkAll" onClick="Javascript:checkAll('frmTakeAction','chkRecords')"/>--></td>
                                    ...
                                    $val_record=mysql_fetch_array($all_records);
										...
										$sel_logsheet="select * from logsheet where enroll_id='".$record_id."' and topic_id='".$data_topic_id['topic_id']."' order by position asc";
										...
										if($data_logsheet['faculty_sign']=='completed')
										{
										 ...
										 <input type="hidden" name=" faculty_sign_date_'.$j.'" value="'.$data_logsheet['faculty_sign_date'].'">
										  <input type="text" name="faculty_sign_date_'.$j.'" disabled="disabled" value="'.$data_logsheet['faculty_sign_date'].'">';
										...
										<option value="5" <?php if($data_logsheet["repeated"] =="5") echo 'selected="selected"' ?>>5</option>
										</select>
                                        <?php
										echo'</td>';
                                        echo '</tr>';
