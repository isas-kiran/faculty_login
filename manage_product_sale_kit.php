...
<?php
$edit_access='';
$sel_acc="select * from edit_previleges where admin_id='".$_SESSION['admin_id']."' and privilege_id='357'";
	...
	if($_POST['formAction'])
	{
		if($_POST['formAction']=="delete")
		{
			for($r=0;$r<count($_POST['chkRecords']);$r++)
			{
				...
				$sql_query= "SELECT kit_sale_id FROM product_kit_sale where kit_sale_id ='".$del_record_id."'";
				if(mysql_num_rows($db->query($sql_query)))
				{
					$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('product_kit_sale','Delete','Sale kit delete','".$del_record_id."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
					...
					$delete_query="delete from product_kit_sale where kit_sale_id='".$del_record_id."'";
					$db->query($delete_query); 
					
					$delete_query1="delete from product_kit_sale_map where kit_sale_id='".$del_record_id."'";
    	...
    	<td class="mid_mid" align="center">
			<table cellspacing="0" cellpadding="0" class="table" width="95%"> 
                ...
                if($_GET['order'] !='' && ($_GET['orderby']=='inventory_id'))
                {
                    ...
                    $select_directory='order by kit_sale_id desc';  
                if($_GET['record_id'] !='')
                {
                    $record_cat_id="and kit_sale_id='".$_GET['record_id']."' ";
                }	
                /*if($pre_keyword=='')
                {                    
                ...
                $sql_query="select * from product_kit_sale where 1 ".$_SESSION['where']." ".$search_cm_id." ".$pre_keyword." ".$select_directory.""; //".$admin_ids."  25-12-17
                ...
                if($no_of_records)
                {
                    ...
                    {	
                        ?>
						<td width="2%" align="center"><input type="checkbox" name="chkAll" onClick="Javascript:checkAll('frmTakeAction','chkRecords')"/>
                        </td>
                        <?php 
                    }
                    ?>
                    ...
                    <td width="20%" align="center"><a href="<?php echo $_SERVER['PHP_SELF'];?>?order=<?php echo $order."&orderby=vendor".$query_string;?>"class="table_font"><strong>Customer Name</strong></a><?php echo $img1;?></td>
                    <td width="10%" align="center"><strong>Course Name</strong></td>
                    <td width="10%" align="center"><strong>Stockiest</strong></td>
                    <td width="10%" align="center"><strong>Kit Qty</strong></td>
                    ...
                    while($val_query=mysql_fetch_array($all_records))
                    {
						...
						$sel_custName="select cust_name from customer where cust_id='".$val_query['customer_id']."'";
                        $ptr_custName=mysql_query($sel_custName);
                        $data_custName=mysql_fetch_array($ptr_custName);
                        $customerName=$data_custName['cust_name'];
                        echo '<td align="center">'.$customerName.'</td>'; 
						...
						echo '<td align="center">'.$val_query['kit_qty'].'</td>';
                        
                        $sql_kit= "select kit_sale_map_id from product_kit_sale_map where kit_sale_id='".$val_query['kit_sale_id']."' ";
                        ...
                        echo '<a href="sale_product_kit.php?record_id='.$listed_record_id.'" ><img src="images/edit_icon.png" title="Edit Record" class="example-fade"/></a>&nbsp;&nbsp;';
