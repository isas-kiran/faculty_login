    ...
    <?php       if($_POST['formAction'])
                {
                	if($_POST['formAction']=="delete")
                    {
                    	for($r=0;$r<count($_POST['chkRecords']);$r++)
                        {
                            ...
                            $sql_query= "SELECT inventory_id FROM inventory where inventory_id ='".$del_record_id."'";
                            if(mysql_num_rows($db->query($sql_query)))
							{
								...
								$sel_inv="select * from inventory_product_map where inventory_id='".$del_record_id."'";
								...
								$delete_query="delete from inventory where inventory_id='".$del_record_id."'";
								$db->query($delete_query);   
								
								$delete_query1="delete from inventory_product_map where inventory_id='".$del_record_id."'";
								$db->query($delete_query1); 
								 
								$delete_query2="delete from inventory_tax_map where inventory_id='".$del_record_id."'";
								$db->query($delete_query2);
								 
								$delete_query3="delete from inventory_invoice where inventory_id='".$del_record_id."'";
...
-->            </tr>
    ...
    <?php
						...
						if($pre_keyword=='')
						{                    
						  $sql_query= "SELECT * FROM inventory where 1 ".$record_cat_id." ".$_SESSION['where']." ".$where_vendor." ".$search_cm_id." ".$from_date." ".$to_date." ".$pre_keyword." ".$select_directory."";//".$_SESSION['user_id']." 25-12-17
						}
						else
						{
							$cm_ids=='';
						   ...
						   $sql_query= "SELECT distinct(i.inventory_id) as inventory_id FROM inventory i, vendor v, inventory_product_map im, product p where 1  and (v.name like '%".$keyword."%' or p.product_name like '%".$keyword."%') and i.vendor_id=v.vendor_id and im.inventory_id=i.inventory_id and im.product_id=p.product_id".$cm_ids." ".$where_vendor_i." ".$from_date_i." ".$to_date_i." ".$search_cm_id_s." order by i.inventory_id desc"; //".$admin_ids."  25-12-17
						}
                        ...
                        if($no_of_records)
                        {
                            ...
                            {
								$sql_inventoery = " select * from inventory where inventory_id='".$val_querys['inventory_id']."' ";
								...
								$sel_product="select inventory_id,product_id,sin_product_qty from inventory_product_map where inventory_id ='".$val_query['inventory_id']."'";
								...
								$tot_paid_amount="SELECT SUM( payable_amount ) as payable_amount FROM `inventory_invoice` WHERE inventory_id=".$listed_record_id."";
								...
								echo '<td><center><a href="inventory_payment_details.php?record_id='.$listed_record_id.'">
								...
								if($_SESSION['type'] =='S' || $edit_access=='yes')
								{
									echo '<a href="add_inventory.php?record_id='.$listed_record_id.'" ><img src="images/edit_icon.png" title="Edit Record" class="example-fade"/></a>&nbsp;&nbsp';
								}
								echo '&nbsp;&nbsp; <a href="" onClick="window.open(\'invoice_inventory.php?record_id='.$listed_record_id.'\', \'win1\',\'status=no,toolbar=no,scrollbars=yes,titlebar=no,menubar=no,resizable=yes,width=900,height=600,directories=no,location=no\'); return false;" ><img src="images/invoice.png" title="View Invoice" class="example-fade"/></a>';
								if($_SESSION['type'] =='S' || $edit_access=='yes')
								{
