...
if($_REQUEST['record_id'])
{
	...
	$sel_invoice="select paid_type,cheque_detail,bank_name,chaque_date,cust_bank_name,credit_card_no from invoice where enroll_id='".$row_record['enroll_id']."'";
...
$.post('coupon_check.php', { discount_coupon: jqueryForm.discount_coupon.value },
	...
	mail1=Array();
<?php
$sel_sms_cnt="select * from sms_mail_configuration_map where previlege_id='40'";
...
{								  
	...
	var users_mail=mail1;		
         ...
         data1='action=enroll&branch_name='+branch_name+'&admission_date='+admission_date+'&inst_student_id='+inst_student_id+'&name='+name+'&contact='+contact+'&mail='+mail+'&qualification='+qualification+'&username='+username+'&pass='+pass+'&photo='+photo+'&address='+address+'&per_address='+per_address+'&dob='+dob+'&source='+source+'&admission_remark='+admission_remark+'&course_id='+course_id+'&paid_type='+paid_type+'&toal_fees='+toal_fees+'&concession='+concession+'&discount_coupon='+discount_coupon+'&net_fees='+net_fees+'&service_tax='+service_tax+'&fees='+fees+'&down_payment='+down_payment+'&down_payment_tax='+down_payment_tax+'&down_payment_wo_tax='+down_payment_wo_tax+'&installment_on='+installment_on+'&dropdown='+dropdown+'&cust_bank_name='+cust_bank_name+'&account_no='+account_no+'&chaque_no='+chaque_no+'&chaque_date='+chaque_date+'&credit_card_no='+credit_card_no+'&final_amt='+final_amt+'&total_amt='+total_amt+'&payment_type='+payment_type+"&users_mail="+users_mail+"&email_text_msg="+email_text_msg;
...
<script type="text/javascript">
function chang_installment(val) {
    ...
    <td class="mid_mid">
        ...
        <?php
...

            {
				...
				$service_taxes_in_per= $_POST['service_taxes'];
                    ...
                    if(count($errors)==0 && $_FILES['photo']["name"])
                    {
                        ...
                        else
                        {
						   ...
						   // $data_record['service_tax']=$service_tax;
							...
							//$data_record_invoice['service_tax']=$service_tax;
							//$data_record_invoice['service_taxes_in_percent']=$service_taxes_in_per;
							
							//$data_record_invoice['added_date'] = $added_date." 00:00:00";
                            ...
                            if($record_id)
                            {
                               /* $data_record['added_date'] = date('Y-m-d H:i:s');
								...
								if($row_record['no_of_installment']==$data_record['no_of_installment'])
								{
									...
									$inv_id="select invoice_id from invoice where enroll_id = '".$record_id."' order by invoice_id desc limit 0, 1";
									$ptr_inv_id=mysql_query($inv_id);
									$data_inv_id=mysql_fetch_array($ptr_inv_id);
									"<br/>1".$invoice_id=$data_inv_id['invoice_id'];
								}
								else
								{
									...
									$data_record_invoice['status']="Change Installment";
									$student_id_in=$db->query_insert("invoice", $data_record_invoice); 
									"<br/>0".$invoice_id=$student_id_in;
								}
								"<br/>".$delete_inst="DELETE FROM `installment` WHERE ".$where_record."  ";
								...
								"<br/>".$sel_invoice="select invoice_id from invoice where ".$where_record." ";
								...
								if($data_record['no_of_installment'] !=0)
							   	{
									for($i=1;$i<=$data_record['no_of_installment'];$i++)
									{
										...
										if($_POST['installments'][$i-1] !='')
										{
											...
											if($i=='1')
											{
												$inst_id=$installment_id;
											}
											if($_POST['installment_date'][$i-1])
											{
												$sep_date =  explode('/',$_POST['installment_date'][$i-1]);
												$installment_date = $sep_date[2].'-'.$sep_date[1].'-'.$sep_date[0];
											}
											"<br/>".$insert_query ="insert into installment(enroll_id,`installment_ref_id`, course_id, installment_amount, installment_date, status, invoice_id) values('".$record_id."','".$inst_id."','".$data_record['course_id']."','".$_POST['installments'][$i-1]."','".$installment_date."','not paid','".$invoice_id."' ) ";
											$insert_ptr = mysql_query($insert_query);
											
											$insert_query1 = "  insert into installment_history(enroll_id,`installment_ref_id`, course_id, installment_amount, installment_date, status, invoice_id) values('".$record_id."','".$inst_id."','".$data_record['course_id']."','".$_POST['installments'][$i-1]."','".$installment_date."','not paid','".$invoice_id."') ";
								...
								"<br>".$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('enrollment','Edit Installment','".$name."','".$record_id."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
                            ...
                            else
                            {
								...
								"<br>".$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('Installment Edit','Edit','".$name."','".$student_id."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
								$query=mysql_query($insert);  
								...
								/*------------send a mail to admin about this---------------------*/
											...
											$mail->msgHTML(file_get_contents('http://www.isasbeautyschool.com/faculty_login/invoice-generate.php?record_id='.$student_id_in), dirname(__FILE__));
													...
													$mesg ="Enrollment of ".$name." is done";
													...
													"<br/>".$sel_sms_cnt="select * from sms_mail_configuration_map where previlege_id='40'";
													$ptr_sel_sms=mysql_query($sel_sms_cnt);
													while($data_sel_cnt=mysql_fetch_array($ptr_sel_sms))
													{
              ...
              <td width="20%"class="orange_font">* Date Format should be [ DD/MM/YYYY ]</td>
                       ...
                       <?php }?>
                            ...
                            <tr>
	                            ...
	                            <td width="49%"><input type="text" id="admission_date" name="admission_date" class="input_text datepicker" value="<?php if($_POST['admission_date']) echo $_POST['admission_date']; else if($row_record['admission_date']!=''){$arrage_date= explode('-',$row_record['admission_date'],3); echo $arrage_date[2].'/'.$arrage_date[1].'/'.$arrage_date[0]; 
									}
                            ...
                            <tr>
	                            ...
	                            <td width="49%"><input type="text" name="inst_student_id" id="inst_student_id" class="input_text" value="<?php if($_POST['inst_student_id']) echo $_POST['inst_student_id']; elseif($record_id==''){ echo trim($comma_separated.'/'.$largestNumber) ;} else echo $row_record['installment_display_id']; ?>"  /> </td>
							</tr>            
                            <tr>
            				...
            				<tr>
                                        ...
                                        <tr>
                              			...
                              			<tr>
                                    		...
                                    		<td width="49%">
                                    		<input type="text" class="validate[required] input_text datepicker" name="dob" id="dob" value="<?php if($_POST['dob']) echo $_POST['dob']; else if($row_record['dob']!='')
											{
            							...
            							<tr>
                   							...
                   							<select id="source" name="source">
                        						...
                        						<?php 
													...
													{
													?>
													<option value = "<?php echo $data_src['source_id']?>" <? if($_POST['source'] == $data_src['source_id']) echo "selected"; else if ( $data_src['source_id'] == $row_record['source']) echo "selected";?> > <?php echo $data_src['source_name'] ?> </option>
												<?
													}
												?>
                          					</select>
            							...
            							<tr>
                    						...
                    						<select readOnly name="course_id" id="course_id" class="validate[required] input_select customized_select_box" onChange="show_course(this.value);" >  
                        				...
                        				<td colspan="3"><div id="custome_div" style="display:none">
                            				<table width="100%">
                            					<tr>
                                					...
                                					<td width="40%"><input disabled type="text" class="inputText" name="costomize_courses" id="costomize_courses" value="<?php if($_POST['costomize_courses']) echo $_POST['costomize_courses']; else echo $row_record['customised_course'];?>"/></td>
                                					<td width="20%">&nbsp;</td>
                              					</tr>
                          					</table>
                          				</div></td>
...
<input type="radio" disabled name='paid_type' id="paid_type" value="installments_zero"  <?php if($_POST['paid_type'] =="installments_zero")  echo 'checked="checked"'; else if($row_record['paid_type'] == "installments_zero") echo 'checked="checked"';  ?> onChange="show_record()" />Installment 0% add
       ...
       <!--==========================================================================================================================-->                                           
           <tr>
                ...
                <input type="text" class="input_text" disabled name="total_fees" id="toal_fees"  onblur="show_record(this.value,1)"  value="<?php if($_POST['total_fees']) echo $_POST['total_fees']; else echo $row_record['course_fees'];?>" />
          ...
          <tr>
                <td width="20%">Discount in <input type="radio" disabled name='discount_type' value="percent" <?php if($_POST['discount_type']=="percent") echo 'checked="checked"'; else if($row_record['discount_type']=="percent") echo 'checked="checked"'; else echo 'checked="checked"';?>  onChange="show_record()" />% or <input disabled type="radio" name='discount_type' value="cash" <?php if($_POST['discount_type']=="cash") echo 'checked="checked"'; else if($row_record['discount_type']=="cash") echo 'checked="checked"'; ?> onChange="show_record()" />Cash</td>
                ...
                <input type="text" disabled class="input_text" name="concession" id="concession" value="<?php if($_POST['concession']) echo $_POST['concession']; else if($row_record['discount'] !=0) echo $row_record['discount']; else echo 0; ?>" onBlur="show_record()"/></td>                
                <td width="31%"></td>
            </tr>
            <tr>
                <td width="20%">Discount Coupon Code</td>
                <td width="49%">
                <input type="text" disabled class="input_text" name="discount_coupon" id="discount_coupon" value="<?php if($_POST['discount_coupon']) echo $_POST['discount_coupon']; else echo $row_record['discount_coupon_code'];?>" onBlur="show_discount()" />
                </td>                
                <td width="31%"><div id="coupon"></div><input type="hidden" name="discount_coupon_price" id="discount_coupon_price" value="<?php if($_POST['discount_coupon_price']) echo $_POST['discount_coupon_price']; else echo $row_record['discount_coupon_price'];?>" /></td>
            </tr>
            <!--<tr>
   ...
   <!--============================================================================================================================================================-->                                         
                                            ...
                                            <tr>    
                                                  ...
                                                  <td><input type="text" disabled class="validate[required] input_text" readonly="readonly" name="net_fees" id="net_fees" value="<?php if($_POST['net_fees']) echo $_POST['net_fees']; else echo $row_record['net_fees'];?>" /></td>
                                            </tr>
                                            <tr>      
                                                  <td width="20%" disabled class="heading">Service Tax <span id="service_tax_id"><?php if($_SESSION['type']!='S'){ echo $_SESSION['service_tax'];} ?></span>%<span class="orange_font">*</span></td><input type="hidden" id="service_taxes" name="service_taxes" value="<?php if($_SESSION['type']!='S'){ echo $_SESSION['service_tax'];} ?>"  />
                                                  <td><input type="text" disabled class="validate[required] input_text" readonly="readonly" name="service_tax" id="service_tax"  value="<?php if($_POST['service_tax']) echo $_POST['service_tax']; else echo $row_record['service_tax'];?>" /></td>
                                            ...
                                            <tr>      
                                                  ...
                                                  <td><input type="text" disabled class="validate[required] input_text" readonly="readonly" name="fees" id="fees"  value="<?php if($_POST['fees']) echo $_POST['fees']; else echo $row_record['total_fees'];?>" /></td> 
                                             </tr>
                                            <tr>
                                                  <td width="20%" class="heading">Down Payment(Including tax)<span class="orange_font">*</span></td>
                                                  <td><input type="text" disabled class="validate[required] input_text" name="down_payment" id="down_payment" onKeyUp="show_record(this.value,1)" value="<?php if($_POST['down_payment']) echo $_POST['down_payment']; else if($row_record['down_payment']!=0) echo $row_record['down_payment']; else echo 0;?>" /></td>
                                            </tr> 
                                            <tr>
                                                  <td width="20%" class="heading">Service Tax in Down Payment<span class="orange_font">*</span></td>
                                                  <td><input type="text" disabled class="validate[required] input_text" readonly="readonly" name="down_payment_tax" id="down_payment_tax" onKeyUp="show_record()" value="<?php if($_POST['down_payment_tax']) echo $_POST['down_payment_tax']; else if($row_record['down_payment_tax']!=0) echo $row_record['down_payment_tax']; else echo 0;?>" /></td>

                                            </tr>
                                            <tr>

                                                  <td width="20%" class="heading">Down Payment(Without tax)<span class="orange_font">*</span></td>
                                                  <td><input type="text" disabled class="validate[required] input_text" readonly="readonly" name="down_payment_wo_tax" id="down_payment_wo_tax" onKeyUp="show_record()" value="<?php if($_POST['down_payment_wo_tax']) echo $_POST['down_payment_wo_tax']; else if($row_record['down_payment_wo_tax']!=0) echo $row_record['down_payment_wo_tax']; else echo 0;?>" /></td>
                                            </tr>
                                            <tr>
                                            ...
                                            <td colspan="2">
                                            	  <select name="numDep" id="dropdown" onchange="chang_installment(this.value)" >
                                                     <?php 
													 for ($i = 0; $i <= 24; $i++)
													 {
													 	$selc = '';
											...
											<tr>
                                                ...
                                                <?php if($record_id && $row_record['no_of_installment'] !=0)
												{
                                                    ...
                                                    <table width="100%">
                                                    <?php
													$selec_installemnt="select * from invoice where enroll_id='".$record_id."' and course_id='".$row_record['course_id']."' and amount>0 order by invoice_id asc";
													...
													while($dat_installment = mysql_fetch_array($ptr_installment))
													{
														...
														?>
														<tr <?php echo $display; ?>><td width="50%" class="heading">Installment <?php echo $j-1; ?> </span></td><td width="41%" ><?php echo $dat_installment['amount'];  ?></td><td><?php echo $dat_installment['added_date'] ;?></td></tr>
														<?php 
														$j++;
											...
											<tr>
                                                ...
                                                <?php if($record_id && $row_record['no_of_installment'] !=0)
												{
													...
													while($dat_installment = mysql_fetch_array($ptr_installment))
													{
														...
														$inst_dates=$sep_da[2].'/'.$sep_da[1].'/'.$sep_da[0];
														?>
														<tr><td width="40%" class="heading">Installment <?php echo $i; ?> </span></td><td width="40%" colspan=2""><input type="text" class="input_text" name="installments[]" id="ins_price" value='<?php if($_POST['installments']) echo $_POST['installments']; else echo $dat_installment['installment_amount'];  ?>' /></td><td><input type="text" name="installment_date[]" class="datepicker" placeholder="installment date" id="date_class<?php echo $i; ?>" value="<?php if($_POST['installment_date']) echo $_POST['installment_date']; else echo $inst_dates ;?>"></td></tr>
														<?php 
														$i++;
										...
										<table>
                                           ...
                                           <table width="100%">
	                                           <tr>
		                                           <td width="19%" class="tr-header" align="">Customer Bank Name</td>
		                                           <td width="35%">
		                                           <input type="text" name="cust_bank_name" id="cust_bank_name" value="<?php if($_POST['cust_bank_name']) echo $_POST['cust_bank_name']; else echo $data_invoice['cust_bank_name']; ?>"/>
				                                   </td>
...
{
var insta=document.getElementById("dropdown").value;
chang_installment(insta);
}
</script>
