...
<?php
$message .= '
<table cellpadding="0" align="left" cellspacing="0" width="100%" border="1px">
 ';
...
<tr bgcolor="#AFD8E0">
  ...
  //===================Total receive balance===============================
	...
	$sel_total_inc2="select SUM(amount) as total_amt from receipt where added_date='".date('Y-m-d')."'";
...
//==============================================================================
 ...
 $message.=' <td align="left" colspan="10" style="color:#F00"><strong>Total Incomming</strong> :'.$total_todays_bal.'</td> <input type="hidden" name="total_incoming" value="'.$total_todays_bal.'"  />
  </tr>';
 /* $message.='<tr>
 <td colspan="10" >
 ...
 <td align="center" colspan="10">&nbsp;</td>
  ...
  $select_amnt1="select SUM(amount) as total from expense where and added_date='".date('Y-m-d')."'";
	$ptr_amt1=mysql_query($select_amnt1);
	$total_amount1=0;
	if(mysql_num_rows($ptr_amt1))
	{
...
$message.='  <td align="left" colspan="10" style="color:#F00"><strong>Total Outgoing</strong> :'.$total_amount1.'</td><input type="hidden" name="total_outgoing" value="'.$total_amount1.'"  />
  </tr>';					
 ...
 <tr style="background-color:#999;color: black">
    ...
    <td width="5%" align="center" style="border:1px solid #CCC"><strong>Sr. No.</strong></td>
    <td width="10%" align="center" style="border:1px solid #CCC"><strong>Expense Type</strong></td>
    <td width="20%" align="center" style="border:1px solid #CCC"><strong>Employee</strong></td>
    <td width="15%" align="center" style="border:1px solid #CCC"><strong>Payment Mode</strong></td>
    <td width="20%" align="center" style="border:1px solid #CCC"><strong>Vendor</strong></td>
 ...
 <tr style="background-color:#999;color: black">
	...
	<td width="15%" align="center" style="border:1px solid #CCC"><strong>Total Incomming</strong></td>
...
From receipt C left join
     invoice D on C.bank_id = D.bank_name where c.added_date = '".date('Y-m-d')."' and (c.bank_id !='') and (c.bank_id !='select')";
		...
		if($total_bank=mysql_num_rows($ptr_amnt1))
		{
			...
			$message.= '<tr>';
			$message.= '<td align="center" style="border:1px solid #CCC">'.$k.'</td> <input type="hidden" name="total_banks" value="'.$total_bank.'"  />';       
			$message.= '<td align="center" style="border:1px solid #CCC">'.$data_bank_name['bank_name'].'</td><input type="hidden" name="bank_id_'.$k.'" value="'.$data_bank_name['bank_name'].'"  />';
			$message.= '<td align="center" style="border:1px solid #CCC">'.$data_bank_name['account_no'].'</td><input type="hidden" name="account_no_'.$k.'" value="'.$data_bank_name['account_no'].'"  />';
			$message.= '<td align="center" style="border:1px solid #CCC">'.$total_todays_bal.'</td><input type="hidden" name="incoming_'.$k.'" value="'.$total_todays_bal.'"  />';
			$message.= '<td align="center" style="border:1px solid #CCC">'.$data_sel_expense['total_amt'].'</td><input type="hidden" name="outgoing_'.$k.'" value="'.$data_sel_expense['total_amt'].'"  />';
			$message.= '<td align="center" style="border:1px solid #CCC">'.$data_yesterday['todays_balance'].'</td> <input type="hidden" name="yesterday_bal_'.$k.'" value="'.$data_yesterday['todays_balance'].'"  />';
			$message.= '<td align="center" style="border:1px solid #CCC">'.$incomming.'</td> <input type="hidden" name="todays_bal_'.$k.'" value="'.$incomming.'"  />';
			$k++;
			}
		}
		else
...
$message.='  </table>
 ...
 <tr style="background-color:#999;color: black">
    ...
    <td width="8%" align="center" style="border:1px solid #CCC"><strong>Cash In Hand</strong></td>
				...
				$sel_inv="select SUM(amount) as total_amt from invoice where  DATE(`added_date`) = '".date('Y-m-d')."'";
				$ptr_amnt=mysql_query($sel_inv);
				$total=mysql_num_rows($ptr_amnt);
				$data_ptr_sel=mysql_fetch_array($ptr_amnt);
				
				$sel_receipt="select SUM(amount) as total_amt from receipt where  added_date='".date('Y-m-d')."'";
				$ptr_bank_id=mysql_query($sel_receipt);
				$data_sel_recipt=mysql_fetch_array($ptr_bank_id);
			 ...
			 $message.= '<tr>';
				$message.= '<td align="center" style="border:1px solid #CCC">'.$opening_cash1.'</td> <input type="hidden" name="opening_cash" value="'.$opening_cash1.'"  />';
				...
				$message.= '<td align="center" style="border:1px solid #CCC">Fees- '.$data_total_inc['total_amt'].'<br /><hr >Product- '.$data_total_inc2['total_amt'].'</td>';
				$message.= '<td align="center" style="border:1px solid #CCC">'.$data_expense_taken['total_amt'].'</td> <input type="hidden" name="total_cash_taken_from_bank" value="'.$data_expense_taken['total_amt'].'"  />';
				$message.= '<td align="center" style="border:1px solid #CCC">'.$data_cash_received_sir['total_amt'].'</td> <input type="hidden" name="cash_received_from_director" value="'.$data_cash_received_sir['total_amt'].'"  />';
				$message.= '<td align="center" style="border:1px solid #CCC">'.$data_total_expense['total_amt'].'</td> <input type="hidden" name="total_cash_expense" value="'.$data_total_expense['total_amt'].'"  />';
				$message.= '<td align="center" style="border:1px solid #CCC">'.$data_cash_from_sir['total_amt'].'</td> <input type="hidden" name="cash_given_to_director" value="'.$data_cash_from_sir['total_amt'].'"  />';
				...
				{
					$sel_cash_from_bank="select SUM(amount) as total_amt from receipt where bank_id ='".$data_dist_bank_id['bank_id']."' ";
				...
				$cash_in_hands=$opening_cash + $data_ptr_sel['total_amt'] +$data_sel_recipt['total_amt'] + $data_expense_taken['total_amt'] + $data_cash_received_sir['total_amt'] - $data_total_expense['total_amt'] - $data_cash_from_sir['total_amt'] - $data_cash_deposite_in_bank['total_amt'] + $data_cash_from_innocent['total_amt'];
				//==============================================================================
				$message.= $xx.'</td>';
				$message.= '<td align="center" style="border:1px solid #CCC">'.$data_cash_from_innocent['total_amt'].'</td> <input type="hidden" name="cash_received_from_innocent" value="'.$data_cash_from_innocent['total_amt'].'"  />';
				
				$message.= '<td align="center" style="border:1px solid #CCC">'.$cash_in_hands.'</td> <input type="hidden" name="cash_in_hand" value="'.$cash_in_hands.'"  />' ;
...
/* $message.='<tr>
 ...
 <td align="left" colspan="10" style="color:#F00"><strong>Todays Outgoing</strong> : '.$total_amount1.'</td>
  ...
  $message.='</table>';
						...
						$insert_dsr="insert into dsr(`total_incoming`, `total_outgoing`, `yesterday_bal`, `total_cash_fees`, `total_cash_product`, `total_cash_taken_from_bank`, `cash_received_from_director`, `total_cash_expense`, `cash_given_to_director`,`cash_received_from_innocent`,`cash_in_hand`,`added_date`) values ('".$total_todays_bal."','".$total_amount1."','".$opening_cash1."','".$data_ptr_sel['total_amt']."','".$data_sel_recipt['total_amt']."','".$data_expense_taken['total_amt']."','".$data_cash_received_sir['total_amt']."','".$data_total_expense['total_amt']."','".$data_cash_from_sir['total_amt']."','".$data_cash_from_innocent['total_amt']."','".$cash_in_hands."','".date('Y-m-d')."')";
	  ...
	  $sel_inv1="select DISTINCT(bank_name) from invoice where DATE(`added_date`) = '".date('Y-m-d')."' and (bank_name !='') and (bank_name !='select')";
		...
		if($total_bank=mysql_num_rows($ptr_amnt1))
		{
			...
			while($data_ptr_sel1=mysql_fetch_array($ptr_amnt1))
			{
			 ...
			 $insert_into_dsr_bank=" insert into dsr_bank_summery (`bank_id`,`account_no`,`incoming`,`outgoing`,`yesterdas_balance`,`todays_balance`,`added_date`) values('".$data_bank_name['bank_name']."','".$data_bank_name['account_no']."','".$total_todays_bal."','".$data_sel_expense['total_amt']."','".$data_yesterday['todays_balance']."','".$incomming."','".date('Y-m-d')."')";
