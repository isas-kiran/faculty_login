...
if($_REQUEST['cat_id'])
{
    $record_id=$_REQUEST['cat_id'];		
	$sql_point= "SELECT * FROM assets_category where asset_category_id='".$record_id."'";
...
<title><?php if($record_id) echo "Edit"; else echo "Add";?>Asset Edit</title>
  ...
  <tr>
    ...
    <td class="mid_mid">
		...
		{  
			...
			{
				$success=1;
				$data_record['name'] =$pcategory_name;							
				$data_record['added_date'] = date('Y-m-d H:i:s');
				if($record_id )
				{
					$where_record="asset_category_id='".$record_id."'";   
					$db->query_update("assets_category", $data_record,$where_record); 
					//=============================For Type1====================================
					...
					{ 
						if($_POST['sub_name'.$j] !='' || $_POST['del_floor'.$j]=='yes')
						{
							...
							{
								$delete_row="delete from assets_type where asset_type_id='".$_POST['floor_id'.$j]."' ";
								$ptr_delete = mysql_query($delete_row);
							}
						} 
						if($_POST['sub_name'.$j] !='' && $_POST['del_floor'.$j] !='yes')
						{ 
							$data_record_type1['category_id']=$record_id;  
							$data_record_type1['asset_type'] = addslashes(trim($_POST['sub_name'.$j])); 
							'<br/>type1_id=>'.$_POST['floor_id'.$j];
							if($_POST['floor_id'.$j]=='' && $data_record_type1['asset_type'] !='')
							{
								$type1_id=$db->query_insert("assets_type", $data_record_type1);
							}else
							{ 
								$where_record="asset_type_id='".$_POST['floor_id'.$j]."'";
								$type1_id= $_POST['floor_id'.$j];
								$db->query_update("assets_type", $data_record_type1,$where_record);
							}
							unset($data_record_type1);
						}
					}
					?><div id="statusChangesDiv" title="Record added"><center><br><p>Record Updated successfully</p></center></div>
						<script type="text/javascript">
                            $(document).ready(function() {
                            ...
                            setTimeout('document.location.href="manage_assets_category.php";',1000);
                        </script>
                        <?php
				}		
			}
		...
		if($success==0)
		{
            ...
            <tr>
            	<td width="10%">Enter Asset Category</td>.
               	<td width="50%"><input type="text" name="pcategory_name" id="pcategory_name" class="validate[required] input_text" value="<?php if($_POST['save_changes']) echo $_POST['pcategory_name']; else echo $row_record_point['name'];?>" style="width:500px"></td>
            </tr>
            <tr>
            	<td colspan="3">
                	<table  width="80%" style="border:0px solid gray; ">
						...
						<tr>-->
							...
							if($record_id)
							{
								$select_exc ="select * from assets_type where category_id='".$record_id."' order by asset_type_id asc ";
								$ptr_fs = mysql_query($select_exc);
								$t=1;
								$total_conditions= mysql_num_rows($ptr_fs);
								while($data_exclusive = mysql_fetch_array($ptr_fs))
								{ 
									$pcategory_id= $data_exclusive['asset_type_id'];
									?> 
									...
									<td valign="top" width="40%" align=""><input type='text' style="width:300px" name='sub_name<?php echo $t ?>' id='sub_name<?php echo $t ?>' value='<?php echo $data_exclusive['asset_type'] ?>'/></td>
									<input type="hidden" name="floor_id<?php echo $t; ?>" id="floor_id_<?php echo $t; ?>" value="<?php echo $data_exclusive['asset_type_id'] ?>" />
									<td width="9%"><input type="button" title="Delete Options(-)" onClick="delete_rec(<?php echo $t; ?>,'floor');" class="delBtn" name="del"><input type="hidden" name="del_floor<?php echo $t; ?>" id="del_floor<?php echo $t; ?>" value="" /></td>
									 </tr></table>
									 </div>
									<?php
									$t++;
