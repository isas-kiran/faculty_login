				...
				if((trim(strtolower($_POST['captcha'])) == $_SESSION['captcha']))
				{
					...
					if($_COOKIE[$cookiess_name]==$username || $row_site_setting['type']=="S")
					{
						if($row_site_setting['status']=='Inactive')
						{
							...
							{
								$select_prev_admin= "SELECT u.privilege_id,u.previlege_parent_id,u.admin_id,o.previlege_parent_id,o.privilege_id  FROM admin_previleges u, previleges o where 1 AND admin_id='".$_SESSION['admin_id']."' AND o.previlege_parent_id=u.previlege_parent_id ORDER BY o.seq_no DESC";
								$ptr_previlege = mysql_query($select_prev_admin);
								$array_previ =array();
								$array_previ_parent =array();
								$k=0;
								while($data_prevo = mysql_fetch_array($ptr_previlege))
								{
									...
									$select_prev= "SELECT u.privilege_id,u.previlege_parent_id,u.admin_id,o.previlege_parent_id,o.privilege_id  FROM admin_previleges u, previleges o where 1 AND admin_id='".$_SESSION['admin_id']."' AND    previlege_parent_id='".$data_prevo['previlege_parent_id']."' AND o.previlege_parent_id=u.previlege_parent_id ORDER BY o.seq_no DESC"; 
