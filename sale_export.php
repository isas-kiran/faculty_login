...
$total_found =0;	
 ...
 if($_REQUEST['keyword']!="Keyword")
  	...
  	$sel_for_Num_rows="SELECT distinct(sp.sales_product_id) as sales_product_id FROM sales_product sp, customer c,product p ,sales_product_map sm where 1  and (c.cust_name like '%".$keyword."%' or p.product_name like '%".$keyword."%') and sp.customer_id=c.cust_id and sm.sales_product_id=sp.sales_product_id and sm.product_id=p.product_id ".$cm_ids."  ".$from_date_sp." ".$to_date_sp." ".$search_cm_id_s." order by sp.sales_product_id desc "; 
...
$objPHPExcel->setActiveSheetIndex(0);
 ...
 $objPHPExcel->setActiveSheetIndex()->setCellValueByColumnAndRow('3','3', 'Mo No','3');
  ...
  $objPHPExcel->setActiveSheetIndex()->setCellValueByColumnAndRow('13','3', 'Status','3');*/
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('D4', 'Product Details','6');
	$objPHPExcel->getActiveSheet()->SetCellValue('E4', 'Total Product Price','6');
	$objPHPExcel->getActiveSheet()->SetCellValue('F4', 'Discount','6');
	$objPHPExcel->getActiveSheet()->SetCellValue('G4', 'Total Discounted Price','6');
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('J4', 'Payment Mode','6');
	...
	$sql_query= "SELECT distinct(sp.sales_product_id) as sales_product_id,sp.product_price as product_price,sp.discount as discount,sp.total_price as total_price,sp.remaining_amount as remaining_amount,sp.payment_mode_id as payment_mode_id,sp.added_date as added_date,sp.customer_id as customer_id,sp.type as type,sp.cm_id as cm_id FROM sales_product sp, customer c,product p ,sales_product_map sm where 1  and (c.cust_name like '%".$keyword."%' or p.product_name like '%".$keyword."%') and sp.customer_id=c.cust_id and sm.sales_product_id=sp.sales_product_id and sm.product_id=p.product_id ".$cm_ids."  ".$from_date_sp." ".$to_date_sp." ".$search_cm_id_s." order by sp.sales_product_id desc "; 
	...
	while($val_query=mysql_fetch_array($query_order))
	{
		...
		//==========================NEW AND INSTALLMENT===============================================================
		$sel_prod=  " select sales_product_id,product_id,product_qty from sales_product_map where sales_product_id ='".$val_query['sales_product_id']."'";
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('E'.$rowCount, $val_query['product_price']);
		$objPHPExcel->getActiveSheet()->SetCellValue('F'.$rowCount, $val_query['discount']);
		$objPHPExcel->getActiveSheet()->SetCellValue('G'.$rowCount, $val_query['total_price']);
		$tot_paid_amount="SELECT SUM( payable_amount ) as payable_amount FROM `sales_product_invoice` WHERE sales_product_id=".$val_query['sales_product_id']."";
		...
		$sel_name="select payment_mode, payment_mode_id from payment_mode where payment_mode_id='".$val_query['payment_mode_id']."' ";
		$ptr_name=mysql_query($sel_name);
		$data_name=mysql_fetch_array($ptr_name);
		
		$payment_mode=$data_name['payment_mode'];
		$objPHPExcel->getActiveSheet()->SetCellValue('J'.$rowCount, $payment_mode);
