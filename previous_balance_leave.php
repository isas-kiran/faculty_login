        		...
        		<?php
                    ...
                    if($_POST['save_changes'])
                    {
                        ...
                        else
                        {
							...
							$data_record['staff_id']=$data_att_id['attendence_id'];
                ...
                <tr>
                    ...
                    <td width="40%">
                        <select id="staff_id" name="staff_id" onChange="getmonthdays(this.value);" >
                            ...
                            if($record_id)
                            {
								$sel_staff="select admin_id,attendence_id,name from site_setting where 1 and system_status='Enabled' and attendence_id IS NOT NULL and cm_id='".$row_record['cm_id']."' order by attendence_id asc";	 
...
<div id="right_info">
	<table border="0" cellspacing="0" cellpadding="0" width="100%">
		<?php if($_POST['formAction'])
        {
            if($_POST['formAction']=="delete")
            {
                for($r=0;$r<count($_POST['chkRecords']);$r++)
                {
                    ...
                    $sql_query= "SELECT previous_leave_id FROM pr_previous_leave_management where previous_leave_id ='".$del_record_id."'";
        ...
        if($_REQUEST['deleteRecord'] && $_REQUEST['record_id'])
        {
            ...
            if(mysql_num_rows($db->query($sql_query)))
            {
                $sql_query= "SELECT name FROM pr_previous_leave_management where previous_leave_id ='".$del_record_id."' ";              
                $query=mysql_query($sql_query);
                $fetch=mysql_fetch_array($query);
                    
                $insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('previous_leaves','Delete','Previous Leaves','".$del_record_id."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
                $query=mysql_query($insert);                     
                $delete_query="delete from ".$GLOBALS["pre_db"]."pr_previous_leave_management where previous_leave_id='".$del_record_id."'";
                $db->query($delete_query);
                ?>
		...
		<tr>
            ...
            <td class="mid_mid" align="center">    
        		<table cellspacing="0" cellpadding="0" class="table" width="95%">
                    ...
                    if($keyword !='')
                    {                            
                       $pre_keyword="and ((s.name like '%".$keyword."%') || (p.year like '%".$keyword."%') || (p.month like '%".$keyword."%'))";
                    }                            
                    else
                        {$pre_keyword='';}
                    if($_REQUEST['page'])
                    ...
                    if($_SESSION['where']!='')
                    {
                        $where="and p.cm_id='".$_SESSION['cm_id']."'";
                    }
                    else
                    {
                        $where="";
                    ...
                    $sql_query= "SELECT p.* ,s.name FROM pr_previous_leave_management p, site_setting s where 1 and s.admin_id=p.employee_id ".$where." ".$pre_keyword." ORDER BY p.previous_leave_id DESC"; 
...
function getmonthdays(i)
{
	...
	$.ajax({ 
		...
		if(responseData==1)
		{
			alert("Record Allready Exist For This Selection");
...
<?php 
//if($_SESSION['type']!="S"  && $_SESSION['type']!='Z' && $_SESSION['type']!='LD' ) // commented on 25-3-21
//{ 
	if($record_id=='')
	{
	?>
