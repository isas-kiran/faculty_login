...
function select_me(ids)
{
	...
	$strs = explode('Firefox',$_SERVER['HTTP_USER_AGENT']);if($strs[1]){echo '<input name="split" type="hidden">';}else{ echo '<input type="hidden" name="split">';} 
	?>');
                    ...
                    <?php
                        ...
                        else
                        {
                            ...
                            if($record_id)
                            {
								...
								if($course_type=="parent")
								{
									for($i=0;$i<count($child_course_id);$i++)
									{
										...
										"<br/>".$update_course="update courses set parent_id='".$record_id."' where course_id='".$child_course_id[$i]."'";
										$update_courses = mysql_query($update_course);
									}
								}
								else
								...
								"<br>".$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('add_course','Edit','".$fetch['course_name']."','".$record_id."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
                            ...
                            else
                            {
								...
								"<br>".$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('add_course','Add','".$course_name."','".$courses_id."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
								$query=mysql_query($insert);
...
//                            $discount['for_edit']=$_POST['discount_course'];
	...
	<table border="0" cellspacing="15" cellpadding="0" width="100%">
						...
						<table width="100%">
							<tr>
								...
								while($row_member = mysql_fetch_array($query_tel))
								{
									...
									echo  "<input type='checkbox' name='requirment_id[]'  value='".$row_member['subject_id']."' id='requirment_id$i'  onClick='show_ques($i)' class='case' $checked /> ".$row_member['name']." ";
									echo  '</td>';
									if($i%7==0)
										echo  '<tr></tr>';  
									$i++;
								...
								{ 
									...
									if(in_array($row_member['subject_id'],$subject_array))
									{
										...
										while($row_contact = mysql_fetch_array($query_contact))
										{
											...
											echo  "<td id='qs".$row_contact['topic_id']."' ".$display."><input type='checkbox' name='chapter_id[]' onclick='select_me(\"qs".$row_contact['topic_id']."\")' value='".$row_contact['topic_id']."' id='chapter_id$i' class='case ".$row_contact['topic_id']."' style='vertical-align:top'/><input type='hidden' name='split'>".$row_contact['topic_id'].". ".$row_contact['topic_name']." ";
											echo "<input type='hidden' name='split'><input type='hidden' name='selected_subject' id='selected_subjects".$row_contact['topic_id']."' value='".$row_member['name']."' >";
							...
							//while($row_member = mysql_fetch_array($query_top))

							   {
								 ...
								 //  if($record_id)
								   ...
								   //echo  "<input type='checkbox' name='topic_id[]'  value='".$row_member['topic_id']."' id='topic_id$i' onClick='show_ques($i)' class='case' $checked /> ".$row_member['topic_name']." ";
            ...
            <? }?>
             ...
             <tr><td>Select Topics </td>

                <td>

                  <div id="topic_list" style='overflow:auto;' >Select Subject first....!
           ...
           <!-- <tr>
                ...
                <td width="12%">Discount course</td>
