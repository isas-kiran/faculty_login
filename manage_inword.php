    ...
    <?php       if($_POST['formAction'])
                    {
                        if($_POST['formAction']=="delete")
                        {
                            for($r=0;$r<count($_POST['chkRecords']);$r++)
                            {
                                ...
                                $sql_query= "SELECT inword_id FROM inword_inventory where inword_id ='".$del_record_id."'";
                                if(mysql_num_rows($db->query($sql_query)))
								{
									...
									$delete_query="delete from inword_inventory where inword_id='".$del_record_id."'";
									$db->query($delete_query);   
									
									 $delete_query1="delete from inword_product_map where inword_id='".$del_record_id."'";
...
-->            </tr>
    ...
    <?php
                        ...
                        if($_GET['order'] !='' && ($_GET['orderby']=='inventory_id'))
                        {
                            ...
                            $select_directory='order by inword_id desc';  
						if($_GET['record_id'] !='')
						{
							$record_cat_id="and inword_id='".$_GET['record_id']."' ";
						}	
						if($pre_keyword=='')
						{                    
						...
						else
						{
						    ...
						    $sql_query= "SELECT distinct(i.inword_id) as inword_id FROM inword_inventory i, vendor v, site_setting s, inword_product_map im, product p where 1  and (v.name like '%".$keyword."%' or p.product_name like '%".$keyword."%' or s.name like '%".$keyword."%') and i.vendor_id=v.vendor_id and im.inword_id=i.inword_id and im.product_id=p.product_id".$cm_ids." ".$from_date_i." ".$to_date_i." ".$search_cm_id_s." "; //".$admin_ids."  25-12-17
						}
                     ...
                     <input type="hidden" name="formAction" id="formAction" value=""/>
                      <tr class="grey_td" >
                        ...
                        <td width="10%"><a href="<?php echo $_SERVER['PHP_SELF'];?>?order=<?php echo $order."&orderby=vendor".$query_string;?>"class="table_font"><strong>Vender</strong></a><?php echo $img1;?></td>
						 <td width="20%" align="center"><strong>Stockiest</strong></td>
                        <td width="20%" align="center"><strong>Products</strong></td>
                       
                        <td width="5%" align="center"><strong>Added_date</strong></td>
                      ...
                      <style>
                            ...
                            while($val_querys=mysql_fetch_array($all_records))
                            {
								...
								$sql_inventoery = " select * from inword_inventory where inword_id='".$val_querys['inword_id']."' ";
                                ...
                                $listed_record_id=$val_query['inword_id']; 
							   ...
							   $sql_vendor = " select name, vendor_id from vendor where vendor_id='".$val_query['vendor_id']."' ";
                                ...
                                echo '<td >'.$data_vendor['name'].'</td>';
								 ...
								 $sql_vendor1 = " select name, admin_id from site_setting where admin_id='".$val_query['admin_id']."' ";
								$ptr_vendor1 = mysql_query($sql_vendor1);
								$data_vendor1 = mysql_fetch_array($ptr_vendor1);
									  
                                echo '<td >'.$data_vendor1['name'].'</td>';
								...
								echo '&nbsp;&nbsp; <br/><a href="" onClick="window.open(\'product_inword_invoice.php?record_id='.$listed_record_id.'\', \'win1\',\'status=no,toolbar=no,scrollbars=yes,titlebar=no,menubar=no,resizable=yes,width=900,height=600,directories=no,location=no\'); return false;" ><img src="images/invoice.png" title="View Invoice" class="example-fade"/></a>';
								if($_SESSION['type'] =='S')
									{
