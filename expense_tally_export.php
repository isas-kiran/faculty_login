...
$objPHPExcel->getProperties()->setDescription("Test document for Office 2007 XLSX, generated using PHP classes.");
	...
	{                            
		...
		$pre_keyword_i =" and (total_price like '%".$keyword."%')";
	}                            
	else
	{
		$pre_keyword="";
	...
	{
		$pay_type=" and paid_type='".$_REQUEST['pay_type']."'";
	}
	$bank_ids="";
	if($_REQUEST['bank_id'] !='')
	{
	...
	if($_GET['order'] !='' && ($_GET['orderby']=='firstname'))
	{
		...
		$select_directory='order by added_Date asc';
...
//$objPHPExcel->setActiveSheetIndex(0)->getStyle('J1:J'.$objPHPExcel->setActiveSheetIndex()->getNumberFormat())->setFormatCode('0.000'); 
//$objPHPExcel->setActiveSheetIndex(0)->getStyle('J1:J100')->setActiveSheetIndex()->getNumberFormat()->setFormatCode('0.000'); 
...
$objPHPExcel->setActiveSheetIndex(0);
 ...
 $objPHPExcel->setActiveSheetIndex()->setCellValueByColumnAndRow('3','3', 'Mo No','3');
  ...
  $objPHPExcel->setActiveSheetIndex()->setCellValueByColumnAndRow('13','3', 'Status','3');*/
	...
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('D1', 'Party','6');
	...
	{
		$total_amnts=0;
		$total_price=$val_query['total_price'];
		...
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('C'.$rowCount, 'Expense');
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('D'.$rowCount, $cust_name.' - '.$cust_id);
		//=====================PRODUCT INFORMATION=============
		$tot_price_gst=0;
		$tot_disc_price_gst=0;
		$gst_total='';
		$sel_prod="select SUM(i.cgst_tax) as cgst_total,SUM(i.discounted_price * i.product_qty) as total_val,SUM(i.base_prod_price * i.product_qty) as base_total, i.product_disc,SUM(i.discounted_price) as total_cgst_val, s.total_price, (i.cgst_tax_in_per + i.sgst_tax_in_per) as total_gst from expense_map i, expense s where 1 and i.expense_id='".$val_query['expense_id']."' and i.expense_id=s.expense_id and (i.cgst_tax_in_per >0 || i.sgst_tax_in_per >0) group by i.cgst_tax_in_per";
			
		$ptr_prod= mysql_query($sel_prod); //sin_product_base_price and i.sin_product_total>0 and s.total_cost > 0
		if($tot_prod_gst=mysql_num_rows($ptr_prod))
		{
			while($data_prod=mysql_fetch_array($ptr_prod))
			{
				...
				{
					if($val_query['discount_type']=='percentage')
					{
						$tot_disc=number_format((float)($data_prod['total_val'] * $val_query['discount'] ) / 100,3,'.','');
						$disc=$val_query['discount'];
					}
					else
					{
						$tot_disc=$val_query['discount'];
						$tot_disc_per_org=round(($val_query['discount'])*100 / $data_prod['total_val'],3);
						$tot_disc_per=round($tot_disc_per_org,3);
						$disc=$tot_disc_per;
					}
					$tot_disc_price_gst=$data_prod['total_val'] - $tot_disc;
				}
				if(($data_prod['total_val'] <=0 || $data_prod['total_val']='') &&( $data_prod['discount']>='100' && $data_prod['discount_type']=='percentage'))
				{
					$tot_price_gst=$data_prod['base_total'];
					$disc=100;
				}
				if($data_prod['total_gst']>0)
				{
					$gst_total='Purchase @ '.$data_prod['total_gst'].'%';
				}
				else
					$gst_total='Non GST Purchase';
					
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('A'.$rowCount, $date);
				//$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('K'.$rowCount, $sel_prod);	
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('B'.$rowCount, $val_query['ext_invoice_no']);
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('E'.$rowCount, $gst_total);	
				...
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('H'.$rowCount, round($tot_price_gst,3));
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('I'.$rowCount, $disc);
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('J'.$rowCount, round($tot_disc_price_gst,3));
				$rowCount++;
				$total_amnts =number_format((float)$tot_disc_price_gst,3,'.','');
				$k++;
			}
		}
		//============================= FOR IGST Prod Information============================
		$tot_price_igst=0;
		$tot_disc_price_igst=0;
		$gst_total='';
		$sel_prod="select SUM(i.igst_tax) as igst_total,SUM(i.discounted_price * i.product_qty) as total_val,SUM(i.base_prod_price * i.product_qty) as base_total, i.product_disc,SUM(i.discounted_price) as total_cgst_val, s.total_price, (i.igst_tax_in_per) as total_gst from expense_map i, expense s where 1 and i.expense_id='".$val_query['expense_id']."' and i.expense_id=s.expense_id and (i.igst_tax_in_per >0) group by i.igst_tax_in_per";
		
		$ptr_prod= mysql_query($sel_prod); //sin_product_base_price and i.sin_product_total>0 and s.total_cost > 0
		if($tot_prod_igst=mysql_num_rows($ptr_prod))
		{
			while($data_prod=mysql_fetch_array($ptr_prod))
			{
				$total_price=$data_prod['total_price'];
				...
				if($val_query['discount'] >0)
				{
					if($val_query['discount_type']=='percentage')
					{
						$tot_disc=number_format($data_prod['total_val'] * $val_query['discount']) / 100;
					}
					else
					{
						$tot_disc=$val_query['discount'];
					...
					$tot_disc=number_format($data_prod['total_val'] * $val_query['discount']) / 100;
					$tot_disc_price_igst=number_format($data_prod['total_val'] - $tot_disc,3) ;
					$disc=$val_query['discount'];
				}
				if(($data_prod['total_val'] <=0 || $data_prod['total_val']='') &&( $data_prod['discount']>='100' && $data_prod['discount_type']=='percentage'))
				{
					$tot_price_igst=round($data_prod['base_total'],3);
					$disc=100;
				}
				if($data_prod['total_gst']>0)
				{
				...
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('F'.$rowCount, 'Item');
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('G'.$rowCount, '1');
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('H'.$rowCount,$tot_price_igst);
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('I'.$rowCount,$disc);
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('J'.$rowCount,$tot_disc_price_igst);
				$rowCount++;
				$total_amnts =$tot_disc_price_igst;
				$k++;
			}
		}
		//============================= FOR GST Null Prod Information============================
		$tot_disc_price=0;
		$tot_price=0;
			
		$sel_prod="select SUM(i.igst_tax) as igst_total,SUM(i.discounted_price * i.product_qty) as total_val,SUM(i.base_prod_price * i.product_qty) as base_total, i.product_disc,SUM(i.discounted_price) as total_cgst_val, s.total_price, (i.igst_tax_in_per) as total_gst from expense_map i, expense s where 1 and i.expense_id='".$val_query['expense_id']."' and i.expense_id=s.expense_id and (i.cgst_tax_in_per <=0 and i.sgst_tax_in_per <=0 and  i.igst_tax_in_per <=0) group by i.igst_tax_in_per";
		$ptr_prod= mysql_query($sel_prod); //sin_product_base_price and i.sin_product_total>0 and s.total_cost > 0
		if($tot_prod_nullgst=mysql_num_rows($ptr_prod))
		{
			while($data_prod=mysql_fetch_array($ptr_prod))
			{
				$total_price=$data_prod['total_price'];
				...
				if($val_query['discount'] >0)
				{
					...
					$tot_disc=number_format($data_prod['total_val'] * $val_query['discount'] ) / 100;
					$tot_disc_price=round($data_prod['total_val'] - $tot_disc,3) ;
					$disc=$val_query['discount'];
				}
				if(($data_prod['total_val'] <=0 || $data_prod['total_val']='') &&( $data_prod['discount']>='100' && $data_prod['discount_type']=='percentage'))
				{
					$tot_price=round($data_prod['base_total'],3);
					$disc=100;
				}
				...
				//$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('K'.$rowCount, $sel_prod);
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('B'.$rowCount, $val_query['ext_invoice_no']);
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('E'.$rowCount, 'Non GST Purchase');	
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('F'.$rowCount, 'Item');
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('G'.$rowCount, '1');
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('H'.$rowCount, $tot_price);
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('I'.$rowCount, $disc);
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('J'.$rowCount, $tot_disc_price);
				$rowCount++;
				$total_amnts=round($tot_disc_price,3);
				$k++;
			}
		}
		//==========================For No Prod maping Prod Information=================================
		$tot_disc_price_map=0;
		$tot_price_map=0;
		
		//$tot_prod_gst."-".$tot_prod_igst."-".$tot_prod_nullgst;
		if($tot_prod_gst<=0 && $tot_prod_igst <=0 && $tot_prod_nullgst <=0 )
		{
			$sel_prod="select total_price,amount_wo_tax as total_val,discount from expense where 1 and expense_id='".$val_query['expense_id']."' ";
			$ptr_prod= mysql_query($sel_prod); //sin_product_base_price and i.sin_product_total>0 and s.total_cost > 0
			if($tot_prod_nullgst=mysql_num_rows($ptr_prod))
			{
				while($data_prod=mysql_fetch_array($ptr_prod))
				{
					$total_price=$data_prod['total_price'];
					$tot_disc_price_map=$data_prod['total_val'];
					$tot_price_map=$data_prod['total_val'];
					$disc=0;
					if($val_query['discount'] >0)
					{
						if($val_query['discount_type']=='percentage')
						...
						$tot_disc=number_format($data_prod['total_val'] * $val_query['discount'] ) / 100;
						$tot_disc_price_map=round($data_prod['total_val'] - $tot_disc,3) ;
						$disc=$val_query['discount'];
					}
					if(($data_prod['total_val'] <=0 || $data_prod['total_val']='') &&( $data_prod['discount']>='100' && $data_prod['discount_type']=='percentage'))
					{
						$tot_price_map=round($data_prod['amount_wo_tax'],3);
						$disc=100;
					}
					...
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('F'.$rowCount, 'Item');
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('G'.$rowCount, '1');
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('H'.$rowCount, $tot_price_map);
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('I'.$rowCount, $disc);
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('J'.$rowCount, $tot_disc_price_map);
					$rowCount++;
					$total_amnts =number_format((float)$tot_disc_price_map,3,'.','');
					$k++;
				}
			}	
		}
		...
		//=========================CGST=======================
		$tot_cgst=0;
		$sel_cgst_prod="select SUM(i.cgst_tax) as cgst_total,SUM(i.base_prod_price) as base_total,i.product_disc, SUM(i.discounted_price) as total_cgst_val,i.cgst_tax_in_per from expense_map i, expense s where i.expense_id='".$val_query['expense_id']."' and i.expense_id=s.expense_id and i.sales_product_price>0 group by i.cgst_tax_in_per";
		...
		if($tot_cgst_num=mysql_num_rows($ptr_cgst_prod))
		{
			while($data_cgst=mysql_fetch_array($ptr_cgst_prod))
			{
				if($data_cgst['cgst_tax_in_per']>0)
				{
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('A'.$rowCount, $date);
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('B'.$rowCount, $val_query['ext_invoice_no']);
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('E'.$rowCount, 'Input CGST '.$data_cgst['cgst_tax_in_per'].'%');	
					...
					$tot_cgst=($tot_disc_price_gst*$data_cgst['cgst_tax_in_per']/100);
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('J'.$rowCount, round($tot_cgst,3));
					$rowCount++;
					$total_amnts += number_format((float)$tot_cgst,3,'.','');
					$k++;
				}
			}
		}
		//========================SGST===========================
		$tot_sgst=0;
		$sel_sgst_prod="select SUM(i.sgst_tax) as sgst_total,SUM(i.base_prod_price) as base_total,i.product_disc, SUM(i.discounted_price) as total_sgst_val,i.sgst_tax_in_per from expense_map i, expense s where i.expense_id='".$val_query['expense_id']."' and i.expense_id=s.expense_id and i.sales_product_price>0 group by i.sgst_tax_in_per";
		...
		if($tot_sgst_num=mysql_num_rows($ptr_sgst_prod))
		{
			while($data_sgst=mysql_fetch_array($ptr_sgst_prod))
			{
				if($data_sgst['sgst_tax_in_per']>0)
				{
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('A'.$rowCount, $date);	
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('B'.$rowCount, $val_query['ext_invoice_no']);
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('E'.$rowCount, 'Input SGST '.$data_sgst['sgst_tax_in_per'].'%');	
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('F'.$rowCount, '');
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('G'.$rowCount, '');
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('H'.$rowCount, '');
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('I'.$rowCount, '');
					$tot_sgst=($tot_disc_price_gst*$data_sgst['sgst_tax_in_per']/100);
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('J'.$rowCount, round($tot_sgst,3));
					$rowCount++;
					$total_amnts +=number_format((float)$tot_sgst,3,'.','');
					$k++;
				}
			}
		}
		...
		$sel_igst_prod="select SUM(i.igst_tax) as igst_total,SUM(i.base_prod_price) as base_total,i.product_disc, SUM(i.discounted_price) as total_igst_val,i.igst_tax_in_per from expense_map i, expense s where i.expense_id='".$val_query['expense_id']."' and i.expense_id=s.expense_id and i.sales_product_price>0 group by i.igst_tax_in_per";
		...
		if($tot_igst_num=mysql_num_rows($ptr_igst_prod))
		{
			while($data_igst=mysql_fetch_array($ptr_igst_prod))
			{
				if($data_igst['igst_tax_in_per']>0)
				{
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('A'.$rowCount, $date);	
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('B'.$rowCount, $val_query['ext_invoice_no']);
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('E'.$rowCount, 'Input IGST '.$data_igst['igst_tax_in_per'].'%');	
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('F'.$rowCount, '');
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('G'.$rowCount, '');
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('H'.$rowCount, '');
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('I'.$rowCount, '');
					$tot_igst=($tot_disc_price_igst*$data_igst['igst_tax_in_per']/100);
					$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('J'.$rowCount, round($tot_igst,3));
					$rowCount++;
					$total_amnts +=number_format((float)$tot_igst,3,'.','');
					$k++;
				}
			}
		}
		...
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('H'.$rowCount, '');
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('I'.$rowCount, '');
		$total_actual= number_format($tot_disc_price_gst+$tot_disc_price_igst+$tot_disc_price+$tot_disc_price_map+$tot_cgst+$tot_sgst+$tot_igst,3,'.','');
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('J'.$rowCount, round($total_actual,3));
		//==========================================================		
		$k++;
	...
	header('Content-Disposition: attachment;filename="Expense Tally Report.xls"');
