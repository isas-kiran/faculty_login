...
<?php
$edit_access='';
$sel_acc="select * from edit_previleges where admin_id='".$_SESSION['admin_id']."' and privilege_id='259'";
					...
					<?php
                        ...
                        if($no_of_records)
                        {
                           ...
                           // $pager = new PS_Pagination($sql_query,$_SESSION['show_records'],$_SESSION['show_records_pages'],$query_string1);
                            ...
                            {
                                ?>
                                <td width="2%" align="center" ><strong>Branch</strong></td>
                                <?php
                            }
                            ?>
                        		<td width="10%" ><a href="<?php echo $_SERVER['PHP_SELF'];?>?order=<?php echo $order."&orderby=service_name".$query_string;?>" class="table_font"><strong>Staff Name</strong></a> <?php echo $img1;?></td>
                        		<td width="10%" align="center" ><strong>Incentive Price</strong></td>
								...
								<td width="8%" align="center" ><strong>Including GST Cost</strong></td>
                       			<td width="8%" align="center" ><strong>Excluding GST Cost</strong></td>
                                <td width="8%" align="center" ><strong>After CC/Web Payment Deduction</strong></td>
							...
							while($val_querys=mysql_fetch_array($all_records))
                            {
								...
								$incentive_rs=0;
								$select_installments = " SELECT i.sales_product_price,i.product_id,i.sales_product_id,i.staff_id,i.cgst_tax_in_per,i.cgst_tax,i.sgst_tax_in_per,i.sgst_tax,s.payment_mode_id FROM sales_product_map i,sales_product s where i.sales_product_id =s.sales_product_id and i.sales_product_price >0 and i.staff_id='".$val_querys['admin_id']."' ".$paid_from_date_i." ".$paid_to_date_i." ";
								$ptr_installment = mysql_query($select_installments);
								while($data_installment = mysql_fetch_array($ptr_installment))
								{
									$sel_brand="select brand from product where product_id='".$data_installment['product_id']."'";
									$ptr_brand=mysql_query($sel_brand);
									$data_brand=mysql_fetch_array($ptr_brand);
									$brand_per=5;
									$sel_brand_per="select inc_percenage from product_brand where brand_id='".$data_brand['brand']."'";
									$ptr_brand_per=mysql_query($sel_brand_per);
									if(mysql_num_rows())
									{
										$data_brand_per=mysql_fetch_array($ptr_brand_per);
										$brand_per=$data_brand_per['inc_percenage'];
									}
									
									
									$tot_amnt=$data_installment['sales_product_price'];
									$total_amnt=$total_amnt+$data_installment['sales_product_price'];
									...
									$total_ded_value=0;
									if($data_installment['payment_mode_id']=='4')
									{
									...
									$incentive_rs =intval($after_cc_deduction*$brand_per)/100;
								...
								if($nos!='')
								{
									...
									else
									{
   									...
   									echo '<td align="center">'.number_format((float)$incentive_rs, 2, '.', '').'</td>';                             
									echo '</tr>';
								   	$bgColorCounter++;
								}    
                                ?>
