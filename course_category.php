    ...
    <?php       if($_POST['formAction'])
                    {
                        if($_POST['formAction']=="delete")
                        {
                            for($r=0;$r<count($_POST['chkRecords']);$r++)
                            {
                                ...
                                $sql_query= "SELECT category_id FROM ".$GLOBALS["pre_db"]."course_category where category_id ='".$del_record_id."'";
                                if(mysql_num_rows($db->query($sql_query)))
                                    {                                                
                                        $delete_query="delete from ".$GLOBALS["pre_db"]."course_category where category_id='".$del_record_id."'";
  ...
  <tr class="head_td">
    ...
    <?php
                        ...
                        if($_GET['order'] !='' && ($_GET['orderby']=='category_name'))
                        {
                            ...
                            $select_directory='order by category_name asc';                      
                            $sql_query= "SELECT category_id,category_name FROM course_category where 1  ".$pre_keyword." ".$select_directory." "; 
                       //echo $sql_query;
...
<!--    <td width="10%"><strong>Post date</strong></td>
	...
	<?php } ?>
                            ...
                            while($val_query=mysql_fetch_array($all_records))
                            {
                                ...
                                $listed_record_id=$val_query['category_id']; 
                                ...
                                echo '<td ><a href="add_category.php?record_id='.$listed_record_id.'" class="table_font">'.$val_query['category_name'].'</a></td>';
//                                echo '<td >'.$val_query['country_id'].'</td>';
								if($_SESSION['type'] =='S')
								{ 
                                echo '<td align="center"><a href="add_category.php?record_id='.$listed_record_id.'" ><img src="images/edit_icon.png" title="Edit Record" class="example-fade"/></a>&nbsp;&nbsp;
