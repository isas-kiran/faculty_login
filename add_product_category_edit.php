...
if($_REQUEST['record_id'])
{
    $record_id=$_REQUEST['record_id'];
     $sql_record= "SELECT * FROM product_category where pcategory_id='".$record_id."'";
    if(mysql_num_rows($db->query($sql_record)))
	{
        $row_record=$db->fetch_array($db->query($sql_record));
		
		$sql_point= "SELECT * FROM product_subcategory where pcategory_id='".$record_id."'";
    ...
    <td class="mid_mid">
        ...
        <?php
                    ...
                    {  
                        ...
                        {
                                $success=0;
                                $errors[$i++]="Enter product_category";
                        }
									...
									{ 
 												 if($_POST['sub_name'.$j] !='' || $_POST['del_floor'.$j]=='yes')
												{
													...
													if($_POST['floor_id'.$j]!='' && $_POST['del_floor'.$j]=='yes' )
													{
														 ...
														 $delete_row = " delete from  product_subcategory where sub_id='".$_POST['floor_id'.$j]."' ";
											...
											if($_POST['sub_name'.$j] !='' && $_POST['del_floor'.$j] !='yes')
											{ 
												    $data_record_type1['pcategory_id']=$_GET['record_id'];  
                                             ...
                                             else
                                            {
                                             $data_record_type1['subaction'] = 'Inactive';
												 ...
												 if($_POST['floor_id'.$j]=='' && $data_record_type1['sub_name'] !='')
												   {
														  $type1_id=$db->query_insert("product_subcategory", $data_record_type1);
													...
													{ 
														...
														$db->query_update("product_subcategory", $data_record_type1,$where_record);
            ...
            <tr><td colspan="3" class="orange_font">* Mandatory Fields</td></tr>
             ...
             <tr>
                	<td width="10%">Category Name</td>.
               		<td width="50%"><input type="text" name="pcategory_name" id="pcategory_name" class="validate[required] input_text" value="<?php if($_POST['save_changes']) echo $_POST['pcategory_name']; else echo $row_record['pcategory_name'];?>" style="width:500px"></td>
                        ...
                        <table cellpadding="5" width="100%" >
                         ...
                         <script language="javascript">
									...
									function floors(idss)
									{
										...
										var shows_data='<div id="floor_id'+idss+'"><table cellspacing="3" id="tbl'+idss+'" width="80%"><tr><td style="width:12%;">Subcategory '+idss+'</td><td valign="top" width="10%" ><input type="text" name="sub_name'+idss+'" style="width:300px" id="sub_name'+idss+'" /></td><td><input type="checkbox" name="choose'+idss+' id="choose'+idss+' value="Active" ></td><input type="hidden" name="row_deleted'+idss+'" id="row_deleted'+idss+'" value="" /><tr></table></div>';
...
<input type="button" name="Add"  class="delBtn"  onClick="javascript:create_floor('delete');" alt="Delete(-)" >
  ...
  <tr>-->
                            <td colspan="6">
                             ...
                             <?php
							$select_exc = "select * from product_subcategory where pcategory_id='".$_GET['record_id']."' order by  sub_id  asc ";
                           ...
                           <td> <input type="checkbox" name="choose<?php echo $t; ?>" id="choose<?php echo $t; ?>" value="Active" <?php echo ($data_exclusive['subaction']=='Active')?'checked':'' ?>></td>
