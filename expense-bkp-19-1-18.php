...
function calc_product_price(prod_id)
{
	...
	document.getElementById("net_amnt"+prod_id).value=prod_price_new;
  ...
  <tr>
	...
	<?php
		...
		else
		{
			...
			{
				$insert_for_receipt = "insert into expense (`payment_mode_id`,`bank_id`,`account_no`,`chaque_no`,`chaque_date`,`amount_wo_tax`,`amount`,`discount_type`,`discount`,`discount_price`,`total_price`,`description`,`vendor_id`,`employee_id`,`expense_category_id`,`expense_type_id`,`credit_card_no`,`sac_code`,`added_date`,`cm_id`) values('".$data_record['payment_mode_id']."','".$data_record['bank_name']."','".$data_record['account_no']."','".$data_record['chaque_no']."','".$data_record['date']."','".$data_record['amount_wo_tax']."','".$data_record['amount']."','".$data_record['discount_type']."','".$data_record['discount']."','".$data_record['discount_price']."','".$data_record['total_price']."','".$data_record['description']."','".$data_record['vendor']."','".$data_record['employee']."','".$data_record['expense_category_id']."','".$data_record['expense_type']."','".$data_record['credit_card_no']."','".$sac_code."','".$added_date."','".$cm_id."')";
				...
				for($i=1;$i<=$total_floor;$i++)
				{
					...
					$data_record_service['expense_name']=( ($_POST['expense_name'.$i])) ? $_POST['expense_name'.$i] : "0";
...
<select name="bank_name" class="input_text" id="bank_name"  style="width:200px" onchange="show_acc_no(this.value)">
 ...
 <tr>
	...
	<tr>
		<td width="20%" valign="top" align="center">Discount <input type="radio" name="discount_type" onChange="calculte_total_cost()" id="discount_type" checked="checked" value="percentage" style="width:200px" <?php if($_POST['discount_type']=="percentage") {echo 'checked="checked"';}else if($row_expense['discount_type']=="percentage") { echo 'checked="checked"';} ?>  />in %
	...
	<?php } ?>
                            ...
                            while($val_query=mysql_fetch_array($all_records))
                            {
								...
								if($data_payment_mode['payment_mode']!="cash")
								{
									...
									echo "-----";
								echo'</td>';
								echo '<td align="center"><span style="float:left">Amount -</span><span style="float:right">'.$val_query['amount_wo_tax'].'</span><br/>';
								...
								if(mysql_num_rows($ptr_tax))
								{
									while($data_tax=mysql_fetch_array($ptr_tax))
									{
										echo '<span style="float:left">'.$data_tax['tax_type'].'('.$data_tax['tax_value'].'%) -</span><span style="float:right">'.$data_tax['tax_amount'].'</span><br/>';
									}
									echo '<hr/ style="color:gray">';
									echo '<b><span style="float:left">Total Amount -</span><span style="float:right">'.$val_query['amount'].'</span></b>';
								}
								echo'</td>';
