...
$objPHPExcel->getProperties()->setCreator("Maarten Balliauw")
	...
	$arr_alph=array('L','M','N','O','P','Q','R','S','T','U','V','W','X','Y','Z','AA','AB','AC','AD','AE','AF','AG','AH','AI','AJ','AK','AL','AM','AN','AO','AP','AQ','AR','AS','AT','AU','AV','AW','AX','AY','AZ','BA','BB','BC','BD','BE','BF','BG','BH','BI','BJ','BK','BL','BM','BN','BO','BP','BQ','BR','BS','BT','BU','BV','BW','BX','BY','BZ');
	...
	foreach ($yearArray as $year) {
		...
		foreach ($monthArray as $month) {
			...
			/*if($st==0 || $st%4==0)
			{
				...
				$objPHPExcel->setActiveSheetIndex(0)->setSharedStyle($sharedStyle3,$arr_alph[$pre_st+1]."5:".$arr_alph[$pre_st+1].$no_of_rows);
	...
	if($no_of_records)
	{
		...
		while($val_query=mysql_fetch_array($db))
		{
			...
			$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('J'.$rowCount, $data_bal['total_balance']);
						...
						if($tot_rel=mysql_num_rows($ptr_installment))
						{
							...
							while($data_installment=mysql_fetch_array($ptr_installment))
							{
								if($data_installment['amount'] > 0)
								{
									...
									$objPHPExcel->setActiveSheetIndex(0)->SetCellValue($arr_alph[$pre_st+2].$newRowcount,$amount_realised,'6');
						...
						if($tot_pay=mysql_num_rows($ptr_installment))
						{
							...
							while($data_installment=mysql_fetch_array($ptr_installment))
							{
								if($data_installment['amount'] >0)
								{
									...
									$objPHPExcel->setActiveSheetIndex(0)->SetCellValue($arr_alph[$pre_st+3].$newRowcount, $pay_mode,'6');
									$objPHPExcel->setActiveSheetIndex(0)->setSharedStyle($sharedStyle4,$arr_alph[$pre_st+2].$newRowcount.":".$arr_alph[$pre_st+2].$newRowcount);
