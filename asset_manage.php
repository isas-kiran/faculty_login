  ...
  <tr class="head_td">
    ...
    <?php
						...
						{*/
							...
							$sql_query= "SELECT p.asset_name, p.asset_code,p.status, p.description, p.size, p.price, p.category_id, p.asset_id,p.quantity,p.quantity_in_shelf,p.quantity_in_consumable,p.added_date,p.admin_id,p.hsn_code,p.brand FROM assets p, assets_category pc where 1  and (p.asset_name like '%".$keyword."%' or p.asset_code like '%".$keyword."%' or p.description like '%".$keyword."%' or p.size like '%".$keyword."%' or p.commission like '%".$keyword."%' or p.price like '%".$keyword."%' or pc.name like '%".$keyword."%' ) ".$status_type." and p.category_id=pc.asset_category_id ".$vendor_ids." ".$cm_ids."  ".$search_cm_id_S." ".$search_admin_id_S." ".$select_directory.""; //".$admin_ids."
						//}
                       //echo $sql_query;
