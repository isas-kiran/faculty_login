...
$edit_access='';
$sel_acc="select * from edit_previleges where admin_id='".$_SESSION['admin_id']."' and privilege_id='106'";
...
<link rel="stylesheet" href="css/validationEngine.jquery.css" type="text/css"/>
	...
	{            
		...
		$("#bank_name").chosen({allow_single_deselect:true});
		//$("#from_bank_name").chosen({allow_single_deselect:true});
   });
	</script>
	...
	function payment(value,type)
	{
		...
		if(payment_mode[0]=="cheque")
		{
			...
			// document.getElementById("bank_details").style.display = 'block';
			 ...
			 //document.getElementById("ex_bank_details").style.display = 'none';
			// show_bank_for_transfer(branch_name,'cheque')
		}
		else if(payment_mode[0]=="Credit Card")
		{
			 ...
			 //document.getElementById("ex_bank_details").style.display = 'none';
			 //show_bank_for_transfer(branch_name,'credit_card')
		}
		else if(payment_mode[0]=="online")
		{
			 ...
			 document.getElementById("credit_details").style.display = 'none';
			 //document.getElementById("ex_bank_details").style.display = 'block';
			// show_bank_for_transfer(branch_name,'online')
		}
		else
		{
			 document.getElementById("chaque_details").style.display = 'none';
			 ...
			 document.getElementById("credit_details").style.display = 'none';
			// document.getElementById("ex_bank_details").style.display = 'none';
			// show_bank_for_transfer(branch_name,'')
		}
	}
	</script>
 <script>   
...
function show_acc_no(bank_id,vals)
{
	//alert(bank_id);
...
function show_acc_no_to(bank_id)
{
	//alert(bank_id);
	var data1="action=show_account&bank_id="+bank_id;
	//alert(data1);
	...
	success: function (html)
	{
		document.getElementById('from_account_no').value=html;
	}
	});
}
...
function show() 
{
	...
	document.getElementById('cash_transfer_mode_bank_to_bank').style.display = 'none'; 
	document.getElementById('cash_transfer_mode_cash_to_cash').style.display = 'none'; 
	document.getElementById('hide_b').style.display = 'none'; 
	document.getElementById('text_type').innerHTML = 'Cash Withdrawl From bank'; 
	payment('','inward');
}
...
function hide() 
{
	...
	document.getElementById('cash_transfer_mode_cash_to_cash').style.display = 'none'; 
	document.getElementById('hide_b').style.display = 'none'; 
	document.getElementById('text_type').innerHTML = 'Cash Deposited In bank'; 
	payment('','outward');
}
function hide_all() 
{
	...
	document.getElementById('cash_transfer_mode_inword').style.display = 'none';  
	document.getElementById('cash_transfer_mode_outword').style.display = 'none'; 
	document.getElementById('cash_transfer_mode_bank_to_bank').style.display = 'block'; 
	document.getElementById('cash_transfer_mode_cash_to_cash').style.display = 'none'; 
	document.getElementById('hide_b').style.display = 'block'; 
	document.getElementById('text_type').innerHTML = 'Cash Transfer From One Bank to Another bank';
	payment('','bank_to_bank');
}
function hide_all_category() 
{
	...
	document.getElementById('cash_transfer_mode_outword').style.display = 'none'; 
	document.getElementById('cash_transfer_mode_bank_to_bank').style.display = 'none'; 
	document.getElementById('cash_transfer_mode_cash_to_cash').style.display = 'block'; 
	document.getElementById('bank_details').style.display = 'none'; 
	document.getElementById('bank_record').style.display = 'none'; 
	document.getElementById('hide_b').style.display = 'none'; 
	document.getElementById('text_type').innerHTML = 'Cash Transfer From Business Cash to Petty Cash';
	//payment('','cahs_to_cash');
}

function show_bank_for_branch(branch_id)
{
	//alert(branch_id);
	document.getElementById("bank_record").style.display="none";	
	...
	success: function(retbank)
	{
		...
		show_bank_for_transfer(branch_id)
	}
	});
...
function show_bank_for_transfer(branch_id)
{
	//alert(branch_id);
	document.getElementById("from_bank_record").style.display="none";
	var bank_data="show_bnk=1&action_for=from_bank&branch_id="+branch_id;
	//alert(bank_data);
	$.ajax({
	url: "show_bank.php",type:"post", data: bank_data,cache: false,
	success: function(retbank)
	{
		document.getElementById("from_bank_id").innerHTML=retbank;
		//$("#from_bank_name").chosen({allow_single_deselect:true});
	}
...
<table border="0" cellspacing="0" cellpadding="0" width="100%">
                        ...
                        if($_POST['formAction']=="delete")
                        {
                            for($r=0;$r<count($_POST['chkRecords']);$r++)
                            {
                                ...
                                if(mysql_num_rows($db->query($sql_query)))
                                    {
										"<br>".$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('cash_transfer','Delete','cash_transfer','".$del_record_id."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
  ...
  <tr>
	...
	if($_POST['save_changes'])
	{
		...
		else if($cash_transfer_mode == "outword")
		{
			$payment_mode_outword=$_POST['payment_mode_outword'];
			$sep_outword=explode("-",$payment_mode_outword);
			$payment_mode_id=$sep_outword[1];
		}
		else if($cash_transfer_mode == "bank_to_bank")
		{
			$payment_mode_bank_to_bank=$_POST['payment_mode_bank_to_bank'];
			$sep_bank_to_bank=explode("-",$payment_mode_bank_to_bank);
			$payment_mode_id=$sep_bank_to_bank[1];
			$from_bank_name=$_POST['from_bank_name'];
			$from_account_no=$_POST['from_account_no'];
		}
		else if($cash_transfer_mode == "cash_to_cash")
		{
			$payment_mode_cash_to_cash=$_POST['payment_mode_cash_to_cash'];
			$sep_cash_to_cash=explode("-",$payment_mode_cash_to_cash);
			$payment_mode_id=$sep_cash_to_cash[1];
		}
	...
	<?php
		...
		//==================================================================
			...
			if($record_id)
			{
				...
				$update="update receipt set category='".$data_record['category']."',`cash_type`='".$data_record['cash_type']."',`cash_transfer_mode`='".$data_record['cash_transfer_mode']."',amount='".$data_record['amount']."', bank_id='".$data_record['bank_id']."', payment_mode_id='".$payment_mode_id."', chaque_no='".$chaque_no."', chaque_date='".$date."', credit_card_no='".$credit_card_no."' , description='".$description."', cm_id='".$cm_id."', from_bank_name='".$from_bank_name."', from_account_no='".$from_account_no."'  where receipt_id='".$record_id."'";
				$ptr_update=mysql_query($update);
				
				$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('cash_transfer','Edit','cash_transfer','".$record_id."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
				$query=mysql_query($insert); 
			...
			{
				$insert_for_receipt = "insert into receipt (`category`,`cash_type`,`cash_transfer_mode`,`from_bank_name`, `from_account_no`,`bank_id`,`amount`,`added_date`, `payment_mode_id`,`chaque_no`,`chaque_date`,`credit_card_no`,`description`,`cm_id`) values('".$data_record['category']."','".$data_record['cash_type']."','".$data_record['cash_transfer_mode']."','".$from_bank_name."','".$from_account_no."','".$data_record['bank_id']."','".$data_record['amount']."','".date('Y-m-d')."','".$payment_mode_id."','".$chaque_no."','".$date."','".$credit_card_no."','".$description."','".$cm_id."')";
				$ptr_insert_receipt = mysql_query($insert_for_receipt);
				$ins_id=mysql_insert_id();
			
				"<br>".$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('cash_transfer','Add','cash_transfer','".$ins_id."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
				$query=mysql_query($insert); 
	...
	{	
        ...
        ?>
<tr>
<td align="center" width="20%">Select Branch</td>
<td>
<?php 
...
echo ' <select id="branch_name" style="width:200px" class="input_text" name="branch_name" onchange="show_bank_for_branch(this.value)">';
...
{
	echo '<input type="hidden" name="branch_name" id="branch_name" value="'.$_SESSION['branch_name'].'"  />';
} 
?>
 <tr>
 <input type="hidden" name="category" id="category" value="cash_transfer"  />
    <td width="20%" class="tr-header" align="center">Select Category<span class="orange_font">*</span></td>
    <td width="40%"><input type="radio" name="cash_transfer_mode" style="width:30px" id="cash_transfer_mode_in" value="inword" onclick="show()" <?php if($row_record['cash_transfer_mode'] =="inword") echo 'checked="checked"'; else echo 'checked="checked"'; ?>  > Inward <input type="radio" name="cash_transfer_mode" id="cash_transfer_mode_out" value="outword" <?php if($row_record['cash_transfer_mode'] =="outword") echo 'checked="checked"'; ?> onclick="hide()" > Outward <input type="radio" name="cash_transfer_mode" id="cash_transfer_mode_bank" value="bank_to_bank" <?php if($row_record['cash_transfer_mode'] =="bank_to_bank") echo 'checked="checked"'; ?> onclick="hide_all()" > Bank To Bank <input type="radio" name="cash_transfer_mode" id="cash_transfer_mode_cash" value="cash_to_cash" <?php if($row_record['cash_transfer_mode'] =="cash_to_cash") echo 'checked="checked"'; ?> onclick="hide_all_category()" >Business To Petty Cash</td>
	<td></td>
 </tr>
<tr>
 <td colspan="2">
	<div id="text_type" style="font-size:14px; font-weight:700;color:red;text-align:center">
 ...
 <div id="cash_transfer_mode_inword" <?php if($row_record['cash_transfer_mode'] =="inword") echo 'style="display:block"'; else if($row_record['cash_transfer_mode'] =="outword") echo 'style="display:none"'; else if($row_record['cash_transfer_mode'] =="bank_to_bank") echo 'style="display:none"'; ?>>
 <table width="86%">
  <tr>
    <td width="20%" class="tr-header" align="center">Select Payment Mode<span class="orange_font">*</span></td>
    <td width="25%"><select name="payment_mode_inword" style="width:200px" class="input_text" id="payment_mode_inword" onchange="payment(this.value,'inward')">
  ...
  <td colspan="2">
	<div id="cash_transfer_mode_outword"  <?php if($row_record['cash_transfer_mode'] =="outword") echo 'style="display:block"'; else echo 'style="display:none"'; ?>>
  		<table width="86%">
  			<tr>
                <td width="20%" class="tr-header" align="center">Select Payment Mode<span class="orange_font">*</span></td>
                <td width="25%"><select name="payment_mode_outword" style="width:200px" class="input_text" id="payment_mode_outword" onchange="payment(this.value,'outward')">
                <!--<option value="">--Select--</option>-->
                <?php
                $sel_payment_mode="select payment_mode,payment_mode_id from payment_mode where payment_mode_id='1'"; //payment_mode_id='1' or
                $ptr_payment_mode=mysql_query($sel_payment_mode);
                while($data_payment=mysql_fetch_array($ptr_payment_mode))
                {
                    $selected='';
	...
	<div id="cash_transfer_mode_bank_to_bank" <?php if($row_record['cash_transfer_mode']=="bank_to_bank") echo 'style="display:block"'; else echo 'style="display:none"'; ?>>
	<table width="90%">
    	<tr>
            <td width="20%" class="tr-header" align="center">Select Payment Mode<span class="orange_font">*</span></td>
            <td width="25%"><select name="payment_mode_bank_to_bank" style="width:200px" class="input_text" id="payment_mode_bank_to_bank" onchange="payment(this.value,'bank_to_bank')">
            <option value="">--Select--</option>
            <?php
            $sel_payment_mode="select payment_mode,payment_mode_id from payment_mode where payment_mode_id='2' or payment_mode_id='5'";
            $ptr_payment_mode=mysql_query($sel_payment_mode);
            while($data_payment=mysql_fetch_array($ptr_payment_mode))
            {
                $selected='';
		...
		<tr>
    		<td width="20%" class="tr-header" align="center">Select From Bank<span class="orange_font">*</span></td>
            <td>
            <?php 
            if($_SESSION['type'] !="S" && $_SESSION['type']!='Z' && $_SESSION['type']!='LD'  )
            {
				?>
				<select name="from_bank_name" style="width:200px" class="input_text" id="from_bank_name" onchange="show_acc_no_to(this.value)">
				<option value="">--Select--</option>
				<?php
				$sle_bank_name="select bank_id,bank_name from bank where 1 ".$_SESSION['where'].""; 
				$ptr_bank_name=mysql_query($sle_bank_name);
				while($data_bank=mysql_fetch_array($ptr_bank_name))
				{
					$selected='';
					if($data_bank['bank_id'] == $row_record['from_bank_name'])
					{
						$selected='selected="selected"';
					}
            ...
            ?></div>
            <div id="from_bank_id"></div>
            </td>
		</tr>
        <tr>
             <td width="15%" class="tr-header" align="center">Enter Account No<span class="orange_font">*</span></td>
             <td><input type="text" name="from_account_no" style="width:200px" class="input_text" readonly="readonly" id="from_account_no" value="<?php if($_POST['from_account_no']) echo $_POST['from_account_no']; else echo $row_record['from_account_no']; ?>" /></td>
        </tr>
	</table>
</div>
<div id="cash_transfer_mode_cash_to_cash"  <?php if($row_record['cash_transfer_mode'] =="cash_to_cash") echo 'style="display:block"'; else echo 'style="display:none"'; ?>>
<table width="86%">
    <tr>
        <td width="20%" class="tr-header" align="center">Select Payment Mode<span class="orange_font">*</span></td>
        <td width="25%"><select name="payment_mode_cash_to_cash" style="width:200px" class="input_text" id="payment_mode_cash_to_cash" >
        <!--<option value="">--Select--</option>-->
        <?php
        $sel_payment_mode="select payment_mode,payment_mode_id from payment_mode where payment_mode_id='1'"; //payment_mode_id='1' or
        $ptr_payment_mode=mysql_query($sel_payment_mode);
...
<div id="cash_type" <?php if($row_record['cash_transfer_mode'] == "inword" || $row_record['cash_transfer_mode'] =="outword") echo 'style="display:block"'; else if($row_record['cash_transfer_mode']=="bank_to_bank" || $row_record['cash_transfer_mode']=="cash_to_cash") echo 'style="display:none"'; ?>>
	<table>
		<tr>
        	<td width="12%" class="tr-header" align="center">Select Cash Type <span class="orange_font">*</span></td>
            <td width="25%"><input type="radio" name="cash_type" id="cash_type" value="petty_cash" <?php if($_POST['cash_type']=='petty_cash') echo 'checked="checked"'; else if($row_record['cash_type']=='petty_cash') echo 'checked="checked"'; else echo 'checked="checked"'; ?> /> Petty Cash &nbsp;&nbsp;<input type="radio" name="cash_type" id="cash_type" value="business_cash" <?php if($_POST['cash_type']=='business_cash') echo 'checked="checked"'; else if($row_record['cash_type']=='business_cash') echo 'checked="checked"'; ?> /> Business Cash
            </td>
        </tr>
	</table>
...
<tr>
<td colspan="2">
<div id="bank_details" <?php if($record_id !='') {  if($row_record['cash_transfer_mode'] =="inword") echo 'style="display:block"'; else if($row_record['cash_transfer_mode'] =="outword") echo 'style="display:block"'; else if($row_record['cash_transfer_mode'] =="bank_to_bank") echo 'style="display:block"'; else if($row_record['cash_transfer_mode'] =="cash_to_cash") echo 'style="display:none"'; } ?>>
<table width="100%">
<tr>
<td width="33%" class="tr-header" align="center"><span id="hide_b" style="display:none">Select To </span>Bank Name<span class="orange_font">*</span></td>
<td>
<?php 
if($_SESSION['type'] !="S" && $_SESSION['type']!='Z' && $_SESSION['type']!='LD'  )
{
	?>
	<select name="bank_name" style="width:200px" class="input_text" id="bank_name" onchange="show_acc_no(this.value)">
...
<div id="bank_id"></div>
 ...
 <table width="100%">
	<tr>
		<td width="33%" class="tr-header" align="center">Enter Account No<span class="orange_font">*</span></td>
...
<div id="chaque_details" <?php  if($data_payment_mode1['payment_mode']=='cheque') echo ' style="display:block"'; else echo ' style="display:none"'; ?>>
    <table width="100%">
        <tr>
            <td width="37%" class="tr-header" align="center">Enter Chaque No<span class="orange_font">*</span></td>
        ...
        <tr>
            <td width="37%" class="tr-header" align="center">Enter Chaque Date<span class="orange_font">*</span></td>
...
<div id="credit_details" <?php  if($data_payment_mode1['payment_mode']=='Credit Card') echo ' style="display:block"'; else echo ' style="display:none"'; ?>>
	<table width="100%">
		<tr>
			<td width="37%" class="tr-header" align="center">Enter Credit Card No<span class="orange_font">*</span></td>
			<td><input type="text" name="credit_card_no" style="width:200px" class="input_text" id="credit_card_no" value="<?php if($_POST['credit_card_no']) echo $_POST['credit_card_no']; else echo $row_record['credit_card_no']; ?>" /></td>
		</tr>
	</table>
</div>
<tr>
    <td width="20%" class="tr-header" align="center">Amount</td>
    <td width="25%"><input type="text" style="width:200px" class="input_text" name="amount" id="amount" value="<?php if($_POST['amount']) echo $_POST['amount']; else echo $row_record['amount']; ?>"  /></td>
</tr>
<tr>
    <td width="20%" class="tr-header" align="center">Description</td>
    <td width="25%"><textarea name="description" id="description"  style="width:300px;height:70px" ><?php if($_POST['description']) echo $_POST['description']; else echo $row_record['description']; ?></textarea></td>
</tr>
<tr>
	<?php
                        ...
                        if($no_of_records)
                        {
                           ...
                           // $pager = new PS_Pagination($sql_query,$_SESSION['show_records'],$_SESSION['show_records_pages'],$query_string1);
    						...
    						<form method="post"  name="frmTakeAction" action="<?php echo $_SERVER['PHP_SELF'].'?#msgbox';?>" >
								...
								<td width="8%"><a href="<?php echo $_SERVER['PHP_SELF'];?>?order=<?php echo $order."&orderby=category".$query_string;?>" class="table_font"><strong>Category Name</strong></a> 	<?php echo $img1;?></td>
								<td width="12%" align="center"><strong>Description</strong></td>
								<td width="8%" align="center"><strong>Payment Mode</strong></td>
								<td width="12%" align="center"><strong>Bank Name</strong></td>
								<td width="8%" align="center"><strong>Chaque No</strong></td>
								<td width="8%" align="center"><strong>Chaque Date</strong></td>
								...
								while($val_query=mysql_fetch_array($all_records))
								{
									...
									if($val_query['cash_transfer_mode']=="bank_to_bank")  
									{
										$cash_mode="Bank To Bank";
									}
									else
										$cash_mode=$val_query['cash_transfer_mode'];
										
									if($_SESSION['type']=='S'  || $edit_access=='yes')
									{
										echo '<td ><a href="cash_transfer.php?record_id='.$listed_record_id.'" class="table_font">'.$cash_mode.'</a></td>';
									}
									else
										echo '<td >'.$cash_mode.'</td>';
