...
$sel_for_Num_rows="SELECT * FROM batch_timetable where 1 and c_b_id='".$record_id."' ".$pre_keyword." order by day asc";
...
$sql_query= "SELECT * FROM batch_timetable where 1 and c_b_id='".$record_id."' ".$pre_keyword." order by day asc"; 
...
while($val_query=mysql_fetch_array($query_order))
{
	...
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('C'.$rowCount, $new_date);
...
header('Content-Disposition: attachment;filename="Course_Batch_Timetable_'.date('Y-m-d').'.xls"');
