<?php include 'inc_classes.php';
	...
	if($action=='show_data')
    {
		...
		if($action_page=="issue_book")
		{
			$style='style="width:20.5%;" align="left"';
			$style2='style="width:20.5%;" align="left"';
		}
		else if($action_page=="receipt")
		{
			$style='style="width:27%;" align="center"';
			$style2='style="width:27%" align="center"';
		}
		else if($action_page=="sales_product")
		{
    	...
    	if($_POST['id']=='Customer')
		{
			...
			<tr>
                ...
                <td <?php echo $style; ?>>Search by Mobile no.</td>
                <td id="mobiles">
    		...
    		<tr>
        		<td <?php echo $style2; ?>>Select Customer<span class="orange_font">*</span></td>
        		...
        		<?php  
        		$sql_cust="select cust_name, cust_id from customer where 1 ".$user_cm." order by cust_id asc";
				$ptr_cust = mysql_query($sql_cust); // and type='centre' or (1212 = Ahmednagar, 1113= PCMC, 1805=Pune,18=Ahmedbad)
				while($data_cust = mysql_fetch_array($ptr_cust))
				{
					$selecteds="";
		...
		else if($_POST['id']=='Student')
		{
			...
			<table style="width:100%;">
            	...
            	<tr>
            		<td <?php echo $style2; ?>>Select Student<span class="orange_font">*</span></td>
            		<td width="70%" id="sel_cust" class="customized_select_box">
            		<select name="customer_id" id="customer_id" style="width:200px;" onChange="show_mobile_no(this.value,'<?php echo $_POST['id']; ?>')" >
		...
		else if($_POST['id']=='Employee')
		{
			...
			<table style="width:100%;">
            	...
            	<tr>
            		<td <?php echo $style2; ?>>Select Employee<span class="orange_font">*</span></td>
            		<td width="70%" id="sel_cust" class="customized_select_box">
            		<select name="customer_id" id="customer_id" style="width:200px;" onChange="show_mobile_no(this.value,'<?php echo $_POST['id']; ?>')" >
		...
		else if($_POST['id']=='Vendor')
		{
			...
			<table style="width:100%;">
				<tr>
            		<input type="hidden" name="res1" id="res1" />
            		<input type="hidden" name="res_tax" id="res_tax" />
            		<td style="width:<?php echo $width; ?>%;" align="center">Search by Mobile no.</td>
            		<td>
                	<select id="realtxt" name="realtxt" style="width:200px" onChange="searchSel(this.value,'<?php echo $_POST['id']; ?>')">
                	<option value="">Select Mobile No.</option>
                	<?php  
            	...
            	<tr>
            		<td style="width:<?php echo $width; ?>%;" align="center">Select Vendor<span class="orange_font">*</span></td>
            		<td width="70%" id="sel_cust" class="customized_select_box">
            		<select name="customer_id" id="customer_id" style="width:200px;" onChange="show_mobile_no(this.value,'<?php echo $_POST['id']; ?>')" >
	...
	if($action=='get_staff_presence')
    {
		$branch_id=$_POST['branch_id'];
		$staff_id=$_POST['staff_id'];
		$date=$_POST['date'];
		$frm_date=explode("/",$date);
		$frm_dates=$frm_date[2]."-".$frm_date[1]."-".$frm_date[0];
		$added_date=$frm_dates;
		
		$sel_branch="select cm_id from site_setting where branch_name='".$branch_id."' and type='A'";
		$ptr_branch=mysql_query($sel_branch);
		$data_branch=mysql_fetch_array($ptr_branch);
		$cm_id=$data_branch['cm_id'];
		
		$sel_pres="select attendance_id,total_hrs from pr_daily_attendance where employee_id='".$staff_id."' and DATE(date)='".$added_date."' and cm_id='".$cm_id."'";
		$ptr_pres=mysql_query($sel_pres);
		$data_pres=mysql_fetch_array($ptr_pres);		
		echo $data_pres['total_hrs'].'##'.$data_pres['attendance_id'];
	}
	if($action=='check_logsheet_exist')
    {
		$course_id=$_POST['course_id'];
		$sel_timetable="select logsheet_id from courses where course_id='".$course_id."'";
		$ptr_course=mysql_query($sel_timetable);
		if(mysql_num_rows($ptr_course))
		{
			$data_log=mysql_fetch_array($ptr_course);
			if($data_log['logsheet_id']!='')
			{
				echo "exist##".$data_log['logsheet_id'];
			}
		}
	}
	if($action=='dsr_matched')
    {
		$dates=$_POST['dates'];
		$cm_id=$_POST['cm_id'];
		$vals=$_POST['vals'];
		...
		$date_dsr=$date_Exp[2].'-'.$date_Exp[1].'-'.$date_Exp[0];
		
		$sel_dsr="select dsr_id from dsr_match_reprot where dsr_date='".$date_dsr."' and cm_id='".$cm_id."'";
		$ptr_dsr=mysql_query($sel_dsr);
		if(mysql_num_rows($ptr_dsr))
		{
			$data_dsr=mysql_fetch_array($ptr_dsr);
			
			$update_dsr_matched="update dsr_match_reprot set dsr_date='".$date_dsr."', status='".$vals."', added_by='".$_SESSION['admin_id']."' where dsr_date='".$date_dsr."' and cm_id='".$cm_id."' ";
			$ptr_uodate=mysql_query($update_dsr_matched);
		}
		else
		{
			$ins_dsr="insert into dsr_match_reprot (`dsr_date`,`status`,`added_by`,`admin_id`,`cm_id`,`added_date`) values ('".$date_dsr."','".$vals."','".$_SESSION['admin_id']."','".$_SESSION['admin_id']."','".$cm_id."','".date('Y-m-d')."')";
			$ptr_ins=mysql_query($ins_dsr);
		}
	}
...
//===============================================================ADD Sequence Number =======================================================
    ...
    {	
		$val=$_POST['val'];
		$id=$_POST['id'];
     $update_pre_id=" update previleges set seq_no='".$val."' where privilege_id='".$id."' ";
     $updt_id=mysql_query($update_pre_id); 
    }
//===============================================================End Sequence Number =========================================================
...
//=================================================================================================================================================================================
			if($action=='show_course')
			{
				...
				$select_cm_id= "select cm_id from site_setting where 1 and branch_name='".$branch_name."' and type='A' limit 0,1 ";
				$ptr_cm_id= mysql_query($select_cm_id);
				$data_cm_id= mysql_fetch_array($ptr_cm_id);
				$cm_id=$data_cm_id['cm_id'];
				
				$sel_class = "select course_duration from courses where course_id='".$course_id."' ";
				$execute_class = mysql_query($sel_class);
				$courses_data= mysql_fetch_array($execute_class);
            
				$sel_course_price= "select * from courses_price where course_id='".$course_id."' and cm_id='".$cm_id."' ";
				$ptr_course_price= mysql_query($sel_course_price);
				$courses_price= mysql_fetch_array($ptr_course_price);
				
				$course_price =trim($courses_price['course_price']); 
				$discount_otp =trim($courses_price['discount_otp']); 
				$discount_inst =trim($courses_price['discount_inst']); 
...
//============================================================SHOW CATEGORY===================================================================================
			if($action=='show_course_gst')
			{
				...
				$sel_course_price= "select course_price from courses_price where course_id='".$course_id."' and cm_id='".$cm_id."' ";
				$ptr_course_price= mysql_query($sel_course_price);
				$courses_price= mysql_fetch_array($ptr_course_price);
				...
				/*$sel_tax= "SELECT * FROM general_settings where branch_name='".$branch_name."'";
				$ptr_tax=mysql_query($sel_tax);
				$data_tax=mysql_fetch_array($ptr_tax);
				
				$service_taxes=$data_tax['service_tax'];
				$cgst_taxes=$data_tax['cgst'];
				$sgst_taxes=$data_tax['sgst'];
				if($data_tax['tax_type']=='GST')
				{
				...
				else if($data_tax['tax_type']=='service_tax')
				{
					$tax=($service_taxes*$course_price)/100;
					$course_with_tax=intval($course_price+$tax);
				}*/
				echo $course_duration.'###'.$course_price;				  
			}
//============================================================SHOW CATEGORY===================================================================================
...
if($action=='show_expense_cat')
{
	...
	if($_POST['type']!='for_exp_report') { echo '<td width="11%" align="center">Select Expense Type</td>'; } 
	echo '<td width="44%"><select name="expense_type" id="expense_type" style="width: 200px;" onchange="show_type(this.value)">'; 
	if($_POST['type']=='for_exp_report') {
...
//=======================================================ASSETS CATEGORY============================================================================
if($action=='show_asset_cat')
{
	$category_id=$_POST['category_id'];
	$expense_type_id=$_POST['asset_type_id'];
	$sel_expense = "select * from assets_type where category_id='".$category_id."'";
	$ptr_expense = mysql_query($sel_expense);
	echo '<table width="100%"><tr>';
	if($_POST['type']!='for_exp_report') { echo '<td width="11%" align="center">Select Assets Type</td>'; } 
	echo '<td width="44%"><select name="expense_type" id="expense_type" style="width: 200px;" onchange="show_type(this.value)">'; 
	if($_POST['type']=='for_exp_report') {
		echo '<option value="">--Select Assets Type--</option>';
	}
	while($data_expense=mysql_fetch_array($ptr_expense))
	{
		$sel='';
		if($expense_type_id)
		{
			if($expense_type_id ==$data_expense['asset_type_id'])
			{
				echo $sel='selected="selected"';
			}
		}
		echo '<option '.$sel.' value="'.$data_expense['asset_type_id'].'">'.$data_expense['asset_type'].'</option>';
	}
	echo '</select></td></tr></table>';
}
//============================================================================================================================================================
...
//=============================================================Campaign ==============================================================================
if($action=='show_campaign')
{
	$branch=$_POST['branch'];
	$where_cm_id='';
	if($branch !='')
	{
		$select_cm_id= "select cm_id from site_setting where 1 and branch_name='".$branch."' and type='A' limit 0,1 ";
		$ptr_cm_id= mysql_query($select_cm_id);
		$data_cm_id= mysql_fetch_array($ptr_cm_id);
		$cm_id=$data_cm_id['cm_id'];
		$where_cm_id=" and cm_id='".$cm_id."'";
	}
	$sel_expense="select * from campaign where 1 ".$where_cm_id." and status='Active' order by campaign_for asc";
	$ptr_expense=mysql_query($sel_expense);
	echo '<select name="campaign_name" id="campaign_name" style="width: 250px;" class="">'; 
	echo '<option value="">Select Campaign </option>';
	
	while($data_expense=mysql_fetch_array($ptr_expense))
	{
		echo '<option '.$sel.' value="'.$data_expense['campaign_id'].'">'.$data_expense['campaign_name'].'</option>';
	}
	echo '</select>';
}
if($action=='show_campaign_expense')
{
	$where_branch='';
	$where_cm_id='';
	if($_POST['branch'] !='')
	{
		$branch=$_POST['branch'];
		$where_branch=" and branch_name='".$branch."'";
	...
	echo '<table width="100%"><tr>';
	echo '<td width="20%" align="center">Select Campaign Type</td>';
	echo '<td>';
	echo '<select name="campaign_name" id="campaign_name" style="width: 200px;" class="">'; 
	echo '<option value="">Select Campaign Name </option>';
	$course_category ="select DISTINCT(cm_id),branch_name from site_setting where type='A' ".$where_branch." ";
    $ptr_course_cat = mysql_query($course_category);
    while($data_cat = mysql_fetch_array($ptr_course_cat))
    {
        echo "<optgroup label='".$data_cat['branch_name']."'>";
		$sel_expense = "select * from campaign where 1 ".$where_cm_id." and campaign_for='institute' and status='Active' order by campaign_name asc"; //branch_name='".$branch."'
		$ptr_expense = mysql_query($sel_expense);
		while($data_expense=mysql_fetch_array($ptr_expense))
		{
			echo '<option '.$sel.' value="'.$data_expense['campaign_id'].'">'.$data_expense['campaign_name'].'</option>';
...
if($action=='show_campaign_type')
{
	...
	$type=$_POST['type'];
	$sel_expense="select * from campaign where 1 ".$where_cm_id." and campaign_for='".$type."' and status='Active' order by campaign_name asc";
	$ptr_expense=mysql_query($sel_expense);
...
if($action=='show_course_enrolled')
{
	...
	$discount_otp =trim($courses_price['discount_otp']); 
	$discount_inst =trim($courses_price['discount_inst']);
	$discount_now_otp =trim($courses_price['discount_now_otp']); 
	$discount_now_inst =trim($courses_price['discount_now_inst']);
	$discount_otp_inst =trim($courses_price['discount_otp_inst']); 
	$discount_now_otp_inst =trim($courses_price['discount_now_otp_inst']);
	
	$course_duration = trim($courses_data['course_duration']);
	$tax=($service_taxes*$course_price)/100;
	$course_with_tax=intval($course_price+$tax);
	echo $course_duration.'###'.$course_with_tax.'###'.$discount_otp.'###'.$discount_inst.'###'.$discount_now_otp.'###'.$discount_now_inst.'###'.$discount_otp_inst.'###'.$discount_now_otp_inst;
}
//==========================================================================================================================================================
//=========================================================GET COURSE PRICE AND DISCOUNT TYPE==================================================================
if($action=='show_course_enrolled_gst')
{
	...
	$discount_now_otp_inst =trim($courses_price['discount_now_otp_inst']); 
	//======kit offer=======
	$off_course_price =trim($courses_price['off_course_price']);
	$off_discount_otp =trim($courses_price['off_discount_otp']);
	$off_discount_otp_inst =trim($courses_price['off_discount_otp_inst']);
	$off_discount_inst =trim($courses_price['off_discount_inst']);
	$off_discount_now_otp =trim($courses_price['off_discount_now_otp']);
	$off_discount_now_inst =trim($courses_price['off_discount_now_inst']);
	$off_discount_now_otp_inst =trim($courses_price['off_discount_now_otp_inst']);
	
	$course_duration = trim($courses_data['course_duration']);
	
	$sel_tax="select * from general_settings where 1 and cm_id='".$cm_id."'";
	$ptr_tax=mysql_query($sel_tax);
	$data_tax=mysql_fetch_array($ptr_tax);
	...
	if($data_tax['tax_type']=='GST')
	{
		...
		$cgst_tax=($cgst_taxes*$course_price)/100;
		$sgst_tax=($sgst_taxes*$course_price)/100;
		$course_with_tax=round($course_price+$cgst_tax+$sgst_tax);
		//--------------course price for regular offer----------------
		$o_cgst_tax=($cgst_taxes*$off_course_price)/100;
		$o_sgst_tax=($sgst_taxes*$off_course_price)/100;
		
		$off_course_with_tax=round($off_course_price+$o_cgst_tax+$o_sgst_tax);
	}
	else
	{
		$serv_taxes=$data_tax['service_tax'];
		$services_tax=($serv_taxes*$course_price)/100;
		$course_with_tax=round($course_price+$services_tax);
		//--------------course price for Kit offer-----------------
		$serv_taxes=$data_tax['service_tax'];
		$o_services_tax=($serv_taxes*$off_course_price)/100;
		$off_course_with_tax=round($off_course_price+$o_services_tax);
	}
	
	echo $course_duration.'###'.$course_with_tax.'###'.$discount_otp.'###'.$discount_inst.'###'.$discount_now_otp.'###'.$discount_now_inst.'###'.$discount_otp_inst.'###'.$discount_now_otp_inst.'###'.$off_course_with_tax.'###'.$off_discount_otp.'###'.$off_discount_otp_inst.'###'.$off_discount_inst.'###'.$off_discount_now_otp.'###'.$off_discount_now_otp_inst.'###'.$off_discount_now_inst.'###'.$course_with_tax;
}
//============================================ END ========================================================
//=========================Update Count Down Time for Now Discount in enrollment================================
if($action=='enrollment_disc_countdown')
{
	$studs_id=$_POST['studs_id'];
	$sel_studs="select apply_now_disc,countdown_time from inquiry where inquiry_id='".$studs_id."' ";
	$ptr_studs=mysql_query($sel_studs);
	if(mysql_num_rows($ptr_studs))
	{
		$data_std=mysql_fetch_array($ptr_studs);
		if($data_std['apply_now_disc']=='')
		{
			$update_cnt="update inquiry set apply_now_disc='yes',countdown_time='".date("Y-m-d H:i:s", strtotime('+2 hours'))."' where inquiry_id='".$studs_id."' ";
			$ptr_cnt=mysql_query($update_cnt);
			$datatime=date("Y-m-d H:i:s", strtotime('+2 hours'));
		}
		else
		{
			$datatime=$data_std['countdown_time'];
		}
		echo trim($datatime);
	}
}
//======================================================================== END ==================================================================================
//=========================================================Update Count Down Time for Now Discount in enrollment==================================================================
if($action=='enroll_offer_countdown')
{
	$studs_id=$_POST['studs_id'];
	$sel_studs="select apply_now_disc,countdown_time from inquiry where inquiry_id='".$studs_id."' ";
	$ptr_studs=mysql_query($sel_studs);
	if(mysql_num_rows($ptr_studs))
	{
		...
		if($data_std['apply_now_disc']=='')
		{
			$update_cnt="update inquiry set apply_now_disc='yes',countdown_time='".date("Y-m-d H:i:s", strtotime('+3 days'))."' where inquiry_id='".$studs_id."' ";
			$ptr_cnt=mysql_query($update_cnt);
			$datatime=date("Y-m-d H:i:s", strtotime('+3 days'));
		}
		else
		{
			$datatime=$data_std['countdown_time'];
...
//======================================================================== END ==================================================================================
//=========================================================GET COURSE PRICE AND DISCOUNT TYPE==================================================================
if($action=='get_course_discount')
{
	$course_id=$_POST['course_id'];
	$record_id=$_POST['record_id'];
	
	$select_cm_id= "select cm_id from enrollment where 1 and enroll_id='".$record_id."' ";
	$ptr_cm_id= mysql_query($select_cm_id);
	$data_cm_id= mysql_fetch_array($ptr_cm_id);
	$cm_id=$data_cm_id['cm_id'];
	...
	echo $discount_otp.'###'.$discount_inst.'###'.$discount_now_otp.'###'.$discount_now_inst.'###'.$discount_otp_inst.'###'.$discount_now_otp_inst ;
}
...
if($action=='update_course_status')
{
	...
	$update_course_status="update enrollment set course_status='".$status."' where enroll_id='".$enroll_id."' ";
	$ptr_course_status= mysql_query($update_course_status);
}
//======================================================== Update Enrollment Status =============================================================
if($action=='update_enroll_status')
{
	$status=$_POST['status'];
	$enroll_id=$_POST['enroll_id'];
	$update_course_status="update enrollment set status='".$status."' where enroll_id='".$enroll_id."' ";
	$ptr_course_status= mysql_query($update_course_status);
}
...
if($action=='update_student_status')
{
	$status=$_POST['status'];
	$enroll_id=$_POST['enroll_id'];
	$update_course_status="update enrollment set student_status='".$status."' where enroll_id='".$enroll_id."' ";
	$ptr_course_status= mysql_query($update_course_status);
}
//============================================================================================================
if($action=='update_student_name')
{
	...
	$enroll_id=$_POST['enroll_id'];
	$sel_name="select name from enrollment where enroll_id='".$enroll_id."'";
	$ptr_enrolls=mysql_query($sel_name);
	$data_enrolls=mysql_fetch_array($ptr_enrolls);
	
	$update_course_status="update enrollment set name='".$name."', pre_name='".$data_enrolls['name']."' where (enroll_id='".$enroll_id."' or ref_id='".$enroll_id."')";
	$ptr_course_status= mysql_query($update_course_status);
	
	$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('enrollment','change name','".$name."','".$enroll_id."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
	$query=mysql_query($insert);
}
//============================================================================================================

if($action=='show_course_details')
{
	$enroll_id=$_POST['enroll_id'];
	$sel_quantity = "select * from enrollment where enroll_id='".$enroll_id."' ";
	$execute_quantity = mysql_query($sel_quantity);
	$data_quantity= mysql_fetch_array($execute_quantity);
	?>
	<select id="course_id<?php echo $_POST['id']; ?>" name="course_id<?php echo $_POST['id']; ?>" onChange="c_detail(this.value,<?php echo $_POST['id']; ?>);" style="width:130px;">
	<option value="">Select</option>
    <?php
    $get="SELECT course_id,course_name FROM courses where course_id='".$data_quantity['course_id']."' order by course_id";
	$myQuery=mysql_query($get);
	while($row = mysql_fetch_assoc($myQuery))
	{
		$selected_course='';
...
if($action=='get_staff')
{
	...
	?>
	<tr>
	<td width="15%">Select Staff<span class="orange_font">*</span></td>
    <td width="58%">
    <select id="staff_id" name="staff_id" onChange="getmonthdays(this.value);" >
    <option value="">Select Staff</option>
	<?php
	$sel_staff = "select admin_id,attendence_id,name from site_setting where 1 AND attendence_id!='' AND branch_name='".$_POST['branch_name']."' order by attendence_id asc";	 
...
if($action=='get_staff_for_staff_leave')
{
	$branch_name=$_POST['branch_name'];
	?>
	<tr>
	<td width="15%">Select Staff<span class="orange_font">*</span></td>
    <td width="58%">
    <select id="staff_id" name="staff_id" onChange="getmonthdays(this.value);gettotaldays(this.value);" >
    <option value="">Select Staff</option>
	<?php								
	$sel_staff = "select admin_id,attendence_id,name from site_setting where 1 and system_status='Enabled' and attendence_id IS NOT NULL AND branch_name='".$_POST['branch_name']."' order by name asc";	 
	$query_staff = mysql_query($sel_staff);
	if($total_staff=mysql_num_rows($query_staff))
	{
		while($data=mysql_fetch_array($query_staff))
...
if($action=='get_staff_for_advance_deduction')
{
	$branch_name=$_POST['branch_name'];
	?>
	<tr>
   	<td width="30%">Select Staff<span class="orange_font">*</span></td>
    <td width="58%">
    <select id="staff_id" name="staff_id" onChange="getmonthdays(this.value);gettotaldays(this.value);" >
    <option value="">Select Staff</option>
	<?php
	$sel_staff = "select admin_id,attendence_id,name from site_setting where 1 and system_status='Enabled' and attendence_id!='' AND branch_name='".$_POST['branch_name']."' order by name asc";	 
	$query_staff = mysql_query($sel_staff);
	if($total_staff=mysql_num_rows($query_staff))
	{
		while($data=mysql_fetch_array($query_staff))
...
if($action=='get_staff_for_line')
{
	$branch_name=$_POST['branch_name'];
	?>
	<tr>
	<td width="10%">Select Staff<span class="orange_font">*</span></td>
	<td width="58%">
...
if($action=='c_detail')
{
	$enroll_id=$_POST['enroll_id'];
	$course_id=$_POST['course_id'];
	$sel_quantity = "select * from enrollment where enroll_id='".$enroll_id."' AND course_id='".$course_id."' ";
	$execute_quantity = mysql_query($sel_quantity);
	$data_quantity= mysql_fetch_array($execute_quantity);
	...
	echo trim($data_quantity['net_fees'].">".$course['course_name'].">".$data_quantity['cgst_tax_in_percent'].">".$data_quantity['cgst_tax'].">".$data_quantity['sgst_tax_in_percent'].">".$data_quantity['sgst_tax'].">".$data_quantity['igst_tax_in_percent'].">".$data_quantity['igst_tax']);
}
...
//==========================================================================================================================================================
			if($action=='show_product_estimation')
			{
				...
				$igst_per=0;
				$service_per=0;
				$select_product="select cgst_tax_in_per,cgst_tax,sgst_tax_in_per,sgst_tax,igst_tax_in_per,igst_tax,sin_product_total,sin_product_price,service_tax_in_per,service_tax from inventory_product_map where product_id='".$product_id."' order by map_id desc limit 0,1";
				...
				if(mysql_num_rows($ptr_product))
				{
					...
					$service_per=$data_product['service_tax_in_per'];
					...
					else if($service_per >0 )
					{
						$totalgst=intval($service_per);
						$new_total_tax=floatval(($totalgst+100)/100);
						//"\n".$product_price = floatval($product_price / $new_total_tax);
					}
				}
				echo trim($product_qty."-".$product_price."-".$cgst_per."-".$sgst_per."-".$igst_per."-".$service_per);
			}
//=========================================================================================================================================================
...
if($firstname !='' && $lastname !='' && $email_id !='' && $cm_id !='' &&  $course !='')
{
	...
	$insert= "INSERT INTO inquiry (`firstname`,`middlename`,`lastname`,`birth_date`,`gender`,`maritalstatus`,`address`,`mobile1`,`mobile2`,`landline_no`,`email_id`,`education`,`course_interested`,`course_id`,`total_fees`,`enquiry_date`,`batch`,`enquiry_source`,`enquiry_taken`,`inquiry_for`,`added_date`,`cm_id`,`enquiry_from`) 
	...
	$insert_regi = "INSERT INTO `stud_regi` (`admin_id`,`cm_id`, `name`,`username`,`pass`,  `dob`, `address`, `contact`, `mail`, `qualification`,  `added_date`, `status` ,`not_status`)
	...
	if($first_installment !=0)
	{
		...
		if($sep_instl[$i] !='')
		{
			...
			$insert_query = "  insert into installment(enroll_id, course_id, installment_amount, installment_date, status,installment_display_id, invoice_id,cm_id) values('$student_id ','".$course."','".$sep_instl[$i]."','$installment_date','not paid','".$comma_separated."',$student_id_in,'".$cm_id."') ";
