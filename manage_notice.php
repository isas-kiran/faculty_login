    ...
    <?php if($_POST['formAction'])
                    {
                        if($_POST['formAction']=="delete")
                        {
                            for($r=0;$r<count($_POST['chkRecords']);$r++)
                            {
                                ...
                                $sql_query= "SELECT notice_id FROM ".$GLOBALS["pre_db"]."notice where notice_id ='".$del_record_id."'";
                                if(mysql_num_rows($db->query($sql_query)))
                                    {                                                
                                        $delete_query="delete from ".$GLOBALS["pre_db"]."notice where notice_id='".$del_record_id."'";
                    ...
                    if($_REQUEST['deleteRecord'] && $_REQUEST['record_id'])
                    {
                           ...
                           else if($_POST['formAction']=="Active")
                        {
                            for($r=0;$r<count($_POST['chkRecords']);$r++)
                            {
                                ...
                                $update_record= "update ".$GLOBALS["pre_db"]."notice set status='Active' where notice_id='".$update_record_id."'";
                        ...
                        else if($_POST['formAction']=="Inactive")
                        {
                            ...
                            {
                                $update_record_id=$_POST['chkRecords'][$r];
                                $update_record= "update ".$GLOBALS["pre_db"]."notice set status='Inactive' where notice_id='".$update_record_id."'";
                                $db->query($update_record);
                            }
                        ...
                        else if($_POST['formAction']=="Banned")
                        {
                            ...
                            {
                                $update_record_id=$_POST['chkRecords'][$r];
                                $update_record= "update ".$GLOBALS["pre_db"]."notice set status='Banned' where notice_id='".$update_record_id."'";
                                $db->query($update_record);
                            }
                    ...
                    {
                        $update_query1="update ".$GLOBALS["pre_db"]."notice set status='".$_REQUEST['value']."' where notice_id='".$_REQUEST['changeStatus']."'";
                        //echo $update_query1;
...
<table cellspacing="0" cellpadding="0" class="table" width="95%">
  <tr class="head_td">
    ...
    <?php
                        ...
                        if($_GET['order'] !='' && ($_GET['orderby']=='title'))
                        {
                            ...
                            $select_directory='order by notice_id desc';                      
                            $sql_query= "SELECT * FROM  notice where 1  ".$pre_keyword."  ".$_SESSION['where_admin_id']." ".$select_directory.""; 
                       //echo $sql_query;
  ...
  <tr class="grey_td" >
    ...
    <td width="15%"><a href="<?php echo $_SERVER['PHP_SELF'];?>?order=<?php echo $order."&orderby=name".$query_string;?>" class="table_font"><strong>Notice Title</strong></a> <?php echo $img1;?></td>
 
    <td width="20%"><strong>Discription </strong></td>
       <td width="15%"><strong>Staus  </strong></td>
    ...
    <td width="10%" class="centerAlign"><strong>Action</strong></td>
                            ...
                            while($val_query=mysql_fetch_array($all_records))
                            {
                                ...
                                $listed_record_id=$val_query['notice_id']; 
                                ...
                                echo '<td >'.$val_query['title'].'</td>';
                                 echo '<td >'.$val_query['discription'].'</td>';
					...
					echo '</td>';
							    ...
							    echo '<td >'.$val_query['end_date'].'</td>';
                              
                                echo '<td align="center"><a href="add_notice.php?record_id='.$listed_record_id.'" ><img src="images/edit_icon.png" title="Edit Record" class="example-fade"/></a>&nbsp;&nbsp;
