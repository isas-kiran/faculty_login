...
<title>Refund Outgoing GST</title>
...
?>   
    ...
    <td><a href="refund_outgoing_gst_export.php<?php echo $sep_url_string; ?>"><img src="images/excel.png" height="30px" width="30px"/></a></td>  
</tr>          
</table>
</form>	
    </td>
     ...
     <?php
						...
						{                            
							...
							$pre_keyword_i =" and (name like '%".$keyword."%' )";
						}                            
                        else
						{
                            $pre_keyword="";
						...
						if($gst_keyword)
						{
							if($gst_keyword=="with_gst")
							{
								$where_gst =' and cust_gst_no !=""';
								$where_gst_i =' and cust_gst_no !=""';
							}
							else if($gst_keyword=="without_gst")
							{
								$where_gst =' and cust_gst_no=""';
								$where_gst_i =' and (cust_gst_no="" or cust_gst_no is null)';
							}                            
							else
							{
								$where_gst="";
                        ...
                        if($_GET['order'] !='' && ($_GET['orderby']=='firstname'))
                        {
							...
							$select_directory='order by refund_id desc';                     
							$where_cm='';
							if($_SESSION['where']!='')
							{
							...
							$branch_id='';
                       		...
                       		"<br/>".$sql_query= "SELECT refund_id,enroll_id,added_date FROM enrollment_refund WHERE 1 ".$where_cm." ".$search_cm_id." ".$where_gst." ".$pre_keyword." ".$paid_from_date." ".$paid_to_date." and (cgst_tax_in_per >0 or sgst_tax_in_per >0) ".$select_directory." "; 
				...
				<form method="post"  name="frmTakeAction"  action="<?php echo $_SERVER['PHP_SELF'].'?#msgbox';?>">
                     <input type="hidden" name="formAction" id="formAction" value=""/>
                      <tr class="grey_td" >
						...
						<table width="100%">
						<tr><td colspan="4" align="center"><strong>Details</strong></td></tr>
						<tr><td width="40%" align="center">Refund Type</td><td width="10%" align="center">Price (Excl. GST)</td><td width="25%" align="center">GST in %</td><td width="25%" align="center">GST in value</td></tr>
						</table>
                        ...
                        <td width="14%" align="center"><strong>Installments</strong></td>-->
                            ...
                            while($val_query=mysql_fetch_array($all_records))
                            {
								if($bgColorCounter%2==0)
                                    ...
                                    $bgcolor=""; 
									$listed_record_id=$val_query['refund_id'];
                                include "include/paging_script.php";
                                echo '<tr '.$bgcolor.'>';
                                
								$sel_name="select name,cust_gst_no from enrollment where enroll_id='".$val_query['enroll_id']."'";
								...
								"<br/>".$sel_inst_amnt="select * from enrollment_refund where 1 and refund_id='".$val_query['refund_id']."' and (cgst_tax_in_per >0 or sgst_tax_in_per >0)";
								...
								if($total_inst=mysql_num_rows($ptr_ins_amnt))
								{
									...
									{
										echo "<tr>";
										/* $select_prod_name =" SELECT expense_name FROM product where product_id ='".$data_inst_amnt['product_id']."' ";
										$ptr_prod_name= mysql_query($select_prod_name);
										$data_product_name =mysql_fetch_array($ptr_prod_name); */
										echo "<td width='40%' align='center'>Enrollment Refund</td>";
										/* if($data_inst_amnt['discounted_price'] >0)
											echo "<td width='10%' align='center'>".$data_inst_amnt['discounted_price']."</td>";
										else */
										echo "<td width='10%' align='center'>".$data_inst_amnt['total_refund']."</td>";
