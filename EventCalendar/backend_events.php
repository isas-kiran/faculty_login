...
$stmt = $db->prepare('SELECT * FROM customer_service_map WHERE 1 and start_event_time!="" and end_event_time!="" and start_event_time is NOT NULL and end_event_time is NOT NULL and DATE(added_date) >= "'.date('Y-m-d', strtotime('-30 days')).'" and customer_service_id > 0 and start_event_time!="NaN-NaN-NaN NaN:NaN:NaN" and end_event_time!="NaN-NaN-NaN NaN:NaN:NaN" order by customer_service_map_id desc ' );///NOT ((end_event_time <= :start_event_time) OR (start_event_time >= :end_event_time))
...
foreach($result1 as $row1) {
	  ...
	  $e->cust_id = $row['customer_service_id'];
}
