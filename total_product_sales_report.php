...
<title>Total Product Sales Report</title>
...
<?php
$edit_access='';
$sel_acc="select * from edit_previleges where admin_id='".$_SESSION['admin_id']."' and privilege_id='316'";
						...
						<tr class="head_td">
                            ...
                            if($diff)
                            {
                                ...
                                <tr>
                                    ...
                                    <td colspan="2" style="font-size:12px"><strong>To Date- <?php echo $to_dates; ?> <span style="text-align:right; float:right; color:green">Product details format - Customer Name / Sales ID / Invoice ID / Amount / Payment Type</span></strong></td>
                                ...
                                for($i=0;$i<$data_difs;$i++)
                                {
                                    ...
                                    echo '<td align="left">';
                                    $sel_inv="select * from sales_product_invoice where DATE(added_date)='".$newdates."' and payable_amount>0 ".$paid_type." ".$bank_ids." ".$search_cm_id." ";
                                    $ptr_inv=mysql_query($sel_inv);
                                    while($data_inv=mysql_fetch_array($ptr_inv))
                                    {
										$total_ded_value=0;
                                        $sel_prod="select * from sales_product where sales_product_id='".$data_inv['sales_product_id']."'";
										...
										echo "<span style='color:blue'>".$name." - ".$data_inv['sales_product_id']." / ".$data_inv['invoice_id']." - <strong>".$tot_payble."</span></strong> - <span style='color:brown'>".$pay_type['payment_mode']."</span><br/>";
                                        $tot_inv +=$tot_payble;
                                    }
                                    /*if($tot_inv >0)
