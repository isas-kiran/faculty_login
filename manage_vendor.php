...
<?php
$edit_access='';
$sel_acc="select * from edit_previleges where admin_id='".$_SESSION['admin_id']."' and privilege_id='139'";
    ...
    <?php if($_POST['formAction'])
                    {
                        if($_POST['formAction']=="delete")
                        {
                            for($r=0;$r<count($_POST['chkRecords']);$r++)
                            {
                                ...
                                $sql_query= "SELECT vendor_id FROM ".$GLOBALS["pre_db"]."vendor where vendor_id ='".$del_record_id."'";
                                if(mysql_num_rows($db->query($sql_query)))
                                    {                                                
                                        $delete_query="delete from ".$GLOBALS["pre_db"]."vendor where vendor_id='".$del_record_id."'";
	...
	<table border="0" cellspacing="0" cellpadding="0" width="99%" align="center">
              ...
              <td class="width5"></td>
                ...
                <td><input type="image" src="images/search.jpg" name="search" value="Search" title="Search" class="example-fade"  /></td>
                        ...
                        if($keyword)
                            {                            
                                $pre_keyword=" and (name like '%".$keyword."%' or email like '%".$keyword."%' or contact like '%".$keyword."%' or address like '%".$keyword."%' or detail like '%".$keyword."%')";
                            }                            
                        else
                            $pre_keyword="";
                        ...
                        if($_GET['order'] !='' && ($_GET['orderby']=='name'))
                        {
                         ...
                         $sql_query= "SELECT * FROM vendor where 1 ".$_SESSION['where']."  ".$pre_keyword." ".$select_directory.""; 
                       //echo $sql_query;
                ...
                <td width="20%"><a href="<?php echo $_SERVER['PHP_SELF'];?>?order=<?php echo $order."&orderby=name".$query_string;?>" class="table_font"><strong>Vendor Name</strong></a> <?php echo $img1;?></td>
                ...
                <td width="7%"><strong>Wallet Amount</strong></td>
                <?php
                if( $_SESSION['type'] =='S' || $edit_access=='yes')
                {	
                    ?>
                ...
                while($val_query=mysql_fetch_array($all_records))
                {
                    ...
                    */
                    ...
                    echo '<td ><a href="add_vendor.php?record_id='.$listed_record_id.'" class="table_font">'.$val_query['name'].'</a></td>';
                    echo '<td >'.$val_query['contact'].'</td>';
                    echo '<td >'.$val_query['address'].'</td>';
                    echo '<td >'.$val_query['email'].'</td>';
                    echo '<td >'.$val_query['detail'].'</td>'; 
                    echo '<td >'.$val_query['wallet_balance'].'</td>'; 
                    if($_SESSION['type'] =='S' || $edit_access=='yes')
                    {	
                        echo '<td align="center"><a href="add_vendor.php?record_id='.$listed_record_id.'" ><img src="images/edit_icon.png" title="Edit Record" class="example-fade"/></a>&nbsp;&nbsp;
                          ...
                          <a title="Advance Payment" href="vendor_outstanding.php?record_id='.$listed_record_id.'"><img src="images/add1-.png" title="Advance Payment" border="0"></a>';
                        echo '</td>';
                    }
                    echo '</tr>';
                    $bgColorCounter++;
