...
function validme()
{
		...
		{
			 disp_error +='Enter Assigned Name  \n';
  ...
  <tr>
    ...
    <td class="mid_mid">
		...
		if($_POST['save_changes'])
		{
			...
			$brand=$_POST['brand'];
			$employee_id=(($_POST['employee_id'])) ? $_POST['employee_id'] : $_SESSION['admin_id'];
			...
			if($record_id=='')
			{
				  $sel_product="select product_name from product where product_name ='".$product_name."' and cm_id='".$cm_id."'";
				  ...
				  $sel_product_code="select product_code from product where product_code ='".$product_code."' and cm_id='".$cm_id."' ";
			...
			else
			{
				...
				$data_record['admin_id']=$employee_id;					
				if($record_id)
				{
				...
				else
				{
					...
					$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('add_product','Add','".$product_name."','".$product_id."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
			...
			if($success==0)
			{
					...
					?>
					  <tr>
						...
						<td>
							...
							echo ' <select id="branch_name" name="branch_name" onchange="select_data(this.value)">';
            	...
            	<tr>
                    ...
                    <td width="70%" id="pcat_ids">
                        <select name="pcategory_id" style="width:200px;" id="pcategory_id" onchange="show_subcategory(this.value,'');" >
                        <option value="">Select Category</option> 
                          <?php  
						  if($record_id!='')
						  {
							  $sql_dest ="select pcategory_name, pcategory_id from product_category where 1 ".$_SESSION['where']." order by pcategory_id asc";//".$_SESSION['user_id']."
            	...
            	<tr>
                	...
                	<td width="70%">
                	<input type="radio" name="type"  value="powder" <?php if($row_record['type']=='powder') echo 'checked="checked"'; ?> >Powder
                	<input type="radio" name="type" value="liquid" <?php if($row_record['type']=='liquid') echo 'checked="checked"'; ?> >Liquid
                	<input type="radio" name="type" value="creamy" <?php if($row_record['type']=='creamy') echo 'checked="checked"'; ?> >Creamy
                	</td> 
                	<td width="10%"></td>
           		</tr>
                
                <tr>
	               	<td width="20%" valign="top">Assigned to </td>
    	          	<td width="70%" id="emp_ids">
...
<!--footer end-->
<script language="javascript">
function select_data(branch_id)
{
	var bank_data="action=stockiest&branch_id="+branch_id+"&admin_id="+<?php echo $_SESSION['admin_id']; ?>;
	//alert(bank_data);
	$.ajax({
	...
	var vend_data="action=product_vendor&branch_id="+branch_id+"&admin_id="+<?php echo $_SESSION['admin_id']; ?>;
	//alert(vend_data);
	$.ajax({
	url: "show_councellor.php",type:"post", data: vend_data,cache: false,
	success: function(vends)
	{
		//alert(vends);
		document.getElementById("vendor_idst").innerHTML=vends;
		//$("#employee_id").chosen({allow_single_deselect:true});
		$("#vendor_id").multiselect().multiselectfilter();
	}
	}); 
	
	var data1="action=show_category&branch_id="+branch_id;
		$.ajax({
		url: "show_prod_category.php", type: "post", data: data1, cache: false,
		success: function (html)
		{
			//alert(html);
			//$("#pcategory_id").chosen({allow_single_deselect:false});
			document.getElementById('pcat_ids').innerHTML=html;
			$("#pcategory_id").chosen({allow_single_deselect:true});
		}
	});
}
