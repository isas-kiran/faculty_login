...
{		//alert('hi');						  
	...
	//alert(data1);
	$.ajax({
		url:'http://www.htdpt.in/tracking/send_email.php',dataType: "jsonp",type:"post",data:data1,cache:false,crossDomain:true, async:false,
...
<tr bgcolor="#AFD8E0">
  ...
  //===================Total receive balance===============================
	...
	/*$sel_total_inc2="select SUM(amount) as total_amt from receipt where added_date='".date('Y-m-d')."' and category !='cash_transfer' and category !='voucher' ".$branch_id." ";
...
$message.=' <td align="left" colspan="10" style="color:#F00"><strong>Total Incomming</strong> :'.$total_todays_bal.'</td> 
		...
		$sql_query_inv= "SELECT * FROM invoice where DATE(added_date)='".date('Y-m-d')."' ".$branch_id." ".$select_directory1." "; 
...
$sel_total_servicec="select SUM(payable_amount) as total_amt from customer_service where payment_mode_id!='0' and DATE(`added_date`) = '".date('Y-m-d')."' ".$branch_id."";
$ptr_total_service=mysql_query($sel_total_servicec);
$data_total_service=mysql_fetch_array($ptr_total_service);

$sql_customer= "SELECT price FROM customer where DATE(`added_date`)='".date('Y-m-d')."' and membership ='yes' ".$branch_id." "; 
...
$message.=' <td align="left" colspan="10" style="color:#F00"><strong>Total Incomming</strong> :'.$total_service_amount.'</td> </tr>';
						...
						$sql_query= "SELECT * FROM customer_service_invoice where paid_type!='0' and DATE(`added_date`)='".date('Y-m-d')."' ".$branch_id." ".$select_directory." "; 
                      	//echo $sql_query;
                        $no_of_records=mysql_num_rows($db->query($sql_query));
						$message.=' <tr class="grey_td" style="background-color:#999;color: black">
						<td width="5%" align="center" style="border:1px solid #CCC"><strong>Sr. No.</strong></td>
						...
						$sql_query= "SELECT * FROM customer where DATE(`added_date`)='".date('Y-m-d')."' and membership ='yes'  ".$branch_id." ".$select_directory1." "; 
                      	//echo $sql_query;
                        $no_of_records=mysql_num_rows($db->query($sql_query));
						$bgColorCounter=1;
						//$_SESSION['show_records'] = 10;
...
//===================Total receive balance=====================================================================================================
$sel_total_product="select SUM(payable_amount) as total_amt from sales_product where DATE(`added_date`) = '".date('Y-m-d')."' ".$branch_id."";
...
$message.=' <td align="left" colspan="10" style="color:#F00"><strong>Total Incomming</strong> :'.$total_product_amount.'</td> </tr>';
						...
						$sql_query= "SELECT * FROM sales_product_invoice where DATE(`added_date`)='".date('Y-m-d')."' ".$branch_id." ".$select_directory." "; 
                      	//echo $sql_query;
                        $no_of_records=mysql_num_rows($db->query($sql_query));
						$message.=' <tr class="grey_td" style="background-color:#999;color: black">
						<td width="5%" align="center" style="border:1px solid #CCC"><strong>Sr. No.</strong></td>
...
//===================Total receive balance=====================================================================================================
$sel_sales_pack="select SUM(amount) as total_amt from sales_package_voucher_memb where membership_id='0' and DATE(`added_date`) = '".date('Y-m-d')."' ".$branch_id."";
...
$sql_query= "SELECT * FROM sales_package_voucher_memb where membership_id='0' and  DATE(`added_date`)='".date('Y-m-d')."' ".$branch_id." ".$select_directory.""; 
//echo $sql_query;
$no_of_records=mysql_num_rows($db->query($sql_query));
$message.=' <tr class="grey_td" style="background-color:#999;color: black">
<td width="5%" align="center" style="border:1px solid #CCC"><strong>Sr. No.</strong></td>
...
$sele_recep="select receipt_id,category,payment_mode_id,bank_id,amount,added_date from receipt where added_date='".date('Y-m-d')."' ".$branch_id."";
...
<tr>
 <td align="center" colspan="10">&nbsp;</td>
  ...
  <tr>';
  	...
  	$select_inv="select SUM(payable_amount) as total from inventory where DATE(added_date)='".date('Y-m-d')."' ".$branch_id." ";
...
$message.='  <td align="left" colspan="10" style="color:#F00"><strong>Total Outgoing</strong> :'.$total.'</td> </tr>';					
 ...
 <tr style="background-color:#999;color: black">
	...
	$sele_exp="SELECT * FROM inventory_invoice where DATE(`added_date`)='".date('Y-m-d')."' ".$branch_id." order by inventory_id asc";
	$ptr_exp=mysql_query($sele_exp);
	$i=1;
	if(mysql_num_rows($ptr_exp))
	{
	...
	$cm_idC="and C.cm_id= ".$cm_id1."";
		...
		if($total_bank=mysql_num_rows($ptr_amnt1))
		{
			...
			while($data_ptr_sel1=mysql_fetch_array($ptr_amnt1))
			{
				...
				$sel_inv="select SUM(payable_amount) as total_amt from inventory_invoice where bank_id='".$data_ptr_sel1['bank_name']."' and DATE(added_date)='".date('Y-m-d')."' ".$branch_id."";
				...
				$sel_total_inc3="select SUM(payable_amount) as total_amt from customer_service_invoice where bank_id='".$data_ptr_sel1['bank_name']."'  and DATE(added_date)='".date('Y-m-d')."' ".$branch_id."";
				$ptr_total_inc3=mysql_query($sel_total_inc3);
				$data_total_inc3=mysql_fetch_array($ptr_total_inc3);
				
				$sel_total_inc4="select SUM(payable_amount) as total_amt from sales_product_invoice where bank_id='".$data_ptr_sel1['bank_name']."'  and DATE(added_date)='".date('Y-m-d')."' ".$branch_id."";
				$ptr_total_inc4=mysql_query($sel_total_inc4);
				$data_total_inc4=mysql_fetch_array($ptr_total_inc4);
				
				$sel_total_inc5="select SUM(amount) as total_amt from sales_package_voucher_memb where bank_id='".$data_ptr_sel1['bank_name']."'  and DATE(added_date)='".date('Y-m-d')."' ".$branch_id."";
...
$message.='  </table>
 ...
 <tr style="background-color:#999;color: black">
    ...
    // <td width="15%" align="center" style="border:1px solid #CCC"><strong>Cash Received from Innocent</strong></td>
				...
				$sel_receipt="select SUM(amount) as total_amt from receipt where payment_mode_id='1' and category !='cash_transfer' and category !='innocent' and category !='santosh' and category !='voucher' and added_date='".date('Y-m-d')."' ".$branch_id."";
				$ptr_bank_id=mysql_query($sel_receipt);
				$data_sel_recipt=mysql_fetch_array($ptr_bank_id);
				...
				//===================Total Cash Expenses===============================
				$sel_total_expense="select SUM(amount) as total_amt from expense where payment_mode_id ='1' and expense_type_id !='1' and added_date='".date('Y-m-d')."' ".$branch_id."";
				$ptr_total_expense=mysql_query($sel_total_expense);
				$data_total_expense=mysql_fetch_array($ptr_total_expense);
				//============================================================================
				
				//===================Cash Given to sir===============================
				$sel_cash_from_sir="select SUM(amount) as total_amt from expense where expense_type_id='1' and payment_mode_id='1' and added_date='".date('Y-m-d')."' ".$branch_id."";
				...
				//===================Cash in Hand===============================
				$sel_cash_from_cash="select SUM(amount) as total_amt from receipt where category !='cash_transfer' and category !='santosh' and category !='voucher' and payment_mode_id='1' and added_date='".date('Y-m-d')."' ".$branch_id."";
				$ptr_cash_from_cash=mysql_query($sel_cash_from_cash);
				$data_cash_from_cash=mysql_fetch_array($ptr_cash_from_cash);
				//=============================SERVICE===============================================
				$sel_service="select SUM(payable_amount) as total_amt from customer_service where payment_mode_id='1' and DATE(added_date)='".date('Y-m-d')."' ".$branch_id."";
				$ptr_service=mysql_query($sel_service);
				$data_service=mysql_fetch_array($ptr_service);
				
				$sel_memb="select SUM(price) as total_amt from customer where payment_mode_id='1' and DATE(added_date)='".date('Y-m-d')."' ".$branch_id."";
				...
				//============================================================================
				//=============================PRODUCT===============================================
				$sel_product="select SUM(payable_amount) as total_amt from inventory_invoice where paid_type='1' and DATE(added_date)='".date('Y-m-d')."' ".$branch_id."";
				$ptr_product=mysql_query($sel_product);
				$data_product=mysql_fetch_array($ptr_product);
				
				$sel_sales_product="select SUM(payable_amount) as total_amt from sales_product_invoice where paid_type='1' and DATE(added_date)='".date('Y-m-d')."' ".$branch_id."";
				...
				$sel_sales_membership="select SUM(price) as total_amt from customer where payment_mode_id='1' and membership='yes' and DATE(added_date)='".date('Y-m-d')."' ".$branch_id."";
				$ptr_sales_membership=mysql_query($sel_sales_membership);
				$data_sales_membership=mysql_fetch_array($ptr_sales_membership);
				
				$sel_sales_package="select SUM(amount) as total_amt from sales_package_voucher_memb where payment_mode_id='1' and category='Package' and DATE(added_date)='".date('Y-m-d')."' ".$branch_id."";
				$ptr_sales_package=mysql_query($sel_sales_package);
				$data_sales_package=mysql_fetch_array($ptr_sales_package);
				
				$sel_sales_voucher="select SUM(amount) as total_amt from sales_package_voucher_memb where payment_mode_id='1' and category='Voucher' and DATE(added_date)='".date('Y-m-d')."' ".$branch_id."";
				...
				$sel_dist_bank_id="Select DISTINCT(bank_id) from receipt where added_date='".date('Y-m-d')."' and bank_id !='' and category !='cash_transfer' and category !='voucher' ".$branch_id."";
				...
				{
					$sel_cash_from_bank="select SUM(amount) as total_amt from receipt where bank_id ='".$data_dist_bank_id['bank_id']."' and category !='cash_transfer' and category !='voucher' and added_date='".date('Y-m-d')."' ".$branch_id." ";
				...
				$sel_total_inc21="select SUM(amount) as total_amt from receipt where category !='cash_transfer' and category !='voucher' and added_date='".date('Y-m-d')."' and bank_id='' ".$branch_id."";
...
$message.='   <tr>
  ...
  $message.='</table>';
						...
						try {
							...
							$mail->setFrom('vyavaharekiran@gmail.com', 'ISAS');
							
							$mail->addAddress("kiran.vyavahare@waakan.com"); 
							...
							$mail->Subject = 'DSR report on- '.date('d/m/Y').'';
