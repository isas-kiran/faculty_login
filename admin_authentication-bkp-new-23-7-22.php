...
$select_acitve_logins ="select last_login_time, admin_id from site_setting where admin_id='".$_SESSION['admin_id']."' and branch_name='".$_SESSION['branch_name']."'";
...
if(mysql_num_rows($ptr_activr))
{
	...
	while($data_active = mysql_fetch_array($ptr_activr))
	{
		...
		if($time_diff>7200)
		{
			...
			{*/
				...
				if($data_active['admin_id']==$_SESSION['admin_id'])
				{
					...
					$msg=' System Timeout. Please login again.';
				}
			//}
		}
		else
		{
			...
			$my_query_status="UPDATE site_setting SET last_login_time = '".date('Y-m-d H:i:s')."' WHERE admin_id='".$_SESSION['admin_id']."' and branch_name='".$_SESSION['branch_name']."'";
			$ptr_query_status=mysql_query($my_query_status);
		}
	}
...
{
	$found_active_login ='y';
	$msg='Your trying to login both system at a time (ISAS LLP & ISAS Pvt Ltd.). Please logout first system and then login second system';
}
