...
<title>Add Expense Payment</title>
...
"<br/>".$sel_mail_text="select email_text from previleges where privilege_id='147'";
...
{								  
	...
	// alert(discount);
	var total_cost =document.getElementById('total_amnt_paid').value;
	// alert(total_cost);
	...
	data1='action=add_sale&branch_name='+branch_name+'&invoice_no='+invoice_no+'&name='+name+'&price='+price+'&discount='+discount+'&total_cost='+total_cost
	+'&total_amnt_paid='+total_amnt_paid+'&balance_amount='+balance_amount+'&amount_paid='+amount_paid+
	'&payment_type='+payment_type+'&cust_bank_name='+cust_bank_name+'&bank_name='+bank_name+'&account_no='+account_no+'&chaque_no='+chaque_no+
	'&chaque_date='+chaque_date+'&credit_card_no='+credit_card_no+'&remaining_amount='+remaining_amount;
	//alert(data1);
...
<table border="0" cellspacing="0" cellpadding="0" width="100%">
  ...
  <tr>
    ...
    <td class="mid_mid">
		<?php
        $sel_sale_pro="select * from expense where expense_id='".$record_id."'";
        $quer_sale_prod=mysql_query($sel_sale_pro);
        $fetch_query_sales=mysql_fetch_array($quer_sale_prod);
    
        $select_customer="select * from vendor where vendor_id='".$fetch_query_sales['vendor_id']."' ";
        ...
        if($_POST['save_changes'])
        {
            ...
            if($record_id)
            {
                '<br/>'.$update_sales_prod="update expense set remaining_amount='".$_POST['remaining_amount']."', payable_amount=(payable_amount + ".$payable_amount.") where expense_id='".$sales_product_id."'";
                ...
                $sel_recpt="select receipt_no from expense_invoice where cm_id='".$cm_id1."' and receipt_no IS NOT NULL order by invoice_id desc limit 0,1";
            ...
            $insert_sales_invoice = "INSERT INTO `expense_invoice` (`expense_id`,`receipt_no`,`total_price`, `payable_amount`,`remaining_amount`, `paid_type`, `cust_bank_name`,`bank_id`,`cheque_detail`, `chaque_date`, `credit_card_no`, `admin_id`, `added_date`,`status`,`cm_id`,`total_paid`,`invoice_due_date`) VALUES ('".$record_id."','".$receipt_no."', '".$_POST['total_price']."', '".$payable_amount."','".$_POST['remaining_amount']."', '".$payment_type."', '".$cust_bank_name."', '".$bank_name."', '".$_POST['chaque_no']."', '".$sep_check_date."','".$credit_card_no."', '".$_SESSION['admin_id']."', '".$added_date."','".$status."','".$cm_id1."','".$total_amnt_paid."','".$invoice_due_date."'); ";
            $ptr_sales_invoice = mysql_query($insert_sales_invoice);								
            $invoice_id = mysql_insert_id();
            if($payment_type=='2' || $payment_type=='4' || $payment_type=='5')
            {
                $bank="INSERT INTO `bank_records`(`bank_id`, `type`, `record_id`, `invoice_id`, `amount`, `added_date`, `cm_id`, `admin_id`) VALUES ('".$bank_name."','sales product','".$record_id."','".$invoice_id."','".$payable_amount."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
            ...
            $sel_inq="select sms_text from previleges where privilege_id='147'";
            ...
            $sel_sms_cnt="select * from sms_mail_configuration_map where previlege_id='147' ".$_SESSION['where']."";
            $ptr_sel_sms=mysql_query($sel_sms_cnt);
            while($data_sel_cnt=mysql_fetch_array($ptr_sel_sms))
            {
            ...
            //------send notification on Sale voucher--------------------
                ...
                $notification_args['on_action'] = 'product_sale_payment';
            ...
            else
            {
                ...
                <script type="text/javascript">
                    ...
                    setTimeout('document.location.href="expense_payment_details.php?record_id=<?php echo $record_id; ?>";',1000);
                </script>
                <?php
            }
        }
    ...
    <?php
    $sql_records= "SELECT * FROM expense_invoice where expense_id=".$record_id." order by invoice_id desc limit 0,1 ";
        ...
        while($val_record=mysql_fetch_array($all_records))
        {
           ...
           ?>
              <tr>
                ...
                <td width="49%"><?php if($_POST['invoice_due_date']) echo $_POST['invoice_due_date']; else if($row_record['invoice_due_date']!=''){$arrage_date= explode('-',$row_record['invoice_due_date'],3);echo $arrage_date[2].'/'.$arrage_date[1].'/'.$arrage_date[0];}else{echo date("d/m/Y");}?>
                <input type="hidden" id="invoice_due_date" name="invoice_due_date" class="input_text datepicker" value="<?php if($_POST['invoice_due_date']) echo $_POST['invoice_due_date']; else if($row_record['invoice_due_date']!=''){$arrage_date= explode('-',$row_record['invoice_due_date'],3);echo $arrage_date[2].'/'.$arrage_date[1].'/'.$arrage_date[0];}else{echo date("d/m/Y");}?>" />
                </td>
            </tr>
            ...
            //echo '<td align="center">'.$sr_no.'</td>';	?><tr>
                ...
                <input type="hidden" id="added_date" class="input_text datepicker" name="added_date" value="<?php if($_POST['added_date']) echo $_POST['added_date']; else if($row_record['added_date']!=''){$arrage_date= explode('-',$row_record['added_date'],3);echo $arrage_date[1].'/'.$arrage_date[2].'/'.$arrage_date[0];}else{echo date("d/m/Y");}?>" />
                </td>
            </tr>
           <?php
            echo '<tr class="'.$bgclass.'">';
            ...
            echo '<tr><td><strong>Vendor Name</strong></td><td align="left" style="padding-left:5px;"><b>'.$data_customer['cust_name'].'
            <input type="hidden" name="cm_id_branch" value="'.$data_select['cm_id'].'"/><input type="hidden" name="name" id="name" value="'.$data_customer['cust_name'].'"/><input type="hidden" name="cust_ids" id="cust_ids" value="'.$data_customer['cust_id'].'"/></td></tr>';
            ...
            echo '</td><input type="hidden" name="discount" id="discount" value="'.$fetch_query_sales['discount'].'"/></tr>'; 
            
            $tot_paid_amount="SELECT SUM( payable_amount ) as payable_amount FROM `expense_invoice` WHERE expense_id=".$record_id."";
           ...
           <tr>
            ...
            </tr>            <tr><td width="37%"><strong>Remaining </strong></td><td align="left"  width="63%"> 
