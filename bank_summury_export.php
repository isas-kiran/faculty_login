...
$total_found =0;	
	...
	if($_REQUEST['keyword']!="Keyword")
  	...
  	$sel_for_Num_rows="select i.*, e.name from invoice i, enrollment e where 1 and i.enroll_id=e.enroll_id ".$search_cm_id." ".$pre_keyword_i." ".$pay_type." ".$bank_ids." ".$paid_from_date." ".$paid_to_date." "; 
 ...
 $objPHPExcel->setActiveSheetIndex()->setCellValueByColumnAndRow('3','3', 'Mo No','3');
  ...
  $objPHPExcel->setActiveSheetIndex()->setCellValueByColumnAndRow('13','3', 'Status','3');*/
	...
	$select_directory='order by DATE(i.added_Date) ASC';                         
	 $sql_query= "select i.*, e.name from invoice i, enrollment e where 1 and i.enroll_id=e.enroll_id ".$search_cm_id." ".$pre_keyword_i." ".$pay_type." ".$bank_ids." ".$paid_from_date." ".$paid_to_date." "; 
	...
	while($val_query=mysql_fetch_array($query_order))
	{
		...
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('C'.$rowCount, $data_name['name']);		
