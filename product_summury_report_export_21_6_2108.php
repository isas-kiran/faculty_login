 ...
 $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow('3','3', 'Mo No','3');
  ...
  $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow('13','3', 'Status','3');*/
	...
	while($val_query=mysql_fetch_array($query_order))
	{
		...
		$sel_name="select cust_name from customer where 1 and cust_id='".$val_query['customer_id']."'";
		$ptr_name=mysql_query($sel_name);
		$data_name=mysql_fetch_array($ptr_name);
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('F'.$rowCount, $data_bank['bank_name']."\n Acc. No.- ".$data_bank['account_no']);
		  ...
		  $objPHPExcel->getActiveSheet()->SetCellValue('H'.$rowCount, $val_query['total_price']);		
		...
		$rowCount++;
		$i++;
			$totals_tds +=intval($val_query['total_price']);
	}
...
$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');
ob_get_clean();
$objWriter->save('excel_files/Bank_Product_Summury_Report_'.date('Y-m-d').'.xlsx');
//$objWriter->save('php://output');
ob_end_flush();

?>
<a href="product_summery_report.php">Back</a>

<script>
document.location.href='export_product_summury_report.php';
</script>
