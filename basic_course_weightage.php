...
$edit_access='';
$sel_acc="select * from edit_previleges where admin_id='".$_SESSION['admin_id']."' and privilege_id='324'";
...
<title>Manage Reward Point</title>
...
<table border="0" cellspacing="0" cellpadding="0" width="100%">
  ...
  <tr>
	...
	if($_REQUEST['deleteRecord'])
	{
		...
		$sql_query= "SELECT id FROM weightage_basic_course where id='".$del_record_id."'";
		if(mysql_num_rows($db->query($sql_query)))
		{
			$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('basic_course_weightage','Delete','Weightage Basic course','".$del_record_id."','".date('Yt-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
			$query=mysql_query($insert);			
			$delete_query="delete from weightage_basic_course where id='".$del_record_id."'";
...
function check_voucher(values,bank_id,cm_id)
{
	...
	$sql_records= "SELECT * FROM weightage_basic_course where 1 order by id asc";
	...
	if($no_of_records)
	{
		...
		{ ?><td width="8%"><strong>Center</strong></td><?php }?><td width="11%" class="tr-header"><strong>Course Name</strong></td><td width="11%" class="tr-header"><strong>Theory Weightage</strong></td><td width="11%" class="tr-header"><strong>Practical Weightage</strong></td><?php if($_SESSION['type']=="S" || $edit_access=='yes'){ ?><td width="12%" class="tr-header"><strong>Action</strong></td><?php }?></tr><?php
		while($val_record=mysql_fetch_array($all_records))
		{
			...
			echo "<td align='center'>".$val_record['theory_weightage']."</td>";
			echo "<td align='center'>".$val_record['practical_weightage']."</td>";
    ...
    <tr><td valign="middle" align="right">
		...
		<?php
            ...
            if(isset($_POST['save_changes']))
            {
                ...
                $theory_weightage=$_POST['theory_weightage'];
                $practical_weightage=$_POST['practical_weightage'];
                ...
                if($_POST['theory_weightage'] =="")
                {
                        $success=0;
                        $errors[$i++]="Enter Theory Weightage";
                }
                if($_POST['practical_weightage'] =="")
                {
                        $success=0;
                        $errors[$i++]="Enter Practical Weightage";
                }
                $rec_id="";
                ...
                $sel_rew="select id from weightage_basic_course where cm_id='".$cm_id."' and course_id='".$course_id."' ".$rec_id." ";
                ...
                else
                {
                    ...
                    $data_record['theory_weightage'] = $theory_weightage;
                    $data_record['practical_weightage'] = $practical_weightage;
                    ...
                    if($record_id)
                    {
                        ...
                        {
							$update_bank = "update weightage_basic_course set `course_id`='".$data_record['course_id']."',`theory_weightage`='".$data_record['theory_weightage']."',`practical_weightage`='".$data_record['practical_weightage']."',`added_date`='".date('Y-m-d H:i:s')."',`cm_id`='".$cm_id."' where id='".$record_id."'";
							$ptr_bank= mysql_query($update_bank);
                            
                            $insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('manage_basic_wightage','Edit','".$_POST['bank_name']."','".$record_id."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
                            $query=mysql_query($insert);
							...
							setTimeout('document.location.href="basic_course_weightage.php";',1000);
                            </script>
                        	<?php
                        }
                    }
                    else
                    {
                        if($data_record['course_id'] !='')
                        {
                            $insert_for_prevelgegeis ="insert into weightage_basic_course (`course_id`,`theory_weightage`,`practical_weightage`,`cm_id`,`admin_id`,`added_date`) values('".$data_record['course_id']."','".$data_record['theory_weightage']."','".$data_record['practical_weightage']."','".$cm_id."','".$_SESSION['admin_id']."','".date('Y-m-d H:i:s')."')";
                            $ptr_insert_preilgef = mysql_query($insert_for_prevelgegeis);
                            $ins_id=mysql_insert_id();
                            $insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('manage_basic_wightage','Add','".$_POST['bank_name']."','".$ins_id."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
							$query=mysql_query($insert);
            ...
            <tr>
                <td width="52%" class="tr-header" align="center">Theory Weightage</td>
                 <td width="25%" class="customized_select_box">
                <input type="text" name="theory_weightage" id="theory_weightage" value="<?php if($_POST['theory_weightage']) echo $_POST['theory_weightage']; else echo $row_record['theory_weightage']; ?>" >
                </td>
            </tr>
            <tr>
                <td width="52%" class="tr-header" align="center">Practical Weightage</td>
                <td width="25%" class="customized_select_box">
                <input type="text" name="practical_weightage" id="practical_weightage" value="<?php if($_POST['practical_weightage']) echo $_POST['practical_weightage']; else echo $row_record['practical_weightage']; ?>">
            </td>
            </tr>
            <tr>
