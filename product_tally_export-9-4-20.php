 ...
 $objPHPExcel->setActiveSheetIndex()->setCellValueByColumnAndRow('3','3', 'Mo No','3');
  ...
  $objPHPExcel->setActiveSheetIndex()->setCellValueByColumnAndRow('13','3', 'Status','3');*/
	...
	while($val_query=mysql_fetch_array($query_order))
	{
		...
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('D'.$rowCount, $cust_name);		
		
		//=====================PRODUCT INFORMATION=============
		$sel_prod="select SUM(i.discounted_price * i.product_qty) as total_val, s.total_price from sales_product_map i, sales_product s where 1 and i.sales_product_id='".$val_query['sales_product_id']."' and i.sales_product_id=s.sales_product_id and i.sales_product_price > 0 and s.total_price > 0 group by i.sales_product_id";
		...
		if($tot_prod=mysql_num_rows($ptr_prod))
		{
			while($data_prod=mysql_fetch_array($ptr_prod))
			{
				...
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('E'.$rowCount, 'Product Sales');	
				...
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('H'.$rowCount, round($data_prod['total_val'],2));
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('I'.$rowCount, $disc);
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('J'.$rowCount, round($tot_disc_price,2));
				$rowCount++;
				$total_amnts =round($data_prod['total_val'],2);
				$k++;
			}
		}
		//=========================CGST=======================
		...
		if($tot_cgst=mysql_num_rows($ptr_cgst_prod))
		{
			while($data_cgst=mysql_fetch_array($ptr_cgst_prod))
			{
				...
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('J'.$rowCount, round($data_cgst['cgst_total'],2));
				$rowCount++;
		...
		if($tot_sgst=mysql_num_rows($ptr_sgst_prod))
		{
			while($data_sgst=mysql_fetch_array($ptr_sgst_prod))
			{
				...
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('H'.$rowCount, '');
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('I'.$rowCount, '');
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('J'.$rowCount, round($data_sgst['sgst_total'],2));
				$rowCount++;
		...
		if($tot_igst=mysql_num_rows($ptr_igst_prod))
		{
			while($data_igst=mysql_fetch_array($ptr_igst_prod))
			{
				...
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('H'.$rowCount, '');
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('I'.$rowCount, '');
				$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('J'.$rowCount, round($data_igst['igst_total'],2));
				$rowCount++;
		...
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('H'.$rowCount, '');
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('I'.$rowCount, '');
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('J'.$rowCount, round($total_price,2));
		//==========================================================		
		$k++;
	...
	header('Content-Disposition: attachment;filename="Product Tally Report.xls"');
