...
$edit_access='';
$sel_acc="select * from edit_previleges where admin_id='".$_SESSION['admin_id']."' and privilege_id='105'";
...
$edit_access1='';
$sel_acc1="select * from edit_previleges where admin_id='".$_SESSION['admin_id']."' and privilege_id='337'";
$ptr_access1=mysql_query($sel_acc1);
if(mysql_num_rows($ptr_access1))
{
	$edit_access1='yes';
}
...
if($_REQUEST['record_id'])
{
	...
	$sel_tds="select tds_percent,tds_value from tds_details where type='expense' and ref_id='".$record_id."'";
	$ptr_tds=mysql_query($sel_tds);
	$datta_tds=mysql_fetch_array($ptr_tds);
}
else
{
	$record_id='';
...
if($_REQUEST['expense_id'])
{
	...
	$where_expense=" and expense_id='".$expense_id."'";
}
else
{
	$expense_id='';
...
function send()
{
	...
	?>	
	if(document.getElementById('added_date'))
	{
		added_date =document.getElementById('added_date').value;
	}
	else
		added_date='';
	if(document.getElementById('expense_category'))
	{
		category =document.getElementById('expense_category');
		var category_name=category.options[category.selectedIndex].text;
		if(category_name!='')
		{
			expense_type =document.getElementById('expense_type');
			var expense_type=expense_type.options[expense_type.selectedIndex].text;
		}
		else
		{
			var expense_type='';
	...
	if(document.getElementById('payment_mode'))
	{
		...
		else if(payment_mode_name=='Credit Card')
		{
			if(document.getElementById('bank_name'))
			{
				bank_name =document.getElementById('bank_name');
		...
		else if(payment_mode_name=='online')
		{
			if(document.getElementById('bank_name'))
			{
				bank_name =document.getElementById('bank_name');
	...
	if(document.getElementById('vendor'))
	{
		...
		var vendor_name=vendor.options[vendor.selectedIndex].text;
	}
	else
		var vendor_name='';
	...
	if(document.getElementById('floor'))
		var total_prod=document.getElementById('floor').value;
	else
		var total_prod='';
	...
	if(total_prod >0)
	{
		for(t=1;t<=total_prod;t++)
		{
			var prod_expense=parseInt($(".expense_name"+t).val());
			var prod_price=$(".prod_price"+t).val();
			var prod_base_price=$(".prod_base_price"+t).val();
			var prod_disc=$(".product_disc"+t).val();
			var prod_discounted_price=$(".prod_discounted_price"+t).val();
			var prod_qty=$(".product_qty"+t).val();
			var prod_cgst_per=$(".sin_product_cgst"+t).val();
			var prod_cgst_price=$(".sin_prod_cgst_price"+t).val();
			var prod_sgst_per=$(".sin_product_sgst"+t).val();
			var prod_sgst_price=$(".sin_prod_sgst_price"+t).val();
			var prod_igst_per=$(".sin_product_igst"+t).val();
			var prod_igst_price=$(".sin_prod_igst_price"+t).val();
			var prod_total=$(".sales_product_price"+t).val();
			//var concat_prod +='&prod_expense'+t+'='+prod_expense+'&prod_price'+t+'='+prod_price+'&prod_base_price'+t+'='+prod_base_price+'&prod_disc'+t+'='+prod_disc+'&prod_discounted_price'+t+'='+prod_discounted_price+'&prod_qty'+t+'='+prod_qty+'&prod_cgst_per'+t+'='+prod_cgst_per+'&prod_cgst_price'+t+'='+prod_cgst_price+'&prod_sgst_per'+t+'='+prod_sgst_per+'&prod_sgst_price'+t+'='+prod_sgst_price+'&prod_igst_per'+t+'='+prod_igst_per+'&prod_igst_price'+t+'='+prod_igst_price+'&prod_total'+t+'='+prod_total;
		}
	}		
	
	//alert(concat_prod);
	
	/*if(document.getElementById('total_type1'))
		var total_tax=document.getElementById('total_type1').value;
	else*/
	var total_tax='';
	...
	/*if(total_tax >0)
	{
		for(t=1;t<=total_tax;t++)
		{
			var tax_type=parseInt($(".tax_type"+t).val());
			var tax_value=$(".tax_value"+t).val();
			concat_tax +='&tax_type'+t+'='+tax_type+'&tax_value'+t+'='+tax_value;
		}
	}*/
	
	//alert(concat_tax);
	var users_mail=mail1;
	data1='action=expense&branch_name='+branch_name+'&category_name='+category_name+'&expense_type='+expense_type+'&payment_mode_name='+payment_mode_name+'&bank_names='+bank_names+'&account_no='+account_no+'&chaque_no='+chaque_no+'&date='+date+'&cust_bank_name='+cust_bank_name+'&credit_card_no='+credit_card_no+'&bank_ref_no='+bank_ref_no+'&amount_wo_tax='+amount_wo_tax+'&discount_type='+discount_type+'&discount='+discount+'&discount_price='+discount_price+'&total_price='+total_price+'&amount='+amount+'&vendor_name='+vendor_name+'&description='+description+'&employee_name='+employee_name+"&users_mail="+users_mail+concat_tax+"&email_text_msg="+email_text_msg; //concat_prod+
	//alert(data1);
	$.ajax({
...
function validme()
{
	...
	else
	{
		...
		if(payment_types=='online')
		{
			...
			{
				disp_error +='Enter Bank Ref Number\n';
				document.getElementById('bank_ref_no').style.border = '1px solid #f00';
	...
	function show_type(vals)
	{
		type_id=vals;
		var branch=document.getElementById('branch_name').value
		if(type_id=="238")
		{
			var cat_data="action=show_campaign_expense&branch="+branch;
			$.ajax({
			url: "ajax.php",type:"post", data: cat_data,cache: false,
			success: function(retbank)
			{
				document.getElementById("campaign").innerHTML=retbank;
				//$("#expense_type").chosen({allow_single_deselect:true});
	...
	function calculte_amount_tax(val_tax_ids)
	{
		...
		{
			cost_tot_tt=document.getElementById("total_price").value;
			//alert(cost_tot_tt);
...
function calc_product_price(prod_id)
{
	...
	//===================================Calculate Base Price============================== Changes 18/1/18
		...
		else if(igsttax > 0)
		{
			...
			document.getElementById("sin_prod_igst_price"+prod_id).value=precisionRound(total_gst,0);
		}
...
{
	var final_amt=parseFloat(document.getElementById('amount').value);
	//alert(final_amt);
	var payable_amt=Number(parseFloat(document.getElementById('payable_amount').value));
	//alert(payable_amt);	
			 ...
			 ?>
				...
				<script type="text/javascript">
				   // $("#statusChangesDiv").dialog();
					...
					setTimeout('document.location.href="expense.php";',500);
				</script>
			<?php                            
		}
	}
	...
	?>
  	<tr>
		...
		if($_POST['save_changes'] && $_POST['randcheck']==$_SESSION['rand'])
		{
			...
			$invoice_no=($_POST['invoice_no']) ? $_POST['invoice_no'] : "";
			$invoice_date=($_POST['invoice_date']) ? $_POST['invoice_date'] : "";
			...
			if($payment_mode_id !='1')
			{
				...
				"<br/>".$bank_name=( ($_POST['bank_name'])) ? $_POST['bank_name'] : "0";
			...
			$gst_claim=( ($_POST['gst_claim'])) ? $_POST['gst_claim'] : "";
			...
			$description=mysql_real_escape_string( ($_POST['description']) ? $_POST['description'] : "");
			...
			else
			{
				...
				$data_record['invoice_date']=$invoice_date;
				...
				$data_record['gst_claim'] =$gst_claim;
				...
				//$ptr_update=mysql_query($update);
					...
					$sel_exps_in="select invoice_id from expense_invoice where expense_id='".$record_id."' order by invoice_id asc limit 0,1";
					$ptr_exp_in=mysql_query($sel_exps_in);
					if(mysql_num_rows($ptr_exp_in))
					{
						$data_invs=mysql_fetch_array($ptr_exp_in);
						
						$update_inv="update expense_invoice set `total_price`='".$data_record['total_price']."',`payable_amount`='".$payable_amount."',`remaining_amount`='".$remaining_amount."',`paid_type`='".$data_record['payment_mode_id']."',`bank_id`='".$data_record['bank_id']."', `cheque_detail`='".$data_record['chaque_no']."', `chaque_date`='".$data_record['date']."', `credit_card_no`='".$data_record['credit_card_no']."', `admin_id`='".$_SESSION['admin_id']."', `added_date`='".$added_date."',`status`='".$status."',`cm_id`='".$cm_id."',`total_paid`='".$payable_amount."' where expense_id='".$record_id."' and invoice_id='".$data_invs['invoice_id']."'  ";
						$ptr_up=mysql_query($update_inv);
					}
					else
					{
						$insert_sales_invoice = "INSERT INTO `expense_invoice` (`expense_id`,`receipt_no`,`total_price`, `payable_amount`,`remaining_amount`, `paid_type`, `bank_id`, `cheque_detail`, `chaque_date`, `credit_card_no`, `admin_id`, `added_date`,`status`,`cm_id`,`total_paid`) VALUES ('".$record_id."','".$receipt_no."', '".$data_record['total_price']."', '".$payable_amount."','".$remaining_amount."', '".$data_record['payment_mode_id']."','".$data_record['bank_id']."', '".$data_record['chaque_no']."', '".$data_record['date']."','".$data_record['credit_card_no']."', '".$_SESSION['admin_id']."', '".$added_date."','".$status."','".$cm_id."','".$payable_amount."'); ";
						$ptr_sales_invoice = mysql_query($insert_sales_invoice);	
						$ins_id=mysql_insert_id();		
					}
				
					$update_bnk=" update bank_records set amount='".$data_record['amount']."',bank_id='".$bank_name."' where record_id='".$record_id."' ";
					...
					for($z=1;$z<=$total_floor;$z++)
					{
						...
						if($_POST['del_floor'.$z] !='yes')
						{
							...
							$data_record_service['expense_name']=mysql_real_escape_string( ($_POST['expense_name'.$z]) ? $_POST['expense_name'.$z] : "");
							...
							$data_record_service['gst_claim'] =$_POST['gst_claim'] ? $_POST['gst_claim'] : "";
							...
							$data_record_service['product_disc']=mysql_real_escape_string( ($_POST['product_disc'.$z]) ? $_POST['product_disc'.$z] : "0");
					...
					$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('Expense','Edit','expense','".$record_id."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
					$query=mysql_query($insert); 
				...
				{
					//==============Update Invoice No.=====================
					$sel_inv="select ext_invoice_no from expense where cm_id='".$data_record['cm_id']."' and ext_invoice_no IS NOT NULL order by ext_invoice_no desc limit 0,1";
					...
					//======================================================
					$insert_for_receipt = "insert into expense (`payment_mode_id`,`ext_invoice_no`,`cash_type`,`bank_id`,`account_no`,`chaque_no`,`chaque_date`,`gst_claim`,`amount_wo_tax`,`amount`,`discount_type`,`discount`,`discount_price`,`total_price`,`description`,`vendor_id`,`employee_id`,`expense_category_id`,`expense_type_id`,`agent_id`,`credit_card_no`,`sac_code`,`added_date`,`cm_id`) values('".$data_record['payment_mode_id']."','".$ext_invoice_no."','".$data_record['cash_type']."','".$data_record['bank_id']."','".$data_record['account_no']."','".$data_record['chaque_no']."','".$data_record['date']."','".$data_record['gst_claim']."','".$data_record['amount_wo_tax']."','".$data_record['amount']."','".$data_record['discount_type']."','".$data_record['discount']."','".$data_record['discount_price']."','".$data_record['total_price']."','".$data_record['description']."','".$data_record['vendor_id']."','".$data_record['employee_id']."','".$data_record['expense_category_id']."','".$data_record['expense_type_id']."','".$agent_id."','".$data_record['credit_card_no']."','".$sac_code."','".$added_date."','".$cm_id."')";
					...
					$insert_sales_invoice = "INSERT INTO `expense_invoice` (`expense_id`,`receipt_no`,`total_price`, `payable_amount`,`remaining_amount`, `paid_type`, `bank_id`, `cheque_detail`, `chaque_date`, `credit_card_no`, `admin_id`, `added_date`,`status`,`cm_id`,`total_paid`) VALUES ('".$ins_is."','".$receipt_no."', '".$data_record['total_price']."', '".$payable_amount."','".$remaining_amount."', '".$data_record['payment_mode_id']."','".$data_record['bank_id']."', '".$data_record['chaque_no']."', '".$data_record['date']."','".$data_record['credit_card_no']."', '".$_SESSION['admin_id']."', '".$added_date."','".$status."','".$cm_id."','".$payable_amount."'); ";
					$ptr_sales_invoice = mysql_query($insert_sales_invoice);	
					$ins_id=mysql_insert_id();
					
					if($data_record['payment_mode_id']=='2' || $data_record['payment_mode_id']=='4' || $data_record['payment_mode_id']=='5')
					{
			   			$bank="INSERT INTO `bank_records`(`bank_id`, `type`, `record_id`, `amount`, `added_date`, `cm_id`, `admin_id`) VALUES ('".$bank_name."','expense','".$ins_is."','".$data_record['amount']."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
					...
					for($i=1;$i<=$total_floor;$i++)
					{
						if($_POST['expense_name'.$i] !='' )
						{
							...
							$data_record_service['expense_name']=mysql_real_escape_string(( ($_POST['expense_name'.$i])) ? $_POST['expense_name'.$i] : "");
					...
					$ins_tds="insert into tds_details (`type`,`ref_id`,`tds_disc_type`,`tds_percent`,`tds_value`,`admin_id`,`cm_id`,`added_date`) values('expense','".$ins_is."','".$_POST['tds_discount_type']."','".$_POST['tds_discount']."','".$_POST['tds_discount_price']."','".$admin_id."','".$cm_id."','".$added_date."')";
					$ptr_ins=mysql_query($ins_tds);
	...
	if($_SESSION['type']=='S' || $_SESSION['type']=='Z' || $_SESSION['type']=='LD' )
	{
		if($where_expense=='')
		{
			...
			<tr>
                ...
                {
                    ?>
                    <input style="width:200px" type="text" id="added_date" name="added_date" class="input_text datepicker" value="<?php if($_POST['added_date']) echo $_POST['added_date']; else if($row_expense['added_Date']!=''){$arrage_date= explode('-',$row_expense['added_Date'],3);echo $arrage_date[2].'/'.$arrage_date[1].'/'.$arrage_date[0];}else{echo date("d/m/Y");}?>" />
                    <?php
                }
                else
                {
                    if($_POST['added_date']) echo $_POST['added_date']; else if($row_expense['added_Date']!=''){$arrage_date= explode('-',$row_expense['added_Date'],3);echo $arrage_date[2].'/'.$arrage_date[1].'/'.$arrage_date[0];}else{echo date("d/m/Y");}
                    ?>
                    <input type="hidden"  style="width:200px" id="added_date" name="added_date" class="input_text datepicker" value="<?php if($_POST['added_date']) echo $_POST['added_date']; else if($row_expense['added_Date']!=''){$arrage_date= explode('-',$row_expense['added_Date'],3);echo $arrage_date[2].'/'.$arrage_date[1].'/'.$arrage_date[0];}else{echo date("d/m/Y");}?>" />
                    <?php
                }
                ?>
            ...
            <tr>
                <td width="15%" align="center">SAC Number<span class="orange_font"></span></td>
            ...
            <tr>
                <td width="15%" class="tr-header" align="center">Select Payment Mode<span class="orange_font">*</span></td>
            ...
            <div id="bank_details" <?php if($data_payment_mode1['payment_mode']=='Credit Card' || $data_payment_mode1['payment_mode']=='cheque') echo ' style="display:block"'; else echo ' style="display:none"'; ?>>
                <table width="100%">
                    <tr>
                        <td width="22%" class="tr-header"  align="center">Bank Name<span class="orange_font">*</span></td>
                        <td>
                            <div id="bank_record">
                            <?php 
                            if($record_id !='')
                    ...
                    <tr>
                        <td width="22%" class="tr-header" align="center">Enter Account No<span class="orange_font">*</span></td>
            ...
            <div id="bank_ref_no" <?php if($_POST['payment_type'] =='online-5') echo 'style="display:block"'; else if($data_invoice['paid_type'] =='5') echo 'style="display:block"';  else echo 'style="display:none"'; ?>>
                <table width="100%">
                    <tr>
                        <td width="10%" class="tr-header" align="center">Bank Ref. no</td>
            ...
            <div id="chaque_details" <?php  if($data_payment_mode1['payment_mode']=='cheque') echo ' style="display:block"'; else echo ' style="display:none"'; ?>>
                <table width="100%">
                    <tr>
                        <td width="22%" class="tr-header" align="center">Enter Chaque No<span class="orange_font">*</span></td>
                    ...
                    <tr>
                        <td width="22%" class="tr-header" align="center">Enter Chaque Date<span class="orange_font">*</span></td>
            ...
            <div id="credit_details" <?php  if($data_payment_mode1['payment_mode']=='Credit Card') echo ' style="display:block"'; else echo ' style="display:none"'; ?>>
                <table width="100%">
                    <tr>
                        <td width="22%" class="tr-header" align="center">Enter Credit Card No<span class="orange_font">*</span></td>
            ...
            <div id="paytm_details" <?php  if($data_payment_mode1['payment_mode']=='paytm') echo ' style="display:block"'; else echo ' style="display:none"'; ?>>
                <table width="100%">
                    <tr>
                        <td class="tr-header" width="22%" align="center">ISAS Mobile No</td>
                        <td><input type="text" name="isas_paytm_no" class="input_text" style="width:200px" id="isas_paytm_no" maxlength="10" value="<?php if($_POST['isas_paytm_no']) echo $_POST['isas_paytm_no']; else echo $row_expense['isas_paytm_no']; ?>" /></td>
                    </tr>
                    <tr>
                        <td class="tr-header" width="22%" align="center">Cust. Mobile No</td>
                        <td><input type="text" name="cust_paytm_no" class="input_text" style="width:200px" id="cust_paytm_no" maxlength="10" value="<?php if($_POST['cust_paytm_no']) echo $_POST['cust_paytm_no']; else echo $row_expense['cust_paytm_no']; ?>" /></td>
                    </tr>
                </table>
            </div>
            </td>
            ...
            <tr>
            	<td width="15%" colspan="3" ><div id="campaign"></div></td>
            </tr>
            <tr>
            	<td width="15%" class="tr-header" align="center">Select Agent Name</td>
            ...
            <tr>
                <td width="15%" align="center">GST Claim<span class="orange_font"></span></td>
                <td width="25%"><input type="radio" style="" name="gst_claim" id="gst_claim" class="input_radio" value="yes" <?php if($_POST['gst_claim']=='yes') echo 'checked="checked"'; else if($row_expense['gst_claim']=='yes') echo 'checked="checked"'; else if($row_expense['gst_claim']=='') echo 'checked="checked"';  ?>  />Yes&nbsp;&nbsp;&nbsp;<input type="radio" style="" name="gst_claim" id="gst_claim" class="input_radio" value="no" <?php if($_POST['gst_claim']=='no') echo 'checked="checked"'; else if($row_expense['gst_claim']=='no') echo 'checked="checked"'; ?> />No
                </td>
            </tr>
            <tr>
                <td width="15%" align="center">Invoice Number<span class="orange_font"></span></td>
                <td width="25%"><input type="text" style="width:200px" name="invoice_no" id="invoice_no" class="input_text" value="<?php if($_POST['invoice_no']) echo $_POST['invoice_no']; else echo $row_expense['invoice_no']; ?>"  />
                </td>
            </tr>
            <tr>
                <td width="22%" class="tr-header" align="center">Invoice Date<span class="orange_font"></span></td>
                <td><input type="text" name="invoice_date" id="invoice_date" style="width:200px"  class="datepicker input_text" value="<?php if($_POST['invoice_date']) echo $_POST['invoice_date']; else echo $row_expense['invoice_date']; ?>" /></td>
            </tr>
            <tr>
                    ...
                    <tr>
                        <td colspan="6">
                            ...
                            <tr>
                                ...
                                <input type="radio" name="discount1" id="discount1" value="rupees" />in <?php if($_SESSION['tax_type']=='GST') echo 'Rs -/'; else echo 'AED'; ?></td>
								...
								{
									?><td valign="top" width="7%" align="center">VAT</td>
                                    <?php
								}
								?>
			...
			<tr>
                ...
                <td width="25%"><input type="text" name="amount_wo_tax" id="amount_wo_tax" onBlur="calculte_total_cost()" style="width:200px" class="input_text" readonly="readonly" value="<?php if($_POST['amount_wo_tax']) echo $_POST['amount_wo_tax']; else echo $row_expense['amount_wo_tax']; ?>"  /></td>
            </tr>
            <tr>
                ...
                <input type="radio" name="discount_type" id="discount_type" onChange="calculte_total_cost()" value="rupees" <?php if($_POST['discount_type']=="rupees") {echo 'checked="checked"';}else if($row_expense['discount_type']=="rupees") { echo 'checked="checked"';} ?> />in <?php if($_SESSION['tax_type']=='GST') echo 'Rs -/'; else echo 'AED'; ?> <span class="orange_font"> *</span></td>
            ...
            <tr>
                <td width="20%" valign="top" align="center">Discounted Price</td>
            ...
            <tr>
                <td width="20%" valign="top" align="center">TDS <input type="radio" name="tds_discount_type" onChange="calculte_total_cost()" id="tds_discount_type" checked="checked" value="percentage" style="width:20px" <?php if($_POST['tds_discount_type']=="percentage") {echo 'checked="checked"';}else if($row_expense['tds_discount_type']=="percentage") { echo 'checked="checked"';} ?>  />in %
                <input type="radio" name="tds_discount_type" id="tds_discount_type" onChange="calculte_total_cost()" value="rupees" <?php if($_POST['tds_discount_type']=="rupees") {echo 'checked="checked"';}else if($row_expense['tds_discount_type']=="rupees") { echo 'checked="checked"';} ?> /><?php if($_SESSION['tax_type']=='GST') echo 'Rs -/'; else echo 'AED'; ?><span class="orange_font"> *</span></td>
                <td width="70%"><input type="text"  class="input_text" style="width:200px" name="tds_discount" id="tds_discount" onKeyUp="calculte_total_cost()" value="<?php if($_POST['tds_discount']) echo $_POST['tds_discount']; else echo $datta_tds['tds_percent'];?>" /></td> 
                <td width="10%"></td>
            </tr>
            <tr>
                <td width="20%" valign="top" align="center">TDS Price</td>
                <td width="72%"><input type="text" class=" input_text" style="width:200px" name="tds_discount_price" id="tds_discount_price" value="<?php if($_POST['tds_discount_price']) echo $_POST['tds_discount_price']; else echo $datta_tds['tds_value'];?>" /></td> 
                <td width="5%"></td>
            </tr>
            <tr>
            ...
            <tr>
                <td width="15%" align="center">Payable Amount<span class="orange_font">*</span></td>
                <td width="25%"><input type="text" name="payable_amount" id="payable_amount" class="input_text" style="width:200px" value="<?php if($_POST['save_changes']) echo $_POST['payable_amount']; else echo $row_expense['payable_amount']; ?>" onKeyUp="cal_remaining_amt();"/></td>
            </tr>
            <tr>
                <td width="15%" align="center">Remaining Amount</td>
                <td width="25%"><input type="text" name="remaining_amount" id="remaining_amount" class="input_text" style="width:200px" value="<?php if($_POST['save_changes']) echo $_POST['remaining_amount']; else echo $row_expense['remaining_amount']; ?>"  /></td>
            </tr>
            <tr>
                <td width="15%" class="tr-header" align="center">Description<span class="orange_font">*</span></td>
            ...
            <tr>
                <td width="15%" class="tr-header" align="center">Select Vendor<span class="orange_font">*</span></td>
            ...
            <tr>
                <td width="15%" class="tr-header"  align="center">Select Employee<span class="orange_font">*</span></td>
                <td><select name="employee" style="width:200px" class="input_text" id="employee">
                <option value="select">--Select--</option>
                <?php
                $sle_name="select admin_id,name from site_setting where 1 and system_status='Enabled' order by name"; 
...
<table cellspacing="0" cellpadding="0" class="table" width="95%">
  <tr class="head_td">
	...
	$sql_query= "SELECT * FROM expense where 1 ".$record_cat_id." ".$where_expense." ".$where_cm_id_data." ".$_SESSION['where']." ".$pre_keyword." ".$from_date." ".$to_date." ".$select_directory." "; 
   //echo $sql_query;
	...
	if($no_of_records)
	{
	   ...
	   // $pager = new PS_Pagination($sql_query,$_SESSION['show_records'],$_SESSION['show_records_pages'],$query_string1);
        ...
        <tr class="grey_td" >
            ...
            <td width="5%" align="center"><strong>Invoice No.</strong></td>
			<?php
            if($_SESSION['type']=="S" || $_SESSION['type']=='Z' || $_SESSION['type']=='LD')
            {
                ?>
            ...
            <td width="8%" align="center"><strong>View Payment</strong></td>
        ...
        while($val_query=mysql_fetch_array($all_records))
        {
			...
			$sel_expense_inv="select total_paid,remaining_amount from expense_invoice where expense_id='".$val_query['expense_id']."' order by invoice_id desc limit 0,1";
            $ptr_expense_inv=mysql_query($sel_expense_inv);
            $data_expense_inv=mysql_fetch_array($ptr_expense_inv);
            ...
            if($val_query['discount_price']>0)
            {
                ...
                echo '<br/><b><span style="float:left">Discounted price -</span><span style="float:right">'.round($val_query['total_price']).'</span></b>';
            }
            ...
            echo '<br/><b><span style="float:left">Total Amount -</span><span style="float:right">'.round($val_query['amount']).'</span></b>';
            echo '<br/><b><span style="float:left">Total Paid -</span><span style="float:right">'.round($data_expense_inv['total_paid']).'</span></b>';
            echo '<br/><b><span style="float:left">Remaining Amount -</span><span style="float:right">'.round($data_expense_inv['remaining_amount']).'</span></b>';
            echo'</td>';
            ...
            echo '<td align="center"><a href="expense_payment_details.php?record_id='.$listed_record_id.'">
            <img src="images/view_bill.jpg" border="0" width="30px" height="30px" title="View Payment"></a></td>';
	...
	else
	{
		...
		{
		if($array_previ_parent[$e]['privilege_id']==17 || $_SESSION['type']=='S' )
		{
		for($f=0;$f<count($array_prev[$e]);$f++)
		{ 
		if($array_prev[$e][$f]['privilege_id']==105 || $_SESSION['type']=='S')
		{
		...
		if($where_expense=='')
		{
            ...
            <tr>
                <td width="20%" valign="top" align="center">TDS <input type="radio" name="tds_discount_type" onChange="calculte_total_cost()" id="tds_discount_type" checked="checked" value="percentage" style="width:20px" <?php if($_POST['tds_discount_type']=="percentage") {echo 'checked="checked"';}else if($row_expense['tds_discount_type']=="percentage") { echo 'checked="checked"';} ?>  />in %
                <input type="radio" name="tds_discount_type" id="tds_discount_type" onChange="calculte_total_cost()" value="rupees" <?php if($_POST['tds_discount_type']=="rupees") {echo 'checked="checked"';}else if($row_expense['tds_discount_type']=="rupees") { echo 'checked="checked"';} ?> />in Rs -/ <span class="orange_font">*</span></td>
                <td width="70%"><input type="text"  class="input_text" style="width:200px" name="tds_discount" id="tds_discount" onKeyUp="calculte_total_cost()" value="<?php if($_POST['tds_discount']) echo $_POST['tds_discount']; else echo $datta_tds['tds_percent'];?>" /></td> 
                <td width="10%"></td>
            </tr>
            <tr>
            ...
            <tr>
                <td width="15%" align="center">Payable Amount</td>
                <td width="25%"><input type="text" name="payable_amount" id="payable_amount" class="input_text" style="width:200px" value="<?php if($_POST['save_changes']) echo $_POST['payable_amount']; else echo $row_expense['payable_amount']; ?>" onKeyUp="cal_remaining_amt();"/></td>
            </tr>
            <tr>
                <td width="15%" align="center">Remaining Amount</td>
	...
	if($array_prev[$e][$f]['privilege_id']==337)
	{
?>
<table cellspacing="0" cellpadding="0" class="table" width="95%">
  <tr class="head_td">
   ...
   //echo $sql_query;
	...
	if($no_of_records)
	{
	   ...
	   // $pager = new PS_Pagination($sql_query,$_SESSION['show_records'],$_SESSION['show_records_pages'],$query_string1);
        ...
        <tr class="grey_td" >
			<?php
            if($_SESSION['type']=='S'  || $edit_access1=='yes')
            {?>
            <td width="5%" align="center"><input type="checkbox" name="chkAll" onClick="Javascript:checkAll('frmTakeAction','chkRecords')"/></td>
            <?php
            }
...
function calculte_total_cost()
{
	...
	frm = document.jqueryForm;  
	tds_discount_type =frm.tds_discount_type.value;
	
	var tds_total_prods_price=total_cost_new;
	
	if(document.getElementById('tds_discount'))
	{	
		var tds_discount=Number(parseFloat(document.getElementById('tds_discount').value).toFixed(2));
	}
	else
	{
		var tds_discount=0;
	...
	if(tds_discount >0)
	{
		if(tds_discount_type=="percentage")
		{
			var tds_discount_price= tds_total_prods_price * (tds_discount/100);
		}
		else
		{
			var tds_discount_price= tds_discount;
		}
	}
	else
	{
		var tds_discount_price= 0;
	}
	$('#tds_discount_price').val(tds_discount_price);
	//=========================================================================================
	cost_tot_tt=document.getElementById("total_price").value;
	$('#amount').val(cost_tot_tt);
	//calculte_amount_tax();
