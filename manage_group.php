    ...
    <?php if($_POST['formAction'])
                    {
                        if($_POST['formAction']=="delete")
                        {
                            for($r=0;$r<count($_POST['chkRecords']);$r++)
                            {
                                ...
                                $sql_query= "SELECT group_id FROM ".$GLOBALS["pre_db"]."add_group where group_id ='".$del_record_id."'";
                                if(mysql_num_rows($db->query($sql_query)))
                                    {                                                
                                       $delete_query="delete from ".$GLOBALS["pre_db"]."add_group where group_id='".$del_record_id."'";
                                        $db->query($delete_query);
										$delete_query1="delete from ".$GLOBALS["pre_db"]."add_group_detail where group_id='".$del_record_id."'";
...
<table cellspacing="0" cellpadding="0" class="table" width="95%">
  <tr class="head_td">
    ...
    <?php
                        ...
                        if($_GET['order'] !='' && ($_GET['orderby']=='course_name'))
                        {
                            ...
                            $select_directory='order by group_id desc';                      
                         	$sql_query= "SELECT *FROM add_group where 1  ".$pre_keyword."  ".$select_directory."";  //".$_SESSION['where_admin_id']."
                       //echo $sql_query;
  ...
  <tr class="grey_td" >
    ...
    <td width="15%"><a href="<?php echo $_SERVER['PHP_SELF'];?>?order=<?php echo $order."&orderby=course_name".$query_string;?>" class="table_font"><strong>Group Name</strong></a> <?php echo $img1;?></td>
		 <td width="15%"><strong>Send To</strong></td>
	<?php
	if($_SESSION['type'] =='S')
	{ ?> 
	...
	<?php } ?>
                            ...
                            while($val_query=mysql_fetch_array($all_records))
                            {
                                ...
                                $listed_record_id=$val_query['group_id']; 
                                ...
                                echo '<td >'.$val_query['group_name'].'</td>';
                                echo '<td >'.$val_query['send_to'].'</td>';
                               
								
								if( $_SESSION['type'] =='S')
								{
                                	echo '<td align="center">';
									echo '<a href="add_group.php?record_id='.$listed_record_id.'" ><img src="images/edit_icon.png" title="Edit Record" class="example-fade"/></a>&nbsp;&nbsp;';
