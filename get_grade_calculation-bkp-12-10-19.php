...
else if($action=='diploma_course')
{
	...
	if($tot)
	{
		...
		if(mysql_num_rows($ptr_sel))
		{
			$update_grade="UPDATE `action_certificate_grade` set total_percentage='".$total_diploma_per."',grade='".$dip_grade."',cm_id='".$_SESSION['cm_id']."',admin_id='".$_SESSION['admin_id']."',added_date='".date('Y-m-d H:i:s')."' where enroll_id='".$enroll_id."' and course_id='".$course_id."'";
        	$ptr_certificate=mysql_query($update_grade);
		}
		else
		{
			$ins_grade="INSERT INTO `action_certificate_grade`(`enroll_id`, `course_id`,`total_percentage`,`grade`,`cm_id`, `admin_id`, `added_date`) VALUES ('".$enroll_id."','".$course_id."','".$total_diploma_per."','".$dip_grade."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."','".date('Y-m-d H:i:s')."')";
        	$ptr_certificate=mysql_query($ins_grade);
		}
	}
	else
