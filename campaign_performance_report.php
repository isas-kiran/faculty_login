...
<table cellspacing="0" cellpadding="0" class="table" width="95%">
  ...
  <?php
                       ...
                       // $select_directory =" order by campaign_name asc";
						$branch_id='';
						$select_directory='order by campaign_name asc';                      
						$sql_query= "SELECT branch_name,campaign_id,campaign_name,campaign_for FROM campaign where 1  ".$search_cm_id." ".$campaign_src."  ".$select_directory.""; 
    						...
    						<form method="post"  name="frmTakeAction"  action="<?php echo $_SERVER['PHP_SELF'].'?#msgbox';?>">
                     			<input type="hidden" name="formAction" id="formAction" value=""/>
                              	<tr class="grey_td" >
                                	...
                                	<td width="15%" align="center"><strong>Campaign Name</strong></td>
                                	<td width="10%" align="center"><strong>Campaign For</strong></td>
                                	<td width="8%" align="center"><strong>Total Leads</strong></td>
                                    <td width="8%" align="center"><strong>Total Walk-in</strong></td>
                                	<td width="8%" align="center"><strong>Total Enrollments</strong></td>
                                    <td width="8%" align="center"><strong>Amount Spent</strong></td>
                                    <td width="10%" align="center"><strong>Business Booked</strong></td>
                                	<td width="10%" align="center"><strong>Business Realised</strong></td>
                              	</tr>
								<?php
                                $total_inq_source=0;
                                $total_enroll_src=0;
                                while($val_query=mysql_fetch_array($all_records))
                                {
									...
									$sel_inq_walkin="select inquiry_id from inquiry where 1 and enquiry_source ='".$val_query['campaign_id']."' and response='1' ".$_SESSION['where']."  ".$enquiry_from_date." ".$enquiry_to_date." ".$enquiry_src." ";
                                    $ptr_inq_walkin=mysql_query($sel_inq_walkin);
                                    $total_inq_walkin=mysql_num_rows($ptr_inq_walkin);
									...
									$select_cmp="select total_cost from campaign where campaign_id='".$val_query['campaign_id']."'";
									$ptr_cmp=mysql_query($select_cmp);
									$data_cmp=mysql_fetch_array($ptr_cmp);
									$amount_spent=$data_cmp['total_cost'];
									
									$sel_total_booked="select SUM(paid) as total_booked_amt from enrollment where 1 and source ='".$val_query['campaign_id']."' ".$_SESSION['where']." ".$enquiry_from_date." ".$enquiry_to_date." ".$enroll_src."";
									$ptr_total_booked=mysql_query($sel_total_booked);
									$data_total_booked=mysql_fetch_array($ptr_total_booked);
    								$total_booked_amt=$data_total_booked['total_booked_amt'];
									
									$sel_total_inc="select SUM(net_fees) as total_realised_amt from enrollment where 1 and source ='".$val_query['campaign_id']."' ".$_SESSION['where']." ".$enquiry_from_date." ".$enquiry_to_date." ".$enroll_src."";
									$ptr_total_inc=mysql_query($sel_total_inc);
									$data_total_inc=mysql_fetch_array($ptr_total_inc);
    								$total_realised_amt=$data_total_inc['total_realised_amt'];
									...
									echo '<td align="center">'.$total_inq_walkin.'</td>';
                                    echo '<td align="center">'.$total_enroll_src.'</td>';
									echo '<td align="center">'.$amount_spent.'</td>';
									echo '<td align="center">'.$total_booked_amt.'</td>';
									echo '<td align="center">'.$total_realised_amt.'</td>';
                                    echo '</tr>';
                                    $bgColorCounter++;
                                }
                                ?>
