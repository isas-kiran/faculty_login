  ...
  <tr class="head_td">
						...
						?>
                          ...
                          <td ><select id="response" name="response">
                            ...
                            <option value="">Select Resonse</option>
                           	<option value="walk_in" <?php if($_GET['response'] == "walk_in") echo 'selected="selected"'; ?>>Walk In</option>
                            <option value="will_walkin" <?php if($_GET['response'] == "will_walkin") echo 'selected="selected"'; ?>>Will Walk In</option>
                            <option value="call_back_later" <?php if($_GET['response'] == "call_back_later") echo 'selected="selected"'; ?>>Call Back Later</option>
                            <option value="call_not_pickup" <?php if($_GET['response'] == "call_not_pickup") echo 'selected="selected"';  ?>>Call Did Not Pick Up</option>
                            <option value="not_reachable" <?php if($_GET['response'] == "not_reachable") echo 'selected="selected"'; ?>>Not Reachable</option>
                            <option value="call_taken" <?php if($_GET['response'] == "call_taken") echo 'selected="selected"'; ?>>Call Taken</option>
    ...
    <?php
                        ...
                        if($_GET['order'] !='' && ($_GET['orderby']=='name'))
                        {
                           ...
                           $sql_query= "SELECT student_id,status,name,cm_id,contact,address,mail,added_date,course_id,admin_id,enquiry_source,response,qualification,dob FROM stud_regi where 1 ".$_SESSION['where']." ".$pre_keyword." ".$enquiry_added." ".$enquiry_src." ".$response." ".$select_directory.""; 
                       //echo $sql_query;
...
//                                $val_contry = $db->fetch_array($db->query($select_country));
                               ...
                               /* echo '<td align="center">'.$val_query['contact'].'</td>';*/
								...
								if($val_query['response'] == "walk_in")
								{
									$response="Walk in";
								}
								else if($val_query['response'] == "will_walkin")
								{
									$response="Will walk in";
								}
								else if($val_query['response'] == "call_back_later")
								{
									$response="Call Back Later";
								}
								else if($val_query['response'] == "call_not_pickup")
								{
									$response="Call did not pick up";
								}
								else if($val_query['response'] == "not_reachable")
								{
									$response="Not reachable";
								}
								else if($val_query['response'] == "call_taken")
								{
									$response="Call Taken";
								}
								...
								echo '<td style="color:#FF0000" align="center"><a href="add_student.php?record_id='.$listed_record_id.'" ><img src="images/enrolls.png" title="Enrollment" height="30" width="30"></a></td>';
