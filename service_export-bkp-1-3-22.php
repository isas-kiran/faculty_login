...
$objPHPExcel->setActiveSheetIndex(0);
 ...
 $objPHPExcel->setActiveSheetIndex()->setCellValueByColumnAndRow('3','3', 'Mo No','3');
  ...
  $objPHPExcel->setActiveSheetIndex()->setCellValueByColumnAndRow('13','3', 'Status','3');*/
	...
	if($_SESSION['where']!='')
	{
		$where_cm=" and cs.cm_id='".$_SESSION['cm_id']."'";
	}
	$branch_id='';
	$select_directory='order by DATE(cs.added_date) desc';                      
	$sql_query= "SELECT distinct(cs.customer_service_id) as customer_service_id,cs.customer_id as customer_id,cs.cm_id as cm_id,cs.service_price as service_price,cs.nonmemb_discount_price as nonmemb_discount_price,cs.total_cost as total_cost,cs.payable_amount as payable_amount,cs.remaining_amount as remaining_amount,cs.status as status,cs.added_date as added_date FROM customer_service cs, customer c, servies s, customer_service_map csm where 1  and (cs.service_price like '%".$keyword."%' or cs.discount_price like '%".$keyword."%' or cs.service_tax like '%".$keyword."%' or cs.total_cost like '%".$keyword."%' or c.cust_name like '%".$keyword."%' or c.mobile1 like '%".$keyword."%' or s.service_name like '%".$keyword."%' or s.service_price like '%".$keyword."%') and cs.customer_id=c.cust_id and cs.customer_service_id=csm.customer_service_id  and csm.service_id=s.service_id ".$cmids." ".$branch_keyword_p." ".$record_cat_idss." ".$select_directory." "; 
