...
if($_REQUEST['record_id'])
{
	...
	$sel="select * from biometric_device where biometric_id='".$record_id."'";
...
<title>Manage Baiometric Device</title>
    ...
    <td class="mid_mid">
     <?php 
                    if($_REQUEST['deleteRecord'])
                    {
                        ...
                        $sql_query= "SELECT biometric_id FROM biometric_device where biometric_id='".$del_record_id."'";
                        if(mysql_num_rows($db->query($sql_query)))
                        {
							$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('Manage Biometric Device','Delete','Device','".$del_record_id."','".date('Yt-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
							$query=mysql_query($insert);
							
                        	$delete_query="delete from biometric_device where biometric_id='".$del_record_id."'";
	...
	success: function (html)
	{
		//alert(html);
                      ...
                      <?php
								...
								if($_REQUEST['record_id'] !='')
								{
									$record_cat_id="and biometric_id='".$_REQUEST['record_id']."' ";
								}
                                $sql_records= "SELECT * FROM biometric_device where 1 ".$record_cat_id." ".$_SESSION['where']." order by biometric_id asc";
                                ...
                                if($no_of_records)
                                {
                                    ...
                                    <table width="98%" align="center"  cellpadding="0" cellspacing="1"  class="table" style="width: 97%;">
										<tr align="center" class="grey_td" >
                                            ...
                                            <td width="11%" class="tr-header"><strong>Service Tag Id</strong></td>
										...
										while($val_record=mysql_fetch_array($all_records))
										{
											...
											$listed_record_id=$val_record['biometric_id'];
											...
											echo "<td align='center'>".$val_record['service_tag_id']."</td>";
											//echo "<td align='center'>".$val_record['reward_point_redeme']."</td>";
											if($_SESSION['type']=="S")
											{
												echo '<td align="center"><a href="manage_biometric_machine.php?record_id='.$listed_record_id.'" ><img src="images/edit_icon.png" title="Edit Record" class="example-fade"/></a>&nbsp;&nbsp;<a onclick="return validationToDelete(\''.$_SERVER['PHP_SELF'].'\');" href="'.$_SERVER['PHP_SELF'].'?deleteRecord='.$listed_record_id.'"><img src="images/delete_icon.png" title="Delete Record" class="example-fade" /></a>&nbsp;&nbsp;';
												echo '</td>';
											}
											echo '</tr>';
										   $bgColorCounter++;
							...
							if(isset($_POST['save_changes']))
							{
								...
								$service_tag_id=$_POST['service_tag_id'];
								if($service_tag_id=="")
								{
									$success=0;
									$errors[$i++]="Enter Service Tag Id";
								}
								if($_SESSION['type']=='S')
								{
								...
								if($record_id)
								{
									$rec_id=" and biometric_id !='".$record_id."'";
								}
								$sel_rew="select biometric_id from biometric_device where cm_id='".$cm_id."' ".$rec_id." ";
								...
								{
									$data_record['service_tag_id'] = $service_tag_id;
									$data_record['cm_id'] = $cm_id;
									$data_record['added_date'] = date('Y-m-d');
									
									if($record_id)
									{
										if($data_record['service_tag_id'] !='')
										{
											$update_bank ="update biometric_device set `service_tag_id`='".$data_record['service_tag_id']."',`added_date`='".date('Y-m-d')."',`cm_id`='".$cm_id."' where biometric_id='".$record_id."'";
											$ptr_bank= mysql_query($update_bank);
											
											$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('Biometric_Device','Edit','".$_POST['service_tag_id']."','".$record_id."','".date('Y-m-d')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
											$query=mysql_query($insert);
												
											?><div id="statusChangesDiv" title="Record Deleted"><center><br><p>Device Updated Successfully</p></center></div>
											<script type="text/javascript">
												$(document).ready(function() {
													$( "#statusChangesDiv" ).dialog({
														modal: true,
											   	...
											   	setTimeout('document.location.href="manage_biometric_machine.php";',1000);
												</script>
											<?php
										}
									}
									else
									{
										if($data_record['service_tag_id'] !='')
										{
												$insert_bio= "insert into biometric_device (`service_tag_id`,`added_date`,`cm_id`) values('".$data_record['service_tag_id']."','".date('Y-m-d')."','".$cm_id."')";
												$ptr_bio= mysql_query($insert_bio);
												$ins_id=mysql_insert_id();
												"<br>".$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('Biometric_Device','Add','".$_POST['service_tag_id']."','".$ins_id."','".date('Y-m-d')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
												$query=mysql_query($insert);
												
												?><div id="statusChangesDiv" title="Record Deleted"><center><br><p>Device added successfully</p></center></div>
													<script type="text/javascript">
														$(document).ready(function() {
															$( "#statusChangesDiv" ).dialog({
																	modal: true,
                             ...
                             <? if($_SESSION['type']=='S')
							{
                              ...
                              <tr>
                                ...
                                while($row_branch = mysql_fetch_array($query_branch))
                                {
									...
									$sele_branch="select branch_name from site_setting where cm_id='".$row_record['cm_id']."' and type='A' ";
									$ptr_selected=mysql_query($sele_branch);
                        ...
                        <?php } ?>
                            <tr>
                            	<td width="52%" class="tr-header" align="center">Service Tag Id</td>
                                 <td width="25%" class="customized_select_box">
                      			<input type="text" name="service_tag_id" id="service_tag_id" value="<?php if($_POST['service_tag_id']) echo $_POST['service_tag_id']; else echo $row_record['service_tag_id']; ?>">
                            </td>
                            </tr>
							<tr>
