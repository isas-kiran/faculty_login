...
if($_REQUEST['record_id'])
{
	...
	$sele_record="select * from enrollment where enroll_id='".$record_id."'";
	...
	if($_POST['save_changes'])
	{
		...
		else
        {
			...
			if($record_id)
			{
				...
				"<br>".$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('Enroll outstand','Add','".$data_record['enroll_id']."','".$out_ids."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
				$query=mysql_query($insert);
				//------send notification on inquiry addition--------------------
				/*$notification_args['reference_id'] = $receipt_ids;
				$notification_args['on_action'] = 'enroll_outstanding_payment';
				$notification_status = addNotifications($notification_args);*/
				
				/*$update="update enroll_outstanding set enroll_id='".$data_record['enroll_id']."', type='".$data_record['type']."',amount='".$data_record['amount']."', reason='".$data_record['reason']."' where outstand_id='".$record_id."'";
...
<table cellspacing="0" cellpadding="0" class="table" width="95%">
  <tr class="head_td">
    ...
    <?php
                        ...
                        $sql_query= "SELECT * FROM enroll_outstanding where 1 and enroll_id='".$record_id."' ".$_SESSION['where']." ".$pre_keyword." ".$from_date." ".$to_date." ".$select_directory." "; 	
                       //echo $sql_query;
  ...
  <?php
    ...
    <td width="15%"><strong>Added date</strong></td>
     <td width="15%"><strong>Pay</strong></td>
	<?php
	if($_SESSION['type']=='S')
	{
	?>
	...
	<?php } ?>
                            ...
                            while($val_query=mysql_fetch_array($all_records))
                            {
								...
								echo  '<td align="center"><a href="add_outstand_payment.php?record_id='.$record_id.'&installment_id='.$data_installment['installment_id'].'&ref_id='.$data_installment['installment_ref_id'].'"><img src="images/add1-.png" border="0" title="Add outstand Payment"></a></td>';
								if($_SESSION['type']=='S')
								{
