...
if($_REQUEST['record_id'])
{
    ...
    if(mysql_num_rows($db->query($sql_record)))
		...
		$record_id=0;
	$pay_amount='';
	if($row_record['payable_amount']!= $row_record['remaining_amount'])
	{
		$pay_amount="1";
	}
...
/*$sel_prev_id="select DISTINCT(admin_id) from staff_previleges where 1 ".$prev_value."";
$ptr_id=mysql_query($sel_prev_id);
if(mysql_num_rows($ptr_id))
{
	while($data_prev_id=mysql_fetch_array($ptr_id))
	{
		echo "<br/>".$sel_staff = "select admin_id,name from site_setting where 1 and admin_id='".$data_prev_id['admin_id']."' ".$_SESSION['where']." order by admin_id asc";	
	...
	if(cust_id != 'custome' )
	{
        $.ajax({
            ...
            success: function (html)
            {
				...
				if(document.getElementsByName("total_services[]"))
				showUser();	
            }
            });
	}
	...
	//alert(data1);
        $.ajax({
            ...
            success: function (html)
            {
				...
				document.getElementById("sin_service_disc"+val_idss).value = 0;
				document.getElementById("sin_service_disc_price"+val_idss).value = 0;
				total_service_time =Number(parseInt(total_service_time)+parseInt(service_split[1]));
				//alert(total_service_time);
			...
			{		
				...
				document.getElementById("staff_id"+val_idss+"_chosen").classList.remove("chosen-disabled");
			}
			else
			{
				//alert("HI");
...
//alert(data1);
	$.ajax({
	url:'send_email.php',type:"post",data:data1,cache:false,crossDomain:true,async:false, //http://www.htdpt.in/tracking/send_email_inq.php
...
function show_mobile_no(value,id)
{
	...
	success: function (html)
	{
		if(html.trim()!='')
		{
			...
			setTimeout(getMembership(value),200);
			$("#realtxt").chosen({allow_single_deselect:true});
			return true;
		}
	}
    ...
    <td class="mid_mid">
        ...
        <?php
                        ...
                        if($_POST['save_changes'])
                        {
							...
							if($added_date !='' )
							{
								$sep_added_date = explode('/',$added_date);
								$added_date = $sep_added_date[2].'-'.$sep_added_date[1].'-'.$sep_added_date[0]." ".date('H:i:s');
							}
                            ...
                            else
                            {
								...
								if($record_id)
								{
									...
									$update_cust_invoice="update customer_service_invoice set `service_price`='".$service_price."',`total_cost`='".$total_cost."',`amount`='".$amount."',`payable_amount`= '".$payable_amount."',`excluded_tax_amount`='".$payble_excluded_amnt."',`apply_gst`='".$apply_gst."',`show_gst`='".$show_gst."',`gst_type`='". $gst_type."',`cgst_tax`='".$payble_cgst_val."',`sgst_tax`='".$payble_sgst_val."',`igst_tax`='".$payble_igst_val."',`cgst_tax_in_percent`='".$cgst_tax_in_per."',`sgst_tax_in_percent`='".$sgst_tax_in_per."',`igst_tax_in_percent`='".$igst_tax_in_per."',`remaining_amount`='".$remaining_amount."',`total_paid`='".$payable_amount."',`cgst_tax`='".$cgst_tax."',`sgst_tax`='".$sgst_tax."',`cgst_tax_in_percent`='".$cgst_tax_in_per."',`sgst_tax_in_percent`='".$sgst_tax_in_per."',`paid_type`='".$payment_mode_id."',`bank_id`='".$bank_name."',`cust_bank_name`='".$cust_bank_name."', `cheque_detail`='".$chaque_no."', `chaque_date`='".$sep_check_date."', `credit_card_no`='".$credit_card_no."',`bank_ref_no`='".$bank_ref_no."', `admin_id`='".$_SESSION['admin_id']."', `added_date`='".$added_date."',`cm_id`='".$cm_id1."' where customer_service_id='".$record_id."' order by invoice_id asc ";
									$ptr_update=mysql_query($update_cust_invoice);
									
									"<br/>".$ins_followups="update customer_service_followups set customer_id='".$customer_id."',followup_date='".$followup_date."',treatment_followup_date='".$treatment_followup_date."',followup_desc='".$followup_desc."',admin_id='".$_SESSION['admin_id']."',cm_id='".$cm_id1."',added_date='".$added_date."' where customer_service_id='".$record_id."'";
									$ptr_ins=mysql_query($ins_followups);
									
									$update_bnk="update bank_records set amount='".$payable_amount."',bank_id='".$bank_name."' where record_id='".$record_id."' AND type='service' order by invoice_id asc";
									...
									for($z=1;$z<=$total_floor;$z++)
									{
										...
										if($_POST['del_floor'.$z]=='yes')
										{
											...
											{
												$delete_row = " delete from customer_service_map where customer_service_map_id='".$_POST['floor_id'.$z]."' ";
										...
										if($_POST['del_floor'.$z] !='yes')
									  	{
											...
											$start_t=trim(str_replace("T"," ",$_POST['from_time'.$z]));
											$end_t=trim(str_replace("T"," ",$_POST['to_time'.$z]));											
									...
									"<br/>-".$pay_amount."--".$remaining_amount;
									if($pay_amount!='' && $remaining_amount=='0')
									{
										//=============SMS Send================
										...
										"<br/>".$contact=$data_cust_name['mobile1'];
										...
										if(mysql_num_rows($ptr_inq))
										{
											...
											$txt_msg=strip_tags($dta_msg['sms_text']);
											$sep_text=explode("###",$txt_msg);
										}
										
										"<br/>".$txt_msg=$sep_text[1];
										$messagessss =strip_tags($txt_msg);
										$sep=explode(" ",$start);
										...
										echo "<br/>".$messagessss = str_replace($search_by, $replace_by, $messagessss);
									...
									$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('book_service','Edit','".$fetch_cust['cust_name']."','".$record_id."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
								...
								else
								{
									...
									for($i=1;$i<=$total_floor;$i++)
									{
										if($_POST['service_id'.$i] !='')
										{
											...
											$data_record_service['added_date'] =$added_date;
									...
									$sep_check_date=$chaque_date_exp[2].'-'.$chaque_date_exp[1].'-'.$chaque_date_exp[0];
									 ...
									 $insert_cust_service_invoice = " INSERT INTO `customer_service_invoice` (`customer_service_id`, `service_price`, `total_cost`, `amount`, `payable_amount`,`remaining_amount`, `total_paid`,`paid_type`, `bank_id`, `bank_ref_no`, `cust_bank_name`, `cheque_detail`, `chaque_date`, `credit_card_no`, `admin_id`, `added_date`,`status`,`cm_id`) VALUES ('".$record_id."', '".$service_price."', '".$total_cost."', '".$amount."', '".$payable_amount."','".$remaining_amount."', '".$payable_amount."', '".$payment_mode_id."','".$bank_name."','".$bank_ref_no."','".$cust_bank_name."', '".$chaque_no."', '".$sep_check_date."','".$credit_card_no."', '".$_SESSION['admin_id']."', '".date('Y-m-d H:i:s')."','".$status."','".$cm_id1."'); ";
									...
									if($category=="Voucher")
									{
										for($j=1;$j<=$total_type1;$j++)
										{
											...
											if($categories=="amount")
											{
												...
												{
													$update_query="update sales_package_voucher_memb set quantity = (quantity-1) where cust_id='".$customer_id."' and voucher_id='".$voucher_deal_id."'";
													$ptr_qry=mysql_query($update_query);
												}
												$update_vou_code="update voucher_customer_code_map set status ='".$data_record_sales['status']."',redeem_price='".$data_record_sales['amount']."' where voucher_id='".$voucher_deal_id."' and customer_id='".$customer_id."' and voucher_code_id='".$voucher_code_id."'";
											...
											if($categories=="service")
											{
												...
												if($total_vsm=mysql_num_rows($ptr_vsm))
												{
													...
													{
														$sel_svcsm="select svm_id from sales_customer_service_voucher_map where service_id='".$data_vsm['service_id']."' and voucher_id='".$data_vsm['voucher_id']."' and quantity > 0 ";
									...
									$mesg ="Hi ".$name." your service is book";
									$sel_inq="select sms_text from previleges where privilege_id='112'";
									$ptr_inq=mysql_query($sel_inq);
									$txt_msg='';
									...
									"<br/>".$sel_sms_cnt="select * from sms_mail_configuration_map where previlege_id='112'";
									$ptr_sel_sms=mysql_query($sel_sms_cnt);
									while($data_sel_cnt=mysql_fetch_array($ptr_sel_sms))
									{
									...
									?><div id="statusChangesDiv" title="Record Deleted"><center><br><p>Record Added successfully </p></center></div>
										<script type="text/javascript">
                                            ...
                                            setTimeout('document.location.href="manage_cust_services.php";',1000);
                                        </script>
                                    <?php
								}
							}
	...
	?>
		...
		<table border="0" cellspacing="15" cellpadding="0" width="100%">
              <tr>	
				...
				<?php 
					$sel="select end_event_time from customer_service_map where customer_service_id='".$record_id."' order by customer_service_map_id desc limit 0,1";
					$ptr_sel=mysql_query($sel);
					$data_time=mysql_fetch_array($ptr_sel);
					$times=trim($data_time['end_event_time']);
					$strt_time=str_replace(" ","T",$times);
				
				?>
				<input type="hidden" name="last_start" id="last_start" value="<?php echo $strt_time; ?>" />
            ...
            <!--  <tr>
              ...
              <td>
		   	...
		   	if($_SESSION['type']=="S")
		   	{
		   		?>
           		<tr>
					<td width="15%" >Date<span class="orange_font"></span></td>
					<td width="49%"><input type="text" name="added_date" id="added_date" class="input_text datepicker" value="<?php if($_POST['added_date']) echo $_POST['added_date']; else if($row_record['added_date']!='') { $date=explode(" ",$row_record['added_date']); $newDate=explode("-",$date[0]); echo $newDate[2]."/".$newDate[1]."/".$newDate[0]; } else echo date('d/m/Y') ?>"  />
					</td>
				</tr>
           		<?php
		   	}
			...
			{
				?>
                <input type="hidden" name="added_date" id="added_date" value="<?php if($row_record['added_date']!='') { $date=explode(" ",$row_record['added_date']); $newDate=explode("-",$date[0]); echo $newDate[2]."/".$newDate[1]."/".$newDate[0]; } else echo date('d/m/Y') ?>"  />
                <?php
			}
			?>
			<tr>
            ...
            <tr>
                <td width="15%" valign="top">Show GST<span class="orange_font">*</span></td>
            ...
            <tr>           		
                <td width="15%" >Select Customer Type<span class="orange_font">*</span></td>
                <td>
    ...
    <tr>
                        ...
                        <tr>
                            ...
                            { 
                            	...
                            	<tr>
                            	<td valign="top" width="15%" align="center"><input type="hidden" name="from_time<?php echo $t; ?>" id="from_time<?php echo $t; ?>" value="<?php echo $data_exclusive['start_event_time'] ?>" width="40px" /><input type="hidden" name="to_time<?php echo $t; ?>" id="to_time<?php echo $t; ?>" value="<?php echo $data_exclusive['end_event_time'] ?>"  width="40px" /><select name="service_id<?php echo $t; ?>" id="service_id<?php echo $t; ?>" style="width:140px" onChange="getprice(this.value,<?php echo $t; ?>)"><option value="">Select Service</option><?php
								$sel_tel = "select service_id,service_name,service_price,service_time from servies order by service_id asc";	 
								$query_tel = mysql_query($sel_tel);
								if($total=mysql_num_rows($query_tel))
								{
            ...
            <tr>
            	...
            	<table width="100%">
            		<tr>
                		<td width="15%" valign="top">Membership Discount Price</td>
			...
			if($_SESSION['tax_type']=='GST')
			{
                ...
                <tr>      
                      ...
                      <td><input type="text" class="input_text" readonly="readonly" name="cgst_tax" id="cgst_tax"  value="<?php if($_POST['cgst_tax']) echo $_POST['cgst_tax']; else echo $row_record['cgst_tax'];?>" /></td>
                </tr>
                <tr>      
                      ...
                      <td><input type="text" class="input_text" readonly="readonly" name="sgst_tax" id="sgst_tax"  value="<?php if($_POST['sgst_tax']) echo $_POST['sgst_tax']; else echo $row_record['sgst_tax'];?>" /></td>
                </tr>
                <tr>      
                      ...
                      <td><input type="text" class="input_text" readonly="readonly" name="igst_tax" id="igst_tax"  value="<?php if($_POST['igst_tax']) echo $_POST['igst_tax']; else echo $row_record['igst_tax'];?>" /></td>
                </tr>
                <tr>
           ...
           <!-- <tr>
            ...
            <tr>
            	...
            	<td width="15%" valign="top">End Time<span class="orange_font">*</span></td>
             	...
             	<td width="70%"><input type="text"  class=" input_text" style="width:50px" name="end_time" id="end_time" maxlength="2" value="<?php //if($_POST['end_time']) echo $_POST['end_time']; else echo $row_record['end_time'];?>" />
            ...
            <tr>
             ...
             <td colspan="2">
               ...
               <tr>
                ...
                <tr>
                	<td width="15%" class="tr-header">Amount<span class="orange_font">*</span></td>
                ...
                <tr>
					<td width="15%" class="tr-header" align="">Select Employee<span class="orange_font">*</span></td>
...
function showUser(non_memb_disc)
{
	...
	document.getElementById('discount_price').value=roundNumber(total_discount_price,3);
	...
	if(nonmemb_discount_price >0)
	{
		...
		document.getElementById('nonmemb_discount_price').value=roundNumber(total_nonmemb_discount_price,3);
	}
	else
	{
	...
	if(document.getElementById('category'))
	{
		...
		if(category=="loyalty_point")
		{
			...
			document.getElementById('nonmemb_discount_price').value=roundNumber(total,3);
	...
	if(gst_show=='yes')
	{
		...
		if(gst_type=='m_gst')
		{
			...
			//=========================================================================
			var new_tax=roundNumber(parseFloat(new_cgst_tax)+parseFloat(new_sgst_tax),3);
		}
		else
		{
	...
	else if(gst_show=='included')
	{
		...
		if(gst_type=='m_gst')
		{
			...
			document.getElementById('excluded_tax').value=roundNumber(excluded_tax,3);
			//alert(excluded_tax);
			var new_tax=0;
		}
		else
		{
			...
			new_igst_tax=parseFloat(total_gst);
			document.getElementById('igst_tax').value=new_igst_tax;//Math.round
			
			
			excluded_tax=parseFloat(disc_price)-(parseFloat(new_igst_tax));
			document.getElementById('excluded_tax').value=roundNumber(excluded_tax,3);
			//alert(excluded_tax);
			var new_tax=0;
		}
