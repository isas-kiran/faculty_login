...
if($_REQUEST['record_id'])
{
    ...
    $sql_record= "SELECT * FROM inword_inventory where inword_id='".$record_id."'";
  ...
  <tr>
        ...
        <?php
                        ...
                        if($_POST['save_changes'])
                        {
                            ...
                            else
                            {
								...
								$data_record['admin_id']=$_POST['stockiest'];
								
								
                                if($record_id)
                                {
                                    $where_record="inword_id='".$record_id."'";
                                    $db->query_update("inword_inventory", $data_record,$where_record);
									for($z=1;$z<=$total_floor;$z++)
									{
										...
										if($_POST['del_floor'.$z]=='yes')
										{
											...
											{
												 "<br />".$delete_row = " delete from inword_product_map where inword_map_id='".$_POST['floor_id'.$z]."' ";
										...
										if($_POST['del_floor'.$z] !='yes')
										{
											$data_record_service['inword_id'] =$record_id; 
											...
											{
											 	$type1_id=$db->query_insert("inword_product_map", $data_record_service);
											...
											else
											{
												...
												$db->query_update("inword_product_map", $data_record_service,$where_record);
                                ...
                                else
                                {
                                    ...
                                    $record_id=$db->query_insert("inword_inventory", $data_record);
									for($i=1;$i<=$total_floor;$i++)
									{
										...
										$customer_service_id=$db->query_insert("inword_product_map", $data_record_service);
	...
	<table border="0" cellspacing="15" cellpadding="0" width="100%">
           ...
           <tr>
                  ...
                  <td>
                   <select name="vendor_id" id="vendor_id" class="input_select" style="width:200px;">
		    ...
		    <tr>
                  ...
                  <td>
                   ...
                   <?php
				   echo $sql_vendor = " select name, admin_id from site_setting where 1 and type='ST' ".$_SESSION['where']." order by admin_id asc";
					$ptr_vendor = mysql_query($sql_vendor);
					while($data_vendor = mysql_fetch_array($ptr_vendor))
					{ 
						$selecteds = '';
						if($data_vendor['admin_id']==$row_record['admin_id'])
						$selecteds = 'selected="selected"';	
           ...
           <?php 
		      ...
		      $rowSQL = mysql_query("SELECT MAX(inword_id) as max FROM `inword_inventory`" );
                    ...
                    <table cellpadding="5" width="100%" >
                     ...
                     <script language="javascript">
                                function floors(idss)
                                {
                                    ...
                                    var shows_data='<div id="floor_id'+idss+'"><table cellspacing="3" id="tbl'+idss+'" width="100%"><tr><td><div id="prod_list"></div></td></tr><tr><td width="10%" align="left"><input type="hidden" name="exclusive_id'+idss+'" id="exclusive_id'+idss+'" value="'+idss+'" /><select name="product_id'+idss+'" id="product_id'+idss+'" style="width:200px"><option value="">Select Product</option>'+res+'</select></td><td width="8%" align="center"><input type="text" name="sin_product_qty'+idss+'" id="sin_product_qty'+idss+'" style=" width:60px" /></td><td valign="top" width="1%" align="center"><input type="hidden" name="total_product[]" id="total_product'+idss+'" /></td><input type="hidden" name="row_deleted'+idss+'" id="row_deleted'+idss+'" value="" /></tr><tr><td colspan="7"><div style="display:none" id="product_details'+idss+'"><table style="width:100%" align="center"><tr><td width="10%">Price : <span id="price_desc'+idss+'"></span></td><td width="10%">Brand : <span id="brand'+idss+'"></span></td><td width="10%">Unit : <span id="unit_desc'+idss+'"></span><span id="measure_desc'+idss+'"></span></td><td width="70%">Product Desc.: <span id="description_desc'+idss+'"></span></td></tr></table></div></td></tr></table></div>';
                        ...
                        <td colspan="7">
                        	<table cellspacing="3" id="tbl" width="100%">
                        		...
                        		<tr>
									...
									if($record_id!='')
									{
                            			 $select_exc = "select * from inword_product_map where inword_id='".$record_id."' order by inword_map_id asc ";
                            			...
                            			{ 
                                			$slab_id= $data_exclusive['inword_map_id'];
											?> 
											...
											<table cellspacing="5" id="tbl<?php echo $t; ?>" width="100%">
												<tr>
													...
													$sql_dest = " select product_id, admin_id from product where admin_id='".$row_record['admin_id']."' ";
													...
													?>
														...
														<input type="hidden" name="floor_id<?php echo $t; ?>" id="floor_id_<?php echo $t; ?>" value="<?php echo $data_exclusive['inword_map_id'] ?>" />
