	...
	<table border="0" cellspacing="15" cellpadding="0" width="100%">
			...
			<td colspan="3">			
				...
				dp.contextMenu = new DayPilot.Menu({items: [
						...
						{text:"Job Card", onclick: function() {window.open('job-card-generate.php?record_id='+this.source.id() +'','win1','status=no,toolbar=no,scrollbars=yes,titlebar=no,menubar=no,resizable=yes,width=900, height=600,directories=no,location=no')}, target:"_blank"},
        ...
        if($_POST['post_data'])
		{
			...
			$type=( ($_POST['user'])) ? $_POST['user'] : "Customer";
			...
			else
			{
				...
				$data_record['inquiry_id'] =$record_id;
				...
				if($record_id)
				{
					if($cust_name!='')
					{
						"<br/>".$ins_cust="insert into customer(`branch_name`,`cust_name`,`mobile1`,`cm_id`,`added_date`) values ('".$branch_name."','".$cust_name."','".$row_record['mobile']."','".$cm_id1."','".date('Y-m-d')."')";
						$ptr_cust=mysql_query($ins_cust);
					...
					$cust_serv_id=$db->query_insert("customer_service", $data_record);
					...
					"<br/>".$insert_cust_service_invoice = " INSERT INTO `customer_service_invoice` (`customer_service_id`,`receipt_no`, `service_price`, `total_cost`, `amount`, `payable_amount`,`remaining_amount`, `total_paid`,`paid_type`, `bank_id`, `cust_bank_name`,`cheque_detail`, `chaque_date`, `credit_card_no`,`isas_paytm_no`,`cust_paytm_no`, `admin_id`, `added_date`,`status`,`cm_id`) VALUES ('".$cust_serv_id."','".$receipt_no."', '".$service_price."', '".$total_cost."', '".$amount."', '".$payable_amount."','".$remaining_amount."', '".$payable_amount."', '".$payment_mode_id."','".$bank_name."','".$cust_bank_name."', '".$chaque_no."', '".$sep_check_date."','".$credit_card_no."','".$isas_paytm_no."','".$cust_paytm_no."', '".$_SESSION['admin_id']."', '".date('Y-m-d H:i:s')."','".$status."','".$cm_id1."'); ";
					...
					{
				   		$bank="INSERT INTO `bank_records`(`bank_id`, `type`, `record_id`, `invoice_id`, `amount`, `added_date`, `cm_id`, `admin_id`) VALUES ('".$bank_name."','service','".$cust_serv_id."','".$ins_id."','".$payable_amount."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
					...
					$update_serv="update service_inquiry set status='Booked' where inquiry_id='".$record_id."'";
					$ptr_serv=mysql_query($update_serv);
					
					$ins_followups="insert into customer_service_followups (`customer_id`, `customer_service_id`, `followup_date`, `treatment_followup_date`, `followup_desc`, `admin_id`,`cm_id`,`added_date`) values('".$customer_id."','".$cust_serv_id."','".$followup_date."','".$treatment_followup_date."','".$followup_desc."','".$_SESSION['admin_id']."','".$cm_id1."','".date('Y-m-d H:i:s')."')";
					...
					if($category=="Voucher")
					{
						for($j=1;$j<=$total_type1;$j++)
						{
							...
							if($categories=="service")
							{
								$sel_vsm="select service_id,quantity,voucher_id from voucher_service_map where voucher_id='".$voucher_deal_id."'";
										...
										if(mysql_num_rows($ptr_svcsm))
										{
											for($i=1;$i<=$total_floor;$i++)
											{
												if($_POST['service_id'.$i]!='')
												{
													...
													{
														$update_prod_qty="update sales_customer_service_voucher_map set quantity=(quantity -1) where service_id='".$data_vsm['service_id']."' and voucher_id='".$voucher_deal_id."' and voucher_code_id ='".$voucher_code_id."'";
					...
					else if($category=="Package")
					{
						...
						if($total <= 0)
						{
							...
							if(mysql_num_rows($ptr_spcvs))
							{
								$update_query="update sales_package_voucher_memb set status='inactive' where cust_id='".$customer_id."' and package_id='".$packagess_deal_id."'";
								$ptr_qry=mysql_query($update_query);
							}
						}
					}
					
					$sql_query= "SELECT cust_name FROM customer where cust_id ='".$customer_id."' ";              
					$query=mysql_query($sql_query);
					$fetch=mysql_fetch_array($query);
					$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('book_service','Add','".$fetch['cust_name']."','".$ins_id."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
					...
					{
						"<br/>".$messagessss = "Hello ".$name.", We just want to say it was privilege to have the opportunity to serve you at the Innocent Beauty Salon. I hope you enjoyed the service as much as we enjoyed rendering it to you. As a thank you, I would like to offer you 50% of your next same service. Please call us to book an appointment.".$address."";
					}
                   ...
                   <tr>
                    ...
                    <tr>
                        <td colspan="2">
                            ...
                            <table width="100%">
                            	<tr>
                                	<td width="18%">Select Customer</td>
                        			<td><input type="text" name="cust_name" class="input_text" id="cust_name" value="<?php if($_POST['cust_name']) echo $_POST['cust_name']; else echo $row_record['name'] ?>"/></td>
                                </tr>
                            </table>
                            </div>
                        </td>
                        ...
                        <table  width="100%" style="border:1px solid gray; ">
                            ...
                            <table cellpadding="5" width="100%" >
                             ...
                             <script language="javascript">
                                        ...
                                        function floors(idss)
                                        {
                                            ...
                                            //}
                                             ?>
                                             </select><input type="hidden" name="total_services[]" id="total_services'+idss+'" /></td><td width="8%"></td><input type="hidden" name="row_deleted'+idss+'" id="row_deleted'+idss+'" value="" /><tr></table></div>';
							...
							<tr>
                                ...
                                <tr>
                                    ...
                                    <td valign="top" width="8%"  align="center">action</td>
                                </tr>
                                <tr>
                                    <td colspan="7">
                                    <?php
                                    if($record_id)
                                    {
										...
										{ 
											...
											$total_service_time +=$data_exclusive['service_time'];
											?> 
											<div class="floor_div" id="floor_id<?php echo $t; ?>">
											<table cellspacing="5" id="tbl<?php echo $t; ?>" width="100%">
											<tr>
											<td valign="top" width="21%" align="center"><select name="service_id<?php echo $t; ?>" id="service_id<?php echo $t; ?>" style="width:140px" onChange="getprice(this.value,<?php echo $t; ?>)"><option value="">Select Service</option><?php
                                            ...
                                            <script>
											$("#service_id<?php echo $t; ?>").chosen({allow_single_deselect:true});
					...
					<tr>      
						  <td width="15%" class="heading">CGST <span id="cgst_id"><?php if($_SESSION['type']!='S'){ echo $_SESSION['cgst'];} ?></span>%<span class="orange_font">*</span></td><input type="hidden" id="cgst_taxes" name="cgst_taxes" value="<?php if($_SESSION['type']!='S'){ echo $_SESSION['cgst'];} ?>"  />
					...
					<tr>      
						  <td width="15%" class="heading">SGST <span id="sgst_id"><?php if($_SESSION['type']!='S'){ echo $_SESSION['sgst'];} ?></span>%<span class="orange_font">*</span></td><input type="hidden" id="sgst_taxes" name="sgst_taxes" value="<?php if($_SESSION['type']!='S'){ echo $_SESSION['sgst'];} ?>"  />
...
function showUser(non_memb_disc)
{
	...
	var total_cost = Math.round(parseFloat(disc_price) +  parseFloat( new_tax));
	document.getElementById('total_cost').value=Math.round(total_cost);
