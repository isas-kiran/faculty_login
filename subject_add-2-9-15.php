...
if($_REQUEST['record_id'])
{
    ...
    $sql_record= "SELECT * FROM subject where subject_id='".$record_id."'";
...
//             $('input:radio[name=status][value=Inactive]').click();
	 ...
	 function desriptionss(value)
	{
		varsss ='<?php
				...
				$oFCKeditor->Create() ;*/
				 ?> <textarea cols="47" name="description'+value+'" ></textarea>';return varsss;
		
	}
	
	function select_course(value)
	{
	vars ='<select name="course_id'+value+'" id="course_id'+value+'" class="input_select" ><option value=""> Select Course</option><?php
    ...
    function del_degree(del)  // for delete the degree
					 ...
					 function add_degree(no) //for add a degree
					 {
						...
						{
							var course= select_course(i); 
							var description = desriptionss(i);
							var teacher = techerss(i)
					 	var value='<div id="'+i+'"><table class="table_class" width="90%"  cellspacing="0" cellpadding="2"> <tr style="text-align:center;"><td align="center" >'+course+'</td><td align="center"><input type="text" name="name'+i+'" id="name'+i+'"  class="input_text"   ></td><td align="center">'+teacher+'</td><td align="center">'+description+'</td><td align="center" ><input type="text" name="sub_fees'+i+'" id="sub_fees'+i+'" onblur="show_balance()" class="input_text" > </td></tr></table></div> <div id="'+next+'"></div>';
        ...
        <?php
                    ...
                    <?php
                        ...
                        $success=1;
							...
							if($data_record_extra['course_id'] !='' && $data_record_extra['name'] )
							{
	...
	<table border="0" cellspacing="10" cellpadding="0" width="100%">
            ...
            <!--<tr>
                ...
                <div width="12%" style="height:30px">Subject Name <span class="orange_font">*</span></div>
                    <input type="text" class="validate[required] input_text" name="name" id="name" value="<?php if($_POST['save_changes']) echo $_POST['name']; else echo $row_record['name'];?>" />
                      ...
                      <?php
                $sql_sub_cat = "select * from courses where course_id='".$row_record['course_id']."' ".$_SESSION['where_admin_id']." ";
            ...
            <td width="16%" >
                ...
                <option value="" >Select Teacher </option>                        
                        <?php 
                            $select_faculty = "select * from staff_regi ".$_SESSION['where_admin_id_2']." order by staff_id asc";
                            $ptr_faculty = mysql_query($select_faculty);
                            while($data_faculty = mysql_fetch_array($ptr_faculty))
                            { 
                              echo '<option value="'.$data_faculty['staff_id'].'" >'.$data_faculty['staff_name'].'</option>';  
                ...
                <td >
                <div width="12%" style="height:30px">Description</div>
                    <?php
                            include("../FCKeditor/fckeditor.php");
                            $BasePath = "../FCKeditor/";
                            $oFCKeditor 		= new FCKeditor('description') ;
