...
function book_lost(ids)
{
	if(confirm("Are you sure, you want to delete selected record(s)?"))
	{
		var data1="action=lost&book_issue_id="+ids;	
		alert(data1);
		$.ajax({
...
<?php
                        ...
                        if($_GET['orderby']=='book_issue_id' )
                            $img1 = $img;

                        if($_GET['order'] !='' && ($_GET['orderby']=='book_issue_id'))
                        {
                            $select_directory .= " order by ".$_GET['orderby']." " .$_GET['order'] ;
                            $date_query='&order='.$_GET['order'].'&orderby='.$_GET['orderby'];
                        }
                        else
							$select_directory='order by i.book_issue_id desc';  
						$record_cat_id='';
						if($_GET['record_id'] !='')
						{
							$record_cat_id="and i.book_issue_id='".$_GET['record_id']."' ";
						}
						if($_SESSION['where'] !='')
						{
							$cm_ids="and i.cm_id='".$_SESSION['cm_id']."'";
						}
						$sql_query= "SELECT i.book_issue_id,b.book_name,b.isbn_no,i.book_id,i.issue_to,i.issue_date,i.issue_days,i.user_type,i.status FROM lb_book_issue i,lb_book b where 1 and i.status='active' and i.book_id=b.book_id ".$record_cat_id." ".$cm_ids." ".$search_cm_id_S." ".$pre_keyword." ".$select_directory."";
                     ...
                     <input type="hidden" name="formAction" id="formAction" value=""/>
                      <tr class="grey_td" >
					   <?php
                        ...
                        <td width="4%" align="center"><strong>Issued days</strong></td>
                        <td width="4%" align="center"><strong>User type</strong></td>
                        <td width="10%" align="center"><strong>Status</strong></td>
                        <td width="6%" align="center"><strong>Return</strong></td>
                        <td width="5%" align="center"><strong>Lost</strong></td>
						<?php
                        ...
                        while($val_query=mysql_fetch_array($all_records))
                        {
							...
							$listed_record_id=$val_query['book_issue_id']; 
							...
							{
								$sel_user="select name from enrollment where enroll_id='".$val_query['issue_to']."'";
							...
							{
								$sel_user="select name from site_setting where admin_id='".$val_query['issue_to']."'";
								$query_user=mysql_query($sel_user);
								$data_user=mysql_fetch_array($query_user);
								$name=$data_user['name'];
							}	
							...
							echo '<td align="center">'.$val_query['issue_days'].'</td>';
							echo '<td align="center">'.$val_query['user_type'].'</td>';
							echo '<td align="center">'.$val_query['status'].'</td>';
							echo '<td align="center">';
							if($val_query['status']=='active')
								echo '<a href="lb_return_book.php?book_issue_id='.$listed_record_id.'"><button name="return" type="button">Return</button></a></td>';
							else
								echo 'Return';
							echo '</td>';
							echo '<td align="center">';	
							if($val_query['status']=='active')
								echo '<a href="lb_lost_book.php?book_issue_id='.$listed_record_id.'"><button name="lost" type="button">Lost</button></a></td>';
							else
								echo 'Lost';
							echo '</td>';
							echo '<td align="center">';
