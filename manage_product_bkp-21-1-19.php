...
<!--                                <option value="Active">Active</option>
    ...
    <?php
						...
						else
						{
							...
							$sql_query= "SELECT p.product_name, p.product_code,p.status, p.description, p.size, p.commission, p.price, p.pcategory_id, p.sub_id, p.product_id,p.quantity,p.quantity_in_shelf,p.quantity_in_consumable,p.added_date,p.admin_id,p.hsn_code FROM product p, product_category pc, product_subcategory sub where 1  and (p.product_name like '%".$keyword."%' or p.product_code like '%".$keyword."%' or p.description like '%".$keyword."%' or p.size like '%".$keyword."%' or p.commission like '%".$keyword."%' or p.price like '%".$keyword."%' or pc.pcategory_name like '%".$keyword."%' or sub.sub_name like '%".$keyword."%' ) and p.pcategory_id=pc.pcategory_id and p.sub_id=sub.sub_id ".$cm_ids."  ".$search_cm_id_S." ".$search_admin_id_S." ".$select_directory.""; //".$admin_ids."
						}
                       //echo $sql_query;
                        ...
                        if($no_of_records)
                        {
                            ...
                            $query_string='&keyword='.$keyword.'&branch_name='.$_REQUEST['branch_name'].'&stockiest='.$_REQUEST['stockiest'];
    ...
    <form method="post"  name="frmTakeAction" action="<?php echo $_SERVER['PHP_SELF'].'?#msgbox';?>" >
					...
					if( $_SESSION['type'] =='S')
                        ...
                        <td width="4%" align="center"><strong>Qty(Shelf)</strong></td>
                        <td width="4%" align="center"><strong>Qty(Cons.)</strong></td>
