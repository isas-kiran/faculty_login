...
function addNotifications($args)
{
    ...
    if(trim($args['on_action']) == "enquiry_added")
    {
        ...
        if($row_loggedin['type'] == 'A')
        {
            ...
            if($row_Councellor['admin_id'])
            {
                ...
                $data_record['redirect_page_url'] = 'manage_student.php?record_id='.$args['reference_id'];
        ...
        else if($row_loggedin['type'] == 'C')
        {
            ...
            //--second message to the admin---
            $sql_cm = "SELECT admin_id,cm_id FROM site_setting where type='C' and cm_id='".$row_loggedin['cm_id']."'";
			$db1=mysql_query($sql_cm);
			while($row_cm =mysql_fetch_array($db1))
			{
                $data_record['notification'] = 'New inquiry added by '.$row_loggedin['name'];
                $data_record['redirect_page_url'] = 'manage_student.php?record_id='.$args['reference_id'];
                $data_record['added_by'] = $_SESSION['admin_id'];
        ...
        {
            //--first message to the cm---
			"<br/>". $sql_cm = "SELECT admin_id,cm_id FROM site_setting where type='A' and cm_id='".$_SESSION['cm_id_notification']."'";
            ...
            if($row_cm['cm_id'])
            {
				...
				"<br/>".$ins_notifi="insert into notifications(`notification`,`redirect_page_url`,`added_by`,`added_for`,`on_action`,`is_read`,`added_date`) values('".$data_record['notification']."','".$data_record['redirect_page_url']."','".$data_record['added_by']."','".$data_record['added_for']."','".$data_record['on_action']."','".$data_record['is_read']."','".$data_record['added_date']."')";
				$ptr_inst=mysql_query($ins_notifi);
                //$db1->query_insert('notifications',$data_record);
            }
            //--second message to the Councellor and Super admin---
			$sql_councellor = "SELECT admin_id,cm_id FROM site_setting where type='C' or type='S' and admin_id!='".$_SESSION['admin_id']."' and cm_id='".$_SESSION['cm_id_notification']."'";
	...
	else if(trim($args['on_action']) == "enroll")
	{
		...
		//----get logged in user type and based on that send the notification ----
		"<br/>".$sql_councellor = "SELECT admin_id,cm_id FROM site_setting where type='C' or type='S' or type='A' or type='AC' and cm_id='".$row_loggedin['cm_id']."'";
		...
		while($row_Councellor =mysql_fetch_array($db1))
		{
			...
			$data_record['redirect_page_url'] = 'manage_enroll_student.php?record_id='.$args['reference_id'];
			$data_record['added_by'] = $_SESSION['admin_id'];
			$data_record['added_for'] = $row_Councellor['admin_id'];
			$data_record['on_action'] = $args['on_action'];
			$data_record['is_read'] = 'No';
	...
	else if(trim($args['on_action']) == "add_new_course")
	{
		$enroll_id = $args['reference_id'];//----you can use if you want to send the more details regarding the inquiry
		//----get logged in user type and based on that send the notification ----
		$sql_councellor = "SELECT admin_id,cm_id FROM site_setting where type='C' or type='S' or type='A' or type='AC' and cm_id='".$row_loggedin['cm_id']."'";
		$db1=mysql_query($sql_councellor);
		while($row_Councellor =mysql_fetch_array($db1))
		{
	...
	else if(trim($args['on_action']) == "installment_payment")
	{
		$enroll_id = $args['reference_id'];//----you can use if you want to send the more details regarding the inquiry
		//----get logged in user type and based on that send the notification ----
		$sql_councellor = "SELECT admin_id,cm_id FROM site_setting where type='C' or type='S' or type='A' or type='AC' and cm_id='".$row_loggedin['cm_id']."'";
		...
		while($row_Councellor =mysql_fetch_array($db1))
		{
			$data_record['notification'] = 'Installment payment done by faculty'.$row_loggedin['name'];
			$data_record['redirect_page_url'] = 'manage_enroll_student.php?record_id='.$args['reference_id'];
			$data_record['added_by'] = $_SESSION['admin_id'];
			$data_record['added_for'] = $row_Councellor['admin_id'];
			$data_record['on_action'] = $args['on_action'];
	...
	else if(trim($args['on_action']) == "receipt_payment")
	{
		$enroll_id = $args['reference_id'];//----you can use if you want to send the more details regarding the inquiry
		$sql_councellor = "SELECT admin_id,cm_id FROM site_setting where type='S' or type='A' or type='AC' and cm_id='".$row_loggedin['cm_id']."'";
		$db1=mysql_query($sql_councellor);
		while($row_Councellor =mysql_fetch_array($db1))
		{
			$data_record['notification'] = 'Incomming payment by receipt by faculty '.$row_loggedin['name'];
			$data_record['redirect_page_url'] = 'receipt.php?receipt_id='.$args['reference_id'];
			$data_record['added_by'] = $_SESSION['admin_id'];
			$data_record['added_for'] = $row_Councellor['admin_id'];
			$data_record['on_action'] = $args['on_action'];
			$data_record['is_read'] = 'No';
	...
	else if(trim($args['on_action']) == "expense")
	{
		$enroll_id = $args['reference_id'];//----you can use if you want to send the more details regarding the inquiry
		$sql_councellor = "SELECT admin_id,cm_id FROM site_setting where type='S' or type='A' or type='AC' and cm_id='".$row_loggedin['cm_id']."'";
		$db1=mysql_query($sql_councellor);
		while($row_Councellor =mysql_fetch_array($db1))
		{
			$data_record['notification'] = 'Expense added by faculty '.$row_loggedin['name'];
			$data_record['redirect_page_url'] = 'expense.php?expense_id='.$args['reference_id'];
			$data_record['added_by'] = $_SESSION['admin_id'];
			$data_record['added_for'] = $row_Councellor['admin_id'];
			$data_record['on_action'] = $args['on_action'];
			$data_record['is_read'] = 'No';
	...
	else if(trim($args['on_action']) == "book_service")
	{
		$enroll_id = $args['reference_id'];//----you can use if you want to send the more details regarding the inquiry
		$sql_councellor = "SELECT admin_id,cm_id FROM site_setting where type='S' or type='A' and cm_id='".$row_loggedin['cm_id']."'";
		$db1=mysql_query($sql_councellor);
		while($row_Councellor =mysql_fetch_array($db1))
		{
			$data_record['notification'] = ' Customer Service Booked by faculty '.$row_loggedin['name'];
			$data_record['redirect_page_url'] = 'manage_cust_services.php?record_id='.$args['reference_id'];
			$data_record['added_by'] = $_SESSION['admin_id'];
			$data_record['added_for'] = $row_Councellor['admin_id'];
			$data_record['on_action'] = $args['on_action'];
			$data_record['is_read'] = 'No';
	...
	else if(trim($args['on_action']) == "sale_voucher")
	{
		$enroll_id = $args['reference_id'];//----you can use if you want to send the more details regarding the inquiry
		$sql_councellor = "SELECT admin_id,cm_id FROM site_setting where type='S' or type='A' and cm_id='".$row_loggedin['cm_id']."'";
		$db1=mysql_query($sql_councellor);
		while($row_Councellor =mysql_fetch_array($db1))
		{
			$data_record['notification'] = 'Sale Voucher to customer by faculty '.$row_loggedin['name'];
			$data_record['redirect_page_url'] = 'manage_memb_package_voucher.php?record_id='.$args['reference_id'];
			$data_record['added_by'] = $_SESSION['admin_id'];
			$data_record['added_for'] = $row_Councellor['admin_id'];
			$data_record['on_action'] = $args['on_action'];
			$data_record['is_read'] = 'No';
	...
	else if(trim($args['on_action']) == "sale_package")
	{
		$enroll_id = $args['reference_id'];//----you can use if you want to send the more details regarding the inquiry
		$sql_councellor = "SELECT admin_id,cm_id FROM site_setting where type='S' or type='A' and cm_id='".$row_loggedin['cm_id']."'";
		$db1=mysql_query($sql_councellor);
		while($row_Councellor =mysql_fetch_array($db1))
		{
			$data_record['notification'] = 'Sale Package to customer by faculty '.$row_loggedin['name'];
			$data_record['redirect_page_url'] = 'manage_memb_package_voucher.php?record_id='.$args['reference_id'];
			$data_record['added_by'] = $_SESSION['admin_id'];
			$data_record['added_for'] = $row_Councellor['admin_id'];
			$data_record['on_action'] = $args['on_action'];
	...
	else if(trim($args['on_action']) == "product_purchase")
	{
		$enroll_id = $args['reference_id'];//----you can use if you want to send the more details regarding the inquiry
		$sql_councellor = "SELECT admin_id,cm_id FROM site_setting where type='S' or type='A' or type='AC' or type='ST' and cm_id='".$row_loggedin['cm_id']."'";
		$db1=mysql_query($sql_councellor);
		while($row_Councellor =mysql_fetch_array($db1))
		{
			$data_record['notification'] = 'Purchase product from vendor by faculty '.$row_loggedin['name'];
			$data_record['redirect_page_url'] = 'manage_inventory.php?record_id='.$args['reference_id'];
			$data_record['added_by'] = $_SESSION['admin_id'];
			$data_record['added_for'] = $row_Councellor['admin_id'];
			$data_record['on_action'] = $args['on_action'];
			$data_record['is_read'] = 'No';
	...
	else if(trim($args['on_action']) == "product_purchase_payment")
	{
		$enroll_id = $args['reference_id'];//----you can use if you want to send the more details regarding the inquiry
		$sql_councellor = "SELECT admin_id,cm_id FROM site_setting where type='S' or type='A' or type='AC' or type='ST' and cm_id='".$row_loggedin['cm_id']."'";
		$db1=mysql_query($sql_councellor);
		while($row_Councellor =mysql_fetch_array($db1))
		{
			$data_record['notification'] = 'Payment paid to vendor for product purchase by faculty '.$row_loggedin['name'];
			$data_record['redirect_page_url'] = 'inventory_payment_details.php?record_id='.$args['reference_id'];
			$data_record['added_by'] = $_SESSION['admin_id'];
			$data_record['added_for'] = $row_Councellor['admin_id'];
			$data_record['on_action'] = $args['on_action'];
			$data_record['is_read'] = 'No';
	...
	else if(trim($args['on_action']) == "product_sale_payment")
	{
		$enroll_id = $args['reference_id'];//----you can use if you want to send the more details regarding the inquiry
		$sql_councellor = "SELECT admin_id,cm_id FROM site_setting where type='S' or type='A' or type='AC' or type='ST' and cm_id='".$row_loggedin['cm_id']."'";
		$db1=mysql_query($sql_councellor);
		while($row_Councellor =mysql_fetch_array($db1))
		{
			$data_record['notification'] = 'Payment received from customer for product sale by faculty '.$row_loggedin['name'];
			$data_record['redirect_page_url'] = 'sales_prod_payment_details.php?record_id='.$args['reference_id'];
			$data_record['added_by'] = $_SESSION['admin_id'];
			$data_record['added_for'] = $row_Councellor['admin_id'];
			$data_record['on_action'] = $args['on_action'];
			$data_record['is_read'] = 'No';
	...
	else if(trim($args['on_action']) == "product_sale")
	{
		$enroll_id = $args['reference_id'];//----you can use if you want to send the more details regarding the inquiry
		$sql_councellor = "SELECT admin_id,cm_id FROM site_setting where type='S' or type='A' or type='AC' or type='ST' and cm_id='".$row_loggedin['cm_id']."'";
		$db1=mysql_query($sql_councellor);
		while($row_Councellor =mysql_fetch_array($db1))
		{
			$data_record['notification'] = 'Sale product to customer by faculty '.$row_loggedin['name'];
			$data_record['redirect_page_url'] = 'manage_sales_product.php?record_id='.$args['reference_id'];
			$data_record['added_by'] = $_SESSION['admin_id'];
			$data_record['added_for'] = $row_Councellor['admin_id'];
			$data_record['on_action'] = $args['on_action'];
			$data_record['is_read'] = 'No';
	...
	else if(trim($args['on_action']) == "enroll_verify")
	{
		$enroll_id = $args['reference_id'];//----you can use if you want to send the more details regarding the inquiry
		//----get logged in user type and based on that send the notification ----
		$sql_councellor = "SELECT admin_id,cm_id FROM site_setting where type='S' or type='A' or type='AC' and cm_id='".$row_loggedin['admin_id']."'";
		$db1=mysql_query($sql_councellor);
		while($row_Councellor =mysql_fetch_array($db1))
		{
			$data_record['notification'] = 'Enrollment Verified by faculty '.$row_loggedin['name'];
			$data_record['redirect_page_url'] = 'manage_enroll_student.php?record_id='.$args['reference_id'];
			$data_record['added_by'] = $_SESSION['admin_id'];
			$data_record['added_for'] = $row_Councellor['admin_id'];
			$data_record['on_action'] = $args['on_action'];
