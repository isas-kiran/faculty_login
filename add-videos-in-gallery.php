...
if($_REQUEST['record_id'])
{
    ...
    $sql_record= "SELECT gallery_id FROM bp_gallery where gallery_id='".$record_id."'";
        ...
        <tr>
            ...
            <td valign="top" class="contentRight">
                <table cellpadding="0" cellspacing="0" align="left" width="100%">
                    <tr><td class="navigation_heading"><?php  echo "Add";?> Videos In Gallery</td></tr>
				...
				if($_POST['submit'])
				{
							...
							if($icon !='')
							{
								...
								$update_icon= " update  ".$GLOBALS["pre_db"]."gallery_videos set icon='' where gallery_id = '$gallery_id' ";
								...
								if( $_POST['img_src']!='' || $_POST['video_src']!='' )
								{
									
								$insert_pdf_query ="insert into ".$GLOBALS["pre_db"]."gallery_videos values('$gallery_id',0,'".addslashes($_POST['tag'])."','".addslashes($_POST['comment'])."','".addslashes($_POST['video_src'])."','".addslashes($_POST['img_src'])."','".date('Y-m-d H:i:s')."','$this_icon')";
								if($ptr_pdf=mysql_query($insert_pdf_query))
								$video_added='yes';
								}
								...
								*/
								...
								for($i=1;$i<=$extra_pdf;$i++)
								{
									$this_icon='';
										...
										$insert_extra_pdf = "insert into ".$GLOBALS["pre_db"]."gallery_videos values('$gallery_id',0,'".addslashes($_POST['tag'.$i])."','".addslashes($_POST['comment'.$i])."','".addslashes($_POST['video_src'.$i])."''".addslashes($_POST['img_src'.$i])."','".date('Y-m-d H:i:s')."','$this_icon')";
										$ptr_extra_pdf=mysql_query($insert_extra_pdf);
									  $fnames= "../videos".$saved_more_file_name;
									 /* $image_info1 = getimagesize($fnames) or die("no image");
									  ...
									  $dir= "../resize/".$saved_more_file_name;
									  $image->save($dir);
									 if(($image_info1[0] > 120) ||($image_info1[1] >120))
									  {   	  	  	  
										if(($image_info1[0] > 120) && ($image_info1[0] > $image_info1[1]))
									...
									//  $dir= "../videos/".$saved_more_file_name;
									  //$image->save($dir);
									
									
								}
							if($video_added=='yes')
							{	$msg= "<center><font color='green'> Videos(s) Added Successfully...!</font></center>";
							}	
							else 
							{
							
								$msg= "<center><font color='red'>No Video Added...</font></center>";
							}
				}
				?>
                   ...
                   <tr>
                     <td  align="left" >Select Gallery </td>
					  ...
					  <option value="0">==Select Gallery==</option>
					   <?php
					  $select_names = "select * from  ".$GLOBALS["pre_db"]."video";
					...
					<tr><td align="left">Name</td><td align="left">Comment</td>
					<td valign="top" align="left">Embedded Video  <font color="#CC66FF" size="-2">[get video src and paste here  Ex.http://www.youtube.com/embed/EQOb3G99gRk ]</font>             <br />
					  <!--<font color="#CC66FF" size="-2">[Any Format]</font></td>--><td align="center"> Icon </td><td>Image Embedded <br />  <font color="#CC66FF" size="-2">[get Imge src and paste here  Ex.http://i2.ytimg.com/vi/EgCScMxID24/default.jpg ]</font></td><td>&nbsp;</td></tr>
                    <tr>
					<td valign="top" align="left"><div align="left"><input type="text" name="tag"  onKeyPress="if (!isEng){ return change(this,event);} else {return true;}" onFocus="changeCursor(this);showme();" onClick="changeCursor(this);" onKeyUp="changeCursor(this);" 
					...
					<td valign="top" align="left"><div align="left"><input type="text" name="comment"  onKeyPress="if (!isEng){ return change(this,event);} else {return true;}" onFocus="changeCursor(this);showme();" onClick="changeCursor(this);" onKeyUp="changeCursor(this);" 
                    onKeyDown="positionChange(event);"  onblur="hideme();" /> <div style="display:none; position:absolute; padding-top:5%;padding-left:10%" id="hindi_images" >
                    <img src="HindiMap.jpg" alt="Hindi letter map"></div></td>
                      ...
                      <td valign="top" align="left"><div align="left">
                        ...
                        <textarea name="video_src"></textarea>
						
      
                      </div></td><td align="center"><input type="radio" name="icon" value="0" /></td><td><textarea name="img_src"></textarea></td>
                    ...
                    <tr>
                      ...
                      <td  align="right">
                        <input name="submit" type="submit"  value="Add Videos" onClick="return validte_name(this.form);" />
                      </td>
					  <td>&nbsp;</td>
					  <td><div align="left"></div></td>
                    </tr>
