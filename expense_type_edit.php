...
if($_REQUEST['cat_id'])
{
	...
	$sql_point= "SELECT * FROM expense_category where expense_category_id='".$record_id."'";
...
<title><?php if($record_id) echo "Edit"; else echo "Add";?> Expense Edit</title>
  ...
  <tr>
        ...
        <?php
                    ...
                    <?php
                        ...
                        else
                        {
                            ...
                            if($record_id )
                            {
                                $where_record="expense_category_id='".$record_id."'";   
                                $db->query_update("expense_category", $data_record,$where_record); 
									...
									{ 
											if($_POST['sub_name'.$j] !='' || $_POST['del_floor'.$j]=='yes')
											{
												...
												if($_POST['floor_id'.$j]!='' && $_POST['del_floor'.$j]=='yes' )
												{
													 ...
													 $delete_row = " delete from  expense_type where expense_type_id='".$_POST['floor_id'.$j]."' ";
											...
											{ 
												 ...
												 if($_POST['floor_id'.$j]=='' && $data_record_type1['expense_type'] !='')
												   {
														  $type1_id=$db->query_insert("expense_type", $data_record_type1);
														  
												   }else
													{ 
													 
														$where_record="expense_type_id='".$_POST['floor_id'.$j]."'";
														$type1_id= $_POST['floor_id'.$j];
													   
														$db->query_update("expense_type", $data_record_type1,$where_record);
	...
	<table border="0" cellspacing="15" cellpadding="0" width="100%">
            <tr><td colspan="3" class="orange_font">* Mandatory Fields</td></tr>
             ...
             <tr>
                	<td width="10%">Enter Expense Category</td>.
...
<input type="button" name="Add"  class="delBtn"  onClick="javascript:create_floor('delete');" alt="Delete(-)" >
  ...
  <tr>-->
							...
							if($record_id)
							{
								$select_exc = "select * from expense_type where category_id='".$record_id."' order by  expense_type_id  asc ";
								$ptr_fs = mysql_query($select_exc);
								$t=1;
								$total_conditions= mysql_num_rows($ptr_fs);
								while($data_exclusive = mysql_fetch_array($ptr_fs))
								{ 
									$pcategory_id= $data_exclusive['expense_type_id'];
									?> 
									...
									<td valign="top" width="40%" align=""><input type='text' style="width:300px" name='sub_name<?php echo $t ?>' id='sub_name<?php echo $t ?>' value='<?php echo $data_exclusive['expense_type'] ?>'/></td>
								   
									<input type="hidden" name="floor_id<?php echo $t; ?>" id="floor_id_<?php echo $t; ?>" value="<?php echo $data_exclusive['expense_type_id'] ?>" />
