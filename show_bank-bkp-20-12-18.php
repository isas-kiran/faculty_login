...
if($_POST['branch_id']) //$_POST:-because of that we can use this  in to the function for ajax 
{
	...
	echo '<select name="bank_name" class="input_text" style="width:200px" id="bank_name" onchange="show_acc_no(this.value)">';
	...
	"<br/>".$sle_bank_name="select bank_id,bank_name,is_credit_card,is_paytm from bank where cm_id='".$data_cm_id['cm_id']."' "; //".$pay_type."
	...
	if(mysql_num_rows($ptr_bank_name))
	{
		while($data_bank=mysql_fetch_array($ptr_bank_name))
		{
			...
			{
				$sel="select bank_id from inventory_invoice where inventory_id='".$record_id."' order by invoice_id asc limit 0,1";
				$ptr_data=mysql_query($sel);
				if(mysql_num_rows($ptr_data))
				{
					$row_record=mysql_fetch_array($ptr_data);
