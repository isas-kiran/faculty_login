...
<title>Product Incentive Report</title>
...
<table cellspacing="0" cellpadding="0" class="table" width="95%">
  ...
  <tr class="head_td">
              ...
              <tr>
				...
				<td width="15%">
					<select name="therapist_name" id="therapist_name"  class="input_select_login"  style="width: 150px; ">
						<option value="">-Referral Staff Name-</option>
						<?php 
							$therapist="select DISTINCT staff_id from sales_product_map where 1 and staff_id!='0' and staff_id!='' AND staff_id IS NOT NULL ".$_SESSION['where']."";
    ...
    <?php
						...
						if($_REQUEST['therapist_name']!='')
							{
								$therapist_name=" and sm.staff_id='".$_REQUEST['therapist_name']."'";
							}
							else
							{
								$therapist_name='';
						...
						if($_REQUEST['from_date'] && $_REQUEST['from_date']!=="0000-00-00" && $_REQUEST['from_date']!="From Date")
						 {
							...
							$frm_dates=$frm_date[2]."-".$frm_date[1]."-".$frm_date[0];
						  	...
						  	$pre_from_date=" and sp.added_date >='".date('Y-m-d',strtotime($frm_dates))."'";
						...
						if($_REQUEST['to_date']  && $_REQUEST['to_date']!="To Date")
						{
							...
							$to_dates=$to_date[2]."-".$to_date[1]."-".$to_date[0];
							 ...
							 $pre_to_date=" and sp.added_date<='".date('Y-m-d',strtotime($to_dates))."'";
						...
						if($_REQUEST['branch_name'])
						{
							...
							$branch_keyword_p="and sp.cm_id ='".$cm_id."'";
						}
						else {
							$branch_keyword="";
							$branch_keyword_p="";
						...
						if($_GET['record_id'] !='')
						{
							...
							$record_cat_idss="and sm.sales_product_id='".$_GET['record_id']."' ";
							
						}	
						$cmids="";
						if($_SESSION['where']!='')
						{
							$cmids=" and sp.cm_id='".$_SESSION['cm_id']."'";
						}
						
						
						$sql_query= "SELECT distinct(sp.sales_product_id) as sales_product_id,sm.product_id as product_id,sm.sales_product_price as sales_product_price,sp.added_date as added_date,sp.customer_id as customer_id,sm.staff_id as staff_id,sm.sales_product_price as sales_product_price FROM sales_product sp, customer c,product p ,sales_product_map sm where 1  and (c.cust_name like '%".$keyword."%' or p.product_name like '%".$keyword."%') and sp.customer_id=c.cust_id and sm.sales_product_id=sp.sales_product_id and sm.product_id=p.product_id and sm.staff_id!='' ".$cmids." ".$therapist_name." ".$pre_from_date."  ".$pre_to_date." ".$record_cat_idss." order by sp.sales_product_id desc ";
						
						//	$sql_query= "SELECT distinct(cs.customer_service_id) as customer_service_id,csm.service_id as service_id,csm.admin_id as admin_id,csm.total_price as total_price FROM customer_service cs, customer c, servies s, site_setting ss, sales_product_map csm where 1  and (cs.service_price like '%".$keyword."%' or cs.discount_price like '%".$keyword."%' or cs.service_tax like '%".$keyword."%' or cs.total_cost like '%".$keyword."%' or c.cust_name like '%".$keyword."%' or c.mobile1 like '%".$keyword."%' or s.service_name like '%".$keyword."%' or s.service_price like '%".$keyword."%' or ss.name like '%".$keyword."%') and cs.customer_id=c.cust_id and cs.customer_service_id=csm.customer_service_id  and ss.admin_id=csm.admin_id and  cs.status='Completed' and csm.service_id=s.service_id ".$cmids." ".$pre_from_date."  ".$pre_to_date." ".$therapist_name."  ".$branch_keyword_p." ".$record_cat_idss." ".$where_service_id." ".$select_directory.""; 
						
					
					                    
						//	$sql_query= "SELECT * FROM sales_product_map where 1 ".$record_cat_id." ".$where_serv_id." ".$_SESSION['where']." ".$branch_keyword." ".$pre_from_date."  ".$pre_to_date." ".$therapist_name." ".$pre_keyword." ".$select_directory.""; 
                     ...
                     <input type="hidden" name="formAction" id="formAction" value=""/>
					  ...
					  ?>
                        ...
                        <td width="25%" align="center"><strong>Services</strong></td>
						 <td width="25%" align="center"><strong>Referral Staff</strong></td>
                     
                        <td width="8%" align="center"><strong>Service Cost</strong></td>
                        ...
                        <td width="10%" align="center"><strong>Added Date</strong></td>
                            ...
                            while($val_querys=mysql_fetch_array($all_records))
                            {
								...
								$sel_cust_inv="select invoice_id from sales_product_invoice where sales_product_id='".$listed_record_id."'";   
								...
								echo '<td align="center">';
									...
									$select_service_name="select product_name,price from product where product_id='".$val_querys['product_id']."'";
									$ptr_service=mysql_query($select_service_name);
									$data_service_name=mysql_fetch_array($ptr_service);
									echo $data_service_name['product_name']."(Price - ".$data_service_name['price'].")<br />";
								
								echo '</td>';
								
								 $sel_th="select admin_id,name from site_setting where admin_id='".$val_querys['staff_id']."'";
								$ptr_th=mysql_query($sel_th);
								$therapist_name=mysql_fetch_array($ptr_th);
								echo '<td >'.$therapist_name['name'].
									'</td>';
								
								echo '<td align="center">'.$val_querys['sales_product_price'].'</td>';
								$cgst=9;
								$sgst=9;
								 "</br>".$totalgst=$cgst+$sgst;
								 "</br>".$new_total_tax=(($totalgst+100)/100);
								 "</br>".$total_taxable_value = $val_querys['sales_product_price'] / $new_total_tax;
								 "</br>".$total_gst =$val_querys['sales_product_price'] - $total_taxable_value;
                                ...
                                echo '</tr>';
                                $tot +=$val_querys['sales_product_price'];   
 ...
 <b> <?php echo intval($tot_gst); ?></b>
  ...
  <?php
   $inc_range = "select * from pr_add_product_incentive_details where staff_id='".$_REQUEST['therapist_name']."' ORDER BY p_id ASC";
$range = mysql_query($inc_range);

while($row = mysql_fetch_array($range))
{
	$range_diff=$row['p_to']-$row['p_from'];
if($tot_gst > $row['p_to'])
{
	$inc_amount=$range_diff*$row['p_percentage']/100;
}
else 
{
if($tot_gst>$row['p_from'])
		
		{
		$amt=$tot_gst-$row['p_from'];
		}
	else
	{
		$amt=0;
	}
	$inc_amount=($amt)*$row['p_percentage']/100;
}
$tot_ser_incentive+=$inc_amount;
  ...
  <tr>
  <td colspan="7">
  <b>Range - <?php echo $row['p_from']; ?> TO <?php echo $row['p_to']; ?> (<?php echo $row['p_percentage']; ?> %)</b>
  </td>
