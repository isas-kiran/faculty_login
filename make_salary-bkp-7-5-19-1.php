...
.table2 tr th:last-child {
  border-right: 0;
	...
	{
		//===================================================================
		echo "<br/>".$select_exc = "select * from pr_staff_salary_management where branch_name='".$branch_name."' AND month='".$month."' AND year='".$year."' AND staff_id='".$staff_id."' AND payment_action=1";
        $payment1 = mysql_query($select_exc);
        $payment = mysql_fetch_array($payment1);
						
		echo "<br/>".$select_staff = "select attendence_id,admin_id,name,company_name,contact_address,dob,joining_date,designation1,department,alternate_email from site_setting where attendence_id='".$staff_id."' "; 
		...
		if($staff['company_name']=='ISAS')
		{
			...
			$company_name="International School Of Aesthestics & Spa";
		...
		echo "<br/>".$leaves = "select * from pr_staff_leave_management where staff_id='".$staff_id."' AND month='".$month."' AND year='".$year."' order by staff_leave_id desc limit 0,1"; 
		$leave_query=mysql_query($leaves);
		$pre_leaves = mysql_fetch_array($leave_query);
							
		echo "<br/>".$staff_salary = "select * from pr_staff_salary_management where branch_name='".$branch_name."' AND staff_id='".$staff_id."' AND month='".$month."' AND year='".$year."'"; 
		...
		echo "<br/>".$leave_detail = "select * from pr_previous_leave_management where staff_id='".$staff_id."' AND month='".$last_month."' AND year='".$last_year."' order by previous_leave_id desc limit 0,1";     
