...
<title>Restart permission</title>
...
<?php include "ex_include/ps_pagination.php"; ?>
	...
	function reattempt_exam(stud_id,status)
	{
		if(confirm("Are you sure, Do you want to Restart exam?"))
                {					
					document.location.href="ex_stopped_student.php?action=stop&student_id="+stud_id+"&status="+status;
				}
            else
                return false;
...
<table border="0" cellspacing="0" cellpadding="0" width="100%">
  <tr>
	...
	if($_GET['action']=='stop')
	{
		...
		$update_log="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `admin_id`) VALUES ('Restart_exam','Restart','".$reattempt_permission."','".$stud_login_id."','".date('Y-m-d H:i:s')."','".$_SESSION['admin_id']."')";
		...
		?>
                    ...
                    if($_REQUEST['stopexam'] && $_REQUEST['stud_login_id'])
                    {
                        ...
                        {                           
                        	$upd_query="update ex_stud_login set status='inactive',exam_stoped='yes',stopped_time='".date('Y-m-d H:i:s')."' where stud_login_id='".$stud_login_id."'";
  ...
  <tr class="head_td">
    ...
    <?php
                        ...
                        if($_GET['order'] !='' && ($_GET['orderby']=='exam_name'))
                        {
                            ...
                            $sql_query= "SELECT DISTINCT(stud_login_id) FROM ex_stud_login where exam_stoped='yes'  ".$pre_keyword." ".$select_directory.""; 
                       //echo $sql_query;
