				...
				if($_POST['councillior_admin_id']!="")
				{
					...
					for($r=0;$r<count($_POST['chkRecords']);$r++)
					{
						...
						if(mysql_num_rows($my_query))
						{
							...
							{
								$whr_ext_no='';
								if($data_cm['cm_id']!=$data_center_m['cm_id'])
								{
									//==============Update Invoice No.=====================
									$sel_inv="select ext_invoice_no from enrollment where cm_id='".$data_cm['enroll_id']."' and ext_invoice_no IS NOT NULL order by enroll_id desc limit 0,1";
									...
									$whr_ext_no=",ext_invoice_no='".$preinv."'";
									//======================================================
								}
								$update_query="update enrollment set admin_id='".$councillior_id."',assigned_to='".$councillior_id."',cm_id='".$data_center_m['cm_id']."',transfer_from_cm_id='".$data_cm['cm_id']."',transfer_from_admin_id='".$data_cm['admin_id']."' ".$whr_ext_no." where enroll_id='".$data_cm['enroll_id']."'";
			...
			<div class="new_custom_course" style="display: none;">
				<form method="post" id="jqueryForm" name="discount" enctype="multipart/form-data">
					<table border="0" cellspacing="15" cellpadding="0" width="100%">
						...
						<tr>
							<td width="20%">Select Councillor<span class="orange_font">*</span></td>
							<td width="40%">
