    ...
    <td class="mid_mid">
        ...
        <form method="get" id="jqueryForm" name="jqueryForm" enctype="multipart/form-data">
            <table border="0" cellspacing="15" cellpadding="0" width="100%">
                    ...
                    <tr>
                        <td>&nbsp;</td>
                        <td ><input type="submit" class="input_btn" value="Search" onClick="return validin()" name="save_changes"  /></td>
						...
						while($data_exam_ques=mysql_fetch_array($ptr_ques))
						{
							...
							$ptr_ques1=mysql_query($sel_ques1);*/
								...
								while($data_ques1=mysql_fetch_array($ptr_unit_ques1))
								{
									...
									"<br/>--------".$sele_quest="select question_id from ex_student_paper where question_id='".$data_ques1['question_id']."' and exam_no='".$data_examno['exam_number']."'";
									...
									if($stude_total_ques=mysql_num_rows($ptrs_quesrt))
									{
										...
										"<br/>===============".$sel_incorrect="select answer_option,question_id,exam_no from ex_student_paper where exam_no='".$data_examno['exam_number']."' and question_id='".$datas_quest['question_id']."' ";
										$ptr_ques_id=mysql_query($sel_incorrect);
										if($totals_question=mysql_num_rows($ptr_ques_id))
										{
											while($data_totals=mysql_fetch_array($ptr_ques_id))
											{
												...
												"<br/>===>>>>>>>".$sel_ques1="select * from ex_question where question_id='".$data_totals['question_id']."'";
												$ptr_ques1=mysql_query($sel_ques1);
												if($totals_question11=mysql_num_rows($ptr_ques1))
												{
													$data_ques1=mysql_fetch_array($ptr_ques1);
														
													"<br />".$select_options="select * from ex_options where question_id='".$data_totals['question_id']."' and answer='y' limit 0,1";
													$ptr_opt=mysql_query($select_options);
													$total_right=mysql_num_rows($ptr_opt);
													$data_total_rit=mysql_fetch_array($ptr_opt);
            ...
            <tr>
            	<td align="center">
                	<table style="border:1px solid #0066cc;	border-collapse: collapse;" width="95%" border="1px" align="center">
						...
						$select_non_stopped_exam ="select stud_login_id as student_id from ex_stud_login where examination_number='".$data_exams['exam_number']."' "; //and exam_stoped ='' -->28/3/19
							...
							if($total_questions)
							{
								while($data_ques=mysql_fetch_array($ptr_ques))
								{
                        			...
                        			<tr <?php echo $bgcolor; ?> >
                                        ...
                                        <?php
										if($_SESSION['admin_name']=="ISASADMIN")
										{
											?>
