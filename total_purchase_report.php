						...
						<tr class="head_td">
                            ...
                            if($diff)
                            {
                                ...
                                for($i=0;$i<$data_difs+1;$i++)
                                {
                                    ...
                                    echo '<td align="left">';
                                    $sel_inv="select * from inventory_invoice where DATE(added_date)='".$newdates."' and payable_amount>0 ".$paid_type." ".$bank_ids." ".$search_cm_id." ";
                                    $ptr_inv=mysql_query($sel_inv);
                                    while($data_inv=mysql_fetch_array($ptr_inv))
                                    {
										$total_ded_value=0;
                                        $sel_prod="select * from inventory where inventory_id='".$data_inv['inventory_id']."'";
                                        $ptr_prod=mysql_query($sel_prod);
                                        $data_prod=mysql_fetch_array($ptr_prod);
										
										$sql_product = "select name from vendor where vendor_id='".$data_prod['vendor_id']."' ";
										...
										echo "<span style='color:blue'>".$name." - ".$data_inv['inventory_id']." / ".$data_inv['invoice_id']." - <strong>".$tot_payble."</span></strong> - <span style='color:brown'>".$pay_type['payment_mode']."</span><br/>";
                                        $tot_inv +=$tot_payble;
                                    }
                                    /*if($tot_inv >0)
