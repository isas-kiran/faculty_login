...
$total_found =0;	
	...
	$sel_for_Num_rows="select i.*, e.ext_invoice_no from invoice i, enrollment e where 1 and i.enroll_id=e.enroll_id and i.status='paid' ".$search_cm_id." ".$pre_keyword_i." ".$pay_type." ".$bank_ids." ".$paid_from_date." ".$paid_to_date." ".$select_directory." "; //and amount>0
	//and amount>0
...
$objPHPExcel->setActiveSheetIndex(0);
 ...
 $objPHPExcel->setActiveSheetIndex()->setCellValueByColumnAndRow('3','3', 'Mo No','3');
  ...
  $objPHPExcel->setActiveSheetIndex()->setCellValueByColumnAndRow('13','3', 'Status','3');*/
	...
	$sql_query= "select i.*, e.ext_invoice_no from invoice i, enrollment e where 1 and i.enroll_id=e.enroll_id and i.status='paid' ".$search_cm_id." ".$pre_keyword_i." ".$pay_type." ".$bank_ids." ".$paid_from_date." ".$paid_to_date." ".$select_directory." "; //and amount>0
	...
	{		
		...
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('D'.$rowCount, 'Course');
		...
		$name_val = $data_name['name'].' - '.$val_query['enroll_id'];
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('F'.$rowCount,$name_val );		
		...
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('G'.$rowCount, round($val_query['amount'],2));
		//'Product CGST  '.$val_query['cgst_tax_in_per'].'%   SGST  '. $val_query['sgst_tax_in_per'].'%'
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('H'.$rowCount, 'Student Fees Collected');	
	...
	header('Content-Disposition: attachment;filename="Enroolment Receipt Tally Report.xls"');
