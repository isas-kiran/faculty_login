                            ...
                            if($diff)
                            {
                                ...
                                for($i=0;$i<$data_difs;$i++)
                                {
                                    ...
                                    while($data_inv=mysql_fetch_array($ptr_inv))
                                    {
										...
										$tot_stud_gst=intval($data_inv['cgst_tax']+$data_inv['sgst_tax']);
                                    ...
                                    while($data_sp=mysql_fetch_array($ptr_sp))
                                    {
										...
										$ptr_pay=mysql_query($sql_pay);
										$pay_type=mysql_fetch_array($ptr_pay);
										$tot_prod_sale_gst=intval($data_sp['cgst_tax']+$data_sp['sgst_tax']);
                                        echo "<span style='color:blue'>".$tot_prod_sale_gst."</span> <span style='color:brown'>(".$pay_type['payment_mode'].")</span><br/>";
                                    ...
                                    while($data_ser=mysql_fetch_array($ptr_ser))
                                    {
										...
										$tot_serv_gst=intval($data_ser['cgst_tax']+$data_ser['sgst_tax']);
                                    ...
                                    //========================Expemse ==========================
                                    echo '<td align="left">';
                                    $sel_exp="select e.payment_mode_id,e.bank_id,em.gst_claim,e.added_Date,em.cgst_tax,em.sgst_tax from expense e, expense_map em where DATE(e.added_Date)='".$newdates."' and e.total_price>0 and amount>0 and (em.cgst_tax_in_per >0 or em.sgst_tax_in_per >0) and e.expense_id=em.expense_id and e.gst_claim='yes' ".$paymode_type_exp." ".$banks_id_exp." ".$search_cm_id_exp."";
                                    ...
                                    while($data_exp=mysql_fetch_array($ptr_exp))
                                    {
										...
										$ptr_pay=mysql_query($sql_pay);
										$pay_type=mysql_fetch_array($ptr_pay);
										$tot_exp_amnt=intval($data_exp['cgst_tax']+$data_exp['sgst_tax']);
                                        echo "<span style='color:blue'>".$tot_exp_amnt."</span> <span style='color:brown'>(".$pay_type['payment_mode'].")</span><br/>";
                                    ...
                                    while($data_pur=mysql_fetch_array($ptr_pur))
                                    {
										...
										$ptr_pay=mysql_query($sql_pay);
										$pay_type=mysql_fetch_array($ptr_pay);
										$tot_gst_inv=intval($data_pur['cgst_tax']+$data_pur['sgst_tax']);
                                        echo "<span style='color:blue'>".$tot_gst_inv."</span> <span style='color:brown'>(".$pay_type['payment_mode'].")</span><br/>";
                                        $tot_pur +=$tot_gst_inv;
                                    }
                                    if($tot_pur>0)
