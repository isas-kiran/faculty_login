...
<tr><td><?php
                    ...
                    if($_REQUEST['deleteRecord'] && $_REQUEST['record_id'])
                    {
                        ...
                        $sql_query= "SELECT  question_title , ans_dis  FROM question where question_id='".$del_record_id."'";
                        if(mysql_num_rows($db->query($sql_query)))
                        {                           
                            $delete_query="delete from question where question_id='".$del_record_id."'";
                            $db->query($delete_query);   
							
							 $select_option_images = " select option_title, answer from options where question_id='".$del_record_id."' ";
							$ptr_opt = mysql_query($select_option_images);
							
							$delete_querys="delete from options where question_id='".$del_record_id."'";
                            $db->query($delete_querys);
...
<? 
   ...
   $select_quationpapaer="select distinct(subject_id),add_class_id,div_id,sum(mark) as mark  from question where subject_id='".$record_id."' and chapter_id='".$topic_id."'";
...
<td style="text-align:right"> <a href="manage_question.php" style="text-decoration:none"><input type="button" name="back" value="Back"/></a> <a href="show_ans_sheet.php?subject_id=<? echo $record_id;?>" style="text-decoration:none">  <input type="button" name="answer_sheet" value="Answer Sheet"/></a></td>
<td>
                          ...
                          <option value="delete">Delete</option>
							...
							<td width="10%" class="th_blue_bold1" align="center"></td>
</tr>
...
<td  class="th_blue_bold1" align="center">Action</td>
 ...
 <?php 
	  ...
	  // mysql_num_rows($ptr_subject_name);
	   ...
	   $selec_subject_name="select * from  question where  subject_id='".$record_id."' and chapter_id='".$topic_id."'";
		...
		$question_img=$val_subject_name['question_img'];
 ?>
	    
        <tr valign="middle">
        <td style="vertical-align: top"><input type="checkbox" name="chkRecords[]" value= "<?php echo $listed_record_id; ?>" /></td>
		<td width="2%" height="65" style="vertical-align: top"><b> <?php echo $i; ?>)</b></td>
	 	<td width="80%"><?php echo "<b>".ucfirst($subject_name)."</b>"; 
		echo  '<br>';
		if($question_img !='' && file_exists("question_photo/".$question_img) )
		{
		echo "<img src='question_photo/".$question_img."' width='100' height='100'>";
		}
		echo  '<br>'; echo '<hr >'; ?>
        </td>
        <td width="7%" style="text-align:center"><?php echo "Marks: ".$marks; ?></td>
...
<img src="images/delete_icon.png" title="Delete Record" class="example-fade" /></a>&nbsp;&nbsp;                    
       ...
       <td><input type="radio" <?php if ($ans=='y') echo 'checked=checked'?> /></td>
         ...
         <?php 
		 $sel_opt_img="select * from option_image where question_id=".$question_id."";
		...
		$data_img=mysql_fetch_array($ptr_opt_img);
         	...
         	<td></td>
            <td></td>
            <td colspan="2"><?php if($data_img['option_image'.$m] !='' && file_exists("question_photo/".$data_img['option_image'.$m]) ){ ?><img src="question_photo/<?php echo $data_img['option_image'.$m]; ?>" width="100" height="100"  /><?php } ?></td>
            <?php
			 } ?>
         </tr>
         <?php }?>
