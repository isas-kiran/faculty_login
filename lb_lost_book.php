					...
					<?php
                        ...
                        if($_POST['save_changes'])
                        {
                            ...
                            else
                            {
                                ...
                                $data_record['lost_by'] =$customer_id;				
								$data_record['fine'] =$fine;
								$data_record['status'] ="Lost";
                               	...
                               	if($record_id)
                                {
                                    ...
                                    $where_record=" book_lost_id ='".$record_id."'";
                                    $db->query_update("lb_book_lost", $data_record,$where_record);
                                    echo '<br></br><div id="msgbox" style="width:40%;">Record updated successfully</center></div><br></br>';
                                }
                                else
                                {
									$data_record['added_date'] =date("Y-m-d H:i:s");
                                    $record_id=$db->query_insert("lb_book_lost", $data_record);
									$ins_return_id=mysql_insert_id();
									
									$update_query="update lb_book_issue set status='lost' where book_issue_id='".$book_issue_id."' and book_id='".$book_id."' and issue_to='".$customer_id."'";
									$ptr_update=mysql_query($update_query);
									
									$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('Issue_book','Add','lost book','".$ins_return_id."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
									$query=mysql_query($insert);

									//=========================END GALLERY IMAGE===============================
                                    //echo ' <br></br><div id="msgbox" style="width:40%;">Record added successfully</center></div> <br></br>';
									?><div id="statusChangesDiv" title="Record Deleted"><center><br><p>BRecord save successfully</p></center></div>
										<script type="text/javascript">
                                            $(document).ready(function() {
