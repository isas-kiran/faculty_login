...
<title>Manage Staff Event Management</title>
...

                    {
                        ...
                        if($_POST['formAction']=="delete")
                        {
                            ...
                            for($r=0;$r<count($_POST['chkRecords']);$r++)
                            {
								...
								$val=explode('-',$ds);
                                 "</br>".$del_record_id=$val[0];
							 	"</br>".$month=$val[1];
								"</br>".$year=$val[2];
                                $sql_query= "SELECT event_name FROM ".$GLOBALS["pre_db"]."pr_staff_event_management where event_name ='".$del_record_id."' ";
                                if(mysql_num_rows($db->query($sql_query)))
                                {
									"<br>".$sql_query= "SELECT * FROM pr_staff_event_management where event_name ='".$del_record_id."' ";              
                                    ...
                                    $delete_query="delete from ".$GLOBALS["pre_db"]."pr_staff_event_management where event_name='".$del_record_id."' ";
...
<table cellspacing="0" cellpadding="0" class="table" width="95%">
  ...
  <tr class="head_td">
    ...
    <?php
                        ...
                        $sql_query= "SELECT DISTINCT event_name,event_date_from,event_date_to,event_location FROM pr_staff_event_management"; 
...
<?php if($_SESSION['type']=='S')
   ...
   <td width="12%"><a href="<?php echo $_SERVER['PHP_SELF'];?>?order=<?php echo $order."&orderby=name".$query_string;?>" class="table_font"><strong>Event Name</strong></a> <?php echo $img1;?></td>

    <td width="10%"><strong>Event Date</strong></td>
    <td width="9%"><strong>Event Location</strong></td>
    ...
    <td width="7%" class="centerAlign"><strong>Action</strong></td>
...

                            {
                               ...
                               $listed_record_id=$val_query['event_name']; 
							   $listed_record_id1=$val_query['event_name'].'-'.$val_query['month'].'-'.$val_query['year']; 
                                ...
                                */
                                ...
                                echo '<td >'.$val_query["event_date_to"].' To '.$val_query['event_date_from'].'</td>';
                                echo '<td >'.$val_query['event_location'].'</td>';
									echo '<td align="center"><a href="add_event.php?record_id='.$val_query['event_name'].'&month='.$val_query['month'].'&year='.$val_query['year'].'"><img src="images/edit_icon.png" height="20px" width="20px" title="Edit Event" class="example-fade"/></a>&nbsp;
