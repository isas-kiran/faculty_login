...
$edit_access='';
$sel_acc="select * from edit_previleges where admin_id='".$_SESSION['admin_id']."' and privilege_id='280'";
  ...
  <tr>
    ...
    <td class="mid_mid" align="center">
		<table cellspacing="0" cellpadding="0" class="table" width="95%">
                                    ...
                                    <div id="show_type">
                                        <table style="width:100%;">
                                            <tr>
                                                ...
                                                <?php
                                                $sql_cust = "select name, enroll_id from enrollment where 1 and (student_status ='Active' or student_status is NULL) ".$_SESSION['where']." and ref_id='0' order by enroll_id asc";
                                                $ptr_cust = mysql_query($sql_cust);
                                                while($data_cust = mysql_fetch_array($ptr_cust))
                                                {
                                                    $selecteds="";
                                                    if($data_cust['enroll_id']==$_REQUEST['customer_id'])
                                                    {
            ...
            {
                $where_cust_id=" and enroll_id='".$_REQUEST['customer_id']."'";
            }
            if($_REQUEST['page'])
                $page=$_REQUEST['page'];
            else
            ...
            $sql_query= "SELECT enroll_id,name,cm_id FROM enrollment where 1 and (student_status ='Active' or student_status is NULL) and ref_id='0' ".$where_cust_id." ".$where_cm." ".$search_cm_id." ".$pre_keyword." ".$select_directory.""; 
            //echo $sql_query;
            ...
            if($no_of_records)
            {
                ...
                {		
                    ...
                    $sel_enroll="select enroll_id, name, net_fees, balance_amt, added_date from enrollment where 1 and (student_status ='Active' or student_status is NULL) and (enroll_id='".$listed_record_id."' or ref_id='".$listed_record_id."')   ".$paid_from_date." ".$paid_to_date."";
                    $ptr_query= mysql_query($sel_enroll);
                    $k=1;
                    $total_enr_price=0;
                    $total_enr_payble=0;
                    $total_enr_remaining=0;
                    while($data_enroll=mysql_fetch_array($ptr_query))
                    {
                        echo'<tr style="border:1px solid black;">';
                        echo'<td style="border:1px solid black;" align="center">'.$k.'</td>';
                        echo'<td style="border:1px solid black;" align="center">Enrollment</td>';
                        echo'<td style="border:1px solid black;" align="center">'.$data_enroll['enroll_id'].'</td>';
                        
                        $total_enr_price +=$data_enroll['net_fees'];
                        $payble_amnt =$data_enroll['net_fees']-$data_enroll['balance_amt'];
                        $total_enr_payble +=$payble_amnt;
                        $total_enr_remaining +=$data_enroll['balance_amt'];
                        
                        echo'<td style="border:1px solid black;" align="center">'.$data_enroll['net_fees'].'</td>';
                        echo'<td style="border:1px solid black;" align="center">'.$payble_amnt.'</td>';
                        echo'<td style="border:1px solid black;" align="center">'.$data_enroll['balance_amt'].'<br/>';
                        if($data_enroll['balance_amt'] >0)
                        {
                            echo '<span style="font-weight:600; font-style:italic;text-decoration:underline"><a href="add_payment_to_do_gst.php?record_id='.$listed_record_id.'" >pay now</a></span>';
                        }
                        else
                        {
                        }
                        ...
                        $dateexp=explode(' ',$data_enroll['added_date']);
                        echo'<td style="border:1px solid black;" align="center">'.$dateexp[0].'<br/>';
                        echo'</tr>';
                        $k++;
                    }
                    echo'<tr style="border:1px solid black;"><td style="border:1px solid black;font-weight:800" align="center" colspan="3">Total Enrolled</td><td style="border:1px solid black;font-weight:800" align="center">'.$total_enr_price.'</td><td style="border:1px solid black;font-weight:800" align="center">'.$total_enr_payble.'</td><td style="border:1px solid black;font-weight:800" align="center">'.$total_enr_remaining.' </td><td style="border:1px solid black;" align="center">0</td></tr>';
                    $sel_products="select sales_product_id,total_price,payable_amount,remaining_amount,payment_mode_id,added_date from sales_product where customer_id='".$listed_record_id."'and type='Student' ".$paid_from_date." ".$paid_to_date."";
                    $ptr_products=mysql_query($sel_products);
                    $i=1;
                    $total_prod_price=0;
                    $total_prod_payble=0;
                    $total_prod_remaining=0;
                    if(mysql_num_rows($ptr_products))
                    {
                        while($data_products=mysql_fetch_array($ptr_products))
                        {
                            ...
                            echo'<td style="border:1px solid black;" align="center">Product</td>';
                            echo'<td style="border:1px solid black;" align="center">'.$data_products['sales_product_id'].'</td>';
                            
                            $total_prod_price +=$data_products['total_price'];
                            $total_prod_payble +=$data_products['payable_amount'];
                            $total_prod_remaining +=$data_products['remaining_amount'];
                        ...
                        echo'<tr style="border:1px solid black;"><td style="border:1px solid black;font-weight:800" align="center" colspan="3">Total Product</td><td style="border:1px solid black;font-weight:800" align="center">'.$total_prod_price.'</td><td style="border:1px solid black;font-weight:800" align="center">'.$total_prod_payble.'</td><td style="border:1px solid black;font-weight:800" align="center">'.$total_prod_remaining.' </td><td style="border:1px solid black;" align="center">0</td></tr>';	
                    }
                    //==========================Services================================
					$sel_services="select customer_service_id,total_cost,payable_amount,remaining_amount,added_date from customer_service where customer_id='".$listed_record_id."' and type='Student' ".$paid_from_date." ".$paid_to_date." ";
					$ptr_services=mysql_query($sel_services);
					if(mysql_num_rows($ptr_services))
					{
						$total_ser_price=0;
						$total_ser_payble=0;
						$total_ser_remaining=0;
						$i=1;
						...
						echo'<tr style="border:1px solid black;"><td style="border:1px solid black;font-weight:800" align="center" colspan="3">Total Service</td><td style="border:1px solid black;font-weight:800" align="center">'.$total_ser_price.'</td><td style="border:1px solid black;font-weight:800" align="center">'.$total_ser_payble.'</td><td style="border:1px solid black;font-weight:800" align="center">'.$total_ser_remaining.' </td><td style="border:1px solid black;" align="center">0</td></tr>';
					}
					//===============================END Services=======================
					//echo'<tr style="border:2px solid black;"><td style="border:2px solid black;font-weight:800" align="center" colspan="2">Total</td><td style="border:2px solid black;font-weight:800" align="center">'.$total_price+$total_ser_price.'</td><td style="border:2px solid black;font-weight:800" align="center">'.$total_payble+$total_ser_payble.'</td><td style="border:2px solid black;font-weight:800" align="center">'.$total_remaining+$total_ser_remaining.' </td><td style="border:2px solid black;" align="center">0</td></tr>';
					echo'</table></td>';
					echo '</tr>';
					$bgColorCounter++;
                    //}
