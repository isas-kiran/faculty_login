...
<title>add Package Payment</title>
  ...
  <tr>
    ...
    <td class="mid_mid">
    					<?php
							 ...
							 $sel_cust_service="select cust_id, id, amount, total_cost, payable_amount, remaining_amount, cm_id from sales_package_voucher_memb where id='".$record_id."'";
							$quer_cust_service=mysql_query($sel_cust_service);
							$fetch_query_cust_service=mysql_fetch_array($quer_cust_service);
						
							$select_customer = " select cust_name, cust_id, address, mobile1, email from customer where cust_id='".$fetch_query_cust_service['cust_id']."' ";
    						...
    						if($_POST['save_changes'])
                        	{
								...
								if($record_id)
								{
									...
									$update_cust_service="update sales_package_voucher_memb set remaining_amount='".$_POST['remaining_amount']."', payable_amount=(payable_amount + ".$payable_amount.")  where id='".$customer_service_id."'";
									...
									$sel_pay="select SUM(payable_amount) as total_amnt from package_invoice where v_id='".$record_id."'";
									$ptr_pay=mysql_query($sel_pay);
									$data_sep=mysql_fetch_array($ptr_pay);
									$total_paid=$data_sep['total_amnt'] + $payable_amount;
									
									"<br/>".$insert_cust_ser_invoice = " INSERT INTO `package_invoice` (`v_id`,`total_cost`, `amount`, `payable_amount`,`remaining_amount`,`total_paid`, `paid_type`, `cust_bank_name`, `bank_id`, `cheque_detail`, `chaque_date`, `credit_card_no`, `admin_id`, `added_date`,`status`,`cm_id`) VALUES ('".$record_id."', '".$fetch_query_cust_service['total_cost']."', '".$fetch_query_cust_service['amount']."', '".$payable_amount."','".$_POST['remaining_amount']."','".$total_paid."', '".$payment_type."', '".$cust_bank_name."', '".$bank_name."', '".$_POST['chaque_no']."', '".$sep_check_date."','".$credit_card_no."', '".$_SESSION['admin_id']."', '".date('Y-m-d H:i:s')."','".$status."','".$cm_id1."'); ";
                            		...
                            		<script type="text/javascript">
										...
										setTimeout('document.location.href="package_payment_details.php?record_id=<?php echo $record_id; ?>";',1000);
                            		</script>
                                <?php
								//======================================================== END SEND MESSAGE===============================
								if($payment_type_val=="online")
                      ...
                      <?php
                                ...
                                $sql_records= "SELECT * FROM package_invoice where v_id=".$record_id." order by invoice_id desc limit 0,1 ";
