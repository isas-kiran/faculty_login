                    ...
                    {   
                        ...
                        if(count($data->sheets[$i][cells])>0) // checking sheet not empty
                        {
                            ...
                            for($j=1;$j<=count($data->sheets[$i][cells]);$j++) // loop used to get each row of the sheet
                            {
                                ...
                                for($k=1;$k<=count($data->sheets[$i][cells][$j]);$k++) // This loop is created to get data in a table format.
                                {
                                    ...
                                    for($i=2;$i<=$data->sheets[0]['numRows'];$i++)
                                    {
                                        ...
                                        "<br />Email_ID:".$email_id=trim($data->sheets[0]['cells'][$i][2]);
                                       	echo"<br />Fullname:".$fullname=trim($data->sheets[0]['cells'][$i][3]);
								...
								$select_inq="select inquiry_id from inquiry where 1 and (mobile1='".trim($contact_no)."' or mobile1='".trim($contact_no1)."') ";
								...
								else
								{
									if(trim($data->sheets[0]['cells'][$i][4]) !='')
									{
										...
										if($_SESSION['type']=='LD')
										{
											$campaign_type='lead_distribution';
										}
										$insert_inq= "insert into inquiry (`firstname`,`mobile1`,`email_id`,`remark`, `enquiry_source`,`campaign_id`,`campaign_type`,`added_date`,`cm_id`,`status`) values ('".$fullname."','".$contact_no."','".$email_id."','".$course_name."','".$enquiry_src."','".$campaign_id."','".$campaign_type."','".date("Y-m-d H:i:s")."', '".$cm_id."','Enquiry')  ";
                                		$ptr_inqu= mysql_query($insert_inq);
										$inquiry_id=mysql_insert_id();
										
										$insert_stud_regi= "insert into stud_regi (`name`,`contact`,`mail`,`enquiry_source`,`campaign_id`,`campaign_type`,`class_id`,`added_date`,`cm_id`) values ('".$fullname."','".$contact_no."','".$email_id."','".$enquiry_src."','".$campaign_id."','".$campaign_type."','".$inquiry_id."','".date("Y-m-d H:i:s")."', '".$cm_id."')  ";
