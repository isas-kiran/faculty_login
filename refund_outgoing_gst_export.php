...
$total_found =0;	
	...
	if($_SESSION['where']!='')
	{
  	...
  	$sel_for_Num_rows="SELECT refund_id,enroll_id,added_date FROM enrollment_refund WHERE 1 ".$where_cm." ".$search_cm_id." ".$where_gst." ".$pre_keyword." ".$paid_from_date." ".$paid_to_date." and (cgst_tax_in_per >0 or sgst_tax_in_per >0) ".$select_directory." ";
...
$objPHPExcel->setActiveSheetIndex()->setSharedStyle($sharedStyle2, "A4:H4");
...
$objPHPExcel->setActiveSheetIndex(0);
 ...
 $objPHPExcel->setActiveSheetIndex()->setCellValueByColumnAndRow('3','3', 'Mo No','3');
  ...
  $objPHPExcel->setActiveSheetIndex()->setCellValueByColumnAndRow('13','3', 'Status','3');*/
	...
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('B4', 'Student Name','6');
 
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('C4', 'Expense Name','6');
	$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('D4', 'Price (Excl. GST)','6');
	...
	$sql_query= "SELECT refund_id,enroll_id,added_date FROM enrollment_refund WHERE 1 ".$where_cm." ".$search_cm_id." ".$where_gst." ".$pre_keyword." ".$paid_from_date." ".$paid_to_date." and (cgst_tax_in_per >0 or sgst_tax_in_per >0) ".$select_directory." "; 
	...
	{
		echo $sel_name="select name,cust_gst_no from enrollment where enroll_id='".$val_query['enroll_id']."''";
		...
		$gst_per='';
		$gst_val='';
		$sel_inst_amnt="select * from enrollment_refund where 1 and refund_id='".$val_query['refund_id']."' and (cgst_tax_in_per >0 or sgst_tax_in_per >0)";
		...
		if($total_inst=mysql_num_rows($ptr_ins_amnt))
		{
			...
			while($data_inst_amnt=mysql_fetch_array($ptr_ins_amnt))
			{
				...
				$product_name .= 'Enrollment Refund';
				...
				$product_price .=$data_inst_amnt['total_refund'];
...
header('Content-Disposition: attachment;filename="refund_outgoing_gst.xls"');
