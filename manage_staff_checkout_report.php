...
<title>Manage Checkout Product</title>
...
<?php
$edit_access='';
$sel_acc="select * from edit_previleges where admin_id='".$_SESSION['admin_id']."' and privilege_id='295'";
...
{
	var data1="action=all&branch_name="+branch_name;	
	//alert(data1);
	$.ajax({
...
function change_avl_qty(id,values,user_id)
{
	var data1="action=set_avl_qty&id="+id+"&values="+values+"&user_id="+user_id;	
	//alert(data1);
	$.ajax({
		url: "set_quantity.php", type: "post", data: data1, cache: false,
		success: function (html)
...
function change_cons_qty(id,values,user_id)
{
	var data1="action=set_cons_quantity&id="+id+"&values="+values+"&user_id="+user_id;	
	//alert(data1);
	$.ajax({
		url: "set_quantity.php", type: "post", data: data1, cache: false,
		success: function (html)
    ...
    <td colspan="20">
		...
		<table border="0" cellspacing="0" cellpadding="0" width="99%" align="center">
        	<tr>
				...
				<?php
				if($_SESSION['type'] !='S' && $_SESSION['type']!='Z' && $_SESSION['type']!='LD'  || $_REQUEST['branch_name']!='')
				{
					if($_SESSION['type']=='S' || $_SESSION['type']=='Z' || $_SESSION['type']=='LD' )
					{
					...
					$sel_stockiest="select * from site_setting where branch_name='".$branch_name."' and system_status='Enabled'";
					$ptr_stockiest=mysql_query($sel_stockiest);
					while($data_stockist=mysql_fetch_array($ptr_stockiest))
					{
						$sel='';
						if($data_stockist['admin_id']==$_REQUEST['stockiest'])
						{
							$sel='selected="selected"';
						}
				...
				<td> <a href="product_staff_checkout_export.php<?php echo $sep_url_string;?>"><img src="images/excel.png" height="30px" width="30px"/></a></td>  
    ...
    <?php
					...
					if($no_of_records)
					{
					   ...
					   // $pager = new PS_Pagination($sql_query,$_SESSION['show_records'],$_SESSION['show_records_pages'],$query_string1);
                        ...
                        <td width="15%"><a href="<?php echo $_SERVER['PHP_SELF'];?>?order=<?php echo $order."&orderby=product_name".$query_string;?>" class="table_font"><strong>Product Name</strong></a> <?php echo $img1;?></td>
                        <td width="7%" align="center"><strong>Brand</strong></td>
						<!--<td width="5%" align="center"><strong>HSN Code</strong></td>-->
                        <td width="7%" align="center"><strong>Product Category</strong></td>
                        <td width="7%" align="center"><strong>Sub Category</strong></td>
                        ...
                        <td width="5%" align="center"><strong>Last issue qty</strong></td>
                        <td width="5%" align="center"><strong>Last issue date</strong></td>
                        <td width="7%" align="center"><strong>Available Qty</strong></td>
                        <td width="5%" align="center"><strong>Consumed Qty</strong></td>
                        <td width="6%" align="center"><strong>Owner</strong></td>
                        ...
                        while($val_query=mysql_fetch_array($all_records))
                        {
                            ...
                            echo '<td align="center">'.$data_brand['brand_name'].'</td>';
                            //=============Category Name===============
                            ...
                            //=============Unit===============
                            echo '<td align="center">'.$data_pbrand['size']." ".$data_pbrand['unit'].'</td>';
                            //=============Last Issue Qty===============
                            ...
                            echo '<td align="center">'.$data_prods['issue_qty'].'</td>';
                            //=============Last Issue Date===============
                            echo '<td align="center">'.$data_prods['added_date'].'</td>';
                            //=============Available Quantity===============
                            echo '<td align="center">';
                            if($_SESSION['type']=='S')
                            {
                                echo'<input type="text" name="avl_stock_quantity" value="'.$val_query['quantity_in_consumable'].'" style="width:40px"  onblur="change_avl_qty('.$listed_record_id.',this.value,'.$val_query['user_id'].')" >';
                            }
                            else
                                echo $val_query['quantity_in_consumable'];
                            echo'</td>';
                            //=============Consumed Quantity===============
                            echo '<td align="center">';
                            if($_SESSION['type']=='S' || $_SESSION['type']=='Z' || $_SESSION['type']=='A' || $_SESSION['type']=='ST')
                            {
                                echo'<input type="text" name="consumed_quantity" value="'.$val_query['quantity'].'" style="width:40px" onblur="change_cons_qty('.$listed_record_id.',this.value,'.$val_query['user_id'].')" >';
                            }
                            else
