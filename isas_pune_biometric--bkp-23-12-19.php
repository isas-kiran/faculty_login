...
function handle_attendance_log($request_data)
{
	...
	$sel_user="select * from site_setting where attendence_id='".trim($UserId)."' and branch_name='Pune'";
	$ptr_user=mysql_query($sel_user);
	if(mysql_num_rows($ptr_user))
	{
		$data_user=mysql_fetch_array($ptr_user);
