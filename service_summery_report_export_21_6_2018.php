...
$objPHPExcel->setActiveSheetIndex(0);
 ...
 $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow('3','3', 'Mo No','3');
  ...
  $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow('13','3', 'Status','3');*/
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('D4', 'Service Details','6');
	...
	while($val_query=mysql_fetch_array($query_order))
	{
		...
		if($total_inst=mysql_num_rows($ptr_ins_amnt))
		{
			...
			while($data_inst_amnt=mysql_fetch_array($ptr_ins_amnt))
			{
				...
				$service_name .= $data_product_name['service_name'];
				$ins++;
			}
		}
		
		$objPHPExcel->getActiveSheet()->SetCellValue('D'.$rowCount, $service_name);
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('F'.$rowCount,"Bank Name.-". $data_bank['bank_name']."\n Acc. No.- ".$data_bank['account_no']);						
		  ...
		  $objPHPExcel->getActiveSheet()->SetCellValue('H'.$rowCount, $val_query['total_cost']);
		...
		$rowCount++;
		$i++;
		$totals_tds +=intval($val_query['total_cost']);
	}
...
$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');
ob_get_clean();
$objWriter->save('excel_files/Bank_Service_Summury_Report_'.date('Y-m-d').'.xlsx');
//$objWriter->save('php://output');
ob_end_flush();

?>
<a href="service_summery_report.php">Back</a>

<script>
document.location.href='export_service_summury_report.php';
</script>
