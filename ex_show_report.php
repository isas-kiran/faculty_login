...
{  
	...
	$sele_stud="select * from ex_stud_login where stud_login_id='".$student_id."'";
	$ptr_stud=mysql_query($sele_stud);
	$data_stud=mysql_fetch_array($ptr_stud);
	
	$select_ex="select * from ex_exams where exam_number ='".$data_stud['examination_number']."' ";
	...
	echo' <table style="border-top:2px solid #0066cc;" width="100%">
            <tr><td><table width="100%">
             <tr>
				...
				<td align="right" width="5%"><input class="input_btn" type="button" name="print" value="Print" title="Print" border="0" 
					onclick=\'window.open("ex_student_report_print.php?exam_no='.$data_stud['examination_number'].'&student_id='.$_POST['student_id'].'","View Invoice","scrollbars=yes","resizable=yes","width=900,height=600")\' style="cursor:pointer" ></td>
             </tr>
             <tr>
			...
			$select_ques="select * from ex_student_paper where student_id='".$student_id."' and exam_no='".$data_stud['examination_number']."' order by question_id asc";	
			...
			if($total_std=mysql_num_rows($ptr_std))
			{
				...
				{
					"<br />".$sel_ques_name="select question_id,question_title,unit_id from ex_question where question_id='".$data_std['question_id']."' order by question_id asc";
					$ptr_que=mysql_query($sel_ques_name);
					$data_que=mysql_fetch_array($ptr_que);
					
					"<br />".$sel_opt="select * from ex_options where question_id='".$data_std['question_id']."' and answer='y'";
					...
					{ $correct="Wrong"; $mark="0";}
                        ...
                        echo'<tr>
                            <td>'.$j.'</td>
                        	<td>'.$data_que['question_id'].''.'('.$data_que['question_title'].')</td>
