										...
										if(no !='')
										{
											...
											if(previous_floor==0)
											{
												...
												{												
											 	...
											 	previous_floor=no;
												if( pageName === "add_inventory" || pageName === "sales_product" || pageName === "add_checkout" || pageName ==="add_student_kit")
												{
													$("#product_id"+(i-1)).chosen({allow_single_deselect:true});
													/*$("#staff_id"+(i-1)).prop("disabled",false);
													$("#staff_id"+(i-1)).chosen({allow_single_deselect:true});
													$("#staff_id"+(i-1)).prop("disabled",true);*/
												}
												if(pageName === "add_holiday")
												{
													$("#from_date"+(i-1)).datepicker({ changeMonth: true,changeYear: true, showButtonPanel: true, closeText: 'Clear'});
													$("#to_date"+(i-1)).datepicker({ changeMonth: true,changeYear: true, showButtonPanel: true, closeText: 'Clear'});
												}
												if( pageName === "add_cust_services" || pageName === "add_voucher" || pageName === "add_package" )
												{
													//$("#service_id"+(i-1)).chosen({allow_single_deselect:true});
												}
												if( pageName === "add_checkout")
												{
													$("#employee_id"+(i-1)).chosen({allow_single_deselect:true});
													$("#select_from"+(i-1)).chosen({allow_single_deselect:true});
												}
												if( pageName ==="add_timetable")
												{
													$("#subject"+(i-1)).chosen({allow_single_deselect:true});
													$("#topic"+(i-1)).chosen({allow_single_deselect:true});
												}
											}
											else
											{	
								...
								if(no_type1 !='')
								{
									...
									{	
										if(no_type1>previous_floor1)
										{
											...
											if( pageName === "add_cust_services" ||pageName === "add_voucher" ){
												$("#service_id"+(i-1)).chosen({allow_single_deselect:true});
												/*$("#staff_id"+(i-1)).prop("disabled",false);
												$("#staff_id"+(i-1)).chosen({allow_single_deselect:true});
												$("#staff_id"+(i-1)).prop("disabled",true);*/
											}else if( pageName === "add_inventory"  || pageName ==="add_student_kit"){
												$("#product_id"+(i-1)).chosen({allow_single_deselect:true});
												/*$("#staff_id"+(i-1)).prop("disabled",false);
												$("#staff_id"+(i-1)).chosen({allow_single_deselect:true});
												$("#staff_id"+(i-1)).prop("disabled",true);*/
