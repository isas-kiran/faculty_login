...
<?php
$edit_access='';
$sel_acc="select * from edit_previleges where admin_id='".$_SESSION['admin_id']."' and privilege_id='123'";
    ...
    <?php       if($_POST['formAction'])
                    {
                        if($_POST['formAction']=="delete")
                        {
                            for($r=0;$r<count($_POST['chkRecords']);$r++)
                            {
                                ...
                                $sql_query= "SELECT package_id,package_name FROM package where package_id ='".$del_record_id."'";
                                $ptr_query=mysql_query($sql_query);
								if(mysql_num_rows($ptr_query))
								{     
									$data_cust=mysql_fetch_array($ptr_query);   
									"<br>".$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('manage_package','Delete','".$data_cust['package_name']."','".$del_record_id."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
...
<!--                                <option value="Active">Active</option>
    ...
    <?php
						...
						if($_GET['record_id'] !='')
						{
							$record_cat_id="and package_id='".$_GET['record_id']."' ";
						}	
						if($pre_keyword=='')
						{                      
						  $sql_query= "SELECT * FROM package where 1 ".$record_cat_id." ".$_SESSION['where']." ".$pre_keyword." ".$select_directory.""; 
						}
						else
						{
							$wherecmp='';
                        ...
                        <td width="10%" class="centerAlign"><strong>Action</strong></td>
						 <?php } ?>
                            ...
                            while($val_query=mysql_fetch_array($all_records))
                            {
                                ...
                                echo '<td >'.$val_query['package_name'].'</td>';
								...
								if($_SESSION['type']=="S" || $edit_access='yes')
								{
                                	echo '<td align="center"><a href="add_package.php?record_id='.$listed_record_id.'" ><img src="images/edit_icon.png" title="Edit Record" class="example-fade"/></a>&nbsp;&nbsp;<a onclick="return validationToDelete(\''.$_SERVER['PHP_SELF'].'\');" href="'.$_SERVER['PHP_SELF'].'?deleteRecord=1&record_id='.$listed_record_id.'&page='.$_REQUEST['page'].$query_string1.'"><img src="images/delete_icon.png" title="Delete Record" class="example-fade" /></a>&nbsp;&nbsp;';
