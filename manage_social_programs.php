...
<title>Manage social_programs</title>
...
<table border="0" cellspacing="0" cellpadding="0" width="100%">
  <tr>
    ...
    <td class="top_mid" valign="bottom"><?php include "include/social_program_menu.php";?></td>
    ...
    <?php       if($_POST['formAction'])
                    {
                        if($_POST['formAction']=="delete")
                        {
                            for($r=0;$r<count($_POST['chkRecords']);$r++)
                            {
                                ...
                                $sql_query= "SELECT program_id FROM ".$GLOBALS["pre_db"]."`social_programs where program_id ='".$del_record_id."'";
                                if(mysql_num_rows($db->query($sql_record)))
                                    {                                                
                                        $delete_query="delete from ".$GLOBALS["pre_db"]."social_programs where program_id='".$del_record_id."'";
...
<!--                                <option value="Active">Active</option>
    ...
    <?php
                        ...
                        if($_GET['order'] !='' && ($_GET['orderby']=='title'))
                        {
                            ...
                            $select_directory='order by program_id desc';                      
                            $sql_query= "SELECT * FROM bp_social_programs where 1  ".$pre_keyword." ".$select_directory.""; 
                       //echo $sql_query;
    ...
    <form method="post"  name="frmTakeAction" action="<?php echo $_SERVER['PHP_SELF'].'?#msgbox';?>" >
                     <input type="hidden" name="formAction" id="formAction" value=""/>
                      <tr class="grey_td" >
                        ...
                        <td width="10%"><a href="<?php echo $_SERVER['PHP_SELF'];?>?order=<?php echo $order."&orderby=title".$query_string;?>" class="table_font"><strong>Program Nmae</strong></a> <?php echo $img1;?></td>
                       ...
                       <td width="15%"><strong>Tag</strong></td>
                        ...
                        <td width="10%" class="centerAlign"><strong>Action</strong></td>
                            ...
                            while($val_query=mysql_fetch_array($all_records))
                            {
                                ...
                                $listed_record_id=$val_query['program_id']; 
                                ...
                                echo '<td ><a href="add_social_programs.php?record_id='.$listed_record_id.'" >'.$val_query['title'].'</a></td>';
                                echo '<td >'.$post.'</td>';
								
								 echo '<td ><img src="../static_Page_photo/'.$val_query['photo'].'" height=50px; width=80px; > </td>'; 
								 
								 
                                echo '<td align="center">'.$val_query['added_date'].'</td>';
                                echo '<td align="center"><a href="add_social_programs.php?record_id='.$listed_record_id.'" ><img src="images/edit_icon.png" title="Edit Record" class="example-fade"/></a>&nbsp;&nbsp;
