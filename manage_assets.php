    ...
    <?php       if($_POST['formAction'])
                {
                	if($_POST['formAction']=="delete")
                    {
                    	for($r=0;$r<count($_POST['chkRecords']);$r++)
                        {
							...
							{
								"<br>".$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('manage_inventory','Delete','purchase delete','".$del_record_id."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
								$query=mysql_query($insert);
								
								$sel_inv="select * from asset_map where asset_id='".$del_record_id."'";
								...
								$delete_query1="delete from assets_map where asset_id='".$del_record_id."'";
								$db->query($delete_query1); 
								 
								$delete_query3="delete from assets_invoice where asset_id='".$del_record_id."'";
                 ...
                 if($_REQUEST['deleteRecord'] && $_REQUEST['record_id'])
                 {
                    ...
                    if(mysql_num_rows($db->query($sql_query)))
                    {  
						"<br>".$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('manage_assets','Delete','asset delete','".$del_record_id."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
						$query=mysql_query($insert);
							
						$sel_inv="select * from assets_map where asset_id='".$del_record_id."'";
						$ptr_inv=mysql_query($sel_inv);
						while($data_inv=mysql_fetch_array($ptr_inv))
						{
							$qty=$data_inv['sin_asset_qty'];
						...
						$delete_query3="delete from asset_invoice where asset_id='".$del_record_id."'";
                        $db->query($delete_query3);    
...
-->            </tr>
    ...
    <?php
						...
						if($pre_keyword=='')
						{                    
						  $sql_query= "SELECT * FROM assets where 1 ".$record_cat_id." ".$_SESSION['where']." ".$where_vendor." ".$search_cm_id." ".$from_date." ".$to_date." ".$pre_keyword." ".$select_directory."";//".$_SESSION['user_id']." 25-12-17
						}
						else
						{
							$cm_ids=='';
						   ...
						   $sql_query= "SELECT distinct(i.asset_id) as asset_id FROM assets i, vendor v, assets_map im, assets_type p where 1  and (v.name like '%".$keyword."%' or p.asset_name like '%".$keyword."%') and i.vendor_id=v.vendor_id and im.asset_id=i.asset_id and im.asset_type_id=p.asset_type_id".$cm_ids." ".$where_vendor_i." ".$from_date_i." ".$to_date_i." ".$search_cm_id_s." order by i.asset_id desc"; //".$admin_ids."  25-12-17
						}
                        ...
                        if($no_of_records)
                        {
                            ...
                            <form method="post"  name="frmTakeAction" action="<?php echo $_SERVER['PHP_SELF'].'?#msgbox';?>" >
                     		<input type="hidden" name="formAction" id="formAction" value=""/>
                      		<tr class="grey_td" >
                                ...
                                <td width="20%" align="center"><strong>Assets</strong></td>
                            ...
                            {
								$sql_inventoery="select * from assets where asset_id='".$val_querys['asset_id']."' ";
								...
								$sel_product="select asset_id,asset_type_id,sin_asset_qty from assets_map where asset_id ='".$val_query['asset_id']."'";
								$ptr_product=mysql_query($sel_product);
								$k=1;
								$count=mysql_num_rows($ptr_product);
								while($data_product=mysql_fetch_array($ptr_product))
								{
									$select_service_name="select asset_name,price,admin_id from assets_type where asset_type_id='".$data_product['asset_type_id']."'";
									...
									$prod_name=$data_service_name['asset_name'];
									if($keyword)
									{
								...
								$tot_paid_amount="SELECT SUM( payable_amount) as payable_amount FROM `assets_invoice` WHERE asset_id=".$listed_record_id."";
								...
								echo '<td><center><a href="assets_payment_details.php?record_id='.$listed_record_id.'">
						<img src="images/view_bill.jpg" border="0" width="30px" height="30px" title="View Payment"></a></center></td>';
								...
								if($_SESSION['type'] =='S')
								{
									echo '<a href="add_assets.php?record_id='.$listed_record_id.'" ><img src="images/edit_icon.png" title="Edit Record" class="example-fade"/></a>&nbsp;&nbsp';
								}
								echo '&nbsp;&nbsp; <a href="" onClick="window.open(\'invoice_assets.php?record_id='.$listed_record_id.'\', \'win1\',\'status=no,toolbar=no,scrollbars=yes,titlebar=no,menubar=no,resizable=yes,width=900,height=600,directories=no,location=no\'); return false;" ><img src="images/invoice.png" title="View Invoice" class="example-fade"/></a>';
								if($_SESSION['type'] =='S')
								{
