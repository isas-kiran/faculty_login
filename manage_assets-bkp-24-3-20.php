...
if($_REQUEST['record_id'])
{
	...
	$sel="select * from assets where asset_id='".$record_id."'";
	$ptr_data=mysql_query($sel);
	if(mysql_num_rows($ptr_data))
	$row_expense=mysql_fetch_array($ptr_data);
	$expense_type_id=$row_expense['asset_type_id']; 
...
if($_REQUEST['asset_id'])
{
	...
	$where_expense=" and asset_id='".$asset_id."'";
}
else
{
	$expense_id='';
...
{
	$sel_act="select contact_phone,email from site_setting where admin_id='".$data_sel_cnt['staff_id']."' and email!='' ".$_SESSION['where']."";
...
function validme()
{
	...
	{
		category_id= value;
		var cat_data="action=show_asset_cat&category_id="+category_id+"&asset_type_id=<?php echo $expense_type_id; ?>";
...
?>   
	...
	if($_POST['formAction'])
	{
		if($_POST['formAction']=="delete")
		{
			for($r=0;$r<count($_POST['chkRecords']);$r++)
			{
				...
				$sql_query= "SELECT asset_id FROM assets where asset_id ='".$del_record_id."'";
				if(mysql_num_rows($db->query($sql_query)))
				{
					"<br>".$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('Assets','Delete','expense','".$del_record_id."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
					$query=mysql_query($insert); 
					
					$delete_query="delete from assets where asset_id='".$del_record_id."'";
					$db->query($delete_query);
					
					$delete_map_query="delete from assets_map where asset_id='".$del_record_id."'";
					$db->query($delete_map_query);                                                                                        
				}
			 }
			 ?>
				...
				<script type="text/javascript">
				   // $("#statusChangesDiv").dialog();
					...
					setTimeout('document.location.href="manage_assets.php";',500);
				</script>
			<?php                            
		}
	}
	if($_REQUEST['deleteRecord'] && $_REQUEST['record_id'])
	{
		...
		if(mysql_num_rows($db->query($sql_query)))
		{
			"<br>".$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('Assets','Delete','asset','".$del_record_id."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
			$query=mysql_query($insert); 
  	...
  	<tr>
		...
		if($_POST['save_changes'] && $_POST['randcheck']==$_SESSION['rand'])
		{
			...
			else
			{
				...
				$data_record['asset_type_id'] =$expense_type	;
				$data_record['asset_category_id'] =$expense_category;
				...
				//$ptr_update=mysql_query($update);
					...
					$where_records="asset_id='".$record_id."'";
					$db->query_update("assets", $data_record,$where_records);
					...
					for($z=1;$z<=$total_floor;$z++)
					{
						...
						if($_POST['del_floor'.$z]=='yes')
						{
							...
							{
								$delete_row = "delete from assets_map where map_id='".$_POST['floor_id'.$z]."' ";
						...
						if($_POST['del_floor'.$z] !='yes')
						{
							...
							$data_record_service['asset_name']=( ($_POST['expense_name'.$z])) ? $_POST['expense_name'.$z] : "";
							...
							$data_record_service['validity']=( ($_POST['validity'.$z])) ? $_POST['validity'.$z] : "0";
							$data_record_service['base_value']=( ($_POST['base_price'.$z])) ? $_POST['base_price'.$z] : "";
							
							if($_POST['floor_id'.$z]=='' && $_POST['expense_name'.$z] !='')
							{
								 $type1_id=$db->query_insert("assets_map", $data_record_service);
							}
							else
							{
								$where_record="map_id='".$_POST['floor_id'.$z]."'";
								$floor_id= $_POST['floor_id'.$z];
								$db->query_update("assets_map", $data_record_service,$where_record);
							}
					...
					for($x=1;$x<=$total_type1;$x++)
					{
						...
						if($_POST['del_floor_type1'.$x]=='yes')
						{
							...
							{
								"<br />".$delete_row = " delete from assets_tax_map where asset_tax_map_id='".$_POST['type1_id'.$x]."' ";
								$ptr_delete = mysql_query($delete_row);
							}
						}
						if($_POST['del_floor_type1'.$x] !='yes')
						{
							$data_record_tax['asset_id'] =$record_id; 
							...
							{
								$type1_id=$db->query_insert("assets_tax_map", $data_record_tax);
							}
							else
							{
								$where_record="asseet_tax_map_id='".$_POST['type1_id'.$x]."'";
								$type1_id= $_POST['type1_id'.$x];
								$db->query_update("assets_tax_map", $data_record_tax,$where_record);
							}
							unset($data_record_tax);
						}
					}
				
					$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('assets','Edit','asset','".$record_id."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
					$query=mysql_query($insert); 
										
					//echo ' <br></br><div id="msgbox" style="width:40%;">Record added successfully</center></div> <br></br>';
					?><div id="statusChangesDiv" title="Record Deleted"><center><br><p>Assets added successfully</p></center></div>
					<script type="text/javascript">
					$(document).ready(function() {
						$( "#statusChangesDiv" ).dialog({
								modal: true,
				...
				{
					"<br/>".$insert_for_receipt ="insert into assets (`invoice_no`,`invoice_date`,`payment_mode_id`,`cash_type`,`bank_id`,`account_no`,`chaque_no`,`chaque_date`,`gst_claim`,`amount_wo_tax`,`amount`,`discount_type`,`discount`,`discount_price`,`total_price`,`description`,`vendor_id`,`employee_id`,`asset_category_id`,`asset_type_id`,`credit_card_no`,`sac_code`,`added_date`,`cm_id`) values('".$data_record['invoice_no']."','".$data_record['invoice_date']."','".$data_record['payment_mode_id']."','".$data_record['cash_type']."','".$data_record['bank_id']."','".$data_record['account_no']."','".$data_record['chaque_no']."','".$data_record['date']."','".$data_record['gst_claim']."','".$data_record['amount_wo_tax']."','".$data_record['amount']."','".$data_record['discount_type']."','".$data_record['discount']."','".$data_record['discount_price']."','".$data_record['total_price']."','".$data_record['description']."','".$data_record['vendor_id']."','".$data_record['employee_id']."','".$data_record['asset_category_id']."','".$data_record['asset_type_id']."','".$data_record['credit_card_no']."','".$sac_code."','".$added_date."','".$cm_id."')";
					...
					{
			   			$bank="INSERT INTO `bank_records`(`bank_id`, `type`, `record_id`, `amount`, `added_date`, `cm_id`, `admin_id`) VALUES ('".$bank_name."','asset','".$ins_is."','".$data_record['amount']."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
					...
					for($i=1;$i<=$total_floor;$i++)
					{
						...
						{
							$data_record_service['asset_id'] =$ins_is; 
							$data_record_service['asset_name']=( ($_POST['expense_name'.$i])) ? $_POST['expense_name'.$i] : "";
							...
							$data_record_service['validity']=( ($_POST['validity'.$i])) ? $_POST['validity'.$i] : "0";
					...
					for($j=1;$j<=$total_type1;$j++)
					{
						...
						{
							$data_record_tax['asset_id'] =$ins_is; 
							...
							$customer_tax_id=$db->query_insert("assets_tax_map", $data_record_tax);
						}
					}
					/*$ins_tds="insert into tds_details (`type`,`ref_id`,`tds_disc_type`,`tds_percent`,`tds_value`,`admin_id`,`cm_id`,`added_date`) values('assets','".$ins_is."','".$_POST['tds_discount_type']."','".$_POST['tds_discount']."','".$_POST['tds_discount_price']."','".$admin_id."','".$cm_id."','".$added_date."')";
					$ptr_ins=mysql_query($ins_tds);*/
				
					"<br>".$insert="INSERT INTO `log_history`(`category`, `action`, `name`, `id`, `date`, `cm_id`, `admin_id`) VALUES ('Assets','Add','assets','".$ins_is."','".date('Y-m-d H:i:s')."','".$_SESSION['cm_id']."','".$_SESSION['admin_id']."')";
					$query=mysql_query($insert); 
				
					//------send notification on inquiry addition--------------------
					$notification_args['reference_id'] = $ins_is;
					$notification_args['on_action'] = 'assets';
		...
		if($where_expense=='')
		{
            ...
            <tr>
            	<td width="15%" class="tr-header" align="center">Select Asset Category</td>
                ...
                <option value="select">--Select--</option>
                <?php
                $sel_payment_mode="select * from assets_category";
                $ptr_payment_mode=mysql_query($sel_payment_mode);
                while($data_payment=mysql_fetch_array($ptr_payment_mode))
                {
                    $selected='';
                    if($data_payment['asset_category_id'] == $row_expense['asset_category_id'])
                    {
                        $selected='selected="selected"';
                    }
                    echo '<option '.$selected.' value="'.$data_payment['asset_category_id'].'">'.$data_payment['name'].'</option>';
                }
            ...
            <tr>
                <td width="15%" align="center">Invoice No.<span class="orange_font"></span></td>
                ...
                <td colspan="3">
                    ...
                    <tr>
                        <td colspan="6">
                            ...
                            <tr>
                                <td valign="top" align="center" width="22%">Asset Name</td>
                                ...
                                <td valign="top" width="6%" align="center">Validity (in month)</td>
                            ...
                            <tr>
                                ...
                                if($record_id)
                                {
                                $select_exc = "select * from assets_map where asset_id='".$record_id."' order by map_id asc ";
            ...
            <tr>
                ...
                <td width="25%"><input type="text" name="amount_wo_tax" id="amount_wo_tax" onBlur="calculte_total_cost()" style="width:200px" class="input_text" value="<?php if($_POST['amount_wo_tax']) echo $_POST['amount_wo_tax']; else echo $row_expense['amount_wo_tax']; ?>"  /></td>
            </tr>
            <tr>
    ...
    <td colspan="14">
        <form method="get" name="search">
			<table border="0" cellspacing="0" cellpadding="0" width="99%" align="center">
              ...
              <tr>
			  <td><a href="assets_export.php<?php echo $sep_url_string; ?>"><img src="images/excel.png" height="30px" width="30px"/></a></td> 
              <td></td>
              <td class="width5"></td>
...
<?php				
                        ...
                        if($keyword)
						{
							...
							$expense_type_filter='';
							$sel_exp_id="select asset_type_id from assets_type where asset_type like '%".$keyword."%' "; 
							$ptr_exp_id=mysql_query($sel_exp_id);    
							if(mysql_num_rows($ptr_exp_id)) 
							{
								$data_exp_id=mysql_fetch_array($ptr_exp_id);
								$expense_type_filter="|| asset_type_id = '".$data_exp_id['asset_type_id']."'";
							}  
                        ...
                        if($_GET['order'] !='' && ($_GET['orderby']=='asset_id'))
                        {
                            $select_directory .= " order by ".$_GET['orderby']." " .$_GET['order'] ;
                            $date_query='&order='.$_GET['order'].'&orderby='.$_GET['orderby'];
                        }
                        else
                            $select_directory='order by asset_id desc';  
							
						$record_cat_id='';
						if($_GET['record_id'] !='')
						{
							$record_cat_id="and asset_id='".$_GET['record_id']."' ";
							
						}
                        
						$sql_query= "SELECT * FROM assets where 1 ".$record_cat_id." ".$where_expense." ".$where_cm_id_data." ".$_SESSION['where']." ".$pre_keyword." ".$from_date." ".$to_date." ".$select_directory." "; 
                       //echo $sql_query;
  ...
  <tr class="grey_td" >
	...
	<td width="8%" align="center"><strong>Asset Type</strong></td>
    ...
    while($val_query=mysql_fetch_array($all_records))
    {
        ...
        $listed_record_id=$val_query['asset_id']; 
        ...
        $sel_expense_type="select asset_type from assets_type where asset_type_id='".$val_query['asset_type_id']."'";
        ...
        echo '<td align="center">'.$val_query['invoice_no'].'</td>';  
        if($_SESSION['type']=="S")
        {
            echo '<td align="center">'.$data_branch['branch_name'].'</td>';
        }
        echo '<td >'.$data_expense_type['asset_type'].'</td>';
        if($_SESSION['type']=='S')
        {
            echo '<td align="center"><a href="manage_assets.php?record_id='.$listed_record_id.'" class="table_font">'.$data_payment_mode['payment_mode'].'</a></td>';
        }
        else
        ...
        $sele_tax="select tax_type,tax_value,tax_amount from assets_tax_map where asset_id ='".$listed_record_id."'";
        ...
        if($_SESSION['type']=='S')
        {
            echo '<td align="center"><a href="manage_assets.php?record_id='.$listed_record_id.'" ><img src="images/edit_icon.png" title="Edit Record" class="example-fade"/></a>&nbsp;&nbsp;
...
if($record_id)
{
	?>
	<script>
	show_category(<?php echo $row_expense['asset_category_id'] ?>)
	</script>
    <?php
}
?>
