   	...
   	<tr>
    	...
    	<td class="mid_mid" align="center">
    				...
    				<td colspan="17">
        				<form method="get" name="search">
							<table border="0" cellspacing="0" cellpadding="0" width="99%" align="center">
                                ...
                                <td width="10%">
                                <input type="text" name="start_date" class="input_text datepicker" placeholder="From Date" readonly="1" id="start_date" title="From Date" value="<?php if($_REQUEST['start_date']) echo $_REQUEST['start_date']; else echo date('d/m/Y') //$enquiry_from_date=date('d/m/Y',strtotime('-1 days')) ?>">
                                </td>
            ...
            if($_REQUEST['start_date'] && $_REQUEST['start_date']!=="0000-00-00" && $_REQUEST['start_date']!="From Date")
            {
				...
				$enquiry_from_date_i=" and DATE(i.added_date) >='".date('Y-m-d',strtotime($frm_dates))."'";
            ...
            else
            {
                ...
                if($_REQUEST['to_date']=='')
                {
					...
					$enquiry_from_date_i=" and DATE(`i.added_date`) >='".date('Y-m-d')."'"; //date('Y-m-d',strtotime('-1 days'))
            ...
            if($_REQUEST['end_date']  && $_REQUEST['end_date']!="To Date")
            {
				...
				$enquiry_to_date_i=" and DATE(i.added_date)<='".date('Y-m-d',strtotime($to_dates))."'";
            ...
            if($_SESSION['type']=="S" || $_SESSION['type']=='Z' || $_SESSION['type']=='LD')
            {
                if($_REQUEST['branch_name']!='')
                {
                    ...
                    $data_cm_id=mysql_fetch_array($ptr_cm_id);
                    $search_cm_id=" and cm_id='".$data_cm_id['cm_id']."'";
					$search_cm_id_f=" and i.cm_id='".$data_cm_id['cm_id']."'";
                    $cm_ids=$data_cm_id['cm_id'];
                }
                else
                {
            ...
            if($no_of_records)
            {
               ...
               // $pager = new PS_Pagination($sql_query,$_SESSION['show_records'],$_SESSION['show_records_pages'],$query_string1);
                ...
                <td width="7%" align="center"><strong>No. of Fresh Leads Assign</strong></td>
                <td width="7%" align="center"><strong>No. of Invalid Leads</strong></td>
                <td width="7%" align="center"><strong>No. of Not-interested Leads</strong></td>
                <td width="7%" align="center"><strong>No. of Walkin From Fresh Leads</strong></td>
                <td width="7%" align="center"><strong>No. of Admission From Fresh Leads</strong></td>
                <td width="7%" align="center"><strong>Amount Realised From Fresh Leads</strong></td>
                ...
                while($val_query=mysql_fetch_array($all_records))
                {
					...
					//--------------------------------------------------------------------
					$select_inv="select inquiry_id from inquiry where 1 and response='8' and employee_id='".$val_query['admin_id']."' ".$_SESSION['where']." ".$search_cm_id." ".$enquiry_from_date." ".$enquiry_to_date."";
                    $query_inv=mysql_query($select_inv);
                    $count_inv=mysql_num_rows($query_inv);								
                    echo '<td align="center">'.$count_inv.'</td>';
                    $total_inv +=$count_inv;
					//--------------------------------------------------------------------
					$select_invalid="select inquiry_id from inquiry where 1 and response='7' and employee_id='".$val_query['admin_id']."' ".$_SESSION['where']." ".$search_cm_id." ".$enquiry_from_date." ".$enquiry_to_date."";
                    $query_invalid=mysql_query($select_invalid);
                    $count_invalid=mysql_num_rows($query_invalid);								
                    echo '<td align="center">'.$count_invalid.'</td>';
                    $total_invalid +=$count_invalid;
					//--------------------------------------------------------------------
					$count_enq_walkin=0;
					//$select_enquiry="select inquiry_id from inquiry where 1 and employee_id='".$val_query['admin_id']."' ".$search_cm_id." ".$_SESSION['where']."  ".$enquiry_from_date." ".$enquiry_to_date."";				
					$select_enquiry="select DISTINCT(f.student_id) from inquiry i,followup_details f where 1 and i.inquiry_id=f.student_id and f.response='1' and f.admin_id='".$val_query['admin_id']."' ".$search_cm_id_f."   ".$enquiry_from_date_i." ".$enquiry_to_date_i."";
                    $query_enquiery=mysql_query($select_enquiry);
                    if($count_enquiry=mysql_num_rows($query_enquiery))
					{
						/*while($data_cnts=mysql_fetch_array($query_enquiery))
						{
							$select_enq_walkin="select DISTINCT(student_id) from followup_details where 1 and response='1' and admin_id='".$val_query['admin_id']."' and student_id='".$data_cnts['inquiry_id']."' ".$_SESSION['where']." ".$search_cm_id." ".$enquiry_from_date." ".$enquiry_to_date."";
							$query_enq_walkin=mysql_query($select_enq_walkin);
							if(mysql_num_rows($query_enq_walkin))
							{
								$count_enq_walkin +=1;
							}
						}*/
					...
					echo '<td align="center">'.$count_enquiry.'</td>';
                    $total_walkin +=$count_enquiry;
					//--------------------------------------------------------------------
					$select_enr="select inquiry_id from inquiry where 1 and employee_id='".$val_query['admin_id']."' ".$search_cm_id." ".$_SESSION['where']." and status='Enrolled' ".$enquiry_from_date." ".$enquiry_to_date."";
                    $query_enr=mysql_query($select_enr);
                    $count_enr=mysql_num_rows($query_enr);
                    echo '<td align="center">'.$count_enr.'</td>';
                    $total_assign +=$count_enr;
					//--------------------------------------------------------------------
					$total_amnts=0;
					$select_enr="select inquiry_id from inquiry where 1 and employee_id='".$val_query['admin_id']."' ".$search_cm_id." ".$_SESSION['where']." and status='Enrolled' ".$enquiry_from_date." ".$enquiry_to_date."";
                    $query_enr=mysql_query($select_enr);
                    if($count_enr=mysql_num_rows($query_enr))
					{
						while($data_enr=mysql_fetch_array($query_enr))
						{
							$sel_enr="select enroll_id from enrollment where student_id='".$data_enr['inquiry_id']."'";
							$ptr_enr=mysql_query($sel_enr);
							$data_enr=mysql_fetch_array($ptr_enr);
							
							$sel_amnt="select SUM(amount) as total_amnt from invoice where 1 and assigned_to='".$val_query['admin_id']."'  and enroll_id='".$data_enr['enroll_id']."' ".$search_cm_id." ".$_SESSION['where']." ".$enquiry_from_date." ".$enquiry_to_date." ";
							$query_amnt=mysql_query($sel_amnt);
							$data_amnt=mysql_fetch_array($query_amnt);
							$total_amnts +=$data_amnt['total_amnt'];
						}
					}
                    echo '<td align="center">'.$total_amnts.'</td>';
					//--------------------------------------------------------------------
					echo '</tr>';
                    $i++;
                }
