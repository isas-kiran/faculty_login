...
?>   
	...
	if($no_of_records)
	{
		...
		while($val_query=mysql_fetch_array($my_db))
		{
			...
			$sel_name="select name from vendor where vendor_id='".$val_query['vendor_id']."'";
				...
				while($data_prod=mysql_fetch_array($ptr_prod))
				{
					...
					if($val_query['discount'] >0)
					{
						...
						else
						{
							...
							$tot_disc_per=round($tot_disc_per_org,2);
							$disc=$tot_disc_per;
						}
					...
					echo'<tr>';
					echo '<td align="center" width="40%">Product @ '.$data_prod['total_gst'].'%</td>';
					echo '<td align="center" width="20%">Item </td>';
					echo '<td align="center" width="10%">1</td>';				
					echo '<td align="center" width="10%">'.round($tot_price,2).'</td>';
			...
			$sel_prod="select SUM(i.igst_tax) as igst_total,SUM(i.discounted_price * i.product_qty) as total_val,SUM(i.base_prod_price * i.product_qty) as base_total, i.product_disc,SUM(i.discounted_price) as total_cgst_val, s.sales_product_price as total_price, (i.igst_tax_in_per) as total_gst from expense_map i, expense s where 1 and i.expense_id='".$val_query['expense_id']."' and i.expense_id=s.expense_id and (i.igst_tax_in_per >0) group by i.igst_tax_in_per";
			...
			//============================= FOR GST Null ============================
			 $sel_prod="select SUM(i.igst_tax) as igst_total,SUM(i.discounted_price * i.product_qty) as total_val,SUM(i.base_prod_price * i.product_qty) as base_total, i.product_disc,SUM(i.discounted_price) as total_cgst_val, s.sales_product_price as total_price, (i.igst_tax_in_per) as total_gst from expense_map i, expense s where 1 and i.expense_id='".$val_query['expense_id']."' and (i.cgst_tax_in_per <=0 and i.sgst_tax_in_per <=0 and  i.igst_tax_in_per <=0) group by i.igst_tax_in_per";
			$ptr_prod= mysql_query($sel_prod); //sin_product_base_price and i.sin_product_total>0 and s.total_cost > 0
			...
			if($tot_prod_gst<=0 && $tot_prod_igst <=0 && $tot_prod_nullgst <=0 )
			{
				...
				if($tot_prod_nullgst=mysql_num_rows($ptr_prod))
				{
					while($data_prod=mysql_fetch_array($ptr_prod))
					{
						...
						echo'<tr>';
						echo '<td align="center" width="40%">Product @ 0%</td>';
						echo '<td align="center" width="20%">Item </td>';
						echo '<td align="center" width="10%">1</td>';				
						echo '<td align="center" width="10%">'.round($tot_price,2).'</td>';
						echo '<td align="center" width="10%">'.$disc.'</td>';
			...
			if($tot_cgst=mysql_num_rows($ptr_cgst_prod))
			{
				while($data_cgst=mysql_fetch_array($ptr_cgst_prod))
				{
					if($data_cgst['cgst_tax_in_per']>0)
					{
						...
						echo '<td align="center" width="10%"></td>';
						echo '<td align="center" width="10%"></td>';
						echo "<td align='center' width='10%'>".round($data_cgst['cgst_total'],3)."</td>";
						echo '</tr>';
						$total_amnts +=round($data_cgst['cgst_total'],2);
					}
				}
			}
			//=====================SGST===================================
			...
			if($tot_sgst=mysql_num_rows($ptr_sgst_prod))
			{
				while($data_sgst=mysql_fetch_array($ptr_sgst_prod))
				{
					if($data_sgst['sgst_tax_in_per']>0)
					{
						...
						echo '<td align="center" width="10%"></td>';
						echo '<td align="center" width="10%"></td>';
						echo "<td align='center' width='10%'>".round($data_sgst['sgst_total'],3)."</td>";
						echo '</tr>';
						$total_amnts +=round($data_sgst['sgst_total'],2);
					}
				}
			}
			//=============================IGST=========================
			
			$sel_cgst_prod="select SUM(i.igst_tax) as igst_total,SUM(i.base_prod_price) as base_total,i.product_disc, SUM(i.discounted_price) as total_igst_val,i.igst_tax_in_per from expense_map i, expense s where i.expense_id='".$val_query['expense_id']."' and i.expense_id=s.expense_id and i.sales_product_price>0 group by i.igst_tax_in_per";
			...
			if($tot_igst=mysql_num_rows($ptr_igst_prod))
			{
				while($data_igst=mysql_fetch_array($ptr_igst_prod))
				{
					if($data_igst['igst_tax_in_per']>0)
					{
						...
						echo '<td align="center" width="10%"></td>';
						echo '<td align="center" width="10%"></td>';
						echo "<td align='center' width='10%'>".round($data_igst['igst_total'],3)."</td>";
						echo '</tr>';
			...
			echo '<td align="center" width="10%"></td>';
			echo '<td align="center" width="10%"></td>';
			echo "<td align='center' width='10%'>".round($total_price,2)."</td>";
			echo '</tr>';
			echo '</table></td>';
