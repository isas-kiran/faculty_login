    ...
    <?php if($_POST['formAction'])
                    {
                        if($_POST['formAction']=="delete")
                        {
                            for($r=0;$r<count($_POST['chkRecords']);$r++)
                            {
                                ...
                                $sql_query= "SELECT sms_id FROM ".$GLOBALS["pre_db"]."sms_log_history where sms_id ='".$del_record_id."'";
                                if(mysql_num_rows($db->query($sql_query)))
                                    {                                                
                                        $delete_query="delete from ".$GLOBALS["pre_db"]."sms_log_history where sms_id='".$del_record_id."'";
...
<table cellspacing="0" cellpadding="0" class="table" width="95%">
  <tr class="head_td">
    ...
    <?php
                        ...
                        if($keyword)
                            {                            
                                $select_name = "select * from sms_log_history where (sent_name like '%".$keyword."%' or sent_desc like '%".$keyword."%')";                                           
                                if(mysql_num_rows($db->query($select_name)))
                                {
                                    $val_name = $db->fetch_array($db->query($select_name));
                                    $name_to_array = "or sent_name like concat(concat('%',".$val_name['sms_id']."),'%') ";
                                }
                                $pre_keyword=" and (sent_name like '%".$keyword."%')";
                            }                            
                        else
                            $pre_keyword="";
                        ...
                        if($_GET['orderby']=='sms_id' )
                            $img1 = $img;

                        if($_GET['order'] !='' && ($_GET['orderby']=='sms_id'))
                        {
                            $select_directory .= " order by ".$_GET['orderby']." " .$_GET['order'] ;
                            $date_query='&order='.$_GET['order'].'&orderby='.$_GET['orderby'];
                        }
                        else
                            $select_directory='order by sms_id desc';                      
                            $sql_query= "SELECT * FROM sms_log_history where 1  ".$pre_keyword."  ".$select_directory." "; 
                       //echo $sql_query;
  ...
  <tr class="grey_td" >
    ...
    <td width="26%"><a href="<?php echo $_SERVER['PHP_SELF'];?>?order=<?php echo $order."&orderby=name".$query_string;?>" class="table_font"><strong>Sent Nmae</strong></a> <?php echo $img1;?></td>
    <td width="15%"><strong>Mobile No</strong></td>
    <!--<td width="20%"><strong>Suject Fees</strong></td>-->
    <td width="18%"><strong>Description</strong></td>
    <td width="20%"><strong>sms_type</strong></td>
    <td width="16%" class="centerAlign"><strong>Action</strong></td>
                            ...
                            while($val_query=mysql_fetch_array($all_records))
                            {
                                ...
                                $listed_record_id=$val_query['sms_id']; 
                                include "include/paging_script.php";
                                
                                echo '<tr '.$bgcolor.' >
                                      <td align="center"><input type="checkbox" name="chkRecords[]" value="'.$listed_record_id.'"></td>';
                                echo '<td align="center">'.$sr_no.'</td>';                                
                                echo '<td >'.$val_query['sent_name'].'</td>';
                                echo '<td >'.$val_query['sent_mobile'].'</td>';
                                //echo '<td >'.$val_query['sub_fees'].'</td>';
                                echo '<td >'.$val_query['sent_desc'].'</td>';
                                echo '<td >'.$val_query['sms_type'].'</td>';
