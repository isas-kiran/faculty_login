...
$total_found =0;	
	...
	$sel_for_Num_rows="select i.*, e.vendor_id,e.ext_invoice_no from inventory_invoice i, inventory e where 1 and i.inventory_id=e.inventory_id and i.payable_amount > 0 and i.status='paid' ".$search_cm_id." ".$pre_keyword_i." ".$pay_type." ".$bank_ids." ".$paid_from_date." ".$paid_to_date." ".$select_directory." "; //and amount>0
	//and amount>0
...
$objPHPExcel->setActiveSheetIndex(0);
 ...
 $objPHPExcel->setActiveSheetIndex()->setCellValueByColumnAndRow('3','3', 'Mo No','3');
  ...
  $objPHPExcel->setActiveSheetIndex()->setCellValueByColumnAndRow('13','3', 'Status','3');*/
	...
	$sql_query= "select i.*, e.vendor_id,e.ext_invoice_no from inventory_invoice i, inventory e where 1 and i.inventory_id=e.inventory_id and i.payable_amount > 0 and i.status='paid' ".$search_cm_id." ".$pre_keyword_i." ".$pay_type." ".$bank_ids." ".$paid_from_date." ".$paid_to_date." ".$select_directory." "; //and amount>0
	...
	{		
		...
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('F'.$rowCount, $data_name['name'].' - '.$val_query['vendor_id']);		
		...
		$objPHPExcel->setActiveSheetIndex(0)->SetCellValue('H'.$rowCount, 'Paid for product purchase');	
	...
	header('Content-Disposition: attachment;filename="Purchase Payment Tally Report.xls"');
