...
function show_product_qty(product_id,id)
{
	...
	$.ajax({
		...
		success: function(prod_data)
		{
			...
			{	
				var exit_igst=document.getElementById("sin_product_igst"+id).value = 0;
			}
			else if(product_servicetax > 0 )
			{
				var exit_servicetax=document.getElementById("sin_product_vat"+id).value = product_servicetax;	
			}
...
function calc_product_price(prod_id)
{
	...
	if($_SESSION['tax_type']=='GST')
	{
		...
		else if(igsttax > 0)
		{
			...
			document.getElementById("sin_prod_igst_price"+prod_id).value=precisionRound(total_base_taxable_value,0);
		}
		else
		{
	...
	else
	{
		...
		if(servicetax >0 )
		{
			...
			var tot_base_gst =precisionRound(parseFloat(prod_price - total_base_taxable_value),2);
			//==========================================================================
			total_gst=precisionRound(parseFloat(Number(tot_gst)),0);
			//alert(total_gst);
	...
	//prev_prod_qty=document.getElementById("prev_product_qty"+prod_id).value;
	//total_exit_qty=Number(total_prod_qty)+ Number(prev_prod_qty);
	...
	if(base_price==1)
	{
		...
		if($_SESSION['tax_type']=='GST')
		{
			...
			if(cgst_percent >0)
			{
				cgst_value= precisionRound(parseFloat((total_price*cgst_percent)/100),2);
				cgst_base_value= precisionRound(parseFloat((total_base_price*cgst_percent)/100),2); <!--for base price-->
				cgst_price=precisionRound(parseFloat(cgst_value),2);
			...
			if(sgst_percent >0)
			{
				sgst_value= precisionRound(parseFloat((total_price*sgst_percent)/100),2);
				sgst_base_value= precisionRound(parseFloat((total_base_price*sgst_percent)/100),2); <!--for base price-->
				sgst_price=precisionRound(parseFloat(sgst_value),2);
			...
			if(igst_percent >0)
			{
				igst_value= precisionRound(parseFloat((total_price*igst_percent)/100),2);
				igst_base_value= precisionRound(parseFloat((total_base_price*igst_percent)/100),2); <!--for base price-->
				igst_price=precisionRound(parseFloat(igst_value),2);
	...
	//=========================================================END CHANGES==========================18/1/18
		...
		if($_SESSION['tax_type']=='GST')
		{
			...
			{
				cgst_value= precisionRound(parseFloat((total_price*cgst_percent)/100),2);
				cgst_base_value= precisionRound(parseFloat((total_base_price*cgst_percent)/100),2); <!--fors base price-->
				cgst_price=precisionRound(parseFloat(cgst_value),2);
				//alert(cgst_price);
				document.getElementById("sin_prod_cgst_price"+prod_id).value=cgst_price;
			}
	...
	if(base_price==1)
	{
		document.getElementById("prod_price"+prod_id).value=precisionRound(total_mrp_price,2);
		document.getElementById("mrp_price"+prod_id).value=precisionRound(total_mrp_price,2);
		document.getElementById("sales_product_price"+prod_id).value=precisionRound(totals_price,2);
	}
	else
	{
		document.getElementById("sales_product_price"+prod_id).value=precisionRound(totals_price,2);
		document.getElementById("prod_base_price"+prod_id).value=precisionRound(totals_base_price,2);
		document.getElementById("mrp_price"+prod_id).value=precisionRound(total_mrp_price,2);
...
function showUser()
{
	...
	if(discount_price !=0)
	{
		var total_discount_price= Math.round(parseFloat(total_prods_price - discount_price));
		document.getElementById('discount_price').value=discount_price;
	}
	else
	{
		var total_discount_price=Math.round(total_prods_price);
		document.getElementById('discount_price').value=discount_price;
	}
...
function calculte_total_cost()
{
	...
	if(discount !=0)
	{
		if(discount_type=="percentage")
		{
			var discount_price= precisionRound(total_prods_price * (discount/100),2);
		}
		else
		{
			var discount_price= precisionRound(discount,2);
		}
	}
	else
	{
	...
	if(discount_price !=0)
	{
		var total_cost_new= Math.round(parseFloat(total_prods_price - discount_price));
		document.getElementById('discount_price').value=discount_price;
	}
	else
	{
		var total_cost_new=Math.round(total_prods_price);
		document.getElementById('discount_price').value=discount_price;
	}
...
function show_mobile_no(cust_ids,type)
{
	...
	else if(cust_ids == '904')
	{
		var data2="action_for=lemeurice&customer_id="+cust_ids;
		//alert(data2);
		 $.ajax({
		url: "get_stockist.php", type: "post", data: data2, cache: false,
		success: function (html)
	...
	{
		var data2="action_for=isas_pcmc&customer_id="+cust_ids;
		//alert(data2);
		 $.ajax({
		url: "get_stockist.php", type: "post", data: data2, cache: false,
		success: function (html)
...
function delete_product(id,types)
{
	if(confirm('Do you really want to delete record'))
	{
		...
		$('#prod_price'+id).replaceWith(function(){
			...
			return $('<input type="text" name="prod_price'+id+'" id="prod_price'+id+'" value="" />', {html: $(this).html()});
		});
		...
		$('#prod_disc_price'+id).replaceWith(function(){
			
			return $('<input type="hidden" name="prod_disc_price'+id+'" id="prod_disc_price'+id+'" value="" />', {html: $(this).html()});
		});
		...
		$('#prev_product_qty'+id).replaceWith(function(){
			
			return $('<input type="hidden" name="prev_product_qty'+id+'" id="prev_product_qty'+id+'" value="" />', {html: $(this).html()});
		});
  ...
  <tr>
                        ...
                        if($_POST['save_changes'])
                        {
                            ...
                            else
                            {
								...
								if($record_id)
                                {
									...
									"<br/>".$update_cust_invoice="update sales_product_invoice set `receipt_no`='".$receipt_no."',`total_price`='".$total_price."',`payable_amount`='".$payable_amount."',`remaining_amount`= '".$remaining_amount."',`paid_type`='".$payment_mode_id."',`bank_id`='".$bank_name."',`cheque_detail`='".$chaque_no."',`chaque_date`='".$chaque_date."',`credit_card_no`='".$credit_card_no."',`status`='paid',`total_paid`='".$payable_amount."',`added_date`='".$added_date."' where sales_product_id='".$record_id."' order by invoice_id asc ";
									$ptr_update=mysql_query($update_cust_invoice);
									
									if($payment_mode_id=='2' || $payment_mode_id=='4' || $payment_mode_id=='5')
									{
										$update_bnk="update bank_records set amount='".$payable_amount."',bank_id='".$bank_name."',`added_date`='".$added_date."' where record_id='".$record_id."' AND type='sales product' order by b_record_id asc ";
	                            		$upade_bk=mysql_query($update_bnk);
									}
									
									$sel_salemap="select * from sales_product_map where sales_product_id='".$record_id."'";
									$ptr_salemap=mysql_query($sel_salemap);
									while($data_salemap=mysql_fetch_array($ptr_salemap))
									{
										$sel_stockiest="select admin_id from product where product_id='".$data_salemap['product_id']."' ";
										$ptr_stock=mysql_query($sel_stockiest);
										$data_stock=mysql_fetch_array($ptr_stock);
										
										$ins_data="INSERT INTO `product_daily_report`(`product_id`, `cm_id`, `stockiest_id`, `type`, `purchase_qty`, `sales_qty`, `checkout_qty`, `vendor_id`, `cust_type`, `cust_id`, `todays_qty`,`todays_shelf_qty`,`todays_consumable_qty`, `description`,`admin_id`,`added_date`) VALUES ('".$data_salemap['product_id']."','".$cm_id."','".$data_stock['admin_id']."','sales','0','".$data_salemap['product_qty']."','0','0','".$_POST['user']."','".$customer_id."','product_qty +".$data_salemap['product_qty']."','','','Product Sales Edit Remove','".$_SESSION['admin_id']."','".date('Y-m-d H:i:s')."')";
										$ptr_ins=mysql_query($ins_data);	
									}
									//echo "<br/> Total-Floor-".$total_floor;
									for($z=1;$z<=$total_floor;$z++)
									{
										...
										if($_POST['del_floor'.$z]=='yes')
										{
											...
											{
												$delete_row ="delete from sales_product_map where map_id='".$_POST['floor_id'.$z]."' ";
										...
										if($_POST['del_floor'.$z] !='yes')
									  	{
											...
											$data_record_service['product_id']=( ($_POST['product_id'.$z])) ? $_POST['product_id'.$z] : "0";
											...
											$data_record_service['staff_id'] =$_POST['staff_id'.$z] ? $_POST['staff_id'.$z] : "0";
											...
											$data_record_service['service_tax_in_per'] =$_POST['sin_product_vat'.$z] ? $_POST['sin_product_vat'.$z] : "0";
											$data_record_service['service_tax'] =$_POST['sin_prod_vat_price'.$z] ? $_POST['sin_prod_vat_price'.$z] : "0";
											...
											{
												 $type1_id=$db->query_insert("sales_product_map", $data_record_service);
											}
											else
											{
												$where_record=" map_id='".$_POST['floor_id'.$z]."'";
												$floor_id= $_POST['floor_id'.$z];
												$db->query_update("sales_product_map", $data_record_service,$where_record);
											}
											
											"<br/>total qty- ".$quantity_total=intval($_POST['product_total_qty'.$z]);
											...
											$where_qty_record=" product_id='".$data_record_service['product_id']."'";
											$db->query_update("product", $data_qty_record,$where_qty_record);
											
											unset($data_record_service);
											
											$sel_stockiest="select admin_id from product where product_id='".$_POST['product_id'.$z]."' ";
											$ptr_stock=mysql_query($sel_stockiest);
											$data_stock=mysql_fetch_array($ptr_stock);
											
											$ins_data="INSERT INTO `product_daily_report`(`product_id`, `cm_id`, `stockiest_id`, `type`, `purchase_qty`, `sales_qty`, `checkout_qty`, `vendor_id`, `cust_type`, `cust_id`, `todays_qty`,`todays_shelf_qty`,`todays_consumable_qty`, `description`,`admin_id`,`added_date`) VALUES ('".$_POST['product_id'.$z]."','".$cm_id."','".$data_stock['admin_id']."','sales','0','".$_POST['product_qty'.$z]."','0','0','".$_POST['user']."','".$customer_id."','product_qty -".$_POST['product_qty'.$z]."','','','Product Sales','".$_SESSION['admin_id']."','".date('Y-m-d H:i:s')."')";
											$ptr_ins=mysql_query($ins_data);
										}
									}
									...
									<div id="statusChangesDiv" title="Record added"><center><br><p>Record added successfully</p></center></div>
										...
										setTimeout('document.location.href="manage_sales_product.php";',800);
                                        </script>
                                    <?php
                                ...
                                else
                                {
									...
									for($i=1;$i<=$total_floor;$i++)
									{
										...
										{
											$fetch_partial_quantity=mysql_fetch_array($query_quantity);
											"<br/>data- ".$partial_complete_qty=intval($fetch_partial_quantity['qty']);
											"<br/>".$total_remaining_qty= intval($quantity_total) - intval($partial_complete_qty); 
											"<br/>".$data_qty_record['quantity']=$total_remaining_qty;
										}
									...
									$status='paid';
									$sel_recpt="select receipt_no from sales_product_invoice where cm_id='".$cm_id1."' and receipt_no IS NOT NULL order by invoice_id desc limit 0,1";
									...
									$insert_sales_invoice = " INSERT INTO `sales_product_invoice` (`sales_product_id`,`receipt_no`, `total_price`, `payable_amount`,`remaining_amount`, `paid_type`, `bank_id`, `cheque_detail`, `chaque_date`, `credit_card_no`, `admin_id`, `added_date`,`status`,`cm_id`,`total_paid`) VALUES ('".$record_id."','".$receipt_no."', '".$total_price."', '".$payable_amount."','".$remaining_amount."', '".$payment_mode_id."','".$bank_name."', '".$chaque_no."', '".$chaque_date."','".$credit_card_no."', '".$_SESSION['admin_id']."', '".$added_date."','".$status."','".$cm_id1."','".$payable_amount."'); ";
									...
									if($customer_id=="18")
									{
												...
												if(mysql_num_rows($ptr_names))
												{
													...
													if(mysql_num_rows($ptr_sele_catte))
													{
														...
														$data_record_service_inv['product_id']=$data_product_id['product_id'];
														 ...
														 "<br/>".$update_products1="update `product` set `quantity`=(quantity+".$data_record_service_inv['sin_product_qty'].") where `product_id`='".$data_record_service_inv['product_id']."' and admin_id='".$data_record_ahm['admin_id']."' and cm_id='".$cm_ids."'  ";
										...
										"<br/>".$insert_sales_invoice = " INSERT INTO `inventory_invoice` (`inventory_id`, `price`, `total_cost`, `amount1`, `payable_amount`,`remaining_amount`, `paid_type`, `bank_id`, `cheque_detail`, `chaque_date`, `credit_card_no`, `admin_id`, `added_date`,`status`,`cm_id`,`total_paid`) VALUES ('".$record_id."', '".$product_price."', '".$total_price."', '".$amount1."', '".$_POST['payable_amount']."','".$_POST['remaining_amount']."', '".$payment_mode_id."','".$bank_name."', '".$chaque_no."', '".$sep_check_date."','".$credit_card_no."', '".$data_record_ahm['admin_id']."','".$added_date."','".$status."','".$cm_id1."','".$_POST['payable_amount']."'); ";
										$ptr_sales_invoice = mysql_query($insert_sales_invoice);
										//============================================================================
								...
								//=======================================LEMEURICE==================================================
									if($customer_id=="904")
									{
										...
										"<br/>--".$insert_in="insert into inventory (`vendor_id`,`cm_id`,`price`,`discount_type`,`discount`,`discount_price`,`total_cost`,`payment_mode_id`,`chaque_no`,`chaque_date`, `credit_card_no`,`bank_id`,`amount1`,`admin_id`,`payable_amount`,`remaining_amount`,`added_date`) 
										values ('".$data_record_ahm['vendor_id']."','".$data_record_ahm['cm_id']."','".$data_record_ahm['price']."','".$data_record_ahm['discount_type']."','".$data_record_ahm['discount']."','".$data_record_ahm['discount_price']."','".$data_record_ahm['total_cost']."','".$data_record_ahm['payment_mode_id']."','".$data_record_ahm['chaque_no']."','".$data_record_ahm['chaque_date']."',''".$data_record_ahm['credit_card_no'].",'".$data_record_ahm['bank_id']."','".$data_record_ahm['amount1']."','".$data_record_ahm['admin_id']."','".$data_record_ahm['payable_amount']."','".$data_record_ahm['remaining_amount']."','".$data_record_ahm['added_date']."')";				
										$ins=mysql_query($insert_in, $db2);
										"<br/>inv1-".$record_id=mysql_insert_id($db2);
										...
										for($i=1;$i<=$total_floor;$i++)
										{
											if(trim($_POST['product_id'.$i])!='')
											{
												...
												$data_record_service_inv['service_tax'] =$_POST['sin_prod_vat_price'.$i] ? $_POST['sin_prod_vat_price'.$i] : "0";
												$prd_admin_id='';
												"<br/>".$sel_admin_id="select `admin_id` from `site_setting` where 1 and (`type`='ST' or `type`='A')";
												$ptr_admin_id=mysql_query($sel_admin_id,$db2);
												$s=1;
												while($data_cm_id=mysql_fetch_array($ptr_admin_id))
												{
													if($s==1)
														$or="and";
													else
													...
													$prd_admin_id .=" ".$or." admin_id='".$data_cm_id['admin_id']."' ";
													$s++;
												}
												"<br/>inv2-".$record_id;
												"<br/>sele1-".$sel_product_name="select product_id,product_name,product_code,pcategory_id,sub_id,size,unit,commission,price, vender,type,added_date,cm_id,quantity,admin_id from product where product_id='".$product_id."'";
												$ptr_sel=mysql_query($sel_product_name,$con1);
												$ptr_names=$ptr_sel;
												if(mysql_num_rows($ptr_sel))
												{
												}
													$data_product=mysql_fetch_array($ptr_sel);
													"<br/>sel-".$sele_cate="select product_id from product where product_name='".$data_product['product_name']."' ".$prd_admin_id."  ";
													$ptr_cate=mysql_query($sele_cate,$db2);
													if(mysql_num_rows($ptr_cate))
													{
														$data_product_id=mysql_fetch_array($ptr_cate);
														$data_record_service_inv['product_id']=$data_product_id['product_id'];
														$product_ids=$data_product_id['product_id'];	
														echo"<br/>0-".$update_products1="update `product` set `quantity`=(quantity+".$data_record_service_inv['sin_product_qty'].") where `product_id`='".$data_record_service_inv['product_id']."' and cm_id='".$cm_ids."' ";
														$ptr_update=mysql_query($update_products1,$db2);
														//$query_update=mysql_query($update_products1);
													}
													else
													{
														...
														$ptr_category=mysql_query($sel_category,$con1);
														$data_cate=mysql_fetch_array($ptr_category);//=====================
														
														"<br/>2".$sel_subcategory1="select sub_name from product_subcategory where sub_id='".$data_product['sub_id']."'";
														$ptr_subcategory1=mysql_query($sel_subcategory1,$con1);
														$data_subcategory=mysql_fetch_array($ptr_subcategory1);//=================pune=========================
														
														"<br/>3".$sele_cateahm="select pcategory_id from product_category where pcategory_name='".$data_cate['pcategory_name']."' order by  pcategory_id asc";
														$ptr_sele_ahmcatte=mysql_query($sele_cateahm,$db2);
														if(mysql_num_rows($ptr_sele_ahmcatte))
														{
															$data_ahm_cat=mysql_fetch_array($ptr_sele_ahmcatte);
															$cat_id=$data_ahm_cat['pcategory_id'];
														...
														{
															"<br/>4".$insert_cat="insert into product_category (`pcategory_name`,`added_date`,`cm_id`,`admin_id`) values('".$data_cate['pcategory_name']."','".date('Y-m-d H:i:s')."','2','".$data_record_ahm['admin_id']."')";
															$ptr_ins_cat=mysql_query($insert_cat,$db2);
															$cat_id=mysql_insert_id($db2);
														}
														
														"<br/>5".$sele_subcateahm="select sub_id from product_subcategory where sub_name='".$data_subcategory['sub_name']."' order by  sub_id asc";
														$ptr_sele_subcatte=mysql_query($sele_subcateahm,$db2);
														if(mysql_num_rows($ptr_sele_subcatte))
														{
															$data_ahm_subcat=mysql_fetch_array($ptr_sele_subcatte);
															$sub_cat_id=$data_ahm_subcat['sub_id'];
														...
														{
															echo"<br/>6".$insert_subcat="insert into product_subcategory (`sub_name`,`pcategory_id`,`cm_id`,`admin_id`) values('".$data_subcategory['sub_name']."','".$cat_id."','2','".$data_record_ahm['admin_id']."')";
															$ptr_ins_subcat=mysql_query($insert_subcat,$db2);
															$sub_cat_id=mysql_insert_id($db2);
														}
														"<br/>inv4-".$record_id;											
														"<br/>7".$inser_prod="insert into product (`product_name`,`product_code`,`pcategory_id`,`sub_id`,`size`,`unit`,`commission`,`price`,`type`,`added_date`,`cm_id`,`quantity`,`admin_id`) values ('".$data_product['product_name']."','".$data_product['product_code']."','".$cat_id."','".$sub_cat_id."','".$data_product['size']."','".$data_product['unit']."','".$data_product['commission']."','".$data_product['price']."','".$data_product['type']."','".date('Y-m-d H:i:s')."','".$cm_ids."','".$data_record_service_inv['sin_product_qty']."','".$data_record_ahm['admin_id']."')";
														$ptr_mysql_prod=mysql_query($inser_prod,$db2);
														$product_ids=mysql_insert_id($db2);
												...
												"<br/>".$inser_prod1="insert into inventory_product_map (`inventory_id`,`product_id`,`sin_product_price`, `sin_product_base_price`,`sin_product_disc`,`cgst_tax_in_per`,`cgst_tax`, `sgst_tax_in_per`,`sgst_tax`, `igst_tax_in_per`,`igst_tax`,`sin_product_total`,`sin_product_qty`, `sin_prod_disc_price`,`discounted_price`,`admin_id`) values ('".$record_id."','".$data_record_service_inv['product_id']."','".$data_record_service_inv['sin_product_price']."','".$data_record_service_inv['sin_product_disc']."','".$data_record_service_inv['sin_product_base_price']."','".$data_record_service_inv['cgst_tax_in_per']."','".$data_record_service_inv['cgst_tax']."','".$data_record_service_inv['sgst_tax_in_per']."','".$data_record_service_inv['sgst_tax']."','".$data_record_service_inv['igst_tax_in_per']."','".$data_record_service_inv['igst_tax']."','".$data_record_service_inv['sin_product_total']."','".$data_record_service_inv['sin_product_qty']."','".$data_record_service_inv['sin_prod_disc_price']."','".$data_record_service_inv['discounted_price']."','".$data_record_service_inv['admin_id']."')";
												$ptr_query=mysql_query($inser_prod1,$db2);
												"<br/>inv6-".$record_id;
												//$customer_service_id=$db->query_insert("inventory_product_map", $data_record_service_inv);
											}
										}
										...
										/*for($j=1;$j<=$total_type1;$j++) // 25/6/18
										{
											...
											$data_record_tax_inv['tax_value'] =$_POST['tax_value'.$j];
											$data_record_tax_inv['tax_amount']=$_POST['tax_amount'.$j];
											//echo "<br/>inv7-".$record_id;
											"<br/>".$inser_prod_tax="insert into inventory_tax_map (`inventory_id`,`tax_type`,`tax_value`,`tax_amount`) values ('".$data_record_tax_inv['inventory_id']."','".$data_record_tax_inv['tax_type']."','".$data_record_tax_inv['tax_value']."','".$data_record_tax_inv['tax_amount']."'";
											$ptr_ins=mysql_query($inser_prod_tax,$db2);
										}*/
										...
										"<br/>inv8-".$record_id;
										"<br/>".$insert_sales_invoice = "INSERT INTO `inventory_invoice` (`inventory_id`, `price`, `total_cost`, `amount1`, `payable_amount`,`remaining_amount`, `paid_type`, `bank_id`, `cheque_detail`, `chaque_date`, `credit_card_no`, `admin_id`, `added_date`,`status`,`cm_id`,`total_paid`) VALUES ('".$record_id."', '".$product_price."', '".$total_price."', '".$amount1."', '".$_POST['payable_amount']."','".$_POST['remaining_amount']."', '".$payment_mode_id."','".$bank_name."', '".$chaque_no."', '".$sep_check_date."','".$credit_card_no."', '".$data_record_ahm['admin_id']."','".$added_date."','".$status."','".$cm_id1."','".$_POST['payable_amount']."'); ";
										$ptr_sales_invoice = mysql_query($insert_sales_invoice,$db2);
										//============================================================================
										$sel_cust="select name,contact from vendor where vendor_id ='".$data_record_ahm['vendor_id']."'";
										$ptr_cus_name=mysql_query($sel_cust,$db2);
										$data_cust_name=mysql_fetch_array($ptr_cus_name);
										$name=$data_cust_name['name'];
										$contact=$data_cust_name['contact'];
										...
										$mesg ="Hi ".$name." Thanks for purchasing our service";
										$sel_inq="select sms_text from previleges where privilege_id='136'";
										$ptr_inq=mysql_query($sel_inq,$db2);
										$txt_msg='';
									...
									//===========================================ISAS PCMC============================================
										if($customer_id=="1113")
										{
											...
											"admin_id- ".$data_record_ahm['admin_id']=( ($_POST['stockist_id'])) ? $_POST['stockist_id'] : "0";
									...
									//===========================================ISAS Ahmednagar============================================
										if($customer_id=="1212")
										{
											...
											elseif($_SESSION['branch_name']=="Ahmednagar")
											{
									...
									"<br/>".$sel_sms_cnt="select * from sms_mail_configuration_map where previlege_id='138' ".$_SESSION['where']."";
	...
	?>
            	...
            	<tr>
                	...
                	<table  width="100%" style="border:1px solid gray; ">
						...
						<?php if($_SESSION['tax_type']=='GST')
						{
							...
							{
								var shows_data='<div id="floor_id'+idss+'"><table cellspacing="3" id="tbl'+idss+'" width="100%"><tr><td width="20%" align="center"><input type="hidden" name="exclusive_id'+idss+'" id="exclusive_id'+idss+'" value="'+idss+'" /><select name="product_id'+idss+'" id="product_id'+idss+'" style="width:219px" onchange="show_product_qty(this.value,'+idss+')"><option value="">Select Product</option><?php
					...
					<tr>
                        <td colspan="6">
							...
							<tr>
								<td valign="top" align="center" width="20%">Product Name</td>
								...
								<td valign="top" width="12%"  align="center">Referral Staff</td>
							...
							if($record_id!='')
							{
                            	...
                            	{ 
									...
									$sel_tel = "select p.product_id,p.product_name,p.price,p.quantity,p.admin_id from product p, site_setting s where 1 and p.status='Active' and p.admin_id=s.admin_id ".$where_cm." order by product_id asc";//remove ".$_SESSION['user_id']." on 14/5/2018 --and s.type='ST' on 13/6/18
									$query_tel = mysql_query($sel_tel);
									$qty_in_stock='';
									while($data_prod=mysql_fetch_array($query_tel))
									{
										...
										if($data_exclusive['product_id']==$data_prod['product_id'])
										{
											...
											$qty_in_stock=$data_prod['quantity'];
										}
										$sel_emp="select name from site_setting where admin_id='".$data_prod['admin_id']."'";
										$ptr_admin_id=mysql_query($sel_emp);
										$data_name=mysql_fetch_array($ptr_admin_id);
										$name= "(".$data_name['name'].")";
									...
									<td width="5%" align="center"><input type="text" name="product_total_qty<?php echo $t; ?>" readonly="readonly" id="product_total_qty<?php echo $t; ?>" style=" width:40px" value="<?php echo intval($qty_in_stock+$data_exclusive['product_qty']); ?>" onKeyUp="getDiscount(<?php echo $t; ?>)"/></td>
									...
									?>
                                    <td width="4%" align="center"><input type="text" name="mrp_price<?php echo $t; ?>" id="mrp_price<?php echo $t; ?>" readonly="readonly" style="width:60px" value="<?php echo $data_exclusive['prod_price'] ?>" /></td>
                                    <td width="6%" align="center"><input type="text"  name="sales_product_price<?php echo $t; ?>" id="sales_product_price<?php echo $t; ?>" style=" width:60px" value="<?php echo $data_exclusive['sales_product_price'] ?>"></td>
									<td width="10%">
									...
									if($total_staff=mysql_num_rows($query_staff))
									{
										...
										{
											$selected='';
											if($data_exclusive['staff_id']==$data['admin_id'])
											{
           ...
           <!--</tr> -->
            ...
            <tr>
                <td width="14%" valign="top">Discount <input type="radio" name="discount_type"  onChange="calculte_total_cost()" id="discount_type" checked="checked" value="percentage" <?php if($_POST['discount_type']=="percentage") {echo 'checked="checked"';}else if($row_record['discount_type']=="percentage") { echo 'checked="checked"';} ?>  />in %
                ...
                <td width="74%"><input type="text"  class="input_text" name="discount" style="width:200px" id="discount" onKeyUp="calculte_total_cost()" value="<?php if($_POST['discount']) echo $_POST['discount']; else echo $row_record['discount'];?>" /></td> 
                <td width="12%"></td>
            </tr>
            <tr>
            	<td width="14%" valign="top">Discount Price</td>
            	<td width="74%"><input type="text" class=" input_text" style="width:200px" name="discount_price" id="discount_price" value="<?php if($_POST['discount_price']) echo $_POST['discount_price']; else echo $row_record['discount_price'];?>" /></td> 
           ...
           <tr>
             ...
             <tr>
               ...
               <tr>
                ...
                <td width="74%"><input type="text" name="amount1" id="amount1" style="width:200px" onChange="cal_remaining_amt();" value="<?php if($_POST['save_changes']) echo $_POST['amount1']; else echo $row_record['total_price']; ?>"  /></td>
             </tr>
             <tr>
