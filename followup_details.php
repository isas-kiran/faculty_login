...
/*$sel_id="select class_id from stud_regi where student_id='".$rec_id."'";
$ptr_ids=mysql_query($sel_id);
$data_ids=mysql_fetch_array($ptr_ids);
$record_id = $data_ids['class_id'];*/
?>
...
function close_followup(val)
{
	var record_id=val;
	var data="action=close_followup_inquiry&record_id="+record_id;
	$.ajax({
		url:"set_status.php",type:"post",data:data,cache:false,
...
<table border="0" cellspacing="0" cellpadding="0" width="100%">
  ...
  <!-- ================================================Start Table=============================================================================================-->
<?php
 $sql_query= "select inquiry_id,firstname,middlename,lastname, course_id,status from inquiry where inquiry_id = '$record_id'"; 
...
<table cellspacing="0" cellpadding="0" class="table" width="95%">
	...
	<table border="0" cellspacing="0" cellpadding="0" width="99%" align="center">
              <tr>
                ...
                <td><strong>Name- </strong><?php echo $data_ptr['firstname']." ".$data_ptr['lastname']; ?></td>  
                <td><strong>Course Name- </strong><?php echo $val_select_course['course_name']; ?></td>     
				<?php
				if($_SESSION['type']=="S" || $_SESSION['type']=='Z' || $_SESSION['type']=='LD' && $data_ptr['status']!="Cancelled")
				{
				?>
				<td  align="right"><input type="button" name="park_inquiry" onClick="close_followup('<?php echo $record_id; ?>')" value="Close Enquiry" /></td>
                <?php
				}
				?>
				<?php
				if($data_ptr['status']!="Cancelled")
				{
					if($data_ptr['status']!='Enrolled')
					{
						?><td  align="right"><a href="followup_summery.php?record_id=<?php echo $record_id; ?>"><input type="button" name="update_followup" value="Update Followup" /></a></td><?php
					}
				}
				?>
				
                <td  align="right"><a href="followup_summery.php"><input type="button" name="back" value="Back" /></a></td>
                <td class="rightAlign" > 
    ...
    <?php
                        ...
                        if($_GET['order'] !='' && ($_GET['orderby']=='followup_id'))
                        {
                        	...
                        	$sql_query= "select * from followup_details where student_id ='".$record_id."' ".$_SESSION['where']." ".$pre_keyword." ".$select_directory.""; 
                       //echo $sql_query;
					   	$db=mysql_query($sql_query);
