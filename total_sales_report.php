		...
		<form method="get" name="search">
            ...
            ?>
				<tr>
                    ...
                    <td width="15%" align="center"><input type="text" value="<?php if($_REQUEST['keyword']!="Keyword") echo $_REQUEST['keyword'];?>" name="keyword" class="defaultText search_box" title="Keyword" />
                    </td>
                    ...
                    {
                        ?>
                        <td> <a href="total_sales_export.php<?php echo $sep_url_string; ?>"><img src="images/excel.png" height="30px" width="30px"/></a></td>
                        <?php
                    }
                    ?>
        		</tr>
...
{                            
	...
	{
		$pre_null_date =" and DATE(admission_date) !='0000-00-00";
	}
	else
	{
		$pre_null_date='';
	...
	{
		$sql_query= "SELECT DISTINCT(i.enroll_id) FROM invoice i, enrollment e WHERE i.enroll_id=e.enroll_id ".$paid_from_date_i." ".$paid_to_date_i." ".$where_cm." ".$search_cm_id_i." ".$select_directory." ";
	}
	else
	{
		$sql_query= "SELECT DISTINCT(i.enroll_id) FROM invoice i, enrollment e WHERE i.enroll_id=e.enroll_id and e.assigned_to='".$_SESSION['admin_id']."' ".$paid_from_date_i." ".$paid_to_date_i." ".$where_cm." ".$search_cm_id_i." ".$select_directory." ";
	}
	...
	if($no_of_records)
	{
	   ...
	   // $pager = new PS_Pagination($sql_query,$_SESSION['show_records'],$_SESSION['show_records_pages'],$query_string1);
		...
		<tr class="grey_td" >
			<td align="center" width="4%"><strong>Sr. No.</strong></td>
			<td align="center" width="12%"><strong>Student Name</strong></td>
			<td align="center" width="14%"><strong>Course Name</strong></td>
			<td align="center" width="12%"><strong> Total paid in month</strong></td>
			<?php
			//echo "<br/>".$cm_ids."--".$_REQUEST['branch_name'];
			if($_SESSION['type']=="S" || $_SESSION['type']=="A" || $_SESSION['type']=='Z' || $_SESSION['type']=='LD')
			{
			...
			{
				$select_counc="select * from site_setting where 1 and (type='Z' or type='A' or type='C') and system_status='Enabled' and admin_id='".$_SESSION['admin_id']."' ".$_SESSION['where']." ".$search_cm_id." order by admin_id asc";
			}
		...
		while($val_query=mysql_fetch_array($db))
		{
			...
			///$sele_ref="select ref_id from enrollment where ";
			
			$sel_name="select name from enrollment where 1 and (student_status ='Active' or student_status is NULL) and enroll_id='".$val_query['enroll_id']."' ".$pre_keyword." order by enroll_id asc";
			...
			if(mysql_num_rows($ptr_name))
			{
				...
				while($data_total=mysql_fetch_array($ptr_ref))
				{
					...
					echo '<br/>'.$val_course['course_name'].'</b><br><img src="images/indian-rupee-16.ico">'.$data_total['net_fees'].'/-<br/>';
					...
					if(mysql_num_rows($ptr_down))
					{
						...
						echo 'Down Payment- '.$data_amnt['down_payment'].'<br> Paymeent Mode- '.$data_pay['payment_mode'].''; //<br> Discount- '.$data_amnt['discount'].'
						$total_paid_downpayment=$total_paid_downpayment+$data_amnt['down_payment'];
					}										
					echo '<br/>Date: '.$data_total['admission_date'].'';
				...
				if($total_inst=mysql_num_rows($ptr_ins_amnt))
				{
					...
					{
						$select_installments =" SELECT * FROM invoice where enroll_id ='".$data_inst_amnt['enroll_id']."' and course_id='".$data_inst_amnt['course_id']."' and (invoice_id !='".$data_inst_amnt['invoice_no']."' or type!='down_payment') and amount >0 ".$paid_from_date." ".$paid_to_date." ";
						//echo "<br />";
				...
				if($_SESSION['type']=="S" || $_SESSION['type']=="A" || $_SESSION['type']=='Z' || $_SESSION['type']=='LD')
				{
					$select_counc1="select admin_id from site_setting where 1 and (type='Z' or type='A' or type='C') and system_status='Enabled' ".$_SESSION['where']." ".$search_cm_id." order by admin_id asc";
				}
				else
				{
					$select_counc1="select admin_id from site_setting where 1 and (type='Z' or type='A' or type='C') and system_status='Enabled' and admin_id='".$_SESSION['admin_id']."' ".$_SESSION['where']." ".$search_cm_id." order by admin_id asc";
				}
				...
				while($data_counc1=mysql_fetch_array($ptr_counc1))
				{
					...
					<td align="center">
					<?php
					$sel_inst_amnt="select enroll_id,paid,course_id,invoice_no,assigned_to from enrollment where 1 and (student_status ='Active' or student_status is NULL) and enroll_id='".$val_query['enroll_id']."' and assigned_to='".$data_counc1['admin_id']."' ".$pre_keyword." order by enroll_id asc";//Bhakti Sapke
					$ptr_ins_amnt=mysql_query($sel_inst_amnt);
					if($total_inst=mysql_num_rows($ptr_ins_amnt))
					{
		...
		?>
		<!--<tr class="grey_td">
		<td colspan="<?php //echo $colspan; ?>" style="padding:0px">
			<table cellspacing="0" cellpadding="0" width="100%"  style="border:1 px solid #CCC; border-collapse:collapse">-->
				<tr class="grey_td">
				   
					<td colspan="2" style="font-size:14px; font-weight:bold" align="center"><strong>Total</strong></td>
					<td align="center" style="font-size:14px; font-weight:bold"><?php echo $total_paid_downpayment; ?></td>
					<td align="center" style="font-size:14px; font-weight:bold"><?php echo $total_paid1; ?></td>
					<?php
					if($_SESSION['type']=="S" || $_SESSION['type']=="A" || $_SESSION['type']=='Z' || $_SESSION['type']=='LD')
					{
						$select_counc1="select admin_id from site_setting where 1 and (type='Z' or type='A' or type='C') and system_status='Enabled' ".$_SESSION['where']." ".$search_cm_id." order by admin_id asc";
				...
				<tr class="grey_td">
					...
					<td align="center" colspan="2" style="font-size:14px; font-weight:bold"><?php echo intval($total_paid_downpayment + $total_paid1); ?></td>
					...
					$ptr_counc1=mysql_query($select_counc1);
					$tot=mysql_num_rows($ptr_counc1);
					?>
					<td align="center" style="font-size:14px; font-weight:bold" colspan="<?php echo $tot; ?>"></td>
