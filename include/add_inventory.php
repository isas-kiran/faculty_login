...
function getDiscount(idss)
{
	...
	if(disc_type=="rupees")
	{
		total_prod_price=parseFloat(product_price-disc);
	...
	else
	{
		...
		total_prod_price=parseFloat(product_price-discount);
	}
		...
		if($_POST['save_changes'])
		{
			...
			else
			{
				...
				if($record_id)
				{
					...
					$sel_inv="select * from inventory_product_map where inventory_id='".$record_id."'";
					...
					$update_invoice="update `inventory_invoice` set `price`='".$price."', `total_cost`='".$total_cost."', `amount1`='".$amount1."', `payable_amount`='".$_POST['payable_amount']."',`remaining_amount`='".$_POST['remaining_amount']."', `paid_type`='".$payment_mode_id."', `bank_id`='".$bank_name."', `cheque_detail`='".$chaque_no."', `chaque_date`='".$sep_check_date."', `credit_card_no`='".$credit_card_no."', `admin_id`='".$_SESSION['admin_id']."',`status`='".$status."',`total_paid`='".$payable_amount."', `added_date`='".$added_date."' where inventory_id='".$record_id."'";
                        ...
                        <td colspan="7">
                        	<table cellspacing="3" id="tbl" width="100%">
                        		...
                        		<tr>
									...
									if($record_id!='')
									{
                            			echo $select_exc = "select * from inventory_product_map where inventory_id='".$record_id."' and product_id!='' order by map_id asc ";
